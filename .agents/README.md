...
- `2026-09-19-shared-trace-log-csm.json` — 2026-09-19 — plan (ready, NOT_STARTED; csm-plan/1): make traces go to ONE shared per-repo log that is concurrency-safe, worktree-independent (survives worktree closure), and loss-less, and make the temp/worktree registry lock-free with no lost entries — 7 tasks: a git-common-dir path resolver (absolute), single-write bounded loss-less appends, a per-entry registry with one-time atomic migration, wt-session wired to both, concurrency/durability tests, docs + retention policy, Makefile/regen — status: ready
