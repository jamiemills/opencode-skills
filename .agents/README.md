...
- `2026-09-12-dynamic-worker-runtime-csm.json` — 2026-09-12 — plan (23 tasks across 6 phases; 18 completed in working tree, 5 pending — see broker-upstream for remaining; body status field not yet reconciled): dynamic-worker runtime from the agreed grill approach (versioned contracts/capabilities, wired concurrency, worker observability + on-demand reducer, worker lease/heartbeat, hybrid validated scheduler, build-shaped sandbox + mediated egress broker with immutable logging + hardened worktree + thin child-side entry + prototype gate); hard-gated by spikes T001/T002 — status: reference
...
- `2026-09-13-dynamic-worker-runtime-broker-completion-csm.json` — 2026-09-13 — plan (blocked, recoverable, cycle 1; supersedes broker-upstream; csm-build cycle 1: T004 egress limits + sustained session COMPLETE, T005 true completed-run resume COMPLETE, T001 broker-mediated upstream Docker-proven (drop->recordDrop pending), T002 live isolation wiring / T003 decision gate / T006 commit+lineage pending): 6 tasks — status: reference
- `2026-09-13-dynamic-worker-runtime-gap-remediation-csm.json` — 2026-09-13 — plan (ready, NOT_STARTED; remediates ALL blockers + non-blockers from reviews A/B of the broker-completion build; independent critique applied): 8 tasks — on-the-wire broker listener + limits separation, real drop source + observable capture policy (spike), durability/reliability non-blockers, schema/registry owner + same-commit regen, mode-aware effective-isolation gate + live provider/broker wiring + telemetry, provider/policy honesty + listener-routed e2e + digest-bound attestation, decision gate + six-condition isolation matrix, durable delivery (regen/commit/lineage/docs) — status: reference
