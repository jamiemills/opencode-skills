...
3. [completed] Artifact format markers + corpus retrofit (P2)
   - Task ID: T003
   - Depends on: T002
   - Parallel group: G3
   - Risk: standard
...
| 2026-08-16 | 3 | SELECT -> DISPATCH (T003 primary-led) -> INTEGRATE -> VERIFY -> CHECKPOINT | T003 | 11 files retrofitted; formatMarkerOf helper + FORMAT_VERSIONS (contracts.mjs) bound versions; approaches corpus loop w/ runtime grill-template extraction; probes: stripped marker, /99, rogue plan all fail; .agents/README.md stale-fix + approaches + this-plan index. Gate 415 | SELECT (Wave 4) |
