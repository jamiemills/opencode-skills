...
6. A per-repo/per-directory toggle: `.agents/token-efficiency.json` (`{"enabled": false}` disables) resolved by nearest-wins walk-up from the working directory to the git root, defaulting to ENABLED when absent; it switches the repo-local efficiency layer — the check-suite lint checks (1), the monitor (2), and the AGENTS.md guidance (4) — while skill-internal economics (3) and the docs-only config block stay global.
...
- The efficiency lint is opt-out per repo/directory via `.agents/token-efficiency.json` — when disabled, check-suite skips the two new checks with a visible notice (count 516 instead of 525); absent or malformed toggle = enabled (fail-closed: protections stay on). The toggle gates ONLY the repo-local efficiency layer (checks 1/2/4); csm-build's prefix rule and the docs-only config block are global (A9).
...
7. Toggle semantics verified: `scripts/lib/token-efficiency.mjs` (pure functions) unit-tested for absent → enabled, `{"enabled": true}` → enabled, `{"enabled": false}` → disabled, malformed/invalid JSON → enabled with warning, and nearest-wins walk-up from a subdirectory (a nested `.agents/token-efficiency.json` in a subdir wins over the repo root's); check-suite with a disabled toggle in a full-tree /tmp copy (via `check-suite --root`) skips the two efficiency checks with a visible notice and exits 0 (count 516); `scripts/cache-health.mjs` run from a directory with a disabled toggle prints the notice and exits 0 without querying the DB; AGENTS.md and the dated doc document the toggle format and semantics (with a disabled example).
...
| A9 | Toggle semantics: `.agents/token-efficiency.json` (`{"enabled": bool}`) resolved nearest-wins from the working directory up to the git root; absent or malformed = ENABLED (fail-closed); gates ONLY the repo-local efficiency layer (check-suite lint, monitor, AGENTS.md guidance); csm-build's prefix rule (provider economics) and the docs-only config block stay global | User-dictated intent | User request 2026-08-20: "turn on or off token efficiency for the repo or directory i am in using a toggle file in .agents"; NORMS.md detection precedent (walk-up) | Accepted |
