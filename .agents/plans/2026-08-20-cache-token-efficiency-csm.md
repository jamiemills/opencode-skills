...
Deliverables:
1. Gate-level protection of prefix stability and the frontmatter word budget: new check-suite checks rejecting volatile content (dates, years, versions, env vars, absolute paths) in skill descriptions and enforcing the ≤220-word total, so cache hits cannot be silently invalidated or the budget silently regressed by future edits — gated by a per-repo/per-directory toggle (see 6).
2. A zero-dependency cache-health monitor (`scripts/cache-health.mjs`) that reads opencode's SQLite database via the bundled `opencode db` CLI and reports per-session and per-day cache read/write ratios and actual cost for deepseek-v4-flash — granularity the built-in `opencode stats` does not provide; honors the toggle (refuses with a notice when disabled for the working directory).
3. A documented subagent prefix-sharing rule in csm-build DISPATCH: parallel dispatches share a byte-identical static prefix so DeepSeek's automatic prefix caching serves requests #2..N at ~97% input discount; provider-specific notes (Anthropic-style stagger-first-request) recorded. This rule is skill-level provider economics and is NOT toggle-gated (decision A9).
...
6. A per-repo/per-directory toggle: `.agents/token-efficiency.json` (`{"enabled": false}` disables) resolved by nearest-wins walk-up from the working directory to the git root, **ON by default** — absence of a file, `{"enabled": true}`, or a malformed file all resolve to ENABLED (fail-closed), and THIS repo ships a committed default toggle `{"enabled": true}` so the default is explicit and visible; it switches the repo-local efficiency layer — the check-suite lint checks (1), the monitor (2), and the AGENTS.md guidance (4) — while skill-internal economics (3) and the docs-only config block stay global.
...
- The efficiency lint is opt-out per repo/directory via `.agents/token-efficiency.json` — **ON by default**: absent file, `{"enabled": true}`, or malformed content all resolve to enabled (fail-closed: protections never silently disappear); only an explicit `{"enabled": false}` disables it, in which case check-suite skips the two new checks with a visible notice (count 516 instead of 525). This repo ships a committed default toggle `.agents/token-efficiency.json` containing `{"enabled": true}` (added by T004). The toggle gates ONLY the repo-local efficiency layer (checks 1/2/4); csm-build's prefix rule and the docs-only config block are global (A9).
...
7. Toggle semantics verified: `scripts/lib/token-efficiency.mjs` (pure functions) unit-tested for absent → enabled, `{"enabled": true}` → enabled, `{"enabled": false}` → disabled, malformed/invalid JSON → enabled with warning, and nearest-wins walk-up from a subdirectory (a nested `.agents/token-efficiency.json` in a subdir wins over the repo root's); THIS repo's committed default `.agents/token-efficiency.json` exists with `{"enabled": true}` and the gate reports 525 with it in place; check-suite with a disabled toggle in a full-tree /tmp copy (via `check-suite --root`) skips the two efficiency checks with a visible notice and exits 0 (count 516); `scripts/cache-health.mjs` run from a directory with a disabled toggle prints the notice and exits 0 without querying the DB; AGENTS.md and the dated doc document the toggle format and semantics (ON-by-default contract + a disabled example).
...
| A9 | Toggle semantics: `.agents/token-efficiency.json` (`{"enabled": bool}`) resolved nearest-wins from the working directory up to the git root; **ON by default** — absent or malformed = ENABLED (fail-closed); only an explicit `{"enabled": false}` disables; THIS repo commits a default toggle file `{"enabled": true}` so the default is explicit and visible; gates ONLY the repo-local efficiency layer (check-suite lint, monitor, AGENTS.md guidance); csm-build's prefix rule (provider economics) and the docs-only config block stay global | User-dictated intent | User request 2026-08-20: "turn on or off token efficiency for the repo or directory i am in using a toggle file in .agents"; follow-up "token efficiency is on by default" (2026-08-20); NORMS.md detection precedent (walk-up) | Accepted |
...
- LATENT GATE DEFECT — FIXED (user-directed immediate fix, 2026-08-20; residual = review coverage in T001's independent gate review): the F-050 marker fix added `format: csm-grill/1` (and `csm-plan/1`) as a BARE first line inside the producer template fences, but the corpus checks (`formatMarkerOf`, check-suite.mjs:53-66) only accepted YAML-frontmatter markers (`---\nformat: ...\n---`). Old corpus files carry YAML frontmatter (e.g. 2026-08-16-suite-coherence-contracts-approach.md:1-3); a session that follows the template literally (e.g. the concurrent csm-grill commit 8882c5d, 2026-08-20) produced a bare-marker file that failed the gate and blocked ALL commits. Fixed by extending formatMarkerOf to also accept a bare top-of-file marker (the F-050 template contract form), additive, YAML path unchanged; gate green 522 checks. T001's mandatory independent review of gate changes covers this edit (verification: bare-marker fixture passes, YAML fixtures still pass, no existing corpus check weakened).

## Design

**Toggle (user-requested, 2026-08-20).** `.agents/token-efficiency.json` switches the repo-local efficiency layer per repo or per directory — **ON by default**: only an explicit `{ "enabled": false }` disables it; anything else (absent file, `true`, malformed JSON, non-boolean value — with a warning notice) keeps it ENABLED (fail-closed: protections never silently disappear on a corrupt file). THIS repo ships a committed default `.agents/token-efficiency.json` = `{"enabled": true}` (added by T004) so the ON default is explicit, discoverable, and easily flipped to false. Resolution is a nearest-wins walk-up from the start directory to the nearest git root (`.git` ancestor; falls back to the filesystem root outside any repo), checking `<dir>/.agents/token-efficiency.json` at each level — a nested project's toggle overrides its parent repo's. Consumers: (a) check-suite's two new efficiency checks (T001) are skipped with a visible notice when disabled for the gate root (count drops from 525 to 516); (b) cache-health.mjs (T002) prints the notice and exits 0 without querying the DB when disabled for the working directory; (c) AGENTS.md's efficiency rules (T004) state they apply only when enabled. NOT gated: csm-build's prefix-sharing rule (skill-level provider economics, applies in every repo) and the docs-only opencode.jsonc recommendation (global config guidance). The resolver is a small pure module `scripts/lib/token-efficiency.mjs` (parse + walk-up + isEnabled) with hermetic unit tests.

**Lint (T001).** Add two additive checks to check-suite.mjs main loop right after the existing description checks (check-suite.mjs:402-405): (a) `VOLATILE_DESC_RE` = date/year/version/$ENV/absolute-path patterns (ISO date `\d{4}-\d{2}-\d{2}`, bare year `\b20\d{2}\b`, version `\b\d+\.\d+(\.\d+)?\b|v\d+(\.\d+)+`, env `\$[A-Z][A-Z0-9_]*`, abs paths `\/home\/|\/Users\/|\/tmp|\/etc\/|\/opt\/|\/usr\/|\/var\/`; NOT bare `\d{4}`) — rejects volatile content in descriptions that would silently invalidate prefix cache units; (b) a total-word-budget check summing the 8 descriptions, failing above 220 — protects AC1 of the completed efficiency plan. Both are skipped when the toggle is disabled for the gate root. Both additive; negative fixtures in /tmp (2 violation->failure pairs: planted year + 221-word set; plus the disabled-toggle skip case). Gate changes are public-interface changes: independent review required (precedent: T006 of the efficiency plan).

**Monitor (T002).** `scripts/cache-health.mjs` — no-npm-dependency Node script: `execFileSync('opencode', ['db', sql, '--format', 'tsv'])` (or equivalent verified flag) against the live DB; queries deepseek-v4-flash sessions (model LIKE '%deepseek-v4-flash%' or providerID/modelID fields) with tokens_input/tokens_cache_read/tokens_cache_write/cost, plus time for per-day grouping; computes hit ratio read/(read+input+write) with zero-denominator guards; prints per-session and per-day report in the repo's plain check-suite style; `--days N` filter via time_created. When the toggle is disabled for the working directory, prints the notice and exits 0 without touching the DB. Parsing/aggregation extracted as pure functions over the TSV text so hermetic unit tests use fixture strings (no DB in tests); a live smoke run on this machine records the real numbers as acceptance evidence. No writes anywhere; no DB copies.
...
1. [pending] check-suite: volatile-description and word-budget lint
   ...
   - Owned scope: scripts/check-suite.mjs (additive checks + toggle gating), NEW scripts/lib/token-efficiency.mjs (toggle resolver), NEW tests/token-efficiency.test.mjs + tests/fixtures/token-efficiency/ (fixture toggle trees)
   ...
   - Spike candidate: none — hook point verified (check-suite.mjs:402-405); regex list validated against all 8 live descriptions (R4; bare \d{4} excluded); toggle resolution is a pure walk-up (A9)
   - Actions:
     ...
     4. Implement `scripts/lib/token-efficiency.mjs` (pure functions, no deps): `parseToggle(text) -> {enabled: bool, ok: bool}` (true/absent/malformed -> enabled with ok=false for malformed), `findToggleFile(startDir) -> path|null` (walk up from startDir to the nearest git root — `.git` ancestor, else filesystem root — checking `<dir>/.agents/token-efficiency.json` at each level, nearest wins), `isEnabled(startDir) -> {enabled: bool, source: path|null, warning: string|null}`. Malformed JSON/non-boolean: warning + enabled (fail-closed, A9).
     5. Wire the gating: when `isEnabled(root).enabled === false`, skip the two new checks and print a visible notice `note: token efficiency disabled (<source>) — volatile/budget checks skipped`; when enabled (default), run them. Keep every existing check byte-identical; do not restructure; expected counts: enabled 525 (516 + 8 volatile + 1 budget), disabled 516.
     6. Hermetic tests (tests/token-efficiency.test.mjs): fixture trees under tests/fixtures/token-efficiency/ covering absent -> enabled; `{"enabled": true}` -> enabled; `{"enabled": false}` -> disabled; malformed JSON -> enabled + warning; nested subdir toggle overrides parent (nearest-wins); subdir without toggle inherits parent's enabled/disabled.
     7. Negative-test evidence via a full-tree copy (mechanism: `rsync -a --exclude node_modules --exclude .git --exclude tests/fixtures-real <repo>/ /tmp/plan-negative/`, plant violations in the copy: (a) a volatile token (e.g. "2026-08-20" or "$HOME") in one description, (b) a word added to reach 221 total; run `node scripts/check-suite.mjs --root /tmp/plan-negative` from the repo and record that each violation fails its check with the expected message; the pristine copy passes (control). Then plant `.agents/token-efficiency.json` `{"enabled": false}` in the copy and record that both checks are SKIPPED with the notice and the gate exits 0 (count 516). No repo changes.
   - Acceptance signal: `node scripts/check-suite.mjs` exits 0 with count 525 AND the /tmp negative-run transcript proves both violation->failure pairs AND the disabled-toggle skip case (recorded) AND `node --test tests/token-efficiency.test.mjs` exits 0.
   - Validation: `node --check scripts/check-suite.mjs scripts/lib/token-efficiency.mjs`; `node scripts/gen-readme-matrix.mjs --check`; `node scripts/sync-skill-boilerplate.mjs --check`; all 8 live descriptions still pass (gate proves it); `git diff --stat` shows only scripts/check-suite.mjs, scripts/lib/token-efficiency.mjs, tests/token-efficiency.test.mjs, tests/fixtures/token-efficiency/.
...
2. [pending] Zero-dependency cache-health monitor
   ...
   - Parallel group: G1
   - Risk: standard
   - Owned scope: new files scripts/cache-health.mjs, tests/cache-health.test.mjs, tests/fixtures/cache-health/ (TSV fixture strings); imports the toggle resolver from scripts/lib/token-efficiency.mjs (owned by T001 — do not modify it)
   - Not in scope: any existing file; opencode DB writes; log parsing (logs are unusable — A2/R1); pricing tables (cost comes from the DB cost column, A5); the toggle resolver implementation (T001)
   ...
   - Actions:
     ...
     2. Toggle honor: before any DB query, call `isEnabled(process.cwd())` from scripts/lib/token-efficiency.mjs; when disabled, print `token efficiency disabled for this directory (<source>)` and exit 0 without querying.
     3. Extract parsing + aggregation as PURE functions (e.g. parseSessionRows(tsvText) and aggregateReport(rows)) so tests run on fixture strings without a DB.
     4. Report (check-suite-style plain text): per-session table (session, agent, input, cache read, hit %, cost) and a per-day summary; hit % = cache.read / (cache.read + input + cache.write) with zero-denominator rows skipped or marked n/a; missing cache.read coalesced to 0.
     5. Hermetic tests: tests/cache-health.test.mjs imports the pure functions with fixture TSV (normal rows, zero-denominator row, missing-cache row, multi-session/day grouping); no DB, no network, no writes outside fixtures.
     6. Live smoke run on this machine: `node scripts/cache-health.mjs --days 30` records real per-session numbers as acceptance evidence (expected: general-agent sessions ~88-99% hit; record exact values).
   - Acceptance signal: `node --test tests/cache-health.test.mjs` exits 0 (all pass) AND `node scripts/cache-health.mjs --days 30` exits 0 and prints a per-session + per-day report with hit ratios (recorded) AND a run from a directory with a disabled toggle (fixture dir from tests/fixtures/token-efficiency/) prints the notice and exits 0 without querying AND `node scripts/check-suite.mjs` still exits 0.
...
4. [pending] Cache/token hygiene guidance: AGENTS.md, dated reference doc, README bullet
   ...
   - Parallel group: G2
   - Risk: standard
   - Owned scope: new AGENTS.md (repo root), new .agents/docs/cache-token-efficiency-2026-08-20.md, new .agents/token-efficiency.json (committed default `{"enabled": true}`), README.md (`## Development & testing` bullet), .agents/README.md index line
   ...
   - Spike candidate: none
   - Actions:
     1. Create `.agents/token-efficiency.json` with exactly `{"enabled": true}` — the committed ON-by-default toggle for this repo (visible, discoverable, flip-to-false to disable; resolution by T001's resolver).
     2. AGENTS.md (repo root, concise <100 lines): repo-level rules for future agent sessions — stable-prefix discipline (frontmatter volatility is lint-enforced by T001; never add dates/versions/$ENV/paths to descriptions; the 220-word budget is gate-enforced), fresh-session resume over long transcripts (plan files are the durable record; chat history is never the only record), compaction recall-first when context approaches limits, append-only history (never rewrite earlier turns — cache prefix units require full match), no new skills without re-budgeting the 220 words, monitor usage pointer. State explicitly that these rules apply only while the toggle (`.agents/token-efficiency.json`) is enabled for the directory being worked in — ON by default in this repo; disabled directories follow default behavior.
     3. .agents/docs/cache-token-efficiency-2026-08-20.md (dated reference per the docs convention — date SUFFIX, matching existing entries like csm-suite-performance-baseline-2026-08-15.md): DeepSeek caching mechanics + pricing basis caveat (DeepSeek docs $0.007/$0.22/$0.66 vs models.dev $0.0028/$0.14/$0.28 — cost shown by opencode stats uses models.dev; the monitor uses DB cost), measured hit ratios from this machine, the recommended (docs-only) opencode.jsonc block with rationale — `compaction: {auto: true, prune: true, reserved: 10000}`, `small_model` (title generation currently uses the main model — documented lever), `provider.deepseek.models["deepseek-v4-flash"].limit.output` (documented key) and `agent.<name>.steps`; the deferred note that a DeepSeek thinking/effort knob is undocumented in opencode (passthrough plausible, unverified — needs a live experiment before enabling); monitor usage (`node scripts/cache-health.mjs [--days N]`); AND a dedicated "Per-repo / per-directory toggle" section: file format (`{"enabled": false}` disables), the ON-by-default contract (absent/true/malformed = enabled; this repo's committed default is `{"enabled": true}`), nearest-wins walk-up semantics (from the working directory to the git root; nested subdir toggles override parent repos), fail-closed warning on malformed files, what is gated (check-suite lint, monitor, AGENTS.md rules) vs global (csm-build prefix rule, config block), with a disabled example file.
     4. README.md: add one bullet under `## Development & testing` (or a short `## Cache & token hygiene` section — README has no heading-set gate) referencing AGENTS.md + the dated doc + the monitor + the lint + the toggle; keep the matrix region untouched (regenerate only via gen-readme-matrix).
     5. .agents/README.md: index the new dated doc per the existing convention — one line of the shape `` `<file>` — <date> — <goal> — status: reference `` (match the existing bullets exactly).
     6. Verify no new skill directory appears and the frontmatter total stays exactly 220.
   - Acceptance signal: `node scripts/check-suite.mjs` exits 0 (525 checks — the committed toggle resolves to enabled) AND `node scripts/gen-readme-matrix.mjs --check` exits 0 AND AGENTS.md exists (<100 lines, grep -c) AND `.agents/token-efficiency.json` exists containing `{"enabled": true}` AND `.agents/docs/cache-token-efficiency-2026-08-20.md` exists AND the doc is referenced in .agents/README.md AND README references the doc AND `git status` shows no new skill dir AND frontmatter total = 220 (word-count command).
...
5. [pending] Regenerate payload and run the full verification battery
   ...
   - Actions:
     ...
     3. Run the full battery and record results: check-suite (525 with toggle enabled); `node --test tests/bootstrap-trust.test.mjs tests/package-audit.test.mjs tests/protocol/*.test.mjs tests/offline/*.test.mjs tests/integration/*.test.mjs tests/resume-semantics.test.mjs tests/cache-health.test.mjs tests/token-efficiency.test.mjs`; csm-scan serial `node --test --test-concurrency=1` (# fail 0, baseline >= 1200); csm-browse `node scripts/check-skill.mjs`; payload byte-match `diff -r` all 8 skills.
...
- Toggle misuse (low, T001): a repo disabling the toggle loses the volatile/budget protection silently — mitigated by the fail-closed default (absent/malformed = enabled), the visible skip notice in check-suite output, and the docs' explicit warning that disabling removes regression protection; the check count itself drops (516 vs 525) making the disabled state observable in gate output.
- Toggle resolution edge cases (low, T001): nested-repo ambiguity (a subdirectory with its own git root resolves its own toggle) is covered by the nearest-git-root walk-up rule; a toggle outside any git repo resolves to the filesystem root — documented, tested.
- Approach/plan corpus format-marker mismatch — FIXED (user-directed immediate fix 2026-08-20; pre-existing, discovered at amendment-commit time when the concurrent grill commit 8882c5d's bare-marker approach file failed formatMarkerOf): formatMarkerOf now accepts the bare top-of-file marker form the producer templates emit (F-050 contract), additive to the YAML path; gate green 522. Residual: the change is a gate edit — T001's mandatory independent gate review must re-verify it (bare-marker pass, YAML fixtures unchanged, no check weakened).
...
| 2026-08-20 | 0 | VERIFY -> SAVED (amended) | — | Plan amended post-save at user request: per-repo/per-directory toggle `.agents/token-efficiency.json` (`{"enabled": false}` disables) with nearest-wins walk-up to the git root, fail-closed default enabled, gating the repo-local efficiency layer (check-suite lint, cache-health monitor, AGENTS.md rules) while csm-build's prefix rule and the docs-only config block stay global (A9). Threaded through: Goal deliverable 6, Constraints, AC7, A9, Design "Toggle" paragraph, T001 (resolver module scripts/lib/token-efficiency.mjs + gated checks + hermetic tests + disabled-toggle negative fixture), T002 (monitor honors toggle), T004 (docs section + AGENTS.md applicability), Risks (misuse + edge cases), T005 battery (+tests/token-efficiency.test.mjs). Gate re-verified green after amendment | SAVED |
