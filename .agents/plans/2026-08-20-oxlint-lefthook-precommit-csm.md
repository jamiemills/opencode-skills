...
- Risk summary: 6 tasks — 1 high-risk (T002 replaces the repo's commit gate; requires independent review), 5 standard. Tasks that always require independent review: T002.
...
- Cycle: 2
- Commits: allowed
- Last checkpoint: 2026-08-20 cycle 2 — T005 final battery green (check-suite 454, hook suite 7/7, csm-browse check-skill PASS, package-audit 1/1, pnpm-only sweep clean, scratch install/uninstall/reinstall + csm-browse frozen install proven). All 6 tasks completed.
...
Add oxlint + lefthook to this repository and REPLACE the existing custom pre-commit gate (scripts/hooks/pre-commit, installed via scripts/install-hooks.mjs which sets `core.hooksPath=scripts/hooks`) with a lefthook-managed pre-commit hook. The new hook must (a) preserve the current gate's behaviors — reject commits with unstaged tracked changes, run `node scripts/check-suite.mjs` (449 checks), syntax-check staged `.mjs` files, run `csm-browse/scripts/check-skill.mjs` when csm-browse files are staged and its deps are installed — and (b) run oxlint on STAGED files only, with oxlint defaults and NO committed `.oxlintrc.json`. Concurrently, the repo moves to **pnpm as its ONLY package installer** (pnpm 10.33.0, corepack-managed): root hook tooling AND csm-browse both install via `pnpm install --frozen-lockfile`; no `npm install`/`npm ci` remains anywhere in docs, hooks, or tests. `npm` is retained exclusively for `npm pack` in pack-bootstrap.mjs (release packaging — not an install) and the opportunistic `npx cyclonedx-npm` SBOM generation.

Deliverables:
1. Root `package.json` (private) with exact-pinned devDependencies `lefthook@2.1.10` + `oxlint@1.79.0`, a `packageManager: pnpm@10.33.0` field, and a committed `pnpm-lock.yaml`.
...
4. `scripts/install-hooks.mjs` rewritten: `pnpm install --frozen-lockfile --ignore-scripts`, ensure core.hooksPath, `pnpm exec lefthook install --force`, one-time `pre-commit.old` cleanup, uninstall path.
...
7. csm-browse converted to pnpm: `package-lock.json` deleted, `pnpm-lock.yaml` committed, its install instructions in README + csm-browse/SKILL.md use pnpm.
...
- **pnpm is the ONLY package installer**: every install in the repo (root tooling, csm-browse) uses `pnpm install` (frozen-lockfile in CI/installer contexts); `npm install`/`npm ci` must not appear in any committed doc, hook, script, or test. `npm pack` (pack-bootstrap.mjs:161) and opportunistic `npx cyclonedx-npm` are NOT installs and stay as-is.
...
Exclusions:
- No change to csm-browse's dependency VERSIONS or its `package.json` scripts; only its lockfile and install docs change (pnpm swap, T006).
- No oxlint rule configuration, no `--fix`, no CI workflows.
- No change to check-suite.mjs or scripts/lib/contracts.mjs.
- No changes to the bootstrap payload or envelope (hooks are dev tooling, not packaged); `scripts/pack-bootstrap.mjs` keeps using `npm pack` (deterministic release packaging is out of the pnpm-only scope — it is not an install).
- No change to `tests/bootstrap-trust.test.mjs:133` — the `npm install evil` string there is a shell-policy fixture for the envelope's steps_markdown, not a repo install.

## Acceptance Criteria

1. `node_modules/.bin/lefthook --version` prints 2.1.10 and `node_modules/.bin/oxlint --version` prints 1.79.0; root `package.json` (with `packageManager: pnpm@10.33.0`) + `pnpm-lock.yaml` are committed; `pnpm install --frozen-lockfile --ignore-scripts` reproduces the lockfile cleanly from a scratch clone; no `package-lock.json` exists at the repo root.
2. `git config --get core.hooksPath` returns `scripts/hooks`; `scripts/hooks/pre-commit` is the lefthook shim (contains the `LEFTHOOK` fingerprint); `pnpm exec lefthook validate` passes; no `scripts/hooks/pre-commit.old` remains; `scripts/install-hooks.mjs` runs end-to-end in a scratch clone (pnpm install + install --force + verify) and its uninstall path resets core.hooksPath.
...
- **Package-manager evidence:** pnpm 10.33.0 installed (`/home/jamiemills/.npm-global/bin/pnpm`, corepack available at /usr/bin/corepack); host node v20.20.2 (repo targets >=22 — binaries unaffected). npm usage sites today: README.md:96 (`npm install --no-audit --no-fund` for csm-browse), README.md:107 (dependency inventory: `csm-browse/package-lock.json` authoritative + `npx cyclonedx-npm` SBOM), README.md:184 (layout comment "deterministic npm pack"), csm-browse/SKILL.md:22 (`npm install --no-audit --no-fund`), csm-browse/package-lock.json (exists, will be swapped), scripts/pack-bootstrap.mjs:161 (`npm pack --json` — packaging, retained), tests/bootstrap-trust.test.mjs:133 (shell-policy fixture string, untouched). pnpm keeps `node_modules/.bin` layout (symlinked) so hook binaries resolve identically.
...
| A3 | Root package.json is the tooling carrier: exact pins lefthook@2.1.10 + oxlint@1.79.0, private, engines node >=22, `packageManager: pnpm@10.33.0`, committed pnpm-lock.yaml | User-dictated (pnpm-only) + planning | pnpm 10.33.0 present (global + corepack); verified no test/gate breaks; offline deterministic hook | Accepted |
| A9 | pnpm is the ONLY package installer repo-wide; `npm pack` (release packaging) and `npx cyclonedx-npm` (opportunistic SBOM) are not installs and are retained; the bootstrap-trust `npm install evil` string is a policy fixture, not a repo install | User-dictated | "only install packages using pnpm"; npm retained only where it is not installing | Accepted |
| A10 | Two independent lockfiles (root + csm-browse) — pnpm is NOT used as a workspace; each installs its own node_modules (root: lefthook+oxlint; csm-browse: its runtime deps) | Planning | Existing repo layout has no root manifest and a separate csm-browse package; minimal-change principle | Accepted |
...
| R5 | pnpm availability and npm usage inventory | `which pnpm corepack`; pnpm --version; grep npm install/ci/pack across repo | Read-only | pnpm 10.33.0 (global + corepack); npm install sites: README:96, csm-browse/SKILL.md:22; npm pack at pack-bootstrap.mjs:161; csm-browse/package-lock.json exists; trust-test fixture string untouched | pnpm-only conversion: T001 root, T006 csm-browse, T002/T005 pnpm commands |
...
- lefthook npm postinstall auto-runs `lefthook install -f` — always install with `--ignore-scripts` (pnpm's flag: `pnpm install --ignore-scripts`) in install-hooks.mjs to keep installs deterministic; lefthook's postinstall also fires under pnpm unless ignored.
- pnpm specifics: `--frozen-lockfile` fails if the lockfile would change (CI/installer determinism); `node_modules/.bin` symlinks keep `./node_modules/.bin/lefthook`/`oxlint` working unchanged; `packageManager` field enables corepack (`corepack pnpm`), present at /usr/bin/corepack; pnpm store lives outside the repo (~/.local/share/pnpm) so no new ignores are needed.
...
`scripts/hooks/pre-commit` becomes the committed lefthook-generated shim (approach (a)). `scripts/install-hooks.mjs` is rewritten: resolve root -> `pnpm install --frozen-lockfile --ignore-scripts` -> ensure `git config core.hooksPath scripts/hooks` -> `pnpm exec lefthook install --force` -> remove `pre-commit.old` if present -> verify (`git config --get core.hooksPath`, `pnpm exec lefthook validate`). Uninstall path: `pnpm exec lefthook uninstall` + `git config --unset core.hooksPath` (restoring the pre-existing state).

**Tooling.** Root `package.json` (private, exact pins, engines >=22, `packageManager: pnpm@10.33.0`, no `"type"` — root `.mjs` files are unaffected) + committed `pnpm-lock.yaml`. csm-browse keeps its own independent `package.json` + new `pnpm-lock.yaml` (no pnpm workspace — two standalone packages).
...
- G1: T001 (root package.json + pnpm-lock.yaml) — base.
- G2: T002 (hook replacement: .lefthook.yml, shim, installer) || T004 (README + dependent-plan amendments) || T006 (csm-browse pnpm conversion) — all depend on T001; disjoint files (hooks/config/installer vs README + plan files vs csm-browse package/SKILL.md).
- G3: T003 (hook test suite rewrite) — depends on T002.
- G4: T005 (final battery + scratch-clone installer proof) — depends on T002, T003, T004, T006.
...
1. [completed] Root tooling manifest: package.json + committed pnpm lockfile
   ...
   - Owned scope: `package.json`, `pnpm-lock.yaml` (root, new)
   - Not in scope: any `csm-*/package.json` (T006); node_modules content; hook files; README
   - Spike candidate: none
   - Actions:
     1. Create root `package.json`: `name: csm-skills-suite`, `version: 0.0.0`, `private: true`, `engines.node: ">=22"`, `packageManager: "pnpm@10.33.0"`, `devDependencies: { "lefthook": "2.1.10", "oxlint": "1.79.0" }` — exact pins, no `"type"` field, no scripts.
     2. Generate the lockfile deterministically: `pnpm install --ignore-scripts` (generates pnpm-lock.yaml without running the lefthook postinstall), then verify `pnpm install --frozen-lockfile --ignore-scripts` reproduces node_modules with both binaries.
     3. Record binary versions: `node_modules/.bin/lefthook --version` and `node_modules/.bin/oxlint --version`.
   - Acceptance signal: `node_modules/.bin/lefthook --version` prints 2.1.10 AND `node_modules/.bin/oxlint --version` prints 1.79.0 AND `git status --short` shows only `package.json` + `pnpm-lock.yaml` as additions AND no `package-lock.json` at the root.
   - Validation: `pnpm install --frozen-lockfile --ignore-scripts` in a /tmp scratch copy of the two files exits 0; `node --test` untouched suites still pass (spot: package-audit); `grep -c '"packageManager"' package.json` = 1.
   - Acceptance evidence: version outputs; lockfile diff stat; scratch pnpm transcript.
   - Repair attempts: 0
   - Recovery note: revert both files to remove; re-run `pnpm install` to fix a malformed lockfile; versions are exact-pinned so drift is impossible without intent.

2. [completed] Replace the pre-commit gate with lefthook (config + shim + installer)
   - Task ID: T002
   - Depends on: T001
   - Parallel group: G2
   ...
   - Actions:
     ...
     2. Run `pnpm exec lefthook install --force` to generate `scripts/hooks/pre-commit` (the committed shim carrying the `LEFTHOOK` fingerprint); remove `scripts/hooks/pre-commit.old` if the legacy script was renamed.
     3. Rewrite `scripts/install-hooks.mjs`: resolve repo root; `pnpm install --frozen-lockfile --ignore-scripts`; ensure `git config core.hooksPath scripts/hooks` (set if absent — keep the current set-if-absent semantics); `pnpm exec lefthook install --force`; remove `pre-commit.old` if present; verify `git config --get core.hooksPath` and `pnpm exec lefthook validate`; print the same style of completion notice as today; add an uninstall path (`pnpm exec lefthook uninstall` + `git config --unset core.hooksPath`).
   ...
   - Acceptance signal: `git config --get core.hooksPath` prints `scripts/hooks` AND `pnpm exec lefthook validate` exits 0 AND `head -1 scripts/hooks/pre-commit` shows a `#!/bin/sh` lefthook shim AND `ls scripts/hooks/` contains no `pre-commit.old` AND `node scripts/check-suite.mjs` exits 0.
...
3. [completed] Rewrite the hook test suite for the lefthook gate
   - Task ID: T003
   - Depends on: T002
   - Parallel group: G3
...
4. [completed] Amend README and dependent plans (skill-suite T001, journal-learnings T002)
   - Task ID: T004
   - Depends on: T001
   - Parallel group: G2
   ...
   - Spike candidate: none
   - Actions:
     1. README.md: update Installation (`node scripts/install-hooks.mjs` section — note it now installs root devDeps via `pnpm install --frozen-lockfile` + lefthook), Quickstart optional-hook line, Repository layout tree (add `package.json`, `pnpm-lock.yaml`, `.lefthook.yml` lines if the tree lists root files; adjust the `hooks/` comment "fast advisory gate (check-suite + drift + syntax)" to "lefthook pre-commit gate (guard + check-suite + syntax + oxlint staged)"), Development & testing (hook install + hook test command), dependency-inventory note (:107) to mention the root `pnpm-lock.yaml` as the hook-tooling authority and `csm-browse/pnpm-lock.yaml` for csm-browse (replacing the package-lock.json reference; keep the `npx cyclonedx-npm` SBOM line). Also README.md:96 `npm install --no-audit --no-fund` -> `pnpm install` (csm-browse setup) and README.md:107 inventory wording.
     2. Skill-suite plan T001: mark its hook sub-items (step 5 "delete step 2 in pre-commit", step 6 "hook test SYNC assertions") as superseded-by-this-plan (T002/T003 here) with a pointer line; retarget its acceptance grep `grep -c "sync-skill-boilerplate" scripts/hooks/pre-commit` -> `grep -c "sync-skill-boilerplate" .lefthook.yml` (expected 0) and its validation `node --check scripts/hooks/pre-commit` -> `pnpm exec lefthook validate`; keep its boilerplate.mjs/sync-tooling scope untouched.
...
5. [completed] Convert csm-browse to pnpm
   - Task ID: T006
   - Depends on: T001
   - Parallel group: G2
   - Risk: standard (lockfile swap; runtime deps must stay intact)
   - Owned scope: `csm-browse/package.json` (add `packageManager` field only), `csm-browse/package-lock.json` (delete), `csm-browse/pnpm-lock.yaml` (new, committed), `csm-browse/SKILL.md:22` (install command)
   - Not in scope: csm-browse dependency versions or scripts (unchanged); README.md (T004); root package.json (T001); bootstrap pack (npm pack retained)
   - Spike candidate: none
   - Actions:
     1. Add `"packageManager": "pnpm@10.33.0"` to `csm-browse/package.json`; do NOT touch name/version/deps/scripts.
     2. Delete `csm-browse/package-lock.json`; run `pnpm install --ignore-scripts` in csm-browse to generate `pnpm-lock.yaml` from the existing package.json, then verify `pnpm install --frozen-lockfile --ignore-scripts` reproduces node_modules.
     3. Update `csm-browse/SKILL.md:22`: `npm install --no-audit --no-fund` -> `pnpm install`.
     4. Verify `cd csm-browse && node scripts/check-skill.mjs` still passes with the pnpm-installed deps.
   - Acceptance signal: `cd csm-browse && pnpm install --frozen-lockfile --ignore-scripts` exits 0 AND `ls csm-browse/package-lock.json` fails (absent) AND `git status --short -- csm-browse` shows exactly package.json + pnpm-lock.yaml modifications and the package-lock.json deletion AND `cd csm-browse && node scripts/check-skill.mjs` passes AND `rg -n "npm install" csm-browse/SKILL.md` returns no matches.
   - Validation: `pnpm install --frozen-lockfile --ignore-scripts` in a /tmp scratch copy of csm-browse (package.json + pnpm-lock.yaml) exits 0; csm-browse tests unaffected (e2e is Docker-gated, out of acceptance); `node --check` on any touched .mjs (none expected).
   - Acceptance evidence: pnpm transcripts; check-skill output; git diff --stat for csm-browse.
   - Repair attempts: 0
   - Recovery note: revert csm-browse/package.json + SKILL.md and restore `csm-browse/package-lock.json` from HEAD, then `pnpm install` regenerates the lockfile on retry; the runtime deps are unchanged so check-skill is the regression canary.

6. [completed] Final battery: scratch-clone installer proof and full gate run
   - Task ID: T005
   ...
   - Repair attempts: 0
   - Recovery note: pure verification; any failure routes to the owning task (T001 lockfile, T002 config/installer, T003 tests, T004 docs, T006 csm-browse) and is journaled before re-running this task.

## Verification Strategy

Ordered cheapest-first:
- Fast per-task gates: `node scripts/check-suite.mjs` (every task), `pnpm exec lefthook validate` (T002), `node --check` on edited .mjs (T002/T003/T004), version greps (T001), pnpm-only rg sweep (T004/T005).
- Mid-tier: rewritten hook test suite `node --test scripts/hooks/test/pre-commit.test.mjs` (T003, also re-run at T005); README integrity + matrix checks (T004); `cd csm-browse && node scripts/check-skill.mjs` (T006, re-run at T005).
- Expensive final battery (T005, sequential): scratch-clone installer/uninstall/reinstall (network via pnpm install — sandboxed under /tmp/opencode only), live commit transcript, csm-browse check-skill, full hook suite, pnpm-only sweep.
- Parallelism: per-task gates in parallel within G2; T005 sequential by design.
- Environment sensitivity: hook tests require root `node_modules` (skip-loudly when absent); pnpm install needs network only in the scratch sandbox (store + registry); csm-browse e2e (Docker) is NOT part of acceptance.
...
- Offline hook dependence on node_modules (low): fresh clones must run `install-hooks.mjs` (pnpm install) before commits; the deps-gated skip in tests and a loud installer message mitigate; bypass remains `git commit --no-verify` (unchanged behavior).
- pnpm migration risks (medium): frozen-lockfile strictness surfaces any registry drift at install time (fail fast, fix by regenerating the lockfile deliberately); pnpm's `--ignore-scripts` still suppresses the lefthook postinstall (deterministic); the pnpm store is outside the repo so no new ignore entries; two standalone packages (root + csm-browse) avoid workspace coupling (A10).
...
| 2026-08-20 | 2 | SELECT | — | T003 ready (depends T002, complete) | DISPATCH |
| 2026-08-20 | 2 | DISPATCH+INTEGRATE+VERIFY | T003 | T003 subagent rewrote scripts/hooks/test/pre-commit.test.mjs: 7 tests (config+shim validation, pass cases, guard-first, staged-only oxlint, deny-warnings, --no-verify, clean status), fixture-config harness driven by real git commit, LEFTHOOK_BIN=<repo>/.bin, deps-gated skip, temp repos cleaned. Primary VERIFY: 7/7 pass, check-suite 454, no SYNC | REVIEW |
| 2026-08-20 | 2 | REVIEW | T002 | Independent reviewer: APPROVE WITH MINOR NOTES — all 5 jobs behaviorally proven in temp repo (guard-first piped CHECK_SUITE=0, failing check-suite blocks, staged-error blocks vs untracked passes, deny-warnings blocks, csm-browse conditional incl. failure path); 7/7 suite + package-audit 1/1 + gate 454 green. Minors: (1) `csm-browse/scripts/check-skill.mjs:85` literal `npm install` in error string — FIXED to `pnpm install`; (2) uninstall leaves tracked shim deleted + double-unset throws — FIXED (shim restored via git checkout, unset guarded) | REPAIR |
| 2026-08-20 | 2 | REPAIR | T003 | Repair items integrated (check-skill.mjs:85 pnpm wording; install-hooks.mjs uninstall robustness); oxlint no-useless-escape `\$` -> `$` in test fixture (5 spots) — gate blocked the first T003 commit on these warnings (deny-warnings proof); all fixed; commit 3897d6c through hook all 5 jobs PASS | CHECKPOINT |
| 2026-08-20 | 2 | CHECKPOINT | T003 | T003 [completed] with review fixes committed (3897d6c). Next: T005 final battery (depends T002/T003/T004/T006 — all complete) | SELECT |
| 2026-08-20 | 2 | DISPATCH+VERIFY | T005 | T005 (primary): scratch-clone proof at /tmp/opencode/lh-proof-final (sandboxed HOME/TMPDIR/XDG_*) — install: pnpm frozen + lefthook install --force, hooksPath set, shim present, old-count=0, validate All good; uninstall: core.hooksPath unset, shim restored (git status clean; cosmetic lefthook 'test is a directory' warning noted); reinstall OK; csm-browse frozen install + check-skill PASS in scratch. Full battery in repo: check-suite 454 OK, hook suite 7/7, csm-browse check-skill PASS, package-audit 1/1, pnpm-only word-boundary sweep zero matches. Live-commit proof already recorded (61736b9, 3897d6c — all five jobs) | COMPLETE |
| 2026-08-20 | 2 | COMPLETE | all | Completion Gate (primary, personal): 6/6 tasks completed with acceptance evidence; AC1-AC7 evidenced (versions, gate behavior, staged-only, tests, docs, csm-browse, battery); independent T002 review approved with minors, both fixed and re-verified; working tree contains only this plan's changes; all work committed (8042ad1, 61736b9, 3897d6c + final); nothing pushed | STOP |

## Completion Review

All 6 tasks completed and evidenced across cycles 1-2:

1. **T001** — root `package.json` (exact pins lefthook@2.1.10, oxlint@1.79.0, packageManager pnpm@10.33.0) + committed `pnpm-lock.yaml`; frozen-lockfile repro proven in scratch; no package-lock.json. Committed `8042ad1`.
2. **T002** — `.lefthook.yml` (piped 5-job chain, `assert_lefthook_installed: true`, `output` at root per schema), committed lefthook shim at `scripts/hooks/pre-commit` (core.hooksPath retained), `install-hooks.mjs` rewritten (pnpm frozen install, install --force, .old cleanup, --uninstall path restoring the tracked shim). Independent review: **APPROVE WITH MINOR NOTES**; both minors fixed (check-skill pnpm wording; uninstall robustness). Live-commit proof `61736b9` ran all five jobs.
3. **T003** — hook suite rewritten (7/7): config+shim validation, pass cases, guard-first (no CHECK_SUITE on guard failure), staged-only oxlint (staged error blocks / untracked does not), deny-warnings, --no-verify bypass, clean status; no /SYNC/ assertions. The gate blocked the build's own first commit on `no-useless-escape` warnings — deny-warnings proven in production. Committed `3897d6c`.
4. **T004** — README (pnpm install, lefthook gate, layout tree incl. .lefthook.yml, dependency inventory, dev/testing), `.agents/README.md` index restored (3 new plan lines + consolidated status refresh), skill-suite plan T001 hook sub-items superseded + acceptance/validation retargeted, journal-learnings plan T002 pointer to .lefthook.yml job.
5. **T006** — csm-browse converted: packageManager field, package-lock.json deleted, pnpm-lock.yaml (v9), SKILL.md:22 -> pnpm install; frozen install + check-skill PASS (real + scratch).
6. **T005** — scratch-clone install/uninstall/reinstall cycle + csm-browse frozen install (sandboxed /tmp/opencode/lh-proof-final); full battery: check-suite 454 OK, hook suite 7/7, csm-browse check-skill PASS, package-audit 1/1, pnpm-only sweep (word-boundary `\bnpm (install|ci)\b`) zero matches.

**Result**: the repo now installs packages only via pnpm (root + csm-browse), and every commit runs the lefthook pre-commit gate: unstaged-guard -> check-suite (454 checks) -> staged-.mjs syntax -> staged-only oxlint (errors AND warnings fail) -> conditional csm-browse check. Nothing pushed (requires explicit user request).
