...
- check-suite has zero Control/journal/ordinal/template-marker validation (grep: no `Control`, `Next transition`, or list-ordinal checks); corpus drift exists today (2026-08-19-consolidated-remaining-work-csm.md Control says `Next transition: SCOPE`, not a build-machine state).
...
1. [pending] Condense shared boilerplate, add the ladder quota rung, and remove doubled pre-commit drift check
   ...
   - Actions:
     ...
     5. In scripts/hooks/pre-commit, delete step 2 (the `sync-skill-boilerplate.mjs --check` invocation) because check-suite.mjs:646-649 already runs checkDrift internally; renumber remaining comments/steps; keep the syntax-check and csm-browse steps.
     6. Update scripts/hooks/test/pre-commit.test.mjs: drop the SYNC-related assertions (lines ~65, 89, 104, 118 assert /SYNC/ and CHECK_SUITE-before-SYNC ordering) and assert CHECK_SUITE runs once; keep the unstaged-changes rejection test.
   - Acceptance signal: `node scripts/sync-skill-boilerplate.mjs --check` exits 0 AND `node scripts/check-suite.mjs` exits 0 AND `grep -c "sync-skill-boilerplate" scripts/hooks/pre-commit` shows exactly 0 occurrences AND `node scripts/hooks/test/pre-commit.test.mjs` passes (its own node:test run).
   - Validation: `wc -w` of the tmux section body in each of the 5 skills <= 150 words; phrase checklist present in each tmux body (TMUX check, skip conditions, `-2`/`-3` suffix, notice, end-invocation rule); quota rung text present in the 4 laddered skills; `node --check scripts/hooks/pre-commit scripts/hooks/test/pre-commit.test.mjs`; a dry `git diff --stat` shows no changes outside the synced sections, the hook, and its test.
...
6. [pending] check-suite and plan-validation module: Control/journal/ordinal/template/interface validation and corpus repair
   ...
   - Actions:
     ...
     4. Repair the single known corpus drift: 2026-08-19-consolidated-remaining-work-csm.md `Next transition: SCOPE` -> `none (terminal)` with a journal row recording the repair.
