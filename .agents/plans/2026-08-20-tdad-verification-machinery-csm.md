...
- Cycle: 0
- Commits: allowed
- Last checkpoint: 2026-08-20T21:25Z — plan saved (SAVED); baseline re-recorded for the tracked count; committed
...
Implement Phase 1 of the TDAD verification layer in this repo: every csm-build iteration emits a versioned, content-hashed evidence manifest with honest `pass` / `fail` / `inconclusive` outcomes, driven by a plan-level Verification Contract and diff-grounded test selection; a new TRIAGE state at the start of the build detects novel ecosystems and, when one is found, dispatches a scoped csm-deep-research run that enriches an ecosystem knowledge base and drafts a PR contribution back to this repo (human-sent; nothing is ever pushed by the build).
...
2. `csm-build/SKILL.md` — new TRIAGE state (`RECOVER -> TRIAGE -> VALIDATE -> ...`), scoped csm-deep-research dispatch protocol, three-state VERIFY/CHECKPOINT semantics, `inconclusive` -> BLOCKED decision protocol, per-cycle manifest emission, `Last manifest:` Control bullet, evidence manifest added to the Interface Produces bullet.
...
1. `scripts/lib/verification.mjs` exports the documented API (including `buildPackageMap`) and `node --test tests/verification.test.mjs` passes: contract parsing (this plan's own contract block, absent, malformed, nested-map variants), changed-file collection against a temp git fixture, discovery + selection on fixtures, status derivation for all pass/fail/inconclusive combinations, manifest emit/validate round-trip, and the score-threshold mapping.
2. csm-build/SKILL.md contains the TRIAGE state in its chain with a scoped deep-research dispatch protocol, the `inconclusive` -> BLOCKED protocol, and per-cycle manifest emission; `node scripts/check-suite.mjs` passes (machine, matrix, interface, payload, F-052 checks green) after each task's pack-bootstrap re-sync.
...
| AD-08 | Novel-ecosystem signal = KB miss for the detected primary test framework (or no framework detected); one dispatch per ecosystem per repo | decision | keeps TRIAGE dispatch rare and deliberate | accepted |
...
4. **Three-state semantics**: `deriveOverallStatus(checks, contract)` — per-check status `pass | fail | unavailable | not-applicable`; overall = `fail` if any required check failed; else `inconclusive` if any required check is `unavailable` or `not-applicable` (with the missing-evidence list); else `pass`. `not-applicable` outside required evidence never blocks.
5. **Manifest** (`emitManifest`, `validateManifest`): JSON at `.agents/evidence/<plan-id>-<cycle>-manifest.json`; schema v1 — `schemaVersion`, `change{gitSha, baseSha, diffHash, changedFiles[]}`, `contract{id?, riskTier, changeClass, invariants[]}`, `testSelection{algorithmVersion, confidence, required[], recommended[]}`, `checks[]{name, category, status, command, durationMs, selectedTests[], junitRef?, baselineFailure?}`, `learningRequest?{ecosystem, status}`, `policy{riskTier, decision, escalations[]}`, `overall{status, generatedAt}`. `diffHash` = sha256 of `git diff HEAD` output. Content hash = sha256 of the canonical JSON minus the hash field. Lifecycle per AD-05/AD-06: the latest cycle's manifest persists untracked and is overwritten at each CHECKPOINT; baseline attribution reads the prior manifest (match key = check name + command + sorted selectedTests); the final-cycle manifest is committed with the plan at COMPLETE.
6. **csm-build TRIAGE state** (new `### 2. TRIAGE`): (1) collect changed files + classify change class/risk from the contract; (2) detect novel-ecosystem signal (KB miss for the detected primary runner — KB = `.agents/ecosystems/<ecosystem-slug>.md`; slug rule per AD-16); (3) on signal, offer the human a scoped csm-deep-research dispatch (question: "What test-impact/verification tooling exists for <ecosystem>?" + goal-slug `<ecosystem-slug>-verification-tooling`); on acceptance, dispatch by name (terminal run writing only `.agents/research/`), curate findings into the KB entry, draft `.agents/ecosystems/drafts/<date>-<ecosystem-slug>-pr.md` (title, body, changed files, exact `gh pr create` command), record `learningRequest{status: learned|declined|failed}`; (4) pass changed files + selection input to VALIDATE.
7. **VERIFY/CHECKPOINT semantics**: VERIFY runs checks in the contract's `requiredEvidence` order (cheapest-first per task), recording per-check status + command + durationMs + selectedTests; CHECKPOINT derives overall, emits the manifest, journals `- Last manifest: <path>`, and on `inconclusive` transitions to BLOCKED with exactly one numbered question (accept-with-residual-risk / broaden-verification / repair) per the Blocker Rules format; `fail` follows the existing REPAIR path.

### Interfaces

- `scripts/lib/verification.mjs` exports: `parseVerificationContract`, `collectChangedFiles`, `discoverTestFiles`, `buildStaticImportMap`, `buildPackageMap`, `selectAffectedTests`, `deriveOverallStatus`, `emitManifest`, `validateManifest`.
...
- Ecosystem KB entry shape: `## Overview`, `## Test frameworks and discovery`, `## Affected-test commands`, `## Mutation/property tooling`, `## CLI recipes`, `## Sources` (dated, URL + retrieval date).
...
1. [pending] Implement verification lib + unit tests
   ...
   - Actions:
     1. Write `scripts/lib/verification.mjs` per Design section (zero-dep ESM, 'use strict', JSDoc header; export the 9 documented functions).
     ...
     4. Manifest emit/validate per schema v1 (content hash = sha256 of canonical JSON minus hash field).
     5. Write `tests/verification.test.mjs` (node --test) + fixtures: contract parse on this plan's own block (incl. its nested `humanApproval` map), absent/malformed variants; changed-file collection against a temp git fixture; discovery + selection on `tests/fixtures/verification/` (fake package tree with import edges); score-threshold mapping per AD-15; deriveOverallStatus for all pass/fail/inconclusive combinations; manifest emit/validate round-trip in a temp dir.
...
2. [pending] Add TRIAGE state and three-state semantics to csm-build; extend BUILD_MACHINE
   ...
   - Actions:
     ...
     3. Write the TRIAGE section: diff collection, change classification, novel-ecosystem detection (KB miss; slug rule AD-16), the scoped csm-deep-research dispatch protocol (offer to the human; question + goal-slug format; consume only `.agents/research/` findings; curate KB entry `.agents/ecosystems/<ecosystem-slug>.md`; draft PR file `.agents/ecosystems/drafts/<date>-<ecosystem-slug>-pr.md` with title/body/changed-files/exact `gh pr create` command; never push), `learningRequest` recording, and the KB entry shape (Design section).
     4. Update the VERIFY section: per-check statuses `pass|fail|unavailable|not-applicable`, run order per contract `requiredEvidence`, record command + durationMs + selectedTests, baseline-failure attribution via prior-cycle manifest comparison (match key per AD-06).
     5. Update CHECKPOINT: emit manifest via the lib, add `- Last manifest: <path>` Control bullet, derive overall status; `inconclusive` -> BLOCKED with exactly one numbered question (accept-with-residual-risk / broaden-verification / repair); `fail` -> REPAIR as today.
     6. Update the Blocker Rules section with the inconclusive decision protocol; update the Completion Gate to require the final manifest and its journal reference.
...
| TRIAGE dispatch adds a heavy deep-research run to builds | low (only on KB miss) | medium | human-gated offer (AD-13); one dispatch per ecosystem per repo; decline path records unavailable evidence |
