...
| `csm-build` | saved CSM plan, explicit implementation request | saved plan, optional NORMS.md, BDD/TDD package when present, optional csm-ddd analysis artifacts when the plan cites them | verified implementation, delivery evidence | delivery evidence to a separately invoked csm-browse |
...
| `csm-browse` | need to drive a headful Chromium browser | browser session, CDP verbs, delivery target | screenshots, videos, DOM, console, network, or performance evidence | evidence files to a separately invoked csm-upload |
| `csm-upload` | evidence files ready, configured GitHub Pages destination | screenshots, videos, or evidence files, GitHub configuration | dated GitHub Pages demo page | expected evidence URL to the user; verify Pages deployment separately |
...
|---|---|---|---|---|---|---|---|
| `csm-grill` | `INTAKE -> ... -> SAVED -> STOP` | one approach document; temp notes are disposable | explicit authorization only | not resumable before `SAVED` | `.agents/approaches/<date>-<slug>-approach.md` | rough idea, repository evidence, retrievable research | handoff to a separately invoked `csm-plan` |
| `csm-plan` | `INTAKE -> ... -> SAVED -> STOP` | draft sidecar, then one plan document | explicit authorization only | draft/control state resumes planning | `.agents/plans/<date>-<slug>-csm.md` | brief, repository evidence, optional norms/research/DDD artifacts | handoff to a separately invoked `csm-build` or `csm-bdd-tdd` |
| `csm-bdd-tdd` | `INTAKE -> ... -> SAVED -> STOP` | specs, scenarios, validation, test designs, mutated plan | explicit authorization only | `specs/control.md` resumes mutation | mutated plan plus specs folder | saved non-BDD plan and optional norms | handoff to a separately invoked `csm-build` |
| `csm-make-tests` | `INTAKE -> ... -> OUTPUT` | approved tests, fixtures, benchmarks, ledger, verification report | never unless explicitly authorized | ledger/report cursor resumes maintenance | `.agents/tests/*-tests-ledger.md` and `*-verification.md` | pinned checkout, optional norms/research | suite and reports return to user or later build |
| `csm-build` | `RECOVER -> ... -> CHECKPOINT` | owned implementation, updated plan, checkpoints | explicit authorization only | plan Control/journal cursor resumes build | verified implementation and updated plan | saved plan, optional norms/BDD/DDD artifacts | delivery returns to human; browse is separate |
| `csm-review` | `INTAKE -> ... -> SAVED -> STOP` | one findings report; sandbox is disposable | never by default; explicit report commit only | report is terminal, not a work cursor | `.agents/reviews/<date>-<repo>-review.md` | pinned target and optional norms | findings feed a later explicit plan |
| `csm-review-python` | `INTAKE -> ... -> REPORT -> STOP` | one doctrine report; temporary notes only | no implicit commit | not resumable before `REPORT` | `.agents/doctrine/<date>-<repo>-python-doctrine-review.md` | pinned Python checkout and doctrine artifacts | report returns to user or dispatching review |
| `csm-scan` | `INTAKE -> ... -> SAVED -> STOP` | one `NORMS.md`; target is read-only | no implicit commit | no durable mid-run cursor | `NORMS.md` | repository path(s) and static declarations | file handoff to later skills |
| `csm-ddd` | `INTAKE -> ... -> RENDER -> STOP` | report and graph only; atomic pair publication | no implicit commit | no durable mid-run cursor | `.agents/ddd/*-ddd-report.md` and `*-ddd-graph.json` | pinned repository, optional norms/questions | report and graph return to user |
| `csm-deep-research` | `INTAKE -> ... -> SAVED -> STOP` | one finding plus declared artifacts; temp notes disposable | explicit authorization only | finding Control journal resumes supported runs | `.agents/research/<date>-<slug>-research.md` | research question and retrievable sources | finding returns to user or dispatching skill |
...
- **Output:** `.agents/plans/<date>-<goal-slug>-csm.md`. A commit is explicit-authorization-only; when authorized, use a path-scoped commit and inspect the staged diff so unrelated work is not included.
...
- **Outputs:** executable test files and approved goldens in the target repository, plus `.agents/tests/<date>-<repo-slug>-tests-ledger.md` and `.agents/tests/<date>-<repo-slug>-verification.md`.
...
- **Output:** `.agents/doctrine/<date>-<repo-slug>-python-doctrine-review.md`, containing one evidence-grounded findings report and its remediation guide.
...
Terminal evidence publisher: `node $HOME/.config/opencode/skills/csm-upload/scripts/upload.mjs --label <name> [--desc <text>] [--github <user>] [--repo <name>] <file...>` copies evidence into a dated directory of your GitHub Pages repo (default `csm-browse-pages`; configurable via `~/.agents/csm-upload.json` or flags), commits, pushes, and prints an expected URL. The commit and push are not proof that the effective remote or Pages deployment is correct or publicly live: verify the remote, Pages configuration, deployment status, and resulting URL separately. Requires an authenticated `gh` CLI and a Pages-enabled repo. Review artifacts for credentials, personal data, private URLs, and sensitive DOM/network metadata before public publication. _Full reference: [csm-upload/SKILL.md](csm-upload/SKILL.md) — Usage._
