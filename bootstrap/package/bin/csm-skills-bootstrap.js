...
const USAGE = 'usage: csm-skills-bootstrap --version | payload-index | verify <envelope.json> | --help';
...
const SHELL_DENYLIST = /\b(npx|npm|node|nodejs|bash|sh|python|python3|pip|pip3|git|curl|wget|sudo|rm|powershell|eval|exec|chmod|chown|docker|uvx|bunx|deno)\b/i;
const FIXED_PACKAGE_POLICY = { name: '@jamiemills/csm-skills-bootstrap', version: '0.1.0', bin: 'csm-skills-bootstrap', registry: 'https://registry.npmjs.org' };
...
const policyToSign = envelope => ({
  ...
  ...(Object.prototype.hasOwnProperty.call(envelope, 'payload_index_sha256') ? { payload_index_sha256: envelope.payload_index_sha256 } : {})
});
...
const checkStepsShellPolicy = steps => steps.includes('`') || steps.includes('~~~') || SHELL_DENYLIST.test(steps);
...
function validateEnvelope(envelope, keyring, { now = new Date(), indexSha256 } = {}) {
  ...
  const allowed = ['audience', 'expires_at', 'key', 'payload_index_sha256', 'policy', 'schema', 'signature', 'steps_markdown', 'steps_sha256'];
  const required = ['audience', 'expires_at', 'key', 'policy', 'schema', 'steps_markdown', 'steps_sha256'];
  for (const field of required) if (!Object.prototype.hasOwnProperty.call(envelope, field)) reject('SCHEMA', `missing field ${field}`);
  for (const field of Object.keys(envelope)) if (!allowed.includes(field)) reject('UNEXPECTED_FIELD', `unsigned field ${field} is not allowed`);
  ...
  if (key.fingerprint !== envelope.key.fingerprint || sha256(Buffer.from(key.public_key_der_base64, 'base64')) !== key.fingerprint) reject('FINGERPRINT', 'fingerprint mismatch');
  ...
  if (checkStepsShellPolicy(envelope.steps_markdown)) reject('SHELL_POLICY', 'steps cannot define executable policy');
  if (sha256(envelope.steps_markdown) !== envelope.steps_sha256) reject('STEPS_DIGEST', 'steps digest mismatch');
  if (!checkFixedPackagePolicy(envelope.policy?.package)) reject('PACKAGE_POLICY', 'package policy is not fixed');
  if (indexSha256 !== undefined && Object.prototype.hasOwnProperty.call(envelope, 'payload_index_sha256') && indexSha256 !== envelope.payload_index_sha256) {
  ...
  if (hasSignature && (envelope.signature === null || typeof envelope.signature !== 'object' || Array.isArray(envelope.signature))) reject('UNSIGNED', 'signature is malformed');
  if (hasSignature && envelope.signature !== null) {
    if (envelope.signature.algorithm !== 'Ed25519' || typeof envelope.signature.value !== 'string' || envelope.signature.value === '') reject('UNSIGNED', 'signature is missing');
    const publicKey = createPublicKey({ key: Buffer.from(key.public_key_der_base64, 'base64'), format: 'der', type: 'spki' });
    if (!verify(null, Buffer.from(canonicalJson(policyToSign(envelope))), publicKey, Buffer.from(envelope.signature.value, 'base64'))) reject('BAD_SIGNATURE', 'signature verification failed');
  }
...
async function verifyEnvelope(envelopePath) {
  if (!envelopePath) throw Object.assign(new Error('verify requires an envelope JSON path'), { code: 'USAGE' });
  ...
  try {
    ...
    throw Object.assign(new Error('payload-index.json is missing from the package'), { code: 'MISSING_PAYLOAD_INDEX' });
  }
  const trusted = validateEnvelope(envelope, JSON.parse(KEYRING_JSON), { now: new Date(), indexSha256 });
