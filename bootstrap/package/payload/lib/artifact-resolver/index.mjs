...
function recordSourceDigest(record) {
  return (
    record?.sourceDigest ??
    record?.digest ??
...
export function createArtifactResolver({
  ...
  async function resolveArtifact(
    ...
    try {
      ...
      if (
        requireSourceDigest &&
        validated.some((record) => !/^sha256:[a-f0-9]{64}$/.test(recordSourceDigest(record) ?? ""))
      )
...
export async function resolveArtifactFile(
  ...
  return createArtifactResolver({
    ...
    compatibility,
    owner,
  }).resolve(path, { expectedDigest, expectedArtifactId, consumerRevision });
}
