...
function machineInputPath(root, path) {
  if (typeof path !== "string" || !path || isAbsolute(path))
    throw Object.assign(new Error("path is not relative"), { code: "path-traversal" });
  const target = resolve(root, path);
  const outside = relative(resolve(root), target);
  if (!outside || outside === ".." || outside.startsWith(`..${sep}`) || isAbsolute(outside))
    throw Object.assign(new Error("path escapes input root"), { code: "path-traversal" });
  return target;
}

async function assertMachineInputFile(root, path) {
...
function machineInputRejected(code, message, details = {}) {
  return Object.freeze({ status: "rejected", code, message, ...details });
}

export async function loadMachineInput(input, { root = process.cwd() } = {}) {
  if (typeof input === "string") {
    if (/\.(?:md|html?)$/i.test(input))
      return machineInputRejected(
        "migration-required",
        "legacy projections require explicit JSON reconstruction",
        {
          path: input,
          readOnly: true,
        },
    ...
    if (!/\.jsonl?$/i.test(input))
      return machineInputRejected("unsupported-format", "machine inputs must be canonical JSON", {
        path: input,
      });
    try {
      const path = machineInputPath(root, input);
      await assertMachineInputFile(root, path);
      const value = parseJson(await readFile(path, "utf8"));
      assertMachineInput(value);
      return Object.freeze({ status: "loaded", value, path: input });
    } catch (error) {
      return machineInputRejected(error.code ?? "invalid-json", error.message, { path: input });
    }
  }
  if (
    input?.schema === "csm-projection/1" ||
    input?.mediaType === "text/markdown" ||
    input?.mediaType === "text/html"
  )
    return machineInputRejected(
      "projection-input",
      "projection descriptors are not machine inputs",
    );
  try {
    assertMachineInput(input);
    return Object.freeze({ status: "loaded", value: input, path: null });
  } catch (error) {
    return machineInputRejected(error.code ?? "machine-input-rejected", error.message);
  }
}
