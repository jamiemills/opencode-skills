...
TASK PROGRESS  [████████████████░░░░░░░░░░░░] 53%
Milestones
[Recover ✓ 20%] [Research ✓ 15%] [Design ▶ 45%] [Verify ○ 20%]
```
...
3. If this invocation is already inside an agent session — `CSM_AGENT_SESSION_EXEC=1` or any other `CSM_AGENT_SESSION_*` marker is set, i.e. it is not an interactive human CLI, do not start a detached session and do not end the invocation: skip this step and step 4 and continue the planning workflow in-process (the step 2 rename may still apply). Otherwise, if not in tmux, and the user did not forbid tmux or choose another multiplexer, write the original request to a mode-600 temporary prompt file, then launch it without shell interpolation: `tmux new-session -d -s "$session" -- <agent-cli> run --prompt-file "$prompt_file"`; verify the launched invocation received the exact request before ending this invocation.
4. Print the active session name and attach command: `tmux attach-session -t csm-plan-<goal-slug>`. Only if a new detached session was launched and no agent-session marker is set (an interactive human CLI) end the invocation — tmux does the planning from the start; when this invocation is already inside an agent session — `CSM_AGENT_SESSION_EXEC=1` or any other `CSM_AGENT_SESSION_*` marker is set, i.e. it is not an interactive human CLI, never end the invocation and continue the planning workflow in-process instead.
...
Authentic if it contains either "Generated by csm-scan" or "## Repository Overview". If absent, warn "NORMS.md found but does not appear to be csm-scan output — conventions may not be accurate" and proceed.
...
Each acceptance signal is exactly one positive, runnable assertion with an expected result. Do not write disjunctive signals (`Either ... OR ...`) and never offer an alternative that merely records, documents, waives, skips, or defers the work; that is an escape hatch, not acceptance. A genuine deferral is a `blocked` task carrying the concrete user decision it waits on, never a `pending` task whose signal can "pass" by not doing the work. Run `node csm-plan/lib/loop-evaluator.mjs lint --record <plan>` during DRAFT and resolve every reported signal error before advancing.
...
- every acceptance signal is a single positive assertion; no disjunctive or deferral escape hatch survives, and every genuine deferral is a `blocked` task naming the user decision it waits on;
...
Enforcement is deterministic and in-loop; it never depends on model obedience. This section applies to every cycle of the state machine and to any terminal action on a plan.

1. **Per-cycle evaluator (mandatory).** Before the state machine advances from any state, and always before SAVED or any plan closure, dispatch a fresh, independent evaluator subagent that did not author the current state. It reads only the durable plan record — Control, journal, task statuses, current-state evidence, and the goal/acceptance contract — and returns exactly one binding verdict: `continue`, `complete`, or `blocked`, with evidence. The evaluator must not see or restate chat history as evidence, and it must find its own supporting citations in the record. Journal the verdict as a `csm-plan-evaluator-receipt/1` receipt in the Progress Journal before advancing; a cycle without a journaled receipt does not count as evaluated. The receipt's inputs are restricted to durable state, matching `EVALUATOR_CONTRACT` in `lib/plan.mjs`.
2. **Deterministic loop guard.** Run `node csm-plan/lib/loop-evaluator.mjs guard --record <plan-path>` each cycle and immediately before any terminal action. Exit `0` means no outstanding work and is the only code that permits terminalization; exit `2` means work remains (the loop must continue or recover); exit `1` (unreadable/malformed record) is a fail-closed error that also blocks terminalization. The guard is status-only and deterministic: any non-terminal task, a non-empty `control.activeTasks`, or a non-terminal lifecycle status is outstanding work. It reads the durable record, not the transcript, so skipping the evaluator cannot manufacture completion.
3. **Closure requires a passing receipt.** A plan with pending, in-progress, or blocked tasks cannot be closed as `complete`, and `closePlan` refuses without a journaled `verdict: complete` receipt and a clear task predicate. Use `closePlan` for completion closure; a non-passing verdict cannot advance to a terminal state.
