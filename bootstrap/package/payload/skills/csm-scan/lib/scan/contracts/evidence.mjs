...
export function assertDataOnly(value, ErrorType = EvidenceContractError, limits = DATA_LIMITS) {
  ...
  while (stack.length > 0) {
    ...
    if (++nodes > maximums.maxNodes)
      fail(ErrorType, "NODE_LIMIT", "input exceeds the data node limit");
    if (item.depth > maximums.maxDepth)
      fail(ErrorType, "DEPTH_LIMIT", "input exceeds the data depth limit");
    const current = item.value;
    if (typeof current === "string") {
      if (current.length > maximums.maxString)
        fail(ErrorType, "STRING_LIMIT", "input exceeds the string limit");
      continue;
    }
    ...
    if (item.ancestors.has(current))
      fail(ErrorType, "DATA_ONLY", "input must be an acyclic data tree");
    ...
    for (const key of names) {
      ...
      if (!Object.hasOwn(descriptor, "value"))
        fail(ErrorType, "DATA_ONLY", "accessor fields are not allowed");
      ...
      if (!array && descriptor.enumerable !== true)
        fail(ErrorType, "DATA_ONLY", "hidden fields are not allowed");
...
function stableId(value, pattern, field) {
  if (typeof value !== "string" || value.length > 96 || !pattern.test(value)) {
    fail(
      ...
      "INVALID_ID",
      `${field} must be a stable versioned ASCII identifier`,
    );
  }
  return value;
}
...
function boundedInteger(value, maximum, field, positive = false) {
  if (!Number.isSafeInteger(value) || value < (positive ? 1 : 0) || value > maximum) {
    fail(
      ...
      `${field} is outside the explicit search bound`,
    );
  }
  return value;
}
...
export function normalizeSearchSpace(value) {
  ...
  for (const key of [
    ...
    if (typeof value[key] !== "boolean")
      fail(EvidenceContractError, "INVALID_SEARCH_SPACE", "search-space state must be boolean");
  }
  const result = {
    ...
    recordsInspected: boundedInteger(
      value.recordsInspected,
  ...
  if (
    result.filesInspected > result.fileLimit ||
    result.bytesInspected > result.byteLimit ||
    result.recordsInspected > result.recordLimit
  ) {
    fail(
      EvidenceContractError,
      "INVALID_SEARCH_SPACE",
      "inspected counts must not exceed disclosed limits",
    );
  }
  if (
    result.complete &&
    ...
    fail(
      EvidenceContractError,
      "INVALID_SEARCH_SPACE",
      "complete search-space state is inconsistent",
    );
  }
...
function normalizeApplicabilityDetails(value) {
  ...
  if (!Array.isArray(value.facts) || value.facts.length === 0 || value.facts.length > 16) {
    fail(
      EvidenceContractError,
      "INVALID_DETAILS",
      "applicability facts must be a bounded non-empty array",
    );
  }
    ...
    .map((fact) => {
      ...
      if (!fact.present && fact.value !== null) {
        fail(
          EvidenceContractError,
          "INVALID_DETAILS",
          "absent applicability fact must have null value",
        );
      }
      return {
        field: fact.field,
        present: fact.present,
        value: fact.present ? factValue(fact.field, fact.value) : null,
      };
    })
    .toSorted((left, right) =>
      compareAscii(
...
function normalizeDetails(category, details) {
  ...
  if (details !== null)
    fail(EvidenceContractError, "INVALID_DETAILS", "ordinary evidence details must be null");
  return null;
}
...
export function normalizeEvidencePath(value) {
  if (
    ...
    fail(
      EvidenceContractError,
      "INVALID_PATH",
      "path must be a normalized POSIX repository-relative path",
    );
  }
  return value;
}
...
function normalizeCore(fields) {
  ...
  if ((fields.category === "search_space") !== (fields.sourceKind === "search_result")) {
    fail(
      ...
      "search evidence category and source kind must agree",
    );
  }
  return {
    claimId,
...
export function validateEvidence(record) {
  ...
  if (id !== identityFor(core))
    fail(EvidenceContractError, "ID_MISMATCH", "id does not match canonical evidence content");
