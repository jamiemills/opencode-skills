...
export const PROVIDER_CATEGORIES = deepFreeze(Object.fromEntries(
  PROVIDER_DIMENSION_IDS.map((id) => [id, DIMENSION_EVIDENCE_CATEGORIES[id]]),
));
...
function normalizeCapability(capability) {
  ...
  if (!Array.isArray(capability.categories) || capability.categories.length === 0
      || capability.categories.length > allowed.length) {
    fail('BOUND_EXCEEDED', 'provider categories must be a bounded non-empty array');
  }
  const categories = capability.categories.map((category) => {
...
export function validateProvider(provider) {
  ...
  if (provider.apiVersion !== PROVIDER_SCHEMA_VERSION) fail('INVALID_VERSION', 'provider apiVersion is unsupported');
  if (typeof provider.id !== 'string' || provider.id.length > 96 || !PROVIDER_ID_PATTERN.test(provider.id)) {
    fail('INVALID_ID', 'provider id must be a stable versioned ASCII identifier');
  }
  if (!Array.isArray(provider.dimensions) || provider.dimensions.length === 0
...
export function validateProviders(providers) {
  ...
  if (!Array.isArray(providers) || providers.length > 256) fail('BOUND_EXCEEDED', 'providers must be a bounded array');
