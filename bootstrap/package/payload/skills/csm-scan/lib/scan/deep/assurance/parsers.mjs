...
export function classifyAssurancePath(path) {
  ...
  if (Object.hasOwn(LOCK_FILES, base)) {
    ...
    return { kind: 'lock', format: base === 'deno.lock' || base === 'Pipfile.lock' || base === 'composer.lock' || base === 'package-lock.json' || base === 'npm-shrinkwrap.json' ? 'json' : 'text' };
  }
  if (isDeepManifest(path)) return { kind: 'manifest', format: /\.json$/i.test(base) ? 'json' : 'text' };
  if (base === 'Gemfile' || base === 'go.mod' || base === 'Pipfile' || base === 'setup.cfg' || base === 'setup.py') {
  ...
  if (/\.sbom\.json$/i.test(base) || base === 'bom.json' || base === 'cyclonedx.json' || base === 'sbom.json'
      || /\.cdx\.json$/i.test(base) || /\.spdx\.json$/i.test(base) || base === 'spdx.json') {
  ...
  if (/\.intoto\.jsonl$/i.test(base) || /\.att$/i.test(base) || /\.attestation(?:\.json)?$/i.test(base)
      || /\.sigstore\.json$/i.test(base)) {
    return { kind: 'attestation', format: 'text' };
  }
  if (/^provenance(?:\.intoto)?\.jsonl?$/i.test(base)) return { kind: 'attestation', format: 'json' };
  return null;
}
...
function boundedToken(value, maximum = 128) {
  if (typeof value !== 'string' || value.length === 0 || value.length > maximum
      || /[^\x20-\x7e]/.test(value) || value !== value.trim()) return null;
  return value;
}
...
function licenseIdentifier(value) {
  const token = String(value).split(/\s+(?:AND|OR)\s+/i)[0].trim();
  if (typeof token !== 'string' || token.length === 0 || token.length > 128
      || !/^[A-Za-z0-9][A-Za-z0-9.+-]*$/.test(token)) return null;
  return token;
}
...
function licenseFromDeclared(value, path, records) {
  if (typeof value === 'string') {
    ...
    if (id !== null) {
      records.push(candidate('license', path, 'observed', { declared: 'manifest', identifier: id }));
    }
    return;
  }
...
function parsePackageJson(value, path) {
  ...
  records.push(candidate('manifest', path, 'observed', { format: 'package_json', ecosystem: 'javascript' }));
...
function parseRequirements(text, path) {
  const records = [];
  const diagnostics = [];
  records.push(candidate('manifest', path, 'observed', { format: 'requirements', ecosystem: null }));
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith('-i') || line.startsWith('--index-url') || line.startsWith('--extra-index-url')) {
    ...
    if (eq && pinCount < ASSURANCE_LIMITS.pins) {
      ...
      if (name !== null && version !== null) {
        records.push(candidate('pin', path, 'observed', { package: name, scope: 'requirements', version }));
        pinCount++;
      }
    }
  }
...
function parsePyproject(text, path) {
  ...
  records.push(candidate('manifest', path, 'observed', { format: 'pyproject_toml', ecosystem: 'python' }));
    ...
    if (Array.isArray(project.dependencies)) {
      ...
      for (const spec of project.dependencies) {
        ...
        if (eq) {
          ...
          const version = boundedToken(eq[2], 128);
          if (name !== null && version !== null) {
            records.push(candidate('pin', path, 'observed', { package: name, scope: 'manifest', version }));
            pinCount++;
          }
        }
      }
  ...
  if (tool !== null && typeof tool === 'object' && !Array.isArray(tool)) {
    const uv = tool.uv;
    if (uv !== null && typeof uv === 'object' && !Array.isArray(uv) && typeof uv['index-url'] === 'string') {
      boundedPush(records, sourceRecord(path, 'index', uv['index-url'], 'index'), PARSER_SOURCE_LIMIT);
    }
    const pip = tool.pip;
    if (pip !== null && typeof pip === 'object' && !Array.isArray(pip) && typeof pip['index-url'] === 'string') {
      boundedPush(records, sourceRecord(path, 'index', pip['index-url'], 'index'), PARSER_SOURCE_LIMIT);
    }
    const poetry = tool.poetry;
    if (poetry !== null && typeof poetry === 'object' && !Array.isArray(poetry) && poetry.license != null) {
...
function parseCargoToml(text, path) {
  ...
  records.push(candidate('manifest', path, 'observed', { format: 'cargo_toml', ecosystem: 'rust' }));
...
function parseComposerJson(value, path) {
  ...
  records.push(candidate('manifest', path, 'observed', { format: 'composer_json', ecosystem: 'php' }));
  licenseFromDeclared(value.license, path, records);
...
function parseManifest({ path, text, value, _format }) {
  ...
  if (formatName !== undefined) {
    return { records: [candidate('manifest', path, 'observed', { format: formatName, ecosystem: null })], diagnostics: [] };
  }
  ...
  if (packages !== null && typeof packages === 'object' && !Array.isArray(packages)) {
    ...
    for (const [key, entry] of Object.entries(packages)) {
      ...
      if (name !== null && boundedToken(name) !== null && pinCount < ASSURANCE_LIMITS.pins) {
        records.push(candidate('pin', path, 'observed', { package: name, scope: 'lockfile', version: entry.version }));
        pinCount++;
      }
      if (typeof entry.resolved === 'string') {
        boundedPush(records, sourceRecord(path, 'registry', entry.resolved, 'registry'), PARSER_SOURCE_LIMIT);
      }
    }
  }
  return { records, diagnostics };
...
function parseCargoLock(text, path) {
  ...
  for (const pkg of packages) {
    ...
    if (typeof pkg.source === 'string') {
      boundedPush(records, sourceRecord(path, 'registry', pkg.source, 'crates.io'), PARSER_SOURCE_LIMIT);
    }
  }
  return { records, diagnostics };
}
...
function parseUvLock(text, path) {
  ...
  for (const pkg of packages) {
    if (pkg === null || typeof pkg !== 'object' || Array.isArray(pkg)) continue;
    boundedPush(records, pinRecord(path, pkg.name, pkg.version, 'lockfile'), PARSER_PIN_LIMIT);
    if (pkg.source !== null && typeof pkg.source === 'object' && !Array.isArray(pkg.source)
        && typeof pkg.source.registry === 'string') {
      boundedPush(records, sourceRecord(path, 'index', pkg.source.registry, 'index'), PARSER_SOURCE_LIMIT);
    }
  }
  return { records, diagnostics };
}
...
function parsePipfileLock(value, path) {
  ...
  if (value._meta !== null && typeof value._meta === 'object' && !Array.isArray(value._meta)
...
function parseComposerLock(value, path) {
  ...
  for (const bucket of ['packages', 'packages-dev']) {
    ...
    for (const pkg of list) {
      ...
      const url = typeof distUrl === 'string' ? distUrl : typeof sourceUrl === 'string' ? sourceUrl : null;
      if (url !== null) boundedPush(records, sourceRecord(path, 'repository', url, 'repository'), PARSER_SOURCE_LIMIT);
    }
  }
  return { records, diagnostics };
}
...
function parseYarnLock(text, path) {
  ...
  for (const line of lines) {
    ...
    if (version && currentName !== null && pinCount < ASSURANCE_LIMITS.pins) {
      records.push(candidate('pin', path, 'observed', { package: currentName, scope: 'lockfile', version: version[1] }));
      pinCount++;
      continue;
    }
    const resolved = line.match(/^\s+resolved "([^"]+)"/);
    if (resolved) {
      boundedPush(records, sourceRecord(path, 'registry', resolved[1], 'registry'), PARSER_SOURCE_LIMIT);
    }
  }
  return { records, diagnostics };
}
...
function parseLock({ path, text, value }) {
  ...
  switch (base) {
    ...
    case 'pnpm-lock.yaml':
    case 'pdm.lock':
      return { records: [candidate('lock', path, 'observed', { format: base === 'pnpm-lock.yaml' ? 'pnpm' : 'pdm' })], diagnostics: [] };
    case 'deno.lock':
      return { records: [candidate('lock', path, 'observed', { format: 'deno' })], diagnostics: [] };
    case 'go.sum':
    case 'Gemfile.lock':
    case 'mix.lock':
    case 'bun.lock':
    case 'bun.lockb':
      return { records: [candidate('lock', path, 'observed', { format: { 'go.sum': 'go', 'Gemfile.lock': 'gemfile', 'mix.lock': 'mix', 'bun.lock': 'bun', 'bun.lockb': 'bun' }[base] })], diagnostics: [] };
    default:
      return { records: [], diagnostics: [diagnostic(path, 'unsupported', 'UNSUPPORTED')] };
  }
}
...
function parseVex(value, path) {
  ...
  records.push(candidate('vex', path, 'observed', { format: 'OpenVEX', specVersion, statementCount }));
...
function parseToolResult(path) {
  ...
  return { records: [candidate('tool_result', path, 'observed', { format, tool })], diagnostics: [] };
}
...
function parseAttestation(path) {
  ...
  if (/\.intoto\.jsonl$/i.test(base) || /^provenance(?:\.intoto)?\.jsonl?$/i.test(base)) {
    return { records: [candidate('attestation', path, 'observed', { format: 'in-toto', kind: 'statement' })], diagnostics: [] };
  }
  if (/\.att$/i.test(base)) return { records: [candidate('attestation', path, 'observed', { format: 'in-toto', kind: 'link' })], diagnostics: [] };
  if (/\.attestation(?:\.json)?$/i.test(base)) {
    return { records: [candidate('attestation', path, 'observed', { format: 'in-toto', kind: 'statement' })], diagnostics: [] };
  }
  if (/\.sigstore\.json$/i.test(base)) {
    return { records: [candidate('attestation', path, 'observed', { format: 'sigstore', kind: 'bundle' })], diagnostics: [] };
  }
  return { records: [], diagnostics: [diagnostic(path, 'unsupported', 'UNSUPPORTED')] };
}
...
function parseLicense(path) {
  ...
  return { records: [candidate('license', path, 'observed', { declared: 'file', identifier: null })], diagnostics: [] };
}
...
export function extractAssuranceArtifact({ path, text, value, format, kind = null }) {
  ...
  switch (effectiveKind) {
    case 'manifest': return parseManifest({ path, text, value, format });
    case 'lock': return parseLock({ path, text, value });
    case 'sbom': return parseSbom(value, path);
    case 'vex': return parseVex(value, path);
    case 'sarif': return parseSarif(value, path);
    case 'configuration': return parseConfiguration(path);
    case 'tool_result': return parseToolResult(path);
    case 'accessibility': return parseAccessibility({ path, text, value });
    case 'attestation': return parseAttestation(path);
    case 'license': return parseLicense(path);
    default:
      return { records: [], diagnostics: [diagnostic(path, 'unsupported', 'UNSUPPORTED')] };
  }
}
