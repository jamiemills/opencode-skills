...
function detectHooks(ctx) {
  ...
  for (const file of HOOK_FILES) {
    ...
    const tool = file.startsWith("lefthook")
      ? "lefthook"
...
const BUILD_TOOL_FILES = [
  ...
  {
    name: "vite",
    files: ["vite.config.ts", "vite.config.js", "vite.config.mjs", "vite.config.cjs"],
  },
  {
    name: "rollup",
  ...
  {
    name: "esbuild",
    files: ["esbuild.config.js", "esbuild.config.mjs", "esbuild.config.ts", "esbuild.config.cjs"],
  },
...
function detectDeclaredTools(ctx, ecosystems) {
  ...
  if (parsed && typeof parsed === "object") {
    ...
    if (depGroups && typeof depGroups === "object") {
      for (const [group, list] of Object.entries(depGroups)) {
        ...
        for (const spec of list) {
          const name = leadingDependencyName(spec);
          if (name && DECLARED_TOOL_VOCABULARY.includes(name))
            addSource(name, "dependency-group", group);
        }
      }
    }
    if (parsed.tool && typeof parsed.tool === "object") {
      for (const name of Object.keys(parsed.tool)) {
        if (DECLARED_TOOL_VOCABULARY.includes(name))
          addSource(name, "tool-section", `tool.${name}`);
      }
    }
  }
  ...
  for (const entry of entries.values()) {
    ...
    entry.sources.sort((a, b) =>
      a.kind === b.kind ? (a.ref < b.ref ? -1 : a.ref > b.ref ? 1 : 0) : a.kind < b.kind ? -1 : 1,
    );
...
export async function scan(repoPath, overview) {
  ...
  const hasSignal =
    linters.length > 0 || formatters.length > 0 || typeCheckers.length > 0 || hooks.length > 0;
