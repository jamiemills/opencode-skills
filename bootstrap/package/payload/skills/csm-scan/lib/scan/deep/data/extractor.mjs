...
function extractSql(text, path) {
  ...
  for (const statement of sqlStatements(source)) {
    ...
    const indexMatch = statementText.match(
      /^CREATE(?:\s+UNIQUE)?\s+INDEX\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z0-9_.-]+)\s+ON\s+(["'`]?)([A-Za-z0-9_.-]+)\2\s*\(([^)]*)\)/i,
    );
    if (indexMatch) {
      ...
      records.push(
        recordCandidate({
          ...
          details: {
            ...
            columns: parseColumnList(indexMatch[4]),
          },
          path,
    ...
    const tableMatch = statementText.match(
      /^CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(["'`[]?)([A-Za-z0-9_.-]+)\1\s*\(/i,
    );
    if (!tableMatch) {
      if (/^CREATE\s+TABLE\b/i.test(statementText)) {
        diagnostics.push(
          diagnostic(
            ...
            "unsupported",
            "UNSUPPORTED",
            lineIndexOf(source, sourceOffset(statementText.indexOf("CREATE"))),
          ),
        );
      }
      continue;
    ...
    if (table === null) {
      diagnostics.push(
        diagnostic(
          ...
          "unsupported",
          "UNSUPPORTED",
          lineIndexOf(source, sourceOffset(statementText.indexOf(tableMatch[0]))),
        ),
      );
      continue;
    }
    ...
    for (const raw of splitTopLevel(body, ",")) {
      ...
      if (/^FOREIGN\s+KEY\b/i.test(core)) {
        const fk = core.match(
          /^FOREIGN\s+KEY\s*\(([^)]*)\)\s+REFERENCES\s+(["'`]?)([A-Za-z0-9_.-]+)\2\s*\(([^)]*)\)/i,
        );
        if (fk) {
...
function extractDjangoMigration(text, path) {
  ...
  if (depsMatch) {
    ...
    for (const entry of depsMatch[1].matchAll(
      /\(\s*['"]([^'"]+)['"]\s*,\s*['"]([^'"]+)['"]\s*\)/g,
    )) {
...
function extractSqlMigration(text, path, dialect) {
  ...
  return {
    records,
    edges: sqlResult.edges,
    diagnostics: sqlResult.diagnostics,
  ...
  for (const match of source.matchAll(modelPattern)) {
    ...
    for (const rawLine of bodyLines) {
      ...
      if (trimmed.startsWith("@@")) {
        ...
        if (compound) {
          const columns = compound[2]
            .split(",")
            .map((entry) => safeLabel(entry.trim()))
            .filter((entry) => entry !== null);
    ...
    for (const field of fields) {
      ...
      const relationAttr = field.attributes.match(
        /@relation\(\s*fields\s*:\s*\[([^\]]*)\]\s*,\s*references\s*:\s*\[([^\]]*)\]\s*\)/,
      );
      if (relationAttr) {
        const target = safeLabel(field.type);
        if (target !== null) {
          const columns = relationAttr[1]
            .split(",")
            .map((entry) => safeLabel(entry.trim()))
            .filter((entry) => entry !== null);
          records.push(
          ...
          edges.push(
            relationEdgeCandidate({
              ...
              path,
              line: field.line,
              matchedKey: matchedKeyForRecord(
                "relation",
                relationSignature(entity, target, "foreign_key"),
              ),
            }),
          );
        }
...
function hasDjangoImport(source) {
  return /^\s*(?:from\s+django(?:\.[A-Za-z_]\w*)*\s+import\s+[\w., ]*\bmodels\b|import\s+django(?:\.[A-Za-z_]\w*)*)/m.test(
    source,
  );
}

function djangoModelSignals(baseClasses, body, source) {
  const inheritsModel = /(?:^|[\s,])(?:models\.Model|django\.db\.models\.Model)(?:\s*[,)]|$)/.test(
    baseClasses,
  );
  if (inheritsModel) return true;
  const hasFieldAssignment = /^\s*[A-Za-z_]\w*\s*=\s*models\.[A-Za-z_]\w*(?:Field)?\s*\(/m.test(
    body,
  );
...
function extractPythonModels(text, path) {
  ...
  for (const block of pythonClassBlocks(source)) {
    ...
    const entity =
      table !== null ? table : isDjango ? safeLabel(block.name) : safeLabel(block.name);
    if (entity === null) continue;
    ...
    if (isSqlAlchemy) {
      ...
      for (const relationship of relationships) {
        diagnostics.push(
          diagnostic(path, "unverified", "NAME_ONLY", classLine + relationship.line),
        );
      }
      continue;
    }
    ...
    if (isDjango) {
      ...
      for (const match of block.body.matchAll(fieldPattern)) {
        ...
        if (
          match[2].includes("ForeignKey") ||
          match[2].includes("OneToOneField") ||
          ...
          const kind = match[2].includes("ManyToManyField")
            ? "many_to_many"
            : match[2].includes("OneToOneField")
              ? "one_to_one"
              : "foreign_key";
...
function extractJavascriptModels(text, path) {
  ...
  for (const match of source.matchAll(initPattern)) {
    ...
    const classMatch = source
      .slice(0, match.index)
      .match(/(?:class|const)\s+([A-Za-z_]\w*)\s+extends\s+Model\s*$/m);
  ...
  const associationPattern =
    /([A-Za-z_]\w*)\.(belongsTo|belongsToMany|hasMany|hasOne)\s*\(\s*(?:['"]([^'"]+)['"]|([A-Za-z_]\w*))/g;
...
function extractRustModels(text, path) {
  ...
  const tableNamePattern =
    /#\[table_name\s*=\s*"([^"]+)"\]\s*(?:#\[[^\]]*\]\s*)*pub\s+struct\s+([A-Za-z_]\w*)/g;
  ...
  const belongsPattern =
    /#\[belongs_to\s*\(\s*([A-Za-z_]\w*)\s*\)\]\s*(?:#\[[^\]]*\]\s*)*pub\s+struct\s+([A-Za-z_]\w*)/g;
...
export function extractDataArtifact({ path, text, _value, _format, _ecosystem }) {
  ...
  if (migration !== null) {
    if (migration === "django")
      return boundedCollections(extractDjangoMigration(source, path), path);
    if (migration === "alembic") return boundedCollections(extractAlembic(source, path), path);
    if (migration === "sequelize")
      return boundedCollections(extractSequelizeMigration(source, path), path);
    if (migration === "prisma")
      return boundedCollections(extractSqlMigration(source, path, "prisma"), path);
    return boundedCollections(extractSqlMigration(source, path, migration), path);
  }

  switch (classification.kind) {
    case "sql":
      return boundedCollections(extractSql(source, path), path);
    case "prisma":
      return boundedCollections(extractPrisma(source, path), path);
    case "python":
      return boundedCollections(extractPythonModels(source, path), path);
    case "javascript":
    case "typescript":
      return boundedCollections(extractJavascriptModels(source, path), path);
    case "rust":
      return boundedCollections(extractRustModels(source, path), path);
    default:
      return other;
  }
}
