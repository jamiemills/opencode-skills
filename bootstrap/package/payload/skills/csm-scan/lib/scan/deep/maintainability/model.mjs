...
export const GENERATED_REASONS = Object.freeze(["dir_marker", "filename_marker", "header_comment"]);

export const TOOL_EVIDENCE_KINDS = Object.freeze(["config", "manifest", "dependency"]);
...
function tokenLabel(value, field = "token") {
  if (
    typeof value !== "string" ||
    value.length === 0 ||
    value.length > MAINTAINABILITY_LIMITS.tokenLabel ||
    /[^\x20-\x7e]/.test(value)
  ) {
...
function normalizeFileMetric(value) {
  ...
  if (
    typeof result.sizeBucket !== "string" ||
    !SIZE_BUCKETS.some((bucket) => bucket.label === result.sizeBucket)
  ) {
...
function normalizeComplexityRecord(value) {
  ...
  exactKeys(
    value,
    ["dialect", "distribution", "functions", "functionsCapped", "path"],
    "complexity record",
  );
  if (
  ...
  const functions = value.functions
    .map(normalizeComplexityFunction)
    .toSorted(
      (left, right) =>
  ...
  return deepFreeze({
    ...
    functions,
    distribution:
      value.distribution === null ? null : normalizeComplexityDistribution(value.distribution),
...
function normalizeDuplicateGroup(value) {
  ...
  if (
    ...
    !/^duplicate-[a-z0-9-]+$/.test(value.id) ||
    value.id.length > 48
  ) {
  ...
  if (new Set(identities).size !== identities.length)
    fail("DUPLICATE_ID", "duplicate group spans are not unique");
...
function normalizeExcludedLanguage(value) {
  ...
  if (
    !isValidExcludedExtension(value.extension) &&
...
function privacyFilter(records, diagnostics) {
  ...
  const unique = [];
  const seen = new Set();
  for (const entry of all.toSorted(
    (left, right) =>
...
function normalizeSearchInput(value) {
  ...
  for (const key of [
    ...
    if (typeof value[key] !== "boolean")
      fail("INVALID_SEARCH_SPACE", "search-space state must be boolean");
  }
  const result = {};
...
export function buildMaintainabilityModel({
  ...
  if (
    !Array.isArray(files) ||
    !Array.isArray(branchPoints) ||
    !Array.isArray(duplicateGroups) ||
    !Array.isArray(generatedBoundaries) ||
    !Array.isArray(toolEvidence) ||
    !Array.isArray(sizeDistribution) ||
    !Array.isArray(diagnostics) ||
    !Array.isArray(complexityRecords) ||
    !Array.isArray(deadCode)
  ) {
  ...
  if (
    complexityInput.length > MAINTAINABILITY_LIMITS.files ||
  ...
  for (const entry of allDiagnostics.toSorted(
    (left, right) =>
      compareAscii(left.path, right.path) ||
      compareAscii(left.status, right.status) ||
      compareAscii(left.reason, right.reason) ||
  ...
  safeDeadCode = safeDeadCode.toSorted(
    (left, right) => compareAscii(left.kind, right.kind) || compareAscii(left.path, right.path),
  );
  safeGroups = safeGroups.toSorted((left, right) => compareAscii(left.id, right.id));
  safeBoundaries = safeBoundaries.toSorted(
    (left, right) =>
      compareAscii(left.path, right.path) ||
      compareAscii(left.reason, right.reason) ||
      (left.line ?? 0) - (right.line ?? 0),
  );
  safeTools = safeTools.toSorted(
    (left, right) => compareAscii(left.tool, right.tool) || compareAscii(left.file, right.file),
  );
  ...
  const eligibleFiles = Number.isSafeInteger(measurement.eligibleFiles)
    ? measurement.eligibleFiles
    : safeFiles.length;
  const measuredFiles = safeFiles.length;
  const supportedDialects = [...new Set(safeFiles.map((record) => record.dialect))].toSorted(
    compareAscii,
  );

  const excludedLanguages = (
    Array.isArray(measurement.excludedLanguages) ? measurement.excludedLanguages : []
  )
  ...
  const filesInspected = Number.isSafeInteger(measurement.filesInspected)
    ? measurement.filesInspected
    : (space?.filesInspected ?? safeFiles.length);
  const bytesInspected = Number.isSafeInteger(measurement.bytesInspected)
    ? measurement.bytesInspected
    : (space?.bytesInspected ?? 0);
  const recordsInspected = Number.isSafeInteger(measurement.recordsInspected)
    ? measurement.recordsInspected
    : (space?.recordsInspected ?? 0);
  const omittedCount = Number.isSafeInteger(measurement.omittedCount)
    ? measurement.omittedCount
    : (space?.omittedCount ?? 0);
  const configFilesInspected = Number.isSafeInteger(measurement.configFilesInspected)
    ? measurement.configFilesInspected
    : 0;
  ...
  const partialCoverage =
    Object.values(capped).some(Boolean) ||
...
function disclosedSource(source, marker) {
  return source.length > MAINTAINABILITY_LIMITS.tokenLabel || /[^\x20-\x7e]/.test(source)
    ? marker
    : source;
}
...
function contentTools(path, text, value, format) {
    ...
    for (const section of ["dependencies", "devDependencies"]) {
      ...
      for (const name of Object.keys(deps)) {
        if (PACKAGE_TOOL_PACKAGES.includes(name)) {
          records.push({
            tool: name === "@biomejs/biome" ? "biome" : name === "jshint" ? "jshint" : name,
            kind: "dependency",
            line: 1,
            source: name,
          });
  ...
  if (
    ...
    const pattern =
      /^\[(?:tool\.)?(ruff|black|isort|mypy|pylint|flake8|bandit|pyright|deptry|vulture|pydocstyle)\]/gm;
...
export function detectToolEvidence({ path, format = "text", value = null, text = "" }) {
  ...
  return [...unique.values()].toSorted(
    (left, right) =>
      compareAscii(left.tool, right.tool) ||
...
export function toolConfigCandidatePaths() {
  const paths = new Set(["package.json", "pyproject.toml", "setup.cfg", "tox.ini", "Cargo.toml"]);
...
const NO_UNUSED_VARS_MARKER_PATTERN =
