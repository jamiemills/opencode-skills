...
export async function enrich(deepResults, overview) {
  ...
  if (cfg?.typescript?.strict && conv) {
    if (conv.importStyle?.type === "CJS (require/module.exports)") {
      contradictions.push({
        description:
          "tsconfig has strict:true; conventions import style is CJS (require/module.exports)",
  ...
  if (conv?.importStyle && stk) {
    ...
    if (conv.importStyle.type === "CJS (require/module.exports)" && stk.type === "module") {
      contradictions.push({
        description:
          'conventions import style is CJS (require/module.exports); package.json type is "module"',
        dimensions: ["conventions", "stack"],
      });
    }
  }
  ...
  if (ops?.dockerfiles?.length > 0 && sec) {
    if (!sec.dockerfilesScanned) {
      gaps.push({
        dimension: "security",
        reason: "dockerfiles present; security docker analysis not performed",
      });
    }
  }
  ...
  if (
    tst &&
    Array.isArray(tst.framework) &&
    tst.framework.includes("unknown") &&
    (tst.fileCount ?? 0) > 0
  ) {
    contradictions.push({
  ...
  if (stk?.packageManager === "unknown") {
    ...
    const depCount =
      man?.dependencies && typeof man.dependencies === "object"
...
function applicabilityStatus(dimension, overview) {
  const facts = [];
  if (typeof overview.isGit === "boolean")
    facts.push({ field: "is_git", present: true, value: overview.isGit });
  if (typeof overview.repositoryKind === "string")
    facts.push({ field: "repository_kind", present: true, value: overview.repositoryKind });
...
function searchCompleteness(searchSpace) {
  ...
  const cleanUnsupported =
    ...
    s.malformed !== true &&
    s.ambiguous !== true &&
    (s.filesInspected ?? 0) === 0 &&
    (s.bytesInspected ?? 0) === 0 &&
    (s.recordsInspected ?? 0) === 0 &&
    (s.omittedCount ?? 0) === 0;
  const incomplete =
    s.supported === true &&
    (!s.readable ||
...
function positiveEvidence(dimension, findings) {
  ...
  switch (dimension) {
    ...
    case "maintainability":
      return (
        Number.isSafeInteger(findings.summary?.filesMeasured) && findings.summary.filesMeasured > 0
      );
    case "governance":
 ...
 */
export function computeExpectedClaimCoverage(
  deepResults,
  overview = {},
