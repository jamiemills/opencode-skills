...
function safeLabel(value, label) {
  if (
    typeof value !== "string" ||
    value.length === 0 ||
    value.length > PLUGIN_LIMITS.label ||
    value !== value.trim() ||
...
function fileName(value, label) {
  if (
    typeof value !== "string" ||
    value.length === 0 ||
    value.length > PLUGIN_LIMITS.string ||
...
function artifactToken(value, label) {
  ...
  if (
    segments.length > PLUGIN_LIMITS.tokenSegments ||
    segments.some(
      (segment) => segment === "." || segment === ".." || !TOKEN_SEGMENT_PATTERN.test(segment),
    )
  ) {
    fail("INVALID_PATH", `${label} must contain bounded relative artifact tokens`);
  }
...
function regexSource(value) {
  if (
    typeof value !== "string" ||
    value.length === 0 ||
    value.length > PLUGIN_LIMITS.regexSource ||
    value !== value.trim() ||
    /[^\x20-\x7e]/.test(value)
  ) {
  ...
  for (let index = 0; index < value.length; index++) {
    ...
    if (escaped) {
      if (!SAFE_REGEX_ESCAPE.test(character))
        fail("INVALID_REGEX", "regexSource contains an invalid escape");
      escaped = false;
      if (inClass) {
    ...
    if (inClass) {
      if (character === "]") {
        ...
        if (classRange || classPrevious === null)
        ...
        if (classRange) {
          ...
          for (
            let code = classRangeStart.codePointAt(0);
    ...
    if (character === "(") {
      ...
      if (!["atom"].includes(previous))
        fail("REGEX_COMPLEXITY", "regexSource quantifier placement is unsafe");
      quantifiers++;
      if (value[index - 1] === "." && (character === "*" || character === "+"))
        unboundedWildcards++;
  ...
  if (
    groups > PLUGIN_LIMITS.regexGroups ||
    alternatives > PLUGIN_LIMITS.regexAlternatives ||
    quantifiers > PLUGIN_LIMITS.regexQuantifiers ||
...
function normalizeRule(rule) {
  ...
  const extensions = uniqueStrings(
    rule.extensions ?? [],
    PLUGIN_LIMITS.extensions,
    extension,
    "extensions",
  );
  const basenames = uniqueStrings(
    rule.basenames ?? [],
    PLUGIN_LIMITS.basenames,
    fileName,
    "basenames",
  );
  const manifestNames = uniqueStrings(
    rule.manifestNames ?? [],
    PLUGIN_LIMITS.manifestNames,
    fileName,
    "manifestNames",
  );
  const artifactTokens = uniqueStrings(
    rule.artifactTokens ?? [],
    PLUGIN_LIMITS.artifactTokens,
    artifactToken,
    "artifactTokens",
  );
  ...
  const literal = optionalMatch(rule, "literal", (entry) => {
    if (
      typeof entry !== "string" ||
      entry.length === 0 ||
      entry.length > PLUGIN_LIMITS.literal ||
      entry !== entry.trim() ||
      /[^\x20-\x7e]/.test(entry)
    ) {
...
export function validatePlugin(value) {
  ...
  if (
    ...
    value.rules.length > PLUGIN_LIMITS.rules
  ) {
  ...
  const capabilities = new Set(
    providers.flatMap(({ dimensions }) =>
      dimensions.flatMap(({ dimensionId, categories }) =>
        categories.map((category) => `${dimensionId}:${category}`),
      ),
    ),
  );
  ...
  for (const plugin of plugins) {
    for (const identity of [plugin.id, ...plugin.aliases]) {
      if (identities.has(identity))
        fail("DUPLICATE_ALIAS", "plugin identifiers and aliases must be globally unique");
      identities.add(identity);
    }
    for (const providerEntry of plugin.providers) {
      if (providerIds.has(providerEntry.id))
        fail("DUPLICATE_PROVIDER", "provider identifiers must be globally unique");
