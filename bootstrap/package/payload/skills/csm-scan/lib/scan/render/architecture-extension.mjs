...
export function createArchitectureExtensionRenderer({ context = DEFAULT_RENDER_CONTEXT } = {}) {
  ...
  function renderGraphFacts(facts) {
    ...
    if (components.length > 0) {
      lines.push(
        `- **Strongly-connected components**: ${facts.stronglyConnectedComponents.totalComponents} total (${facts.stronglyConnectedComponents.singletonComponents} singleton; ${components.length} cyclic)`,
      );
    ...
    if (indicatorCount > 0) {
      lines.push(
        `- **Dynamic indicators**: ${indicatorCount} construct(s) recorded (${facts.universe.indicatorsOmitted} omitted by cap)`,
      );
      for (const indicator of facts.dynamicIndicators.slice(0, MAX_LISTED_INDICATORS)) {
        const target =
          indicator.specifier === null ? "(dynamic)" : `\`${esc(indicator.specifier)}\``;
    ...
    if (fanFiles.length > 0) {
      ...
      for (const file of fanFiles) {
        lines.push(
          `| \`${esc(file)}\` | ${esc(facts.fanIn[file] ?? 0)} | ${esc(facts.fanOut[file] ?? 0)} |`,
        );
      }
