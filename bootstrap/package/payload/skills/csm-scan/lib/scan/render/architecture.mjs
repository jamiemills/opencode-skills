...
function renderCanonical(lines, canonical) {
  ...
  if (canonical.adapterIndependence.length > 0) {
    lines.push(`**Adapter independence groups** — ${canonical.adapterIndependence.length} group(s):`);
    lines.push('');
    ...
    for (const group of canonical.adapterIndependence) {
      ...
      const moduleList = shown.join(', ')
        + (group.modules.length > 8 ? ` (+${group.modules.length - 8} more)` : '');
  ...
  if (canonical.compositionRoots.modules.length > 0) {
    lines.push(`**Composition roots** — ${canonical.compositionRoots.modules.length} declared module(s):`);
    lines.push('');
...
export function renderArchitecture(_repoName, findings) {
  ...
  lines.push('');
  if (canonical) {
    lines.push('> Architecture is reported from the declared layer model (`quality/architecture.toml`) plus heuristic import analysis.');
  } else {
