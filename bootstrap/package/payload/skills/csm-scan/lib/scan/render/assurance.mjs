...
function capNotes(model) {
  ...
  for (const [key, label] of [
    ...
    ["vexes", "VEX"],
    ["sarifs", "SARIF"],
    ["configurations", "tool configuration"],
    ["toolResults", "tool result"],
    ["accessibility", "accessibility artifact"],
...
export function renderAssurance(_repoName, model, context = DEFAULT_RENDER_CONTEXT) {
  ...
  const sections = [
    section(
      ...
      (row) => [row.details.format, row.details.ecosystem ?? "—", row.path],
      context,
    ),
    section(
      model,
      ...
      "Lockfiles",
      ["Format", "Path"],
      (row) => [row.details.format, row.path],
      context,
    ),
    section(
      model,
      "pin",
      "Declared pins",
      ["Package", "Version", "Scope", "Path"],
      (row) => [row.details.package, row.details.version, row.details.scope, row.path],
      context,
    ),
    section(
      model,
      ...
      "Dependency sources",
      ["Kind", "Host", "Path"],
      (row) => [row.details.kind, row.details.host ?? "—", row.path],
      context,
    ),
    section(
      model,
      "license",
      "License references",
      ["Identifier", "Declared", "Path"],
      (row) => [row.details.identifier ?? "—", row.details.declared, row.path],
      context,
    ),
    section(
      model,
      ...
      ["Format", "Spec version", "Components", "Path"],
      (row) => [
        row.details.format,
        row.details.specVersion ?? "—",
        String(row.details.projection?.componentCount ?? 0),
        row.path,
      ],
      context,
    ),
    section(
      ...
      "vex",
      "VEX documents",
      ["Format", "Spec version", "Statements", "Path"],
      (row) => [
        row.details.format,
        row.details.specVersion ?? "—",
        String(row.details.statementCount ?? 0),
        row.path,
      ],
      context,
    ),
    section(
      ...
      ["Version", "Runs", "Results", "Path"],
      (row) => [
        row.details.version ?? "—",
        String(row.details.projection?.runCount ?? 0),
        String(row.details.projection?.resultCount ?? 0),
        row.path,
      ],
      context,
    ),
    ...
    section(
      ...
      ["Tool", "Format", "Path"],
      (row) => [row.details.tool, row.details.format ?? "—", row.path],
      context,
    ),
    section(
      model,
      ...
      ["Kind", "Declared", "Path"],
      (row) => [row.details.kind, row.details.declared ?? "—", row.path],
      context,
    ),
    section(
      model,
      ...
      "Attestations",
      ["Format", "Kind", "Path"],
      (row) => [row.details.format, row.details.kind, row.path],
      context,
    ),
    section(
      model,
      ...
      ["Standard", "Edition key", "Disposition", "Path"],
      (row) => [row.details.registryId, row.details.editionKey, row.details.disposition, row.path],
      context,
    ),
  ];
  ...
  if ((model.summary?.records ?? 0) === 0) {
    lines.push(
      `No declared supply-chain evidence detected in ${model.searchSpace.filesInspected} inspected file(s).`,
    );
    lines.push("");
  }
  ...
  if (model.diagnostics.length > 0) {
    ...
    lines.push("");
    for (const entry of [...model.diagnostics].toSorted((left, right) =>
      compareAscii(left.path, right.path),
    )) {
      lines.push(
        `- \`${escapeField(entry.path)}\`: ${escapeField(entry.reason)} (${escapeField(entry.status)})`,
      );
    }
    lines.push("");
  }
