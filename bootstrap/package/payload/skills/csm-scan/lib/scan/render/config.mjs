...
function declaredToolchainRows(declaredTools) {
  ...
  const rows = [...declaredTools].toSorted((a, b) =>
...
export function renderConfig(repoName, findings, context = DEFAULT_RENDER_CONTEXT) {
  ...
  const linterNames =
    Array.isArray(findings.linters) && findings.linters.length
  ...
  const formatterNames =
    Array.isArray(findings.formatters) && findings.formatters.length
  ...
  const typeCheckerNames =
    Array.isArray(findings.typeCheckers) && findings.typeCheckers.length
  ...
  if (findings.typescript) {
    ...
    const details = [
      ...
      .join(", ");
    lines.push(
      `| TypeScript | \`tsconfig.json\` | ${escapeField(details, { inTable: true }) || "present"} |`,
    );
  } else if (typeCheckerNames) {
    lines.push(
      `| Type checking | \`${escapeField(typeCheckerNames, { inTable: true })}\` | detected |`,
    );
  } else {
  ...
  if (toolchainRows) {
    ...
    for (const row of toolchainRows) {
      lines.push(
        `| \`${escapeField(row.name, { inTable: true })}\` | ${escapeField(row.detail, { inTable: true })} |`,
      );
    }
    lines.push("");
  }
  ...
  if (Array.isArray(findings.buildTools) && findings.buildTools.length > 0) {
    const items = findings.buildTools
      .map((b) => `${escapeField(b.name)} (\`${escapeField(b.config)}\`)`)
      .join(", ");
    lines.push(`- **Build tools**: ${items}`);
  }
  if (Array.isArray(findings.runtimes) && findings.runtimes.length > 0) {
    const items = findings.runtimes
      .map((r) => `${escapeField(r.name)} (\`${escapeField(r.config)}\`)`)
      .join(", ");
