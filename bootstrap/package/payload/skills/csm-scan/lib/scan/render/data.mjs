...
export function renderData(_repoName, model, context = DEFAULT_RENDER_CONTEXT) {
    ...
    lines.push(
      renderTable(
        ...
        model.stores.map((store) => [
          store.signature,
          store.details.kind,
          pathCell(context, store.source.path, store.source.line),
        ]),
      ),
    );
    lines.push("");
    ...
    lines.push(
      renderTable(
        ...
        model.schemas.map((schema) => [
          schema.signature,
          pathCell(context, schema.source.path, schema.source.line),
        ]),
      ),
    );
    lines.push("");
  ...
  if (model.migrations.length > 0) {
    ...
    lines.push(
      renderTable(
        context,
        ["Migration", "Dialect", "Revision", "Predecessor", "Path"],
        model.migrations.map((migration) => {
          const predecessor =
            migration.details.downRevision ??
            (migration.details.dependencies.length > 0
              ? migration.details.dependencies.join(", ")
              : null);
          return [
            migration.signature,
            migration.dialect,
  ...
  if (model.entities.length > 0) {
    ...
    lines.push(
      renderTable(
        context,
        ["Entity", "Table", "Fields", "Keys", "Dialect", "Path"],
    ...
    lines.push(
      renderTable(
        ...
        model.caches.map((cache) => [
          cache.signature,
          cache.details.scope,
          pathCell(context, cache.source.path, cache.source.line),
        ]),
      ),
    );
    lines.push("");
    ...
    lines.push(
      renderTable(
        ...
        model.queues.map((queue) => [
          queue.signature,
          queue.details.scope,
          pathCell(context, queue.source.path, queue.source.line),
        ]),
      ),
    );
    lines.push("");
  ...
  if (!hasDeclarations) {
    lines.push(
      `No declaration-backed data architecture detected in ${model.searchSpace.filesInspected} inspected file(s).`,
    );
    lines.push("");
  }
  ...
  if (model.diagnostics.length > 0) {
    ...
    for (const entry of model.diagnostics) {
      ...
      lines.push(
        `- ${location}: ${context.escapeField(entry.reason)} (${context.escapeField(entry.status)})`,
      );
    }
    lines.push("");
  }
