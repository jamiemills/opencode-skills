...
function renderCodeowners(model, context) {
  ...
  lines.push(
    `- Assignees: ${escapeField(String(ownership.assigneeCount))} distinct report-local labels across ${escapeField(String(ownership.assignmentCount))} assignments.`,
  );
  lines.push("");
  if (ownership.rules.length > 0) {
    lines.push(
      table(
        context,
        ["Pattern", "Assigned labels", "Evidence"],
  ...
  if (ownership.assignees.length > 0) {
    lines.push(
      table(
        context,
        ["Report-local label", "Assignments"],
...
function renderAdr(model, context) {
  const decisions = model.entries
    ...
    .toSorted(
      (left, right) =>
        compareAscii(left.details.id ?? "", right.details.id ?? "") ||
  ...
  lines.push(
    table(
      context,
      ["ID", "Declared status", "Declared date", "Evidence"],
      decisions.map((entry) => [
        entry.details.id ?? "—",
...
function renderCategoryEntries(category, model, context) {
  ...
  const heading =
    CATEGORY_SECTIONS.find((entry) => entry.category === category)?.heading ?? category;
  const lines = [];
  lines.push(`### ${heading}`);
  lines.push("");
  lines.push(
    table(
      ...
      entries.map((entry) => [
        entry.source.path,
        entry.details?.kind ?? entry.dialect,
        evidenceCell(entry, context.escapeField),
      ]),
    ),
  );
...
function renderReferences(model, context) {
  const references = model.entries
    ...
    .toSorted(
      (left, right) =>
        compareAscii(left.details.url, right.details.url) ||
        compareAscii(left.source.path, right.source.path) ||
...
export function renderGovernance(_repoName, model, context = DEFAULT_RENDER_CONTEXT) {
  ...
  if (model.summary.entries === 0 && completeSearchSpace(model.searchSpace)) {
    lines.push(
      `No governance or ownership artifacts detected in ${escapeField(String(model.searchSpace.filesInspected))} inspected file(s).`,
    );
    lines.push("");
  }
