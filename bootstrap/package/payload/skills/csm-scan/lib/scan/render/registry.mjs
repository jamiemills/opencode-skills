...
function buildDefaultEntries(context) {
  ...
  return Object.freeze(
    DIMENSION_RENDERER_SOURCES.map((source) => {
 ...
 */
export function verifyRenderRegistry({ entries, order = DIMENSION_RENDERER_ORDER } = {}) {
  ...
  for (const entry of rendererEntries(entries)) {
    ...
    if (typeof label !== "string" || label.length === 0)
      throw new RenderRegistryError("INVALID_LABEL");
    if (
      !Array.isArray(proseLines) ||
      proseLines.length === 0 ||
  ...
  if (
    renderers.size !== DIMENSION_RENDERER_ORDER.length ||
...
function verifyGlobalRenderer(global) {
  ...
  if (!RENDERER_ID_PATTERN.test(global.rendererId))
    throw new RenderRegistryError("INVALID_RENDERER");
  if (
    !Array.isArray(global.prose) ||
    global.prose.length === 0 ||
...
export function createRenderRegistry({
  ...
  if (rendererIds.has(verifiedGlobal.rendererId))
    throw new RenderRegistryError("DUPLICATE_RENDERER");
