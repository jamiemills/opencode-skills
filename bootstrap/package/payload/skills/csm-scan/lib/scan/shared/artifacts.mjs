...
function reference(value, request = false) {
  plainObject(
    value,
    request ? REQUEST_KEYS : REFERENCE_KEYS,
    request ? "artifact request" : "artifact reference",
  );
  let path;
  try {
...
async function safeLstat(path, code = "UNREADABLE") {
  try {
    ...
    fail(
      code,
      code === "INVALID_ROOT" ? "repository root is unavailable" : "artifact is unreadable",
    );
  }
}
...
export async function boundedBytes(path, maximum, resolved) {
  ...
  try {
    ...
    return {
      bytes: buffer.subarray(0, Math.min(bytesRead, maximum)),
      capped: bytesRead > maximum || info.size > maximum,
    };
  } catch (error) {
    if (error instanceof ArtifactError) throw error;
    fail("UNREADABLE", "artifact is unreadable");
...
export async function readArtifacts(root, inputs, options = ARTIFACT_LIMITS) {
  ...
  if (inputs.length > MAX_REQUESTS)
    fail("BOUND_EXCEEDED", "artifact request count exceeds the hard bound");
  ...
  try {
    ...
    requests = inputs
      .map((input) => reference(input, true))
  ...
  for (const request of requests) {
    ...
    if (
      filesInspected >= bound.maxFiles ||
      bytesInspected >= bound.maxBytes ||
  ...
  const complete =
    ...
    statuses.size <= 1 &&
    (statuses.size === 0 || statuses.has("read"));
