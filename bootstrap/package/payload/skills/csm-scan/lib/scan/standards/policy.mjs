...
export const STANDARDS_DISPOSITIONS = Object.freeze(["authored_mapping", "metadata_only"]);

const ID_PATTERN = /^std:[a-z0-9]+(?:-[a-z0-9]+)*:[a-z0-9]+(?:[._-][a-z0-9]+)*$/;
const EDITION_KEY_PATTERN = /^[a-z0-9]+(?:[._-][a-z0-9]+)*$/;
const DATE_PATTERN = /^\d{4}-(?:0[1-9]|1[0-2])(?:-(?:0[1-9]|[12]\d|3[01]))?$/;
const FLOATING_EDITION_PATTERN =
  /(?:^|[^a-z])(?:current|latest|draft|unspecified|next|nightly|rolling|snapshot|provisional|preview|dev|head|trunk)(?:$|[^a-z])/i;
...
export function validateStandardsRegistry(entries) {
  ...
  return Object.freeze(
    validated.toSorted((left, right) => (left.id < right.id ? -1 : left.id > right.id ? 1 : 0)),
  );
}
