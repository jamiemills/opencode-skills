...
```bash
node $HOME/.config/opencode/skills/csm-upload/scripts/upload.mjs --label <name> [--desc <text>] [--github <user>] [--repo <name>] [--confirm-permanent] <file1> [<file2>] [more files]
```
...
The script requires `--confirm-permanent` before commit/push, isolates Git configuration, verifies the effective `origin`, refuses credential-like artifacts, and never claims Pages deployment or URL verification without separate evidence.
