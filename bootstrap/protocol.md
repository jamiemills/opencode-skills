...
| Envelope trust    | fixed package policy, no envelope path/exec fields, advisory shell-guidance check, steps never executed, machine-readable `signed` verify-output marker (true only when Ed25519 verification ran and passed)                                                                                                   | trust-root establishment and approval          |
...
The shipped fixed bin `bootstrap/package/bin/csm-skills-bootstrap.js` embeds the same keyring and a dependency-free copy of the envelope validator and exposes it as the `verify <envelope.json>` subcommand (F-045): it verifies the envelope against the embedded keyring, and when the envelope carries `payload_index_sha256` it asserts equality with the shipped payload-index.json's sha256, exiting 0 with `{verification:{ok:true,signed:true|false}}` or 1 with the refusal code and `signed:false` on failure. The `signed` field (F4-01) makes the flows machine-distinguishable: it is `true` only when the envelope carried a signature and Ed25519 verification ran and passed. Unsigned envelopes report `signed:false` and derive trust from policy+digest only; hard-requiring signatures remains publication-gated. The `payload-index` subcommand verifies payload integrity only — no envelope signature is involved — and always reports `signed:false`.
