...
If a `NORMS.md` file produced by `csm-scan` exists in the repo, load its testing and convention norms before generating the BDD/TDD spec package.

### Detection and validation
Same rules as csm-build: check user reference → git-root/NORMS.md → cwd/NORMS.md. Validate csm-scan authenticity (must contain "Generated by csm-scan" or "## Repository Overview").
