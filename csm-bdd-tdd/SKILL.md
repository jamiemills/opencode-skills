...
Same rules as csm-build: check user reference → git-root/NORMS.md → cwd/NORMS.md. Validate csm-scan authenticity with all canonical markers: contains "Generated by csm-scan" OR "## Repository Overview", plus "## Code Conventions" and "## Architecture" sections. If markers are absent, warn "NORMS.md found but does not appear to be csm-scan output — conventions may not be accurate" and proceed. Note the generation date and warn if stale (>30 days). In every fallback case nothing is blocked — skip and use standard BDD/TDD defaults.
