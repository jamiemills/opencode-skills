...
4. Confirm the CDP endpoint answers. `cdpUrl` in `state.json` already carries `?token=`, so the discovery path must be appended AFTER the port — never after the query. `curl -m 2 "<cdpUrl-sans-query>/json/version?token=<token>"` (the token is the `token` field in `state.json`; both are needed). If it does not answer, the chromium or the gate died — re-run ensure-browser to recreate the session.
