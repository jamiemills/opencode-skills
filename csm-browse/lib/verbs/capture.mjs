...
export async function run({ args, state }) {
  ...
  if (qi !== -1 && args[qi + 1])
    quality = Math.min(100, Math.max(1, parseInt(args[qi + 1], 10) || 80));
  ...
  const outName =
    positional.length > 0
      ? positional[0].endsWith(`.${cfg.ext}`)
        ? positional[0]
        : `${positional[0]}.${cfg.ext}`
  ...
  if (!VALID_OUT_NAME_RE.test(outName) || outName === "." || outName === "..") {
    console.error(
      `Invalid output name: "${outName}". Must match ^[A-Za-z0-9._-]+$ — no path separators, no traversal.`,
    );
    process.exit(1);
  }
  if (!resolve(artifactsDir, outName).startsWith(resolve(artifactsDir) + sep)) {
    console.error(
      `Invalid output name: "${outName}" — resolved path escapes the artifacts directory.`,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (stitch && fullPage) {
      ...
      if (totalTiles > MAX_TILES) {
        console.error(
          `Full page too large (${totalTiles} tiles). Re-run with --viewport for a single-viewport capture.`,
        );
        process.exit(1);
      }
            ...
            await new Promise((res, reject) => {
              const proc = spawn(
                ...
                [
                  ...
                  tmpPath,
                  "-vf",
                  `crop=iw:ih-${stickyHeight}:0:${stickyHeight}`,
                  "-frames:v",
                  "1",
                  "-y",
                  cropPath,
        ...
        try {
          for (const f of await readdir(artifactsDir)) {
            if (f.startsWith(".stitch-")) {
              try {
                await unlink(join(artifactsDir, f));
              } catch {}
            }
          }
        } catch {}
      ...
      const output = {
        path: outPath,
        bytes: (await import("fs").then((m) => m.promises.stat(outPath))).size,
        format: cfg.ext,
        preset,
        stitched: true,
        tiles: totalTiles,
