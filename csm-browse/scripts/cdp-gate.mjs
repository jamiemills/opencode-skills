...
const PROTOCOL_JSON = (() => {
  try { return readFileSync(require.resolve('chrome-remote-interface/lib/protocol.json'), 'utf-8'); }
  catch { return null; }
})();
...
export function checkRequestLine(line, expectedToken) {
  ...
  let target;
  try { target = new URL(parts[1], 'http://localhost'); } catch { return { ok: false }; }
...
function handleConnection(socket, ctx) {
    ...
    if (!accepted) {
      if (!lineDone) {
        if (buf.length > MAX_LINE_BYTES && buf.indexOf('\n') === -1) {
          denied = true; clearTimeout(lineTimer); deny(socket, 413);
          return;
        }
        ...
        if (!verdict.ok) {
          denied = true; clearTimeout(lineTimer); deny(socket, 403);
          return;
        }
  ...
  function openTunnelAsync() {
    ...
    try {
      ...
      if (!closed) { denied = true; deny(socket, 502); }
      return;
    }
    ...
    if (socket.destroyed) { try { child.kill('SIGTERM'); } catch {} return; }
    tunnel = child;
    let stderr = '';
    if (child.stderr) child.stderr.on('data', (d) => { stderr += d; if (stderr.length > 2048) stderr = stderr.slice(-2048); });

    child.stdout.on('data', (d) => {
      wroteBackend = true;
      if (!socket.write(d)) child.stdout.pause();
    });
    child.stdout.on('end', () => { try { socket.end(); } catch {} });
    child.on('error', (err) => {
    ...
    child.on('close', (code) => {
      if (stderr.trim()) log(`tunnel exit ${code}: ${stderr.trim().split('\n').slice(0, 3).join(' ')}`);
      if (!closed) {
    ...
    child.stdin.on('drain', () => { try { socket.resume(); } catch {} });
    socket.on('drain', () => { try { child.stdout.resume(); } catch {} });
    socket.on('end', () => { try { child.stdin.end(); } catch {} });
    ...
    socket.on('close', () => {
      ...
      const killTimer = setTimeout(() => {
        if (!child.exitCode && !child.signalCode) { try { child.kill('SIGTERM'); } catch {} }
...
export function createGate({ port = 0, internalPort, containerName, token, openTunnel, log = () => {} }) {
  ...
  return {
    ...
    listen() {
      return new Promise((resolve, reject) => {
        ...
        const onListen = () => { server.removeListener('error', onError); resolve(server.address().port); };
...
if (isCli) {
  ...
  gate.listen().then(() => log(`listening on 127.0.0.1:${port} -> ${containerName}:${internal}`))
    .catch((err) => { log(`listen failed: ${err.message}`); process.exit(1); });
