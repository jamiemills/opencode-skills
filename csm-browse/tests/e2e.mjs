...
function detectBridgeGateway() {
  ...
  try {
    const gw = execFileSyncProbe("docker", [
      "inspect",
      "chromium-vnc",
      "--format",
...
async function dockerProbeOk() {
  ...
  const ps = await run(
    "docker",
    ["ps", "-a", "--filter", "name=^chromium-vnc$", "--format", "{{.Names}}"],
    { timeout: 15000 },
  );
...
async function ensureSid(sid) {
  const { error, stdout } = await run(
    "node",
    [join(SKILL_DIR, "scripts", "ensure-browser.mjs"), "--session", sid],
    { timeout: 120000 },
  );
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    try {
      state = JSON.parse(lines[i]);
      break;
    } catch {}
  }
  if (!state) {
    throw new Error(
      `ensure-browser did not output valid JSON state. Last line: ${lines[lines.length - 1]}`,
    );
  }
  return { state, stdout };
}
...
async function checkContainerDir(path) {
  const { stdout } = await run(
    "docker",
    dockerArgs(["sh", "-c", `test -d "${path}" && echo exists`]),
  );
...
async function runTests() {
  ...
  try {
    ...
    // ── Step 1: Baseline ──────────────────────────────────────────
    {
      ...
      try {
        const { stdout: pidOut } = await run(
          "docker",
          dockerArgs(["pgrep", "-af", "--", "--remote-debugging-port=9223"]),
        );
        const defaultAlive = pidOut.trim().split("\n").filter(Boolean).length > 0;
        assert(
          step + " - default chromium running (port 9223)",
          defaultAlive,
        ...
        assert(
          step + " - CDP responsive",
          !!(cdpJson && cdpJson.webSocketDebuggerUrl),
    ...
    // ── Step 2: Ensure session ─────────────────────────────────────
    {
      ...
      try {
        ...
        assert(
          step + " - has daemonPid",
          typeof state.daemonPid === "number" && state.daemonPid > 0,
        );
        ...
        const realToken = JSON.parse(
          readFileSync(join(SESSIONS_ROOT, SESSION_ID, "state.json"), "utf-8"),
        ).token;
        assert(
          step + " - transcript state is redacted",
          state.token === "[REDACTED]",
          `logState token must be redacted: ${JSON.stringify(state.token)}`,
        );
        assert(
          step + " - transcript does not leak the token",
          !ensured.stdout.includes(realToken),
          "ensure-browser stdout contained the raw session token",
        );
        assert(
          step + " - transcript redacts the token",
          ensured.stdout.includes("token=%5BREDACTED%5D") ||
            ensured.stdout.includes("token=[REDACTED]"),
    ...
    // ── Step 6: DOM inspection ─────────────────────────────────────
    {
      ...
      try {
        ...
        assert(
          step + " - eval throwing → verb fails",
          !r.ok,
          `ok=${r.ok} stderr="${r.stderr.substring(0, 100)}"`,
        );
        assert(
          step + " - eval throwing → error surfaced",
    ...
    // ── Step 7b: Tall-page full screenshot bounded + leak-free ─────
    {
      ...
      try {
        ...
        assert(
          step + " - page made tall",
          data && data.result && data.result.value >= 20000,
          data ? JSON.stringify(data.result).substring(0, 80) : r.stdout,
        );
        ...
        let leftovers = [];
        try {
          leftovers = readdirSync(join(SESSIONS_ROOT, SESSION_ID, "artifacts")).filter((f) =>
            f.startsWith(".stitch-"),
          );
        } catch {}
        assert(
          step + " - no .stitch temps left",
          leftovers.length === 0,
    ...
    // ── Step 7c: Consent-wall dismissal ────────────────────────────
    {
      ...
      try {
        ...
        assert(
          ...
          data && data.result && data.result.value === true,
        ...
        let st = null;
        try {
          st = data && data.result ? JSON.parse(data.result.value) : null;
        } catch {}
        assert(
          step + " - wall dismissed",
          st && st.wall === false,
    ...
    // ── Step 8: Devtools capture ───────────────────────────────────
    {
      ...
      try {
        ...
        const hasPage1Response200 = netEvents.some((e) => {
          try {
            return (
              e.payload &&
              e.payload.url &&
              e.payload.url.includes("page1.html") &&
        ...
        assert(
          step + " - network has page1.html request",
          hasPage1Request,
    ...
    if (!QUICK) {
      ...
      try {
        ...
        if (stateBefore.daemonPid) {
          try {
            process.kill(stateBefore.daemonPid, "SIGTERM");
          } catch {}
        }
        ...
        if (!dead && stateBefore.daemonPid) {
          try {
            process.kill(stateBefore.daemonPid, "SIGKILL");
          } catch {}
        ...
        assert(
          step + " - console still has page1-ready after restart",
          stillHasPage1Ready,
    ...
    if (!QUICK) {
      ...
      try {
        ...
        assert(step + " - screencast started", r.stdout.includes("Recording started"), r.stdout);
        ...
        console.log(
          `      video: file=${data.file}, frames=${data.frames}, duration=${data.duration}, codec=${data.codec}`,
        );
        ...
        if (videoPath && existsSync(videoPath)) {
          ...
          const videoStream = ((probe && probe.streams) || []).find(
            (s) => s.codec_type === "video",
          );
          assert(
            step + " - ffprobe: codec vp9",
            videoStream && videoStream.codec_name === "vp9",
            videoStream ? videoStream.codec_name : "no video stream",
          );
          assert(
            step + " - ffprobe: valid resolution",
          ...
          assert(
            step + " - ffprobe: duration >= 4s",
            duration >= 4,
            `duration=${duration.toFixed(2)}s`,
          );

          const frameCount =
            (videoStream && parseInt(videoStream.nb_frames, 10)) || data.frames || 0;
          assert(
            step + " - ffprobe: frames >= 10 (or recorded >= 10)",
    ...
    // ── Step 11: Close ─────────────────────────────────────────────
    {
      ...
      try {
        ...
        assert(
          step + " - close returned removed",
          data && data.removed && data.removed.length > 0,
          r.stdout,
        );
        ...
        const containerDirExists = await checkContainerDir(
          `/config/csm-browse/sessions/${SESSION_ID}`,
        );
    ...
    // ── Step 12: After-close isolation ─────────────────────────────
    {
      ...
      try {
        ...
        const defaultAlive = pidOut.trim().split("\n").filter(Boolean).length > 0;
        assert(
          step + " - default chromium still running (port 9223)",
          defaultAlive,
    ...
    // ── Step 13: Session sweep ─────────────────────────────────────
    {
      ...
      try {
        ...
        const mkStale = (dir) => {
          ...
          writeFileSync(
            join(dir, "state.json"),
            JSON.stringify({ sid: dir.split("/").pop() }),
            "utf-8",
          );
        ...
        writeFileSync(
          join(freshDir, "state.json"),
          JSON.stringify({ sid: "sweep-test-fresh" }),
          "utf-8",
        );

        const dry = await run(
          "node",
          [join(SKILL_DIR, "scripts", "ensure-browser.mjs"), "--cleanup-stale", "--dry-run"],
          { timeout: 60000 },
        );
        assert(
          step + " - dry-run lists stale",
          dry.stdout.includes("sweep-test-stale"),
          dry.stdout.substring(0, 160),
        );
        assert(
          step + " - dry-run skips fresh",
          !dry.stdout.includes("sweep-test-fresh"),
          dry.stdout.substring(0, 160),
        );
        ...
        const real = await run(
          "node",
          [join(SKILL_DIR, "scripts", "ensure-browser.mjs"), "--cleanup-stale"],
          { timeout: 60000 },
        );
        assert(
          step + " - sweep removes stale",
          !existsSync(staleDir),
          real.stdout.substring(0, 160),
        );
        ...
        const auto = await run(
          "node",
          [join(SKILL_DIR, "scripts", "ensure-browser.mjs"), "--session", autoSid],
          { timeout: 120000 },
        );
        assert(
          step + " - auto-sweep line printed",
          auto.stdout.includes("Sweep:"),
          auto.stdout.substring(0, 200),
        );
        assert(step + " - auto-sweep removed stale", !existsSync(autoDir));
        await run("node", [
          join(SKILL_DIR, "scripts", "browse.mjs"),
          "close",
          "--session",
          autoSid,
        ]);
    ...
    //    recorder lock / creating.marker protection) ────────────────
    {
      ...
      try {
        ...
        assert(
          step + " - live session created",
          liveState && typeof liveState.daemonPid === "number",
        ...
        for (let port = 9235; port >= 9225 && decoyGatePort === null; port--) {
          ...
          const related = await run(
            "docker",
            dockerArgs(["pgrep", "-af", "--", `--remote-debugging-port=${port - 1}`]),
          );
        ...
        if (decoyGatePort !== null) {
          decoyGateProc = spawn(
            ...
            [
              ...
              `decoy-${Date.now()}`,
              "--port",
              String(decoyGatePort),
              "--internal",
              String(decoyGatePort - 1),
              "--container",
              "chromium-vnc",
            ],
            {
              detached: true,
              stdio: "ignore",
              env: { ...process.env, CSM_CDP_GATE_TOKEN: "decoy-token" },
            },
          );
          decoyGateProc.unref();
          let gateUp = false;
        ...
        writeFileSync(
          join(recDir, "recorder.json"),
          JSON.stringify({ running: true, file: "decoy.webm" }),
          "utf-8",
        );
        ...
        assert(
          step + " - dry-run lists orphan ffmpeg",
          removed.some((r) => r.includes(`orphan ffmpeg sid=${ffmpegSid}`)),
          JSON.stringify(removed),
        );
        assert(
          step + " - dry-run lists orphan gate",
          removed.some((r) => r.includes(`orphan gate port=${decoyGatePort}`)),
          JSON.stringify(removed),
        );
        assert(
          step + " - dry-run lists stale recorder lock",
          removed.some((r) => r.includes(`stale recorder lock sid=${recSid}`)),
          JSON.stringify(removed),
        );
        assert(
          step + " - dry-run omits live session",
          !removed.some((r) => r.includes(liveSid)),
          JSON.stringify(removed),
        );
        const unexpected = removed.filter(
          (r) =>
            !r.includes(ffmpegSid) &&
            !r.includes(`orphan gate port=${decoyGatePort}`) &&
            !r.includes(recSid),
        );
        assert(
          step + " - dry-run lists nothing else",
          unexpected.length === 0,
          JSON.stringify(unexpected),
        );

        // Real sweep: all three decoys gone, live session untouched.
        await run("node", [join(SKILL_DIR, "scripts", "ensure-browser.mjs"), "--cleanup-stale"], {
          timeout: 120000,
        });
        const ffmpegAfter = await run("pgrep", ["-af", `ffmpeg.*${ffmpegSid}`]);
        assert(
          step + " - sweep killed orphan ffmpeg",
          !ffmpegAfter.stdout.trim(),
          ffmpegAfter.stdout.substring(0, 120),
        );
        assert(
          step + " - sweep removed orphan ffmpeg session dir",
          !existsSync(join(SESSIONS_ROOT, ffmpegSid)),
        );
        if (decoyGatePort !== null) {
          const gateAfter = await run("pgrep", [
            "-af",
            `cdp-gate\\.mjs.*--port ${decoyGatePort}\\b`,
          ]);
          assert(
            step + " - sweep killed orphan gate",
            !gateAfter.stdout.trim(),
            gateAfter.stdout.substring(0, 120),
          );
        }
        let recAfter = null;
        try {
          recAfter = JSON.parse(readFileSync(join(recDir, "recorder.json"), "utf-8"));
        } catch {}
        assert(
          step + " - sweep cleared stale recorder lock",
          recAfter && recAfter.running === false,
          recAfter ? JSON.stringify(recAfter) : "unreadable",
        );
        const liveStatus = parseJson((await browseAs(liveSid, "status")).stdout);
        assert(
          step + " - live session untouched",
          liveStatus && liveStatus.daemonAlive === true,
        ...
        writeFileSync(
          join(markerDir, "creating.marker"),
          JSON.stringify({ pid: process.pid, ts: new Date().toISOString() }),
          "utf-8",
        );
        const real3 = await run(
          "node",
          [join(SKILL_DIR, "scripts", "ensure-browser.mjs"), "--cleanup-stale"],
          { timeout: 120000 },
        );
        assert(
          step + " - marker dir untouched by sweep",
          existsSync(join(markerDir, "creating.marker")),
          real3.stdout.substring(0, 160),
        );
        const liveStatus2 = parseJson((await browseAs(liveSid, "status")).stdout);
        assert(
          step + " - live session still alive after marker sweep",
          liveStatus2 && liveStatus2.daemonAlive === true,
        ...
        try {
          if (decoyFfmpegProc && decoyFfmpegProc.pid) process.kill(decoyFfmpegProc.pid, "SIGKILL");
        } catch {}
        try {
          await run("pkill", ["-f", `ffmpeg.*${ffmpegSid}`]);
        } catch {}
        if (decoyGatePort !== null) {
          try {
            await run("pkill", ["-f", "--", `cdp-gate\\.mjs.*--port ${decoyGatePort}\\b`]);
          } catch {}
        }
        if (decoyGateProc && decoyGateProc.pid) {
          try {
            process.kill(decoyGateProc.pid, "SIGKILL");
          } catch {}
        }
        for (const d of [
          join(SESSIONS_ROOT, ffmpegSid),
          join(SESSIONS_ROOT, recSid),
          join(SESSIONS_ROOT, markerSid),
        ]) {
          try {
            rmSync(d, { recursive: true, force: true });
          } catch {}
  ...
  // ── Step 13: Summary ────────────────────────────────────────────
  {
    ...
    if (durationMs > MAX_E2E_MS) {
      fail(
        "13. Total-suite wall cap",
        `suite took ${durationMs}ms, exceeding MAX_E2E_MS=${MAX_E2E_MS}ms`,
      );
    }
