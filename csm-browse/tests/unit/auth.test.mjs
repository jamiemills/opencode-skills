...
const { generateToken, rotateToken, revokeToken, withToken, cdpEndpoint } =
...
function httpRequest(port, request) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      sock.destroy();
      reject(new Error("httpRequest timed out"));
    }, 5000);
    const finish = (err) => {
      if (responded) return;
      responded = true;
...
test("checkRequestLine: correct token opens any path; no token opens only the static schema", () => {
  ...
  assert.equal(
    checkRequestLine(`GET /devtools/browser/abc?token=${token} HTTP/1.1`, token).ok,
    true,
  );
...
test("checkRequestLine: /json/protocol is a static verdict and length-mismatched tokens do not throw", () => {
  ...
  assert.equal(
    checkRequestLine(`GET /json/version?token=${"a".repeat(5)} HTTP/1.1`, token).ok,
    false,
  );
  assert.equal(
    checkRequestLine(`GET /json/version?token=${"z".repeat(200)} HTTP/1.1`, token).ok,
    false,
  );
  assert.equal(checkRequestLine(`GET /json/version?token=${token} HTTP/1.1`, token).ok, true);
});
...
test("an authenticated client can execute a browser command through the gate", async () => {
  ...
  try {
    ...
    assert.equal(
      backend.messages.some((m) => m.method === "Target.getTargets"),
      true,
...
test("pipelined second request on an unauthenticated connection never reaches the backend", async () => {
  ...
  try {
    ...
    const resp = await new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(
        () => finish(new Error("timed out waiting for the static protocol response")),
        5000,
      );
    ...
    assert.match(
      resp,
      /^HTTP\/1\.1 200/,
      `protocol must be served statically: ${resp.substring(0, 60)}`,
    );
    ...
    await waitFor(async () => {
      const n = await new Promise((resolve) =>
        gate.server.getConnections((err, count) => resolve(err ? null : count)),
      );
      return n === 0;
    });
    assert.equal(
      tunnels.length,
      0,
      "no tunnel may be opened for the unauthenticated protocol path",
    );
    assert.equal(
      backend.sockets.length,
      0,
      "a pipelined second request must never reach the backend",
    );
  } finally {
    await gate.close();
    for (const s of tunnels) {
...
test("token rotation invalidates the old token at the gate", async () => {
  ...
  try {
    await sendAndWaitTunnels(
      port,
      `GET /json/version?token=${oldToken} HTTP/1.1\r\nHost: x\r\n\r\n`,
      1,
      tunnels,
    );
  } finally {
  ...
  try {
    ...
    await sendAndWaitTunnels(
      port,
      `GET /json/version?token=${newToken} HTTP/1.1\r\nHost: x\r\n\r\n`,
      2,
      tunnels,
    );
  } finally {
