...
test('clickCoords escapes backslash and quote in the generated selector', async () => {
  ...
  assert.ok(evals[0].params.expression.includes("querySelectorAll('#a\\'b\\\\c')"),
    `selector not escaped: ${evals[0].params.expression}`);
  const clicks = client.calls.filter((c) => c.method === 'Input.dispatchMouseEvent');
  assert.deepEqual(clicks.map((c) => [c.params.type, c.params.x, c.params.y]), [
    ['mousePressed', 10, 21],
    ['mouseReleased', 10, 21],
  ]);
});
...
test('clickCoords throws when the element is not found', async () => {
  ...
  await assert.rejects(clickCoords(client, 's', '#missing', 3), /Element not found: "#missing" at index 3/);
});
