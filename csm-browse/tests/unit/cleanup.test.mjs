...
test("stopDaemon: with a session id, an unverifiable pid is never signaled", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      signals,
      [],
      `no signal may be sent to an unverifiable pid, got ${JSON.stringify(signals)}`,
    );
  }
});
...
test("stopDaemon: live pid -> SIGTERM then true, no SIGKILL", async () => {
  ...
  try {
    ...
    assert.equal(
      sigkills.length,
      0,
      `no SIGKILL expected on clean stop, got ${JSON.stringify(signals)}`,
    );
  }
});
...
test("killGate: kills only the gate owning the exact public port", async () => {
  ...
  setExecLayerForTests({
    hostPgrep: async () => [
      ...
      {
        pid: 222,
        cmd: "node /x/cdp-gate.mjs --sid s2 --port 9225 --internal 9224 --container chromium-vnc",
      },
    ],
...
test("killInstance: pkills both profile patterns; unsafe path rejected", async () => {
  const calls = [];
  setExecLayerForTests({
    pkillMatch: async (container, pattern) => calls.push([container, pattern]),
  });
  try {
  ...
  await assert.rejects(
    () => cleanup.killInstance("chromium-vnc", "/etc/passwd"),
...
test("removeHostSession: removes a session dir inside the root; rejects escapes", async () => {
  ...
  await assert.rejects(
    () => cleanup.removeHostSession(join(root, "..", "escape")),
    /Path escapes csm-browse root/,
  );
});
...
test("releasePorts: releases the chromium instance; does not re-kill the gate (F-067-13b)", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      false,
      `no gate kill expected: ${JSON.stringify(calls)}`,
    );
    assert.ok(
      calls.some((c) => c[0] === "pkillMatch"),
      `pkillMatch missing: ${JSON.stringify(calls)}`,
    );
  } finally {
    setExecLayerForTests();
  }
...
test("truncateLogs: removes the container chromium.log and the host daemon.log", async () => {
  ...
  try {
    ...
    assert.deepEqual(calls, [
      ["chromium-vnc", ["rm", "-f", "/config/csm-browse/sessions/abc123/chromium.log"]],
    ]);
