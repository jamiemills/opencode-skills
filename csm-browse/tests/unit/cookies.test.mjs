...
function makeClient(opts = {}) {
  ...
  const client = {
    ...
    emit(ev, params) {
      for (const fn of handlers.get(ev) || []) fn(params);
    },
    listenerCount(ev) {
      ...
      if (method === "Page.getFrameTree") {
        return {
          frameTree: {
            frame: {
              id: "frame-wall",
              url: opts.wallUrl ?? "https://sp-prod.example.net/index.html",
            },
            childFrames: [],
          },
        };
...
test("all OneTrust selectors are evaluated and sessionId is passed through", async () => {
  ...
  for (const sel of selectors) {
    assert.ok(
      evaluates(client).some((c) =>
        c.params.expression.includes(`document.querySelector('${sel}')`),
      ),
...
test("generic click pattern embeds every accept text", async () => {
  const client = makeClient();
  await dismissCookies(client, "s");
  const generic = evaluates(client).find(
    (c) =>
      c.params.expression.includes("texts=[") &&
      c.params.expression.includes('button,[role="button"],a.button'),
  );
...
test("no consent wall found: returns early, no context work, no wall removal", async () => {
  ...
  assert.ok(
    !evals.some((c) => c.params.expression.includes("removed=n")),
    "wall-removal ran without a wall",
  );
...
test("no matching accept text: single generic attempt, no dismissal value", async () => {
  ...
  await dismissCookies(client, "s");
  const generic = evaluates(client).filter((c) =>
    c.params.expression.includes('button,[role="button"],a.button'),
  );
...
test("consent wall path: frame context click + scroll-unlock removal", async () => {
  const client = makeClient({
    walls: ["https://cmpv2.example.org/wall"],
    emitContext: true,
    wallClick: "clicked",
  });
