...
const server = createServer((req, res) => {
  ...
  if (req.url.startsWith('/error')) { res.writeHead(500, { 'Content-Type': 'text/plain' }); res.end('boom'); return; }
  if (req.url.startsWith('/badjson')) { res.writeHead(200, { 'Content-Type': 'text/plain' }); res.end('{not json'); return; }
  if (req.url.startsWith('/flaky')) {
    ...
    if (!server.flakyCount || server.flakyCount < 2) {
      ...
      res.writeHead(500, { 'Content-Type': 'text/plain' }); res.end('not yet'); return;
    }
  }
...
test('cdpFetchJson aborts when the attempt exceeds the timeout budget (curl -m semantics)', async () => {
  ...
  await assert.rejects(cdpFetchJson(`${base}/slow`, { timeoutMs: 500 }), (err) =>
    err && (err.name === 'AbortError' || err.name === 'TimeoutError' || /abort|timeout/i.test(String(err))));
...
test('cdpProbe retries until ready and gives up after the budget', async () => {
  ...
  const ready = await cdpProbe(`${base}/flaky`, { timeoutMs: 4000, attemptTimeoutMs: 500, delayMs: 50 });
  ...
  const start = Date.now();
  const failed = await cdpProbe(`${base}/error`, { timeoutMs: 400, attemptTimeoutMs: 300, delayMs: 30 });
