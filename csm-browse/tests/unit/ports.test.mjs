...
function stubLayer({ isPortFree, pgrep } = {}) {
  ...
  setExecLayerForTests({
    isPortFree: async (c, p) => {
      ...
      return isPortFree ? isPortFree(p) : true;
    },
...
test("allocate returns a distinct in-range port pair per claim", async () => {
  ...
  await writeFile(
    join(root, "alloc-a", "state.json"),
    JSON.stringify({ internalPort: 9224, publicPort: 9225 }),
  );
  ...
  await writeFile(
    join(root, "alloc-b", "creating.marker"),
    JSON.stringify({ internal: 9226, public: 9227 }),
  );
  ...
  for (const pair of [first, second, third]) {
    assert.ok(
      pair.internal >= 9224 && pair.internal <= 9234,
      `internal out of range: ${pair.internal}`,
    );
...
test("allocate skips pairs whose public port is held by a stale container TCP-LISTEN socat", async () => {
  ...
  setExecLayerForTests({
    ...
    pgrepMatch: async (c, pattern) => {
      calls.pgrep.push(pattern);
      if (pattern === "TCP-LISTEN:9225,")
        return [{ pid: 1, cmd: `socat ${pattern}fork,reuseaddr TCP:127.0.0.1:9224` }];
      return [];
    },
  });
  const pair = await allocate("chromium-vnc");
  assert.ok(
    calls.pgrep.includes("TCP-LISTEN:9225,"),
    "allocate must probe the container for stale socats",
  );
