...
test('browse.mjs prints every err.message through redactTelemetry', async () => {
  const source = await readFile(fileURLToPath(new URL('../../scripts/browse.mjs', import.meta.url)), 'utf-8');
  assert.ok(
  ...
  assert.ok(source.includes("import { redactTelemetry } from '../lib/security.mjs';"), 'redactTelemetry must be imported');
});
