...
const { validateSid, sessionDir, loadState, saveState, removeState, rotateToken } = await import('../../lib/session.mjs');
...
test('token lifecycle round-trips through state.json at 0600', async () => {
  const state = {
    sid: 'tok-rt', token: 'A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8S9t0U1v2',
    tokenGeneration: 1, internalPort: 9224, publicPort: 9225,
    wsUrl: 'ws://127.0.0.1:9225/devtools/browser/x?token=A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8S9t0U1v2',
...
test('saveState fails closed: a token without a matching embedded token is rejected', async () => {
  ...
  await assert.rejects(
    saveState('tok-bad-1', { sid: 'tok-bad-1', token, wsUrl: 'ws://127.0.0.1:9225/devtools/browser/x' }),
    /token mismatch/
  );
  await assert.rejects(
