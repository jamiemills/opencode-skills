...
test('F-009/F-015 a marker-only dir with an aged marker is reaped despite a young dir', async () => {
  ...
  assert.ok(result.swept.some((e) => e.startsWith(`sid=${sid}`)), `marker-only dir not reaped: ${result.swept}`);
...
test('F-009 reaping a marker-only dir frees its claimed port pair', async () => {
  ...
  assert.ok(claimedBefore.has(9224) && claimedBefore.has(9225), `pair must be claimed before reap: ${[...claimedBefore]}`);
  ...
  assert.ok(!claimedAfter.has(9224) && !claimedAfter.has(9225), `pair must be freed after reap: ${[...claimedAfter]}`);
});
...
test('F-009 a corrupt (unparseable) state.json is swept by the age gate', async () => {
  ...
  const result = await sweep(sweepOpts);
  assert.ok(result.swept.some((e) => e.startsWith(`sid=${sid}`)), `corrupt-state dir not reaped: ${result.swept}`);
...
async function waitForProc(child) {
  ...
  while (Date.now() < deadline) {
    try { await readFile(`/proc/${child.pid}/cmdline`, 'utf-8'); return; } catch {}
...
test('F-021 sweep orphan-daemon pass kills only pids whose argv is the session daemon', async () => {
  ...
  try {
    const result = await sweep(sweepOpts);
    assert.ok(result.swept.includes(`orphan daemon sid=orph-m pid=${matched.pid}`),
...
test('F-067-4 one malformed pgrep line does not abort the orphan-daemon pass', async () => {
  ...
  const result = await sweep(sweepOpts);
  assert.ok(result.swept.includes('orphan daemon sid=orph-ok pid=90002'),
...
test('F-067-13b releasePorts does not re-kill the gate, still releases the instance', async () => {
  const calls = [];
  setExecLayerForTests({
    hostPgrep: async () => { calls.push(['hostPgrep']); return [{ pid: 111, cmd: 'node /x/cdp-gate.mjs --sid s1 --port 9224 --internal 9223 --container chromium-vnc' }]; },
  ...
  try {
    ...
    assert.ok(!calls.some((c) => c[0] === 'killPid'), `gate must not be re-killed: ${JSON.stringify(calls)}`);
    assert.ok(!calls.some((c) => c[0] === 'hostPgrep'), 'releasePorts must not scan for gates');
    assert.ok(calls.some((c) => c[0] === 'pkillMatch'), `instance must still be released: ${JSON.stringify(calls)}`);
  } finally {
    setExecLayerForTests();
  }
});
...
test('F-067-14 stale recorder lock flip is atomic (temp+rename, no residue)', async () => {
  ...
  const result = await sweep(sweepOpts);
  assert.ok(result.swept.includes('stale recorder lock sid=rec-atomic'), `not flipped: ${result.swept}`);
