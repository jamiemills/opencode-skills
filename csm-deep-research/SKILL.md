...
The skill is standalone by design: it hands off to no other skill, keeps a strict write allowlist (one research document plus one temp dir), and runs its long multi-agent pipeline under a named tmux session so a detached run survives the invoking terminal. It resumes mid-pipeline from the research document's embedded Control journal, never from chat history. The final state is SAVED; the research document is the single durable artifact of the run.
...
- The finding is the single dated durable artifact; the run stops at SAVED.
