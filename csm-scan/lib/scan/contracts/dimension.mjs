...
export const COVERAGE_STATES = Object.freeze([
  'complete', 'incomplete', 'unsupported', 'excluded',
]);
...
const DIMENSION_KEYS = Object.freeze([
  'applicability', 'expectedClaimIds', 'id', 'order', 'providerCapability',
  'rendererId', 'retryable',
]);
const CLAIM_KEYS = Object.freeze([
  'applicabilityEvidenceIds', 'coverageState', 'derivationId', 'dimensionId',
  'evidenceIds', 'id', 'inputEvidenceIds', 'limitations', 'searchSpace', 'status',
]);
...
function safeValue(field, value, ErrorType) {
  ...
  if (typeof value !== 'string' || value.length === 0 || value.length > CONTRACT_LIMITS.text
...
function normalizeRule(rule) {
  ...
  if (!APPLICABILITY_FIELDS.includes(rule.field) || !APPLICABILITY_OPERATORS.includes(rule.operator)) {
...
function normalizeApplicability(value) {
  ...
  if (!['all', 'any'].includes(value.mode) || !Array.isArray(value.rules)
      || value.rules.length === 0 || value.rules.length > CONTRACT_LIMITS.applicabilityRules) {
    fail(DimensionContractError, 'INVALID_APPLICABILITY', 'applicability predicate is invalid or too large');
  }
...
export function validateDimension(definition) {
  ...
  if (canonicalOrder < 0) fail(DimensionContractError, 'UNKNOWN_DIMENSION', 'dimension id is not registered');
  if (definition.order !== canonicalOrder) fail(DimensionContractError, 'INVALID_ORDER', 'dimension order is not canonical');
  ...
  if (expectedClaimIds.length === 0) fail(DimensionContractError, 'INVALID_EXPECTED', 'dimension must own expected claims');
  if (typeof definition.retryable !== 'boolean' || typeof definition.providerCapability !== 'boolean') {
  ...
  if (definition.providerCapability !== PROVIDER_DIMENSION_IDS.includes(id)) {
    fail(DimensionContractError, 'INVALID_PROVIDER_FLAG', 'providerCapability does not match the canonical dimension mapping');
  }
  const rendererId = identifier(definition.rendererId, RENDERER_ID_PATTERN, DimensionContractError, 'rendererId');
  return deepFreeze({
    id,
...
export function validateDimensions(definitions) {
  ...
  if (new Set(expected).size !== expected.length) {
    fail(DimensionContractError, 'DUPLICATE_ID', 'expected claim identifiers must be globally unique');
  }
...
function normalizeLimitations(value) {
  ...
  const result = value.map((item) => {
    if (typeof item !== 'string' || item.length === 0 || item.length > CONTRACT_LIMITS.text
        || item !== item.trim() || /[^\x20-\x7e]/.test(item)) {
...
function referencedEvidence(ids, claimId, index) {
  return ids.map((id) => {
    ...
    if (!evidence || evidence.claimId !== claimId) {
      fail(ClaimContractError, 'INVALID_EVIDENCE', 'claim references missing or unrelated evidence');
    }
    return evidence;
  });
}
...
function searchState(status, searchSpace) {
  const cleanComplete = searchSpace.supported && searchSpace.readable && searchSpace.complete
    && !searchSpace.capped && !searchSpace.error && !searchSpace.malformed
    && !searchSpace.ambiguous && searchSpace.omittedCount === 0;
  const cleanUnsupported = !searchSpace.supported && !searchSpace.readable && !searchSpace.complete
    && !searchSpace.capped && !searchSpace.error && !searchSpace.malformed
    && !searchSpace.ambiguous && searchSpace.filesInspected === 0
    && searchSpace.bytesInspected === 0 && searchSpace.recordsInspected === 0
    && searchSpace.omittedCount === 0;
  const incomplete = searchSpace.supported && (!searchSpace.readable || !searchSpace.complete
    || searchSpace.capped || searchSpace.error || searchSpace.malformed
    || searchSpace.ambiguous || searchSpace.omittedCount > 0);
  if ((status === 'not_detected' && !cleanComplete)
...
function provesNotApplicable(dimension, evidence) {
  ...
  if (facts.some(({ field }) => !allowedFields.has(field))) {
    fail(ClaimContractError, 'APPLICABILITY_PROOF', 'applicability evidence contains unrelated facts');
  }
  for (const field of allowedFields) {
    ...
    if (selected.some((fact) => fact.present) && selected.some((fact) => !fact.present)) {
      fail(ClaimContractError, 'APPLICABILITY_PROOF', 'applicability evidence contains conflicting facts');
    }
  }
  const outcomes = dimension.applicability.rules.map((rule) => ruleResult(rule, facts));
  const provenFalse = dimension.applicability.mode === 'all'
    ? outcomes.some((outcome) => outcome === false)
    : outcomes.every((outcome) => outcome === false);
  if (!provenFalse) fail(ClaimContractError, 'APPLICABILITY_PROOF', 'applicability evidence does not prove the predicate false');
}
...
function normalizeClaim(record, index, dimension) {
  ...
  if (!CLAIM_STATUSES.includes(record.status)) fail(ClaimContractError, 'INVALID_STATUS', 'status is not allowlisted');
  ...
  const evidenceIds = boundedIdArray(
    record.evidenceIds, EVIDENCE_ID_PATTERN, ClaimContractError, 'evidenceIds', CONTRACT_LIMITS.evidenceIds,
  );
  const inputEvidenceIds = boundedIdArray(
    record.inputEvidenceIds, EVIDENCE_ID_PATTERN, ClaimContractError, 'inputEvidenceIds', CONTRACT_LIMITS.evidenceIds,
  );
  ...
  if (inputEvidenceIds.some((entry) => !evidenceIds.includes(entry))
      || applicabilityEvidenceIds.some((entry) => !evidenceIds.includes(entry))) {
    fail(ClaimContractError, 'INVALID_EVIDENCE', 'specialized evidence must be included in claim evidence');
  }
  ...
  const inadmissible = evidence.some((entry) => !['search_space', 'applicability'].includes(entry.category)
    && !DIMENSION_EVIDENCE_CATEGORIES[dimension.id].includes(entry.category));
  if (inadmissible) {
    fail(ClaimContractError, 'DIMENSION_CATEGORY', 'evidence category is not admissible for the claim dimension');
  }

  if (record.status === 'observed') {
    if (record.derivationId !== null || inputEvidenceIds.length || applicabilityEvidenceIds.length || searchSpace !== null
        || evidence.length === 0 || evidence.some((entry) => ['search_space', 'applicability'].includes(entry.category))) {
      fail(ClaimContractError, 'STATUS_EVIDENCE', 'observed requires only direct canonical evidence');
    }
  } else if (record.status === 'inferred') {
    derivationId = identifier(record.derivationId, DERIVATION_ID_PATTERN, ClaimContractError, 'derivationId');
    const inputs = referencedEvidence(inputEvidenceIds, id, index);
    if (inputs.length === 0 || applicabilityEvidenceIds.length || searchSpace !== null
      ...
      fail(ClaimContractError, 'STATUS_EVIDENCE', 'inferred requires direct canonical input evidence');
    }
  } else if (['not_detected', 'unsupported', 'unverified'].includes(record.status)) {
    if (record.derivationId !== null || inputEvidenceIds.length || applicabilityEvidenceIds.length
        || searchSpace === null || evidence.length === 0
        || evidence.some((entry) => entry.category !== 'search_space')) {
      fail(ClaimContractError, 'STATUS_EVIDENCE', 'search status requires only canonical search-space evidence');
    }
    const canonical = JSON.stringify(searchSpace);
    if (evidence.some((entry) => JSON.stringify(entry.details) !== canonical)) {
      fail(ClaimContractError, 'STATUS_EVIDENCE', 'claim and evidence search-space details must be identical');
    }
    searchState(record.status, searchSpace);
  } else {
    const applicability = referencedEvidence(applicabilityEvidenceIds, id, index);
    if (record.derivationId !== null || inputEvidenceIds.length || searchSpace !== null
        || evidence.length === 0 || applicability.length !== evidence.length
        || evidence.some((entry) => entry.category !== 'applicability')) {
      fail(ClaimContractError, 'STATUS_EVIDENCE', 'not_applicable requires only applicability evidence');
    }
...
export function validateClaims(records, evidenceRecords, dimensionRegistry) {
  ...
  const result = records.map((record) => {
    ...
    exactKeys(record, CLAIM_KEYS, ClaimContractError, 'claim');
    const dimension = record && typeof record.dimensionId === 'string'
      ? dimensionsById.get(record.dimensionId)
      : null;
    if (!dimension) fail(ClaimContractError, 'UNKNOWN_DIMENSION', 'claim dimension is not registered');
    return normalizeClaim(record, index, dimension);
  });
...
export function computeCoverage(records, evidenceRecords, dimensionRegistry) {
  if (dimensionRegistry === undefined) {
    fail(ClaimContractError, 'MISSING_REGISTRY', 'coverage requires an explicit dimension registry');
  }
  const dimensions = validateDimensions(dimensionRegistry);
