...
export function renderCrossRepositoryGlobal(_repoName, model, context = DEFAULT_RENDER_CONTEXT) {
  ...
  lines.push("");
  lines.push(
    `> Declared cross-repository references resolved against exact repository and component identities across ${count(metrics.repositories ?? 0, "repository identity", "repository identities")} and ${count(metrics.components ?? 0, "component identity", "component identities")}.`,
  );
  lines.push("");
  lines.push(
    `Resolved edges: ${metrics.edges ?? 0} (${metrics.crossRepositoryEdges ?? 0} cross-repository, ${metrics.selfEdges ?? 0} self). External references: ${metrics.external ?? 0}. Ambiguous references: ${metrics.ambiguous ?? 0}. Unresolved identities: ${metrics.unresolved ?? 0}.`,
  );
  lines.push("");
  ...
  const repositories = [
    ...(Array.isArray(identityTable.repositories) ? identityTable.repositories : []),
