...
function toRel(repoPath, abs) {
  return toPosix(relative(repoPath, abs));
}
function extOf(rel) {
...
function isDeclarationFile(rel) {
  return /\.d\.(ts|mts|cts)$/i.test(toPosix(rel));
}
function isDirectory(absPath) {
  try {
    return readdirSync(absPath) && true;
  } catch {
    return false;
  }
}
...
function detectPackageRoot(repoPath, manifest, primary) {
  ...
  if (primary === "shell") {
    return {
      pkgRoot: existsSync(join(repoPath, "scripts")) ? "scripts" : "",
      primaryPackage: null,
    };
  }
  ...
  while (pending.length && visited++ < maxDirs) {
    ...
    for (const entry of entries) {
      ...
      if (
        ...
        PYTHON_DISCOVERY_IGNORES.has(entry.name) ||
        /\.egg-info$/i.test(entry.name)
      )
        continue;
...
function discoverTopPackages(repoPath) {
  ...
  for (const { abs, relBase } of roots) {
    ...
    for (const e of entries) {
      ...
      if (e.name.startsWith(".") || e.name === "__pycache__" || /\.egg-info$/i.test(e.name))
        continue;
...
function buildRustContext(repoPath, manifest) {
  ...
  const workspace =
    manifest && manifest.workspace ? manifest.workspace : rootCargo && rootCargo.workspace;
  const members =
    workspace && Array.isArray(workspace.resolvedMembers)
...
function rustCrateRootFor(file, crateRoots) {
  ...
  for (const root of crateRoots) {
    ...
    if (
      (file === root || file.startsWith(`${sourceDir}/`)) &&
      (!match || sourceDir.length > dirnameOf(match).length)
    )
      match = root;
  }
  return match;
...
function parseImports(filePath, ecosystem, ctx) {
  ...
  const add = (target, kind) => {
    ...
    if (target.endsWith("/")) {
      ...
      for (const e of entries) {
        ...
        if (ctx.sourceSet.has(rel) && !seen.has(rel)) {
          seen.add(rel);
          edges.push({ target: rel, kind });
        }
      }
      return;
    }
    if (ctx.sourceSet.has(target) && !seen.has(target)) {
      seen.add(target);
  ...
  if (ecosystem === "python") {
    ...
    for (const raw of joinParenLines(content)) {
      ...
      if (relRe.test(line)) {
        ...
        if (m) {
          ...
          for (const t of resolvePythonRelative(
            filePath,
            m[1].length,
            m[2] || "",
            names,
      ...
      if (absRe.test(line)) {
        ...
        if (m) {
          ...
          for (const t of resolvePythonAbsolute(m[1], names, ctx.topPackages, ctx.repoPath))
            add(t, "from-import");
          continue;
        }
        ...
        if (m) {
          for (const mod of splitNames(m[1])) {
            for (const t of resolvePythonAbsolute(mod, [], ctx.topPackages, ctx.repoPath))
              add(t, "import");
          }
        }
      }
    ...
    while ((m = JS_SIDE_EFFECT_RE.exec(importSource)))
      add(resolveJsSpecifier(m[1], filePath, ctx), "side-effect");
    JS_DYN_RE.lastIndex = 0;
    let dm;
    while ((dm = JS_DYN_RE.exec(importSource)))
      add(resolveJsSpecifier(dm[1], filePath, ctx), "dynamic-import");
    JS_REQUIRE_RE.lastIndex = 0;
    let rm;
    while ((rm = JS_REQUIRE_RE.exec(importSource)))
      add(resolveJsSpecifier(rm[1], filePath, ctx), "require");
    ...
    while ((m = crateRe.exec(content))) {
      ...
      for (const t of resolveRustCrate(segs, filePath, ctx.crateRoots, ctx.repoPath))
        add(t, "use-crate");
    }
    ...
    const bareRe = new RegExp(
      `^\\s*${VIS}use\\s+(?!crate::|self::|super::)([A-Za-z_][\\w]*(?:::[\\w]+)*)`,
      "gm",
    );
    let bm;
    ...
    while ((m = re.exec(content)))
      add(resolveShellRelative(filePath, m[1], ctx.repoPath), "source");
    // P2: best-effort script invocations beyond `source`/`.`.
    const bashRe = /(?:^|[\s;&|(])(?:bash|sh|zsh)\s+['"]?([\w./-]+\.sh)['"]?/g;
    while ((m = bashRe.exec(content)))
      add(resolveShellRelative(filePath, m[1], ctx.repoPath), "script");
  ...
  for (const eco of ecosystems) {
    for (const m of matchDep(deps, DATABASE_INDICATORS[eco] || {})) {
      ...
      if (!seenDb.has(key)) {
        seenDb.add(key);
        dbs.push({
          id: slug(m.name) || `db${dbs.length}`,
          label: m.label,
          desc: m.type || "Database",
    ...
    for (const m of matchDep(deps, EXTERNAL_API_INDICATORS[eco] || {})) {
      ...
      if (!seenApi.has(key)) {
        seenApi.add(key);
        apis.push({
          id: slug(m.name) || `api${apis.length}`,
          label: m.label,
          desc: m.type || "External service",
          relation: "Calls",
        });
      }
    }
...
function generateC4Context(repoName, manifest, technology, nodes) {
  ...
  if (dbs.length)
    dbs.slice(0, 2).forEach((d) => lines.push(`  System_Ext(${d.id}, "${d.label}", "${d.desc}")`));
  ...
  if (dbs.length)
    dbs.slice(0, 2).forEach((d) => lines.push(`  Rel(app, ${d.id}, "${d.relation}")`));
...
function generateC4Container(repoName, layers, technology, nodes) {
  ...
  if (layers.entryPoints.length) {
    const epNames = layers.entryPoints
      .slice(0, 3)
      .map((f) => basenameOf(f).replace(/\.[^.]+$/, ""));
    lines.push(
      `  Container(entry, "Entry Points", "${technology}", "${escapeMermaid(epNames.join(", "))}")`,
    );
  }
  if (layers.libModules.length) {
    lines.push(
      `  Container(lib, "Core Modules", "${technology}", "${layers.libModules.length} modules")`,
    );
  }
  if (layers.shared.length) {
    lines.push(
      `  Container(shared, "Shared Utilities", "${technology}", "${layers.shared.length} modules")`,
    );
  }
  const { dbs } = nodes;
  if (dbs.length)
    dbs.slice(0, 2).forEach((d) => lines.push(`  ContainerDb(${d.id}, "${d.label}", "${d.desc}")`));
  else lines.push('  ContainerDb(fs, "File System", "Local storage")');
  if (layers.entryPoints.length && layers.libModules.length)
    ...
    if (ext === ".py" || ext === ".pyi") {
      for (const line of lines) {
        ...
        if (/^\s*__all__\s*=/.test(line)) {
          exports.push({ kind: "all", name: "__all__" });
          continue;
        }
      }
      ...
      for (const line of lines) {
        ...
        if ((m = line.match(/export\s+(?:async\s+)?class\s+(\w+)/))) {
          exports.push({ kind: "class", name: m[1] });
          continue;
        }
        if ((m = line.match(/export\s+(?:const|let|var)\s+(\w+)\s*=/))) {
          exports.push({ kind: "variable", name: m[1] });
          continue;
        }
        if ((m = line.match(/export\s+(?:type|interface)\s+(\w+)/))) {
          exports.push({ kind: "type", name: m[1] });
          continue;
        }
        if ((m = line.match(/export\s+enum\s+(\w+)/))) {
          exports.push({ kind: "enum", name: m[1] });
          continue;
        }
        if ((m = line.match(/export\s+default\s+(?:async\s+)?function\s+(\w+)/))) {
          exports.push({ kind: "default-function", name: m[1] });
          continue;
        }
        if ((m = line.match(/export\s+default\s+class\s+(\w+)/))) {
          exports.push({ kind: "default-class", name: m[1] });
          continue;
        }
        // Re-exports.
        if ((m = line.match(/export\s+\*\s+(?:as\s+(\w+)\s+)?from\s+['"]([^'"]+)['"]/))) {
          exports.push({ kind: "re-export", name: m[1] || `* from ${m[2]}` });
          continue;
        }
        ...
        if ((m = line.match(/module\.exports\s*=\s*(\w+)/))) {
          exports.push({ kind: "cjs-export", name: m[1] });
          continue;
        }
        if ((m = line.match(/^exports\.(\w+)\s*=/))) {
          exports.push({ kind: "cjs-export", name: m[1] });
          continue;
        }
      }
      ...
      for (const line of lines) {
        if ((m = line.match(new RegExp(`^\\s*${vis}(?:async\\s+)?fn\\s+(\\w+)`)))) {
          exports.push({ kind: "function", name: m[1] });
          continue;
        }
        if ((m = line.match(new RegExp(`^\\s*${vis}struct\\s+(\\w+)`)))) {
          exports.push({ kind: "class", name: m[1] });
          continue;
        }
        if ((m = line.match(new RegExp(`^\\s*${vis}enum\\s+(\\w+)`)))) {
          exports.push({ kind: "enum", name: m[1] });
          continue;
        }
        if ((m = line.match(new RegExp(`^\\s*${vis}trait\\s+(\\w+)`)))) {
          exports.push({ kind: "trait", name: m[1] });
          continue;
        }
        if ((m = line.match(new RegExp(`^\\s*${vis}type\\s+(\\w+)`)))) {
          exports.push({ kind: "type", name: m[1] });
          continue;
        }
        if ((m = line.match(new RegExp(`^\\s*${vis}(?:const|static)\\s+(\\w+)`)))) {
          exports.push({ kind: "constant", name: m[1] });
          continue;
        }
        if ((m = line.match(new RegExp(`^\\s*${vis}mod\\s+(\\w+)`)))) {
          exports.push({ kind: "module", name: m[1] });
          continue;
        }
        if ((m = line.match(new RegExp(`^\\s*${vis}use\\s+([^;]+)`)))) {
          exports.push({ kind: "re-export", name: m[1].trim() });
          continue;
        }
      }
    } else {
...
function generateC4Code(repoName, layers) {
  ...
  const modules = [
    ...(layers.coreModules || layers.libModules || []),
    ...(layers.shared || []),
  ].slice(0, 3);
  ...
  for (const mod of exports) {
    ...
    lines.push(
      `  Component(${cName}, "${escapeMermaid(simpleName)}", "${escapeMermaid(mod.file)}")`,
    );
    let n = 0;
...
async function prepareGraph(repoPath, overview) {
  ...
  const rustCtx = ecosystems.all.includes("rust")
    ? buildRustContext(repoPath, manifest)
    : { crateRoots: new Set(), localCrateRoots: new Map() };
  const alias = ecosystems.all.some((e) => e === "typescript" || e === "javascript")
    ? readAliasConfig(repoPath)
    : { baseUrl: "", entries: [] };
  const workspaceMap = ecosystems.all.some((e) => e === "typescript" || e === "javascript")
    ? buildWorkspaceMap(repoPath, manifest)
    : new Map();
  ...
  const ctx = {
    ...
    primaryPackage,
    topPackages,
    crateRoots: rustCtx.crateRoots,
    localCrateRoots: rustCtx.localCrateRoots,
    alias,
    workspaceMap,
  };
...
export async function scan(repoPath, overview) {
  ...
  const { ecosystems, manifest, moduleFiles, sourceFiles, ctx, entrypointFiles } = prepared;
  const { primary } = ecosystems;
...
export async function analyzeGraphFacts(repoPath, overview, options = {}) {
  ...
  const { graph, edgeKinds, edgesInspected, edgesOmitted } = buildImportGraph(
    analyzed,
    ecosystems.all,
    analyzedCtx,
    limits,
