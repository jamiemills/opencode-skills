...
function looksLikeImage(value) {
  return typeof value === 'string' && !isInterpolated(value)
    && /[/:.]/.test(value) && value.length <= DEPLOYMENT_LIMITS.maxLabel;
}
...
function normalizeK8sKind(kind) {
  const snake = String(kind).replace(/([a-z0-9])([A-Z])/g, '$1_$2').toLowerCase();
...
class ExtractionContext {
  ...
  addEdge(from, to, kind, line) {
    if (!this.declared.has(from) || !this.declared.has(to)) {
      throw new DeploymentModelError('UNCITED_EDGE', 'topology edges must cite declared resources or services');
    }
  ...
  finalize() {
    ...
    for (const stub of this.stubs) {
      ...
      const resolved = stub.kind === 'cloud_resource'
        ? this.labelResolved(stub.label)
        : this.declared.has(exactId) ? exactId : null;
...
function safeBuildContext(value) {
  ...
  if (value.startsWith('/') || value.startsWith('~') || value.startsWith('../') || value.includes('\\')) return null;
  return value;
}
...
function extractCompose(text, path) {
  ...
  if (services && typeof services === 'object' && !Array.isArray(services)) {
    for (const [name, service] of Object.entries(services)) {
      ...
      if (Array.isArray(service.ports)) {
        const ports = service.ports.map(portValue).filter((value) => value !== null).slice(0, 8);
        if (ports.length > 0) attributes.ports = ports;
      }
      const buildContext = safeBuildContext(typeof service.build === 'string' ? service.build : service.build?.context);
      ...
      if (Array.isArray(service.volumes)) {
        for (const entry of service.volumes) {
          ...
          } else if (entry !== null && typeof entry === 'object' && typeof entry.source === 'string') {
            source = entry.source;
          }
          if (source !== null && boundedToken(source) !== null
  ...
  for (const [kind, key] of [['network', 'networks'], ['volume', 'volumes'], ['configmap', 'configs'], ['secret', 'secrets']]) {
  ...
  if (Array.isArray(container.env)) {
    for (const entry of container.env) {
      ...
      if (typeof entry.valueFrom?.configMapKeyRef?.name === 'string') {
        ctx.resolve('configmap', entry.valueFrom.configMapKeyRef.name, resourceIdOf, 'value_from', docIndex);
      }
      if (typeof entry.valueFrom?.secretKeyRef?.name === 'string') {
        ctx.resolve('secret', entry.valueFrom.secretKeyRef.name, resourceIdOf, 'value_from', docIndex);
      }
    }
  }
}
...
function k8sVolumes(ctx, resourceIdOf, spec, docIndex) {
  ...
  for (const volume of spec.volumes) {
    ...
    if (typeof volume.persistentVolumeClaim?.claimName === 'string') {
      ctx.resolve('pvc', volume.persistentVolumeClaim.claimName, resourceIdOf, 'volume_from', docIndex);
    }
  }
}

function handleContainer(ctx, container, resourceLabel, docIndex) {
  if (typeof container.name !== 'string' || container.name === '' || boundedToken(container.name) === null) return;
  let image = null;
  if (typeof container.image === 'string' && !isInterpolated(container.image) && boundedToken(container.image) !== null) {
...
function handleK8sEntry(ctx, entry, docIndex) {
  ...
  if (typeof entry.kind !== 'string' || entry.kind === '' || entry.metadata?.name === undefined) return;
  ...
  if (typeof entry.metadata?.namespace === 'string' && boundedToken(entry.metadata.namespace) !== null) {
    attributes.namespace = entry.metadata.namespace;
  }
  const id = ctx.addResource(kind, label, docIndex, Object.keys(attributes).length > 0 ? attributes : null);

  let templateSpec = entry.spec;
  if (entry.spec && typeof entry.spec === 'object' && !Array.isArray(entry.spec)) {
    templateSpec = entry.spec.template?.spec ?? entry.spec.jobTemplate?.spec?.template?.spec ?? entry.spec;
  }
  ...
  if (kind === 'service' && entry.spec && typeof entry.spec === 'object' && !Array.isArray(entry.spec)) {
    if (entry.spec.selector && typeof entry.spec.selector === 'object' && !Array.isArray(entry.spec.selector)) {
      const selectors = Object.entries(entry.spec.selector)
        .filter(([key, value]) => boundedToken(key) !== null && boundedToken(String(value)) !== null)
  ...
  if (kind === 'ingress' && entry.spec && typeof entry.spec === 'object' && !Array.isArray(entry.spec)
...
function extractHelmChart(text, path) {
  ...
  if (doc && typeof doc === 'object' && !Array.isArray(doc) && typeof doc.name === 'string'
      && boundedToken(doc.name) !== null) {
    const attributes = {};
    if (typeof doc.version === 'string' && boundedToken(doc.version) !== null) {
      attributes.version = doc.version;
    }
    ctx.addResource('chart', doc.name, null, Object.keys(attributes).length > 0 ? attributes : null);
  }
  return ctx.finalize();
}
...
function handleTfAttribute(ctx, block, name, valueStart, idx, lines) {
  ...
  if ((block.type === 'resource' || block.type === 'data') && (name === 'image' || name === 'image_uri' || name === 'ami')) {
...
function extractTerraform(text, path) {
  ...
  for (let idx = 0; idx < lines.length; idx++) {
    ...
    if (dynamic) {
      ...
      stack.push({ type: 'dynamic', label1: dynamic[1], label2: null, line: idx + 1, id: null, record: null });
      continue;
    }
    ...
    if (trimmed.includes('${') && /[{(]$/.test(trimmed)) {
      ...
      stack.push({ type: 'opaque', label1: null, label2: null, line: idx + 1, id: null, record: null });
      continue;
    }
    const nested = trimmed.match(/^([A-Za-z][\w-]*)(?:\s+"([^"]+)")?\s*\{$/);
    if (nested && stack.length > 0) {
      stack.push({ type: 'nested', label1: nested[2] ?? null, label2: null, line: idx + 1, id: null, record: null });
      continue;
    }
  }
  return ctx.finalize();
...
function walkCf(ctx, node, fromId) {
  ...
  for (const [key, value] of Object.entries(node)) {
    ...
    if (key === 'Condition' || (key.startsWith('Fn::') && !['Fn::GetAtt', 'Fn::Sub'].includes(key))) {
      ctx.addIndicator('intrinsic', null);
...
function extractCloudFormation(text, path) {
  ...
  if (doc.Parameters && typeof doc.Parameters === 'object' && !Array.isArray(doc.Parameters)) {
    for (const [name, definition] of Object.entries(doc.Parameters)) {
      ...
      ctx.addResource('parameter', name, null, Object.keys(attributes).length > 0 ? attributes : null);
    }
  }
...
function serverlessVariables(ctx, text) {
  for (const match of String(text).matchAll(/\$\{([^}]+)\}/g)) {
    ...
    if (prefix === 'env' || prefix === 'opt' || prefix === 'ssm' || prefix === 'cf' || prefix === 's3') {
      const label = inner.slice(0, 64);
      if (boundedToken(label) !== null) ctx.addStub('cloud_resource', label, null, 'resolver', null);
...
function extractServerless(text, path) {
  ...
  if (doc.provider && typeof doc.provider === 'object' && !Array.isArray(doc.provider)) {
    ...
    ctx.addResource('provider', boundedToken(providerName) !== null ? providerName : 'default', null,
  ...
  if (doc.functions && typeof doc.functions === 'object' && !Array.isArray(doc.functions)) {
    for (const [functionName, fn] of Object.entries(doc.functions)) {
      ...
      if (Array.isArray(fn.events) && fn.events.length > 0) {
        const events = fn.events.map((event) => {
      ...
      ctx.addService('function', functionName, null, image,
        Object.keys(attributes).length > 0 ? attributes : null);
...
const DEPLOYMENT_DIRS = new Set([
  'cloudformation', 'cfn', 'charts', 'deploy', 'deployment', 'helm', 'iac',
  'infra', 'infrastructure', 'k8s', 'kubernetes', 'manifests', 'sam',
  'terraform',
]);
...
function detectDeploymentKind(path, text) {
  ...
  if (/\.(?:ya?ml|json)$/i.test(base)) {
    if (/\{\{/.test(text) && /\/templates\/[^/]+\.(?:ya?ml|json)$/.test(path)) return 'helm_template';
...
function isPathCandidate(path) {
  ...
  if (base === 'Chart.yaml' || base === 'Chart.lock' || base === 'values.yaml' || base === 'values.yml') return true;
  if (/\.tf(?:\.json)?$/i.test(base)) return true;
  if (base === 'kustomization.yaml' || base === 'kustomization.yml' || base === 'template.yaml'
      || base === 'template.yml') return true;
  if (/\.(?:ya?ml|json)$/i.test(base)) {
...
export function extractArtifact(kind, text, path) {
  switch (kind) {
    case 'dockerfile': return extractDockerfile(text, path);
    case 'compose': return extractCompose(text, path);
    case 'kubernetes': return extractKubernetes(text, path);
    case 'helm_chart': return extractHelmChart(text, path);
    case 'helm_template': return extractHelmTemplate(text, path);
    case 'terraform': return extractTerraform(text, path);
    case 'cloudformation': return extractCloudFormation(text, path);
    case 'serverless': return extractServerless(text, path);
    default: throw new DeploymentModelError('UNSUPPORTED_ARTIFACT', 'no deployment extractor for artifact');
  }
}

export { detectDeploymentKind, extractCloudFormation, extractCompose, extractDockerfile, extractHelmChart, extractHelmTemplate, extractKubernetes, extractServerless, extractTerraform };
