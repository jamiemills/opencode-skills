...
function resourceRecord(value) {
  ...
  const id = value.id;
  if (id !== resourceId(kind, label))
    fail("INVALID_RECORD", "resource id must match kind and label");
  return deepFreeze({
    id,
    kind,
    label,
...
function imageRecord(value) {
  ...
  return deepFreeze({
    ...
    scope: assertMembers(
      value.scope,
      ["config", "container", "from", "image", "value"],
      "image scope",
    ),
...
function serviceRecord(value) {
  ...
  const id = value.id;
  if (id !== resourceId(kind, label))
    fail("INVALID_RECORD", "service id must match kind and label");
  return deepFreeze({
    id,
    kind,
    label,
...
export function createArtifactResult(raw) {
  ...
  return deepFreeze({
    ...
    resources: boundedList(raw.resources, DEPLOYMENT_LIMITS.maxResources, "resources").map(
      resourceRecord,
    ),
    images: boundedList(raw.images, DEPLOYMENT_LIMITS.maxImages, "images").map(imageRecord),
    services: boundedList(raw.services, DEPLOYMENT_LIMITS.maxServices, "services").map(
      serviceRecord,
    ),
    ...
    indicators: boundedList(raw.indicators, DEPLOYMENT_LIMITS.maxIndicators, "indicators").map(
      indicatorRecord,
    ),
    diagnostics: boundedList(raw.diagnostics, DEPLOYMENT_LIMITS.maxDiagnostics, "diagnostics").map(
      diagnosticRecord,
    ),
  });
}
...
export function mergeTopology(artifacts, options = DEPLOYMENT_LIMITS) {
  ...
  images.sort(
    (left, right) =>
      compareAscii(left.reference, right.reference) || compareAscii(left.path, right.path),
  );
  unresolved.sort(
    (left, right) =>
      compareAscii(left.kind, right.kind) ||
      compareAscii(left.label, right.label) ||
