    ...
    for (const entry of d.testConfigFiles) {
      if (entry.includes(":[")) {
        ...
        const hit = candidates.some(
          (c) =>
            files.some((f) => f === c || f.split("/").pop() === c) || existsSync(join(repoPath, c)),
        );
...
function scanDiffCoverEnv(repoPath) {
  ...
  if (examined < ENV_THRESHOLD_CAP) {
    const gates = readBoundedFile(join(repoPath, "quality", "gates.conf"), {
...
function readBounded(repoPath, relPath, cap) {
  const content = readBoundedFile(join(repoPath, relPath), {
    byteLimit: cap,
    containmentRoot: repoPath,
  });
...
function detectNetworkGuard(repoPath) {
  ...
  if (/loopback|127\.0\.0\.0\/8|_LOOPBACK_NAMES|is_loopback_host/.test(content))
    signals.push("loopback-only");
...
function detectHypothesisProfiles(repoPath) {
  ...
  const profiles = [];
  const re =
    /settings\.register_profile\(\s*["']([^"']+)["']\s*,[\s\S]{0,100}?max_examples\s*=\s*(\d+)/g;
  let match;
...
function detectCoverageAuthority(repoPath) {
  ...
  if (checker !== null) {
    if (/sole[^\n]*authority/i.test(checker))
      facts.push("check_module_coverage.py sole module-coverage authority");
    if (/only changed-line|sole[^\n]*changed-line/i.test(checker))
      facts.push("diff-cover sole changed-line authority");
  }
...
export async function scan(repoPath, overview) {
  ...
  const manifest =
    ov.manifest && typeof ov.manifest === "object" ? ov.manifest : readManifest(repoPath);
  let ecosystems;
  ...
  const signal =
    fwDetected && matchedSorted.length > 0
      ? "high"
      : fwDetected || matchedSorted.length > 0
        ? "medium"
        : "low";
