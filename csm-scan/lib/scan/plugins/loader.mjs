...
async function readPluginFile(pluginRoot, pluginsRealRoot) {
  ...
  if (before.size > PLUGIN_LIMITS.fileBytes)
    fail("FILE_TOO_LARGE", "plugin file exceeds the byte limit");
  ...
  if (bytes.length > PLUGIN_LIMITS.fileBytes)
    fail("FILE_TOO_LARGE", "plugin file exceeds the byte limit");
...
export async function loadPlugins({ skillRoot } = {}) {
  if (
    typeof skillRoot !== "string" ||
    skillRoot.length === 0 ||
    skillRoot.includes("\0") ||
    !isAbsolute(skillRoot) ||
    resolve(skillRoot) !== skillRoot
  ) {
  ...
  const pluginNames = (
    await scanNames(pluginsRoot, "plugin root", PLUGIN_LIMITS.plugins + 1)
  ).toSorted((left, right) => (left < right ? -1 : left > right ? 1 : 0));
  if (pluginNames.length > PLUGIN_LIMITS.plugins)
    fail("PLUGIN_LIMIT", "plugin count exceeds the limit");
  ...
  for (const name of pluginNames) {
    ...
    if (name !== plugin.id)
      fail("DIRECTORY_ID_MISMATCH", "plugin directory name must equal its identifier");
    loaded.push(plugin);
  }
