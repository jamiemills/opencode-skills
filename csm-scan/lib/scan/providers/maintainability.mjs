...
export function maintainabilityObservations(model) {
  ...
  for (const file of Array.isArray(model.files) ? model.files : []) {
    push(observation(`file:${file.path}`, {
  ...
  for (const branch of Array.isArray(model.branchPoints) ? model.branchPoints : []) {
    push(observation(`branch:${branch.path}`, {
  ...
  for (const group of Array.isArray(model.duplicateGroups) ? model.duplicateGroups : []) {
    for (const span of group.spans) {
      push(observation(`duplicate:${group.id}:${span.path}:${span.startLine}`, {
  ...
  for (const boundary of Array.isArray(model.generatedBoundaries) ? model.generatedBoundaries : []) {
    push(observation(`generated:${boundary.path}:${boundary.reason}`, {
      category: 'generated_boundary',
      path: boundary.path,
      matchedKey: boundedAssembledKey(`generated:${boundedKey(boundary.path)}:${boundary.reason}`),
      details: {
  ...
  for (const tool of Array.isArray(model.toolEvidence) ? model.toolEvidence : []) {
    push(observation(`tool:${tool.tool}:${tool.file}`, {
  ...
  if (universe !== null && typeof universe === 'object') {
    push(observation('universe:measurement', {
      ...
      details: {
        ...
        excludedLanguages: Object.fromEntries(
          (Array.isArray(universe.excludedLanguages) ? universe.excludedLanguages : [])
        ...
        sizeDistribution: Object.fromEntries(
          (Array.isArray(universe.sizeDistribution) ? universe.sizeDistribution : [])
