...
const DIMENSION_SOURCES = Object.freeze([
  ...
  },
  {
    ...
    claims: Object.freeze(["CLM-git-repository-metadata-v1", "CLM-git-history-v1"]),
  },
  {
    short: "architecture",
...
export const EXPECTED_CLAIM_IDS = deepFreeze(
  DIMENSION_REGISTRY.flatMap(({ expectedClaimIds }) => expectedClaimIds).toSorted(compareAscii),
);
...
export const DIMENSION_RENDERER_MAP = deepFreeze(
  Object.fromEntries(DIMENSION_REGISTRY.map(({ id, rendererId }) => [id, rendererId])),
);
