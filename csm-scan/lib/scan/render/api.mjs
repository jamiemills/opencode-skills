...
function capNotes(model) {
  ...
  for (const [category, label] of [
    ['routes', 'route'], ['contracts', 'contract'], ['rpcs', 'RPC operation'],
    ['events', 'event'], ['cliCommands', 'CLI command'], ['publicExports', 'public export'],
  ]) {
...
function routeRows(operations, escapeField) {
  ...
  for (const operation of operations) {
    rows.push(`| ${methodCell(operation, escapeField)} | \`${escapeField(displayPath(operation), { inTable: true })}\` | ${escapeField(operation.dialect, { inTable: true })} | ${evidenceCell(operation, escapeField)} |`);
  }
  return rows;
}
...
function genericRows(operations, escapeField) {
  ...
  for (const operation of operations) {
    ...
    const secondary = operation.category === 'rpc' ? `${escapeField(operation.details?.service ?? '', { inTable: true })}.${escapeField(operation.details?.method ?? '', { inTable: true })}`
      : operation.category === 'public_export' ? escapeField(operation.details?.kind ?? '', { inTable: true })
      : escapeField(operation.details?.command ?? operation.details?.emitter ?? '', { inTable: true });
    rows.push(`| \`${signature}\` | ${secondary} | ${escapeField(operation.dialect, { inTable: true })} | ${evidenceCell(operation, escapeField)} |`);
  }
  return rows;
}
...
function renderCategory(category, operations, escapeField) {
  const lines = [];
  lines.push(`#### ${CATEGORY_HEADINGS.find((entry) => entry.category === category)?.heading ?? category}`);
  lines.push('');
  if (category === 'route') {
...
export function renderApi(_repoName, model, context = DEFAULT_RENDER_CONTEXT) {
  ...
  if (model.operations.length === 0) {
    lines.push(`No declaration-backed API surface detected in ${model.searchSpace.filesInspected} inspected file(s).`);
    lines.push('');
  }
