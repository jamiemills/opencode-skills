...
export function renderConventions(repoName, findings, context = DEFAULT_RENDER_CONTEXT) {
  ...
  if (findings.symbolNaming && findings.symbolNaming.dominant && findings.symbolNaming.dominant !== 'unknown') {
    const sn = findings.symbolNaming;
    const dominantCount = sn.counts && typeof sn.counts[sn.dominant] === 'number' ? sn.counts[sn.dominant] : null;
  ...
  if (findings.shellHygiene) {
    ...
    lines.push(`- **Shell hygiene**: pipefail adopted in ${escapeField(sh.pipefailAdoption || `${sh.filesWithPipefail}/${sh.totalShellFiles} files`)}`);
    if (sh.shebang && sh.shebang.present > 0) {
      lines.push(`  - Shebangs present in ${sh.shebang.present} file(s)${sh.shebang.envBased ? ` (${sh.shebang.envBased} env-based)` : ''}`);
    }
  ...
  if (findings.tsAnnotations && typeof findings.tsAnnotations.annotationDensity === 'number') {
    ...
    lines.push(`- **TS annotations**: ${ta.interfaceCount} interface(s), ${ta.typeCount} type alias(es); ${ta.annotationDensity}% annotation density`);
  }
  ...
  if (findings.docstrings && Object.keys(findings.docstrings.patterns).length > 0) {
    ...
    if (findings.docstrings.samples.length > 0) {
      ...
      for (const s of findings.docstrings.samples) {
        lines.push(`| ${escapeField(s.language, { inTable: true })} | \`${escapeField(s.file, { inTable: true })}\` | \`${escapeField(s.symbol, { inTable: true })}\` |`);
      }
    }
  }
  ...
  if (findings.largestFiles && findings.largestFiles.length > 0) {
    ...
    for (const f of findings.largestFiles) {
      lines.push(`| \`${escapeField(f.path, { inTable: true })}\` | ${escapeField(f.size, { inTable: true })} |`);
    }
    lines.push('');
  }
