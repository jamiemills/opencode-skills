...
const BEHAVIOUR_FACTS = Object.freeze([
  ...
  { category: "methodology", kind: "no-bdd", heading: "BDD", valueOnly: true },
  {
    ...
    heading: "Plan gate removal",
    valueOnly: true,
  },
  {
    category: "methodology",
    kind: "fuzz-replay-contract",
    heading: "Fuzz replay contract",
    valueOnly: true,
  },
  { category: "methodology", kind: "fuzz-seeds", heading: "Fuzz seeds", noun: "seed" },
  {
    ...
    heading: "Fuzz decomposition",
    noun: "test",
  },
  { category: "methodology", kind: "fuzz-iterations", heading: "Fuzz iterations", noun: null },
  {
    category: "methodology",
    kind: "fuzz-platform-gate",
    heading: "Fuzz platform gate",
    valueOnly: true,
  },
  { category: "methodology", kind: "fuzz-ci-blocking", heading: "Fuzz CI", valueOnly: true },
  {
    ...
    heading: "Policy validators",
    valueOnly: true,
  },
  {
    category: "quality_gate",
    kind: "suppression-policy",
    heading: "Suppression policy",
    valueOnly: true,
  },
  {
    category: "quality_gate",
    kind: "suppression-exit-code",
    heading: "Suppression exit codes",
    valueOnly: true,
  },
  {
    category: "quality_gate",
    kind: "suppression-baseline",
    heading: "Suppression baseline",
    noun: "identity",
    plural: "identities",
  },
  { category: "quality_gate", kind: "ratchet-engine", heading: "Ratchet engine", valueOnly: true },
  {
    ...
    heading: "Mutation exit codes",
    valueOnly: true,
  },
  {
    category: "quality_gate",
    kind: "mutation-actionable",
    heading: "Mutation actionable",
    valueOnly: true,
  },
  {
    category: "quality_gate",
    kind: "mutation-waivers",
    heading: "Mutation waivers",
    valueOnly: true,
  },
  { category: "quality_gate", kind: "mutation-scope", heading: "Mutation scope", valueOnly: true },
  {
    ...
    heading: "Mutation schedule",
    valueOnly: true,
  },
  {
    category: "quality_gate",
    kind: "analyser-contract-registry",
    heading: "Analyser contract registry",
    noun: "analyser",
  },
  {
    category: "quality_gate",
    kind: "analyser-contract-validate",
    heading: "Analyser contract validation",
    valueOnly: true,
  },
  {
    category: "agent_workflow",
    kind: "conventions-block",
    heading: "Enforced conventions block",
    noun: "rule",
  },
  {
    category: "agent_workflow",
    kind: "quality-check-tools",
    heading: "Reactive quality checks",
    valueOnly: true,
  },
  {
    category: "agent_workflow",
    kind: "quality-gate-blocking",
    heading: "Quality gate blocking",
    valueOnly: true,
  },
  {
    category: "agent_workflow",
    kind: "quality-gate-override",
    heading: "Quality gate override",
    valueOnly: true,
  },
  {
    category: "agent_workflow",
    kind: "pre-push-docs-block",
    heading: "Pre-push docs block",
    valueOnly: true,
  },
  { category: "agent_workflow", kind: "npm-check-script", heading: "npm check", valueOnly: true },
  {
    category: "agent_workflow",
    kind: "coverage-thresholds",
...
function behaviourFactEntries(model, fact) {
  return factEntries(model, fact.category, fact.kind).filter((entry) => hasFactValue(fact, entry));
}
...
const STYLE_FACTS = Object.freeze([
  ...
  { category: "style_guide", kind: "line-length", heading: "Line length", noun: null },
  {
    ...
    kind: "docstring-dialect",
    heading: "Docstring dialect",
    valueOnly: true,
  },
  ...
  { category: "style_guide", kind: "exceptions-hub", heading: "Exceptions hub", noun: null },
  {
    ...
    heading: "Exit code constants",
    noun: "code",
  },
  {
    category: "style_guide",
    kind: "exit-code-exception",
    heading: "Exception exit codes",
    noun: "pair",
  },
  {
    category: "style_guide",
...
function gateValueEntries(model) {
  return (Array.isArray(model.entries) ? model.entries : [])
    .filter(
      (entry) =>
        entry.category === "quality_gate" && entry.matchedKey.startsWith(GATE_VALUE_PREFIX),
    )
    .toSorted(
      (left, right) =>
        compareAscii(gateKey(left), gateKey(right)) || compareAscii(left.path, right.path),
    );
}
...
function checkToggleEntries(model) {
  return (Array.isArray(model.entries) ? model.entries : [])
    .filter(
      (entry) =>
        entry.category === "quality_gate" && entry.matchedKey.startsWith(CHECK_TOGGLE_PREFIX),
    )
    .toSorted(
      (left, right) =>
        compareAscii(checkToggleKey(left), checkToggleKey(right)) ||
...
function hookStageEntries(model) {
  return (Array.isArray(model.entries) ? model.entries : [])
    .filter(
      (entry) => entry.category === "enforcement" && entry.matchedKey.startsWith(HOOK_STAGE_PREFIX),
    )
    .toSorted(
      (left, right) =>
        compareAscii(hookStageKey(left), hookStageKey(right)) ||
        compareAscii(left.path, right.path),
    );
}
...
function renderStyleGuideBlock(model, escapeField) {
  ...
  for (const fact of STYLE_FACTS) {
    const entries = factEntries(model, fact.category, fact.kind).filter((entry) =>
      hasFactValue(fact, entry),
    );
...
function renderDiagnostics(model, context) {
  ...
  lines.push("");
  const sorted = [...model.diagnostics].toSorted(
    (left, right) =>
      compareAscii(left.path, right.path) ||
...
export function renderPractices(_repoName, model, context = DEFAULT_RENDER_CONTEXT) {
  ...
  if (groups.length === 0 && completeSearchSpace(model.searchSpace)) {
    lines.push(
      `No development-practice artifacts detected in ${escapeField(String(model.searchSpace.filesInspected))} inspected file(s).`,
    );
    lines.push("");
  }
