...
export function renderSecurity(_repoName, findings, context = DEFAULT_RENDER_CONTEXT) {
  ...
  if (findings.secrets) {
    ...
    if (findings.secrets.count > 0) {
      for (const s of findings.secrets.findings) {
        ...
        const patternLabel = isSecretPatternName(s.pattern)
          ? escapeRaw(s.pattern)
          : escapeField(s.pattern);
        lines.push(
          `  - **${patternLabel}**: ${s.totalFiles} file(s) (e.g. \`${escapeField(s.files[0] || "unknown")}\`)${allowlisted}`,
        );
      }
    }
  }
  ...
  if (findings.auth) {
    ...
    const firstPartyClusters =
      firstParty && firstParty.detected ? firstParty.clusters.join(", ") : "";
    if (findings.auth.detected) {
      lines.push(
        `- **Authentication**: ${findings.auth.frameworks.map((f) => f.label).join(", ")}`,
      );
      ...
      lines.push(
        `- **Authentication**: no third-party auth library; first-party auth subsystem present (${escapeField(firstPartyClusters)})`,
      );
    } else {
  ...
  if (findings.inputValidation) {
    lines.push(
      `- **Input validation**: ${findings.inputValidation.detected ? findings.inputValidation.libraries.map((l) => l.label).join(", ") : "no validation library detected"}`,
    );
  ...
  if (findings.rateLimiting) {
    lines.push(
      `- **Rate limiting**: ${findings.rateLimiting.detected ? "detected" : "not detected"}`,
    );
  ...
  if (findings.dependabotEvidence && typeof findings.dependabotEvidence === "object") {
    ...
    if (status === "configured") {
      ...
      lines.push(
        "- **Dependabot**: not configured (no .github/dependabot.yml); dependabot/* branches present",
      );
      if (Array.isArray(evidence.branches) && evidence.branches.length > 0) {
        const shown = evidence.branches
          .slice(0, 5)
          .map((b) => `\`${escapeField(b)}\``)
          .join(", ");
        lines.push(
          `  - Branch evidence: ${shown}${evidence.branchCount > 5 ? ` (+${evidence.branchCount - 5} more)` : ""}`,
        );
      }
  ...
  if (findings.gitleaks && typeof findings.gitleaks === "object") {
    ...
    if (g.configPresent || g.ignorePresent) {
      ...
      if (g.configPresent) {
        parts.push(
          `.gitleaks.toml present (${g.allowlistPathCount} allowlisted path(s), ${g.stopwordCount} stopword(s))`,
        );
      }
      if (g.ignorePresent) {
        parts.push(
          `.gitleaksignore present (${g.ignoreEntryCount} entr${g.ignoreEntryCount === 1 ? "y" : "ies"})`,
        );
      }
      lines.push(`- **Gitleaks context**: ${parts.join("; ")}`);
      if (Array.isArray(g.fixtureAllowlisted) && g.fixtureAllowlisted.length > 0) {
        lines.push(
          `  - Fixture-allowlisted pattern(s): ${escapeField(g.fixtureAllowlisted.join(", "))} (inferred)`,
        );
      }
    }
  }
  ...
  if (Array.isArray(findings.auditEvidence)) {
    for (const evidence of findings.auditEvidence) {
      ...
      if (evidence.source === "dependency") {
        ...
        const name =
          typeof evidence.location === "string" && evidence.location.startsWith(prefix)
        ...
        const location =
          typeof evidence.location === "string" && evidence.location
