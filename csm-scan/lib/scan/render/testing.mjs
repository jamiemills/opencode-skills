...
export function renderTesting(repoName, findings, context = DEFAULT_RENDER_CONTEXT) {
  ...
  lines.push(
    `- **Test files**: ${findings.fileCount}${findings.naming.length > 0 ? ` (${escapeField(findings.naming.join(", "))})` : ""}`,
  );
  ...
  if (findings.sampleFiles && findings.sampleFiles.length > 0) {
    lines.push(
      `- **Sample files**: \`${escapeField(findings.sampleFiles.slice(0, 10).join("`, `"))}\``,
    );
  }

  if (findings.testDirs && findings.testDirs.length > 0) {
    lines.push(
      `- **Test directories**: \`${escapeField(findings.testDirs.slice(0, 5).join("`, `"))}\``,
    );
  }
  ...
  if (findings.testLanes && typeof findings.testLanes === "object") {
    ...
    if (Array.isArray(findings.testLanes.mutationPropertyFiles)) {
      parts.push(
        `MUTATION_PROPERTY_FILES manifest (${findings.testLanes.mutationPropertyFiles.length} file(s))`,
      );
    }
  ...
  if (Array.isArray(findings.isolationFixtures) && findings.isolationFixtures.length > 0) {
    const fixtures = findings.isolationFixtures
      .map((name) => `\`${escapeField(name)}\``)
      .join(", ");
