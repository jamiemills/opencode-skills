...
export function expandRepositoryDirectoryPatterns(repoPath, patterns, options = {}) {
  ...
  const include = (Array.isArray(patterns) ? patterns : [patterns])
  ...
  const marker =
    ...
    options.marker &&
    !options.marker.includes("/") &&
    !options.marker.includes("\\")
      ? options.marker
      : null;
  const requestedMax = Number.isFinite(options.maxDirectories)
    ? Math.trunc(options.maxDirectories)
  ...
  const matched = include
    .flatMap((pattern) => expandPattern(root, pattern, traversal))
