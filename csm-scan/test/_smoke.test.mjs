...
import {
  ...
  manifest as javascriptManifest,
} from "./fixtures/javascript.mjs";
import {
  files as typescriptFiles,
  manifest as typescriptManifest,
} from "./fixtures/typescript.mjs";
...
for (const fixture of FIXTURES) {
  test(`harness builds ${fixture.name} fixture under os.tmpdir() with manifest present`, () => {
    ...
    try {
      assert.ok(dir.startsWith(os.tmpdir()), `temp dir ${dir} must live under ${os.tmpdir()}`);
      const manifestPath = path.join(dir, fixture.manifest);
      assert.ok(fs.existsSync(manifestPath), `manifest ${fixture.manifest} missing in ${dir}`);
    } finally {
      cleanupFixture(dir);
