...
test("config scan: python fixture detects ruff linter/formatter, pyright typechecker, lefthook hook", async () => {
  await withFixture(
    ...
    async (dir) => {
      ...
      assert.ok(
        linterNames.includes("ruff"),
        `linters should include ruff: ${JSON.stringify(linterNames)}`,
      );
      ...
      assert.ok(
        tcNames.includes("pyright"),
        `typeCheckers should include pyright: ${JSON.stringify(tcNames)}`,
      );
...
test("config scan: JS fixture detects eslint + prettier (flat config style)", async () => {
  await withFixture(
    ...
    async (dir) => {
      ...
      const linterNames = f.linters.map((l) => l.name);
      assert.ok(
        linterNames.includes("eslint"),
        `linters should include eslint: ${JSON.stringify(linterNames)}`,
      );

      const formatterNames = f.formatters.map((x) => x.name);
      assert.ok(
        formatterNames.includes("prettier"),
        `formatters should include prettier: ${JSON.stringify(formatterNames)}`,
      );
      ...
      assert.ok(
        typeof f.format === "string" && f.format.includes("prettier"),
        `format: ${f.format}`,
      );
...
test("config scan: JS fixture with legacy .eslintrc.json reports legacy style", async () => {
  await withFixture(
    ...
    async (dir) => {
      ...
      assert.equal(
        res.findings.lint.style,
        "legacy",
        `.eslintrc.json is legacy: ${res.findings.lint.style}`,
      );
    },
  );
});
...
test("config scan: tsconfig paths map is preserved (not boolean)", async () => {
  ...
  await withFixture(
    ...
    async (dir) => {
      ...
      assert.ok(
        tcNames.includes("tsc"),
        `typeCheckers should include tsc: ${JSON.stringify(tcNames)}`,
      );
      const linterNames = res.findings.linters.map((l) => l.name);
      assert.ok(
        !linterNames.includes("tsc"),
        `tsc must NOT be in linters: ${JSON.stringify(linterNames)}`,
      );
    },
  );
});
...
test("config scan: turbo.json + webpack detected as build tools", async () => {
  await withFixture(
    ...
    async (dir) => {
      ...
      const names = res.findings.buildTools.map((b) => b.name);
      assert.ok(
        names.includes("turbo"),
        `buildTools should include turbo: ${JSON.stringify(names)}`,
      );
      assert.ok(
        names.includes("webpack"),
        `buildTools should include webpack: ${JSON.stringify(names)}`,
      );
    },
  );
});
...
`;
  await withFixture(
    ...
    async (dir) => {
      ...
      const tcNames = f.typeCheckers.map((t) => t.name);
      assert.ok(
        tcNames.includes("rustc"),
        `typeCheckers should include rustc: ${JSON.stringify(tcNames)}`,
      );
...
test("config scan: python fixture declaring missing tools inventories them with provenance", async () => {
  await withFixture(
    ...
    {
      "pyproject.toml": DECLARED_TOOLS_PYPROJECT,
      Makefile: DECLARED_TOOLS_MAKEFILE,
      "src/demo/__init__.py": "",
    },
    async (dir) => {
      const res = await scan(dir);
      ...
      for (const expected of [
        ...
        assert.ok(
          ...
          `declaredTools should include ${expected}: ${JSON.stringify(names)}`,
        );
      }
      ...
      assert.ok(
        mutmut.provenance.includes("declared-in-deps"),
        `mutmut deps provenance: ${JSON.stringify(mutmut.provenance)}`,
      );
      assert.ok(
        mutmut.provenance.includes("declared-config"),
        `mutmut config provenance: ${JSON.stringify(mutmut.provenance)}`,
      );
      assert.ok(
        mutmut.sources.some((s) => s.kind === "dependency-group"),
        "mutmut declared in a dependency group",
      );
      assert.ok(
        mutmut.sources.some((s) => s.kind === "tool-section"),
        "mutmut declared via a tool section",
      );
      ...
      assert.deepEqual(
        ...
        ["declared-config"],
        `actionlint provenance: ${JSON.stringify(actionlint.provenance)}`,
      );
      assert.ok(
        actionlint.sources.some((s) => s.kind === "makefile"),
        "actionlint declared via the Makefile",
      );
      ...
      assert.ok(
        !names.includes("ruff"),
        `descriptor-only tools must not be re-listed: ${JSON.stringify(names)}`,
      );
    },
  );
});
    ...
    async (dir) => {
      ...
      for (const expected of [
        ...
        assert.match(
          markdown,
          new RegExp(`\`${expected}\``),
          `toolchain should render ${expected}`,
        );
      }
      ...
      assert.deepEqual(
        names,
        [...names].toSorted(),
        `toolchain rows not sorted: ${JSON.stringify(names)}`,
      );
    },
  );
});
  ...
  await withFixture(
    ...
    async (dir) => {
      ...
      assert.doesNotMatch(
        markdown,
        /Declared Toolchain/,
        "toolchain block should be absent without declared tools",
      );
    },
  );
});
...
test("config scan: real perplexity-cli -> ruff+pyright+bandit+vulture, ruff format, lefthook hook", async (t) => {
  ...
  for (const expected of ["ruff"]) {
    assert.ok(
      linterNames.includes(expected),
      `linters should include ${expected}: ${JSON.stringify(linterNames)}`,
    );
  }
  ...
  assert.ok(
    hookTools.includes("lefthook"),
    `hooks should include lefthook: ${JSON.stringify(hookTools)}`,
  );
  ...
  if (isPerplexityCli(REAL_REPO)) {
    ...
    for (const expected of [
      ...
      assert.ok(
        ...
        `declaredTools should include ${expected}: ${JSON.stringify(declaredNames)}`,
      );
    }
  } else {
    ...
    const declaredNames = f.declaredTools.map((dt) => dt.name);
    assert.ok(
      declaredNames.includes("hypothesis"),
      `declaredTools should include hypothesis: ${JSON.stringify(declaredNames)}`,
    );
  }

  // Evidence summary for the human reader.
  console.log(
    "  [pxcli config] declaredTools =",
    JSON.stringify(
      f.declaredTools.map((dt) => ({
        name: dt.name,
        provenance: dt.provenance,
        descriptorDetected: dt.descriptorDetected,
      })),
    ),
  );
  console.log(
    "  [pxcli config] linters   =",
    JSON.stringify(f.linters.map((l) => `${l.name}@${l.config}`)),
  );
