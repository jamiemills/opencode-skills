...
test("DESCRIPTORS is keyed by exactly the 5 in-scope ecosystem ids", () => {
  assert.deepEqual(Object.keys(DESCRIPTORS).toSorted(), [...ECOSYSTEM_IDS].toSorted());
});
...
test("every descriptor has id/label and non-empty core fields", () => {
  for (const id of ECOSYSTEM_IDS) {
    ...
    assert.ok(Array.isArray(d.testFileGlobs) && d.testFileGlobs.length > 0, `${id} testFileGlobs`);
    assert.ok(
...
test("frameworks non-empty except shell which may be {}", () => {
  for (const id of ECOSYSTEM_IDS) {
    ...
    if (id === "shell") {
      ...
      assert.ok(Object.keys(frameworks).length > 0, `${id} frameworks should be non-empty`);
    }
  }
});
...
test("T101 P0: typescript linters has no tsc (dedup with typeCheckers); tsc stays a typeChecker", () => {
  ...
  assert.ok(
    ...
    `tsc must not be a linter: ${JSON.stringify(linterNames)}`,
  );
  assert.ok(
...
test("T101: cross-ecosystem lockfile breadth (pdm.lock python; bun.lock js/ts)", () => {
  assert.ok(
    DESCRIPTORS.python.lockfiles.includes("pdm.lock"),
    "python lockfiles must include pdm.lock",
  );
  assert.ok(
    DESCRIPTORS.javascript.lockfiles.includes("bun.lock"),
    "javascript lockfiles must include bun.lock",
  );
  assert.ok(
    DESCRIPTORS.typescript.lockfiles.includes("bun.lock"),
    "typescript lockfiles must include bun.lock",
  );
});
...
test("T101: breadth spot-checks across ecosystems", () => {
  ...
  assert.ok(
    DESCRIPTORS.python.formatters
      .find((s) => s.name === "ruff-format")
      .files.includes(".ruff.toml"),
...
test("T007 b14: non-python testFileGlobs are unchanged", () => {
  assert.deepEqual(DESCRIPTORS.javascript.testFileGlobs, [
    "**/*.test.{js,mjs,cjs,jsx}",
    "**/*.spec.{js,mjs,cjs,jsx}",
  ]);
...
test("T007 b14: fixture counting excludes scripts/smoke_test.py and tests/support/** via the descriptor globs", async () => {
  ...
  await withFixture("eco-t007-b14", files, async (dir) => {
    ...
    assert.equal(
      res.findings.fileCount,
      1,
      `fileCount must count only real test modules: ${res.findings.fileCount}`,
    );
    assert.ok(
