...
function assertContradictionsNeutral(result) {
  for (const c of result.contradictions) {
    assert.equal(
      "severity" in c,
      false,
      `contradiction must not carry severity: ${JSON.stringify(c)}`,
    );
...
test("low scanner signal does not reduce factual coverage", async () => {
  ...
  assert.equal(
    result.gaps.some((note) => /signal/i.test(note.reason)),
    false,
  );
});
    ...
    [
      {
        dimension: "git",
        signal: "high",
        findings: { commitStyle: "Conventional Commits", logCount: 30 },
      },
    ],
    overview,
  );
  assert.ok(
    result.inferredPatterns.some(
      (pattern) =>
        pattern.dimension === "git" &&
        pattern.pattern === "Commit convention: Conventional Commits",
    ),
  );
});

test("empty and missing findings have zero coverage", async () => {
  const result = await enrich([{ dimension: "security", signal: "low", findings: {} }], {
    languages: [],
    ecosystems: {},
    totalFiles: 0,
  });
...
test("cross-observations do not change dimension coverage", async () => {
  ...
  const deep = [
    { dimension: "config", signal: "low", findings: { typescript: { strict: true } } },
    {
      ...
      signal: "low",
      findings: { importStyle: { type: "CJS (require/module.exports)" } },
    },
  ];
...
test("ecosystem-aware inferred pattern is added when ecosystem is known", async () => {
  ...
  assert.ok(
    result.inferredPatterns.some(
      (p) => p.dimension === "stack" && /Primary ecosystem: python/.test(p.pattern),
    ),
  );
});
...
test("cross-observation describes co-existing facts neutrally (tsconfig strict + CJS)", async () => {
  ...
  const deep = [
    {
      dimension: "config",
      signal: "high",
      findings: { typescript: { strict: true }, linters: ["eslint"] },
    },
    {
      dimension: "conventions",
      signal: "high",
  ...
  const result = await enrich(deep, overview);
  const cx = result.contradictions.find(
    (c) => c.dimensions.includes("config") && c.dimensions.includes("conventions"),
  );
...
test("contradictions never carry a severity field across mixed scenarios", async () => {
  ...
  const deep = [
    ...
    { dimension: "testing", signal: "medium", findings: { framework: ["unknown"], fileCount: 4 } },
    {
      ...
      signal: "medium",
      findings: { importGraph: { graph: {} }, layers: { totalFiles: 5, totalEdges: 0 } },
    },
    {
      dimension: "git",
      signal: "high",
    ...
    },
    {
      ...
      signal: "high",
      findings: { changelog: { present: true, format: "plain markdown" } },
    },
  ];

  const result = await enrich(deep, overview);
  assert.ok(
    result.contradictions.length >= 3,
    `expected several cross-observations, got ${result.contradictions.length}`,
  );
  assertContradictionsNeutral(result);
});
