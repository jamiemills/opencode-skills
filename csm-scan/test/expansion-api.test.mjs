...
import { API_PROVIDER_ID, apiObservations, apiProviderResult } from "../lib/scan/providers/api.mjs";
import { ProviderResultError } from "../lib/scan/providers/base.mjs";
import { createApiRenderer, renderApi } from "../lib/scan/render/api.mjs";
...
function sampleCandidates() {
  return [
    ...
    },
    {
      category: "contract",
      dialect: "openapi",
      signature: "openapi:openapi.yaml",
      details: { format: "openapi", version: "3.0.0" },
      path: "openapi.yaml",
      line: 1,
      status: "observed",
    },
...
test("T211 model: deep-frozen deterministic model with exact summary and search space", () => {
  const first = buildApiModel({
    operations: sampleCandidates(),
    diagnostics: [],
  ...
  assert.equal(
    ...
    JSON.stringify(buildApiModel({ operations: sampleCandidates(), searchSpace: SEARCH_OK })),
  );
...
test("T211 model: identities are prefixed stable tokens and matchedKeys encode path params", () => {
  ...
  assert.deepEqual(
    model.operations.map(({ signature }) => signature),
    ["GET:/pets/{petId}"],
  );
...
test("T211 model: exact duplicates collapse while same-signature different-file declarations persist", () => {
  ...
  assert.deepEqual(
    model.operations.map(({ source }) => source.path),
    ["a/app.js", "b/app.js"],
  );
});
...
test("T211 model: invalid categories, statuses, identities, and paths fail with typed errors", () => {
  ...
  assert.throws(
    () => bad({ category: "language" }),
    (e) => e instanceof ApiModelError && e.code === "UNKNOWN_CATEGORY",
  );
  assert.throws(
    () => bad({ status: "observed-evil" }),
    (e) => e instanceof ApiModelError && e.code === "INVALID_STATUS",
  );
  ...
  const candidates = [
    {
      ...
      dialect: "argparse",
      signature: "cli:argparse:alice@example.test",
      details: { command: "alice@example.test" },
      path: "cli.py",
      line: 4,
      status: "observed",
    },
...
test("T211 extractor: classifyPath recognizes contracts, sources, package, and other", () => {
  assert.deepEqual(classifyPath("openapi.yaml"), {
    kind: "contract",
    format: "text",
    ecosystem: null,
  });
  assert.deepEqual(classifyPath("api/openapi.json"), {
    kind: "contract",
    format: "json",
    ecosystem: null,
  });
  assert.deepEqual(classifyPath("schema.graphql"), {
    kind: "contract",
    format: "text",
    ecosystem: null,
  });
  assert.deepEqual(classifyPath("proto/user.proto"), {
    kind: "contract",
    format: "text",
    ecosystem: null,
  });
  assert.deepEqual(classifyPath("service.wsdl"), {
    kind: "contract",
    format: "text",
    ecosystem: null,
  });
  assert.deepEqual(classifyPath("contracts/orders.yaml"), {
    kind: "contract",
    format: "text",
    ecosystem: null,
  });
  assert.deepEqual(classifyPath("src/app.js"), {
    kind: "source",
    format: "text",
    ecosystem: "javascript",
  });
  assert.deepEqual(classifyPath("src/app.ts"), {
    kind: "source",
    format: "text",
    ecosystem: "typescript",
  });
  assert.deepEqual(classifyPath("app.py"), { kind: "source", format: "text", ecosystem: "python" });
  assert.deepEqual(classifyPath("src/main.rs"), {
    kind: "source",
    format: "text",
    ecosystem: "rust",
  });
...
test("T211 extractor: detectContractKind recognizes openapi and asyncapi markers", () => {
  assert.equal(
    detectContractKind({ text: "openapi: 3.0.0\n", value: null, format: "text" }),
    "openapi",
  );
  assert.equal(
    detectContractKind({ text: 'swagger: "2.0"\n', value: null, format: "text" }),
    "openapi",
  );
  assert.equal(
    detectContractKind({ text: "asyncapi: 2.6.0\n", value: null, format: "text" }),
    "asyncapi",
  );
  assert.equal(detectContractKind({ text: "title: demo\n", value: null, format: "text" }), null);
  assert.equal(
    detectContractKind({ text: "", value: { openapi: "3.0.0" }, format: "json" }),
    "openapi",
  );
  assert.equal(
    detectContractKind({ text: "", value: { asyncapi: "2.6.0" }, format: "json" }),
    "asyncapi",
  );
...
test("T211 extractor: OpenAPI JSON and YAML contracts produce a contract plus route operations", () => {
  const yaml = extractTextContract(
    "openapi.yaml",
    [
      "openapi: 3.0.0",
      "info: { title: Demo }",
  ...
  assert.deepEqual(
    model.operations.find(({ category }) => category === "route")?.details.method,
    "GET",
  );
...
test("T211 extractor: AsyncAPI 2.x channels become event operations", () => {
  const result = extractTextContract(
    "asyncapi.yaml",
    [
      "asyncapi: 2.6.0",
      "channels:",
...
test("T211 extractor: AsyncAPI 3.x operations are reported partial/unverified", () => {
  ...
  assert.deepEqual(result.diagnostics, [
    { path: "asyncapi.yaml", status: "unverified", reason: "DYNAMIC", line: null },
  ]);
});
...
test("T211 extractor: GraphQL multi-line argument continuation lines are never captured as fields", () => {
  ...
  assert.ok(ops.includes("graphql:mutation:createUser"));
  assert.equal(
    ops.includes("graphql:query:first"),
    false,
    "argument continuation must not become a field",
  );
  assert.equal(
    ops.includes("graphql:query:after"),
    false,
    "argument continuation must not become a field",
  );
  assert.equal(
    ops.includes("graphql:mutation:input"),
    false,
    "argument continuation must not become a field",
  );
  assert.equal(
    ops.includes("graphql:mutation:notify"),
    false,
    "argument continuation must not become a field",
  );
});
...
test("T211 extractor: protobuf messages, services, and rpcs are extracted with lines", () => {
  ...
  const rpc = result.operations.find(
    ({ signature }) => signature === "protobuf:rpc:UserService:GetUser",
  );
...
test("T211 extractor: YAML anchors and malformed contracts fail with unsupported diagnostics", () => {
  const anchored = extractTextContract(
    "openapi.yaml",
    "openapi: 3.0.0\ndefaults: &base\n  x: 1\npaths:\n  /a:\n    get: *base\n",
  );
  assert.deepEqual(anchored.operations, []);
  assert.deepEqual(anchored.diagnostics, [
    { path: "openapi.yaml", status: "unsupported", reason: "PARSE_UNSUPPORTED", line: null },
  ]);
  const unknown = extractApiSurface({
    path: "api/random.yaml",
    text: "title: demo\n",
    value: null,
    format: "text",
    ecosystem: null,
  ...
  assert.deepEqual(unknown.diagnostics, [
    { path: "api/random.yaml", status: "unsupported", reason: "UNSUPPORTED", line: null },
  ]);
});
...
test("T211 extractor: click, typer, argparse, commander, yargs, and clap CLI trees", () => {
  ...
  const js = extractSource(
    "bin/cli.js",
    "program.command('build').option('-v');\nyargs.command('serve').parse();\n",
    "javascript",
  );
...
test("T211 extractor: package exports, Python __all__, entry JS exports, and Rust pub items", () => {
  ...
  assert.deepEqual(
    nonEntry.operations.filter(({ category }) => category === "public_export"),
    [],
...
test("T211 extractor: calls embedded in string literals produce NO operations (M1)", () => {
  ...
  assert.deepEqual(
    signatures(js),
    ["GET:/real"],
    "string-embedded calls are rejected, real routes still resolve",
  );
  assert.deepEqual(js.diagnostics, []);
  ...
  assert.deepEqual(
    signatures(py),
    ["ANY:/real/", "GET:/real"],
    "string-embedded decorators and path() calls never resolve",
  );
  assert.deepEqual(py.diagnostics, []);
  ...
  assert.deepEqual(
    signatures(rust),
    ["GET:/real"],
    "string-embedded axum route calls never resolve",
  );
  assert.deepEqual(rust.diagnostics, []);
});
...
test("T211 extractor: Axum chainOf never invents methods from string content, real chains still resolve (M1)", () => {
  ...
  assert.equal(
    ops.includes("POST:/api"),
    false,
    "chain method inside a string span is never invented",
  );
  assert.deepEqual(result.diagnostics, []);
});
...
test("T211 extractor: methods= inside string arguments is never accepted, real methods= still resolve (M1)", () => {
  ...
  assert.ok(ops.includes("GET:/users"));
  assert.equal(
    ops.includes("POST:/api/x"),
    false,
    "methods= inside a string argument never adds a method",
  );
  assert.deepEqual(result.diagnostics, []);
});
...
test("T211 extractor: calls embedded in template literals produce NO operations (M1)", () => {
  ...
  assert.deepEqual(
    signatures(js),
    ["GET:/real"],
    "template-literal-embedded calls and constants are rejected",
  );
  assert.deepEqual(js.diagnostics, []);
});
...
test("T211 extractor: GraphQL block strings and # comments produce NO operations (M1)", () => {
  ...
  assert.equal(
    ops.includes("graphql:query:getFake"),
    false,
    "block-string descriptions never become fields",
  );
...
test("T211 extractor: WSDL XML-comment-embedded declarations produce NO operations (M1)", () => {
  ...
  assert.ok(ops.includes("wsdl:service:UserService"));
  assert.equal(
    ops.includes("wsdl:operation:FakeOp"),
    false,
    "XML-comment-embedded operations are never extracted",
  );
  assert.equal(
    ops.includes("wsdl:portType:FakePort"),
    false,
    "XML-comment-embedded portTypes are never extracted",
  );
  assert.deepEqual(result.diagnostics, []);
});
...
test("T211 extractor: unsupported ecosystems and files produce typed diagnostics", () => {
  ...
  assert.deepEqual(shell.diagnostics, [
    { path: "run.sh", status: "unsupported", reason: "UNSUPPORTED", line: null },
  ]);
  const other = extractApiSurface({
    path: "docs/guide.md",
    text: "text",
    value: null,
    format: "text",
  ...
  assert.deepEqual(other.diagnostics, [
    { path: "docs/guide.md", status: "unsupported", reason: "UNSUPPORTED", line: null },
  ]);
});
...
test("T211 extractor: credential URLs are never extracted as routes", () => {
  const result = extractSource(
    "src/app.js",
    "app.get('https://user:pass@example.com/api', h);\napp.post('/api/safe', h);\n",
    "javascript",
  );
...
test("T211 extractor: per-file caps truncate deterministically with a CAP diagnostic", () => {
  const routes = Array.from(
    { length: API_LIMITS.perFileOperations + 10 },
    (_, index) => `app.get('/r/${index}', h);`,
  );
...
test("T211 provider: deterministic, immutable, and empty for empty/foreign input", () => {
  ...
  assert.deepEqual(apiProviderResult({ operations: [] }), [
    { providerId: API_PROVIDER_ID, dimensionId: "DIM-api-v1", observations: [] },
  ]);
...
test("T211 provider: validation bounds align with the model legal caps and never throw a bare TypeError at 5000 operations", () => {
  ...
  const observations = apiObservations(model);
  assert.equal(
    observations[0].observations.length,
    5000,
    "a legal model derives observations without a bare TypeError",
  );
  assert.throws(
    () => apiProviderResult(model),
    (error) =>
      error instanceof ProviderResultError &&
      error.name === "ProviderResultError" &&
...
test("T211 renderer: empty model renders a factual no-detected line and disclosed caps", () => {
  ...
  const cappedModel = buildApiModel({
    operations: sampleCandidates(),
...
test("T211 inertness: API renderer is never registered in the existing-ten map", async () => {
  ...
  assert.equal(
    existingTen.includes("render/api.mjs"),
    false,
    "existing-ten must not import the API renderer",
  );
...
test("T211 scanner: Python fixture extracts FastAPI/Flask/Django routes, CLI, and exports", async () => {
  const files = {
    "app.py": ["@app.route('/users', methods=['GET', 'POST'])", "def users(): ...", ""].join("\n"),
    "api/main.py": ["@app.get('/api/items')", "def items(): ...", ""].join("\n"),
    "project/urls.py": ["urlpatterns = [", "    path('about/', views.about),", "]", ""].join("\n"),
    "pkg/__init__.py": "__all__ = ['create_client']\n",
    "cli.py": ["@click.command()", "def deploy(): ...", ""].join("\n"),
  };
...
test("T211 scanner: TypeScript fixture extracts NestJS-style routes", async () => {
  const files = {
    "src/app.controller.ts": ["@Get('/api/health')", "health() {}", ""].join("\n"),
    "src/app.ts": ["app.get('/api/root', h);", ""].join("\n"),
  };
...
test("T211 scanner: dynamic fixtures produce diagnostics and never invent endpoints", async () => {
  ...
  await withFixture("api-dynamic", files, async (dir) => {
    ...
    assert.deepEqual(
      findings.diagnostics.map(({ reason }) => reason),
      ["DYNAMIC", "DYNAMIC"],
    );
  });
});
...
test("T211 scanner: source sampling cap is disclosed and never overclaims completeness", async () => {
  ...
  await withFixture("api-cap560", files, async (dir) => {
    ...
    assert.equal(
      ...
      false,
      "skipped eligible sources must never claim completeness",
    );
    ...
    assert.equal(
      ...
      0,
      "the skipped lowest-priority route is not claimed as found",
    );
    const serialized = JSON.stringify(findings);
    assert.equal(
      serialized.includes("/only-lowest-priority"),
      false,
      "omitted content must not be invented",
    );
  });
});
