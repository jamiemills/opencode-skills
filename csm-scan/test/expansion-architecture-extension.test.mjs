...
import {
  detectDynamicIndicators,
  INDICATOR_KINDS,
} from "../lib/scan/deep/architecture/indicators.mjs";
import {
...
import {
  createArchitectureExtensionRenderer,
  DEFAULT_ARCHITECTURE_EXTENSION_RENDERER,
} from "../lib/scan/render/architecture-extension.mjs";
...
test("T217 SCC order is deterministic regardless of insertion order", () => {
  ...
  assert.deepEqual(
    tarjanStronglyConnectedComponents(left),
    tarjanStronglyConnectedComponents(right),
  );
});
...
test("T217 JS dynamic constructs produce indicators; literals are recorded, non-literals are not", () => {
  ...
  assert.ok(
    !indicators.some(({ line }) => line >= 12),
    "comments and strings must not produce indicators",
  );
  for (const indicatorKind of indicators.map(({ kind }) => kind)) {
    assert.ok(
      INDICATOR_KINDS.includes(indicatorKind),
      `unexpected indicator kind ${indicatorKind}`,
    );
  }
});
...
test("T217 Python dynamic constructs produce indicators only", () => {
  ...
  assert.ok(
    !indicators.some(({ line }) => line >= 14),
    "comments and docstrings must not produce indicators",
  );
});
...
test("T217 unknown ecosystems and unsafe specifiers are handled defensively", () => {
  ...
  const unsafe = detectDynamicIndicators(
    "import('/etc/passwd');\nimport('a b');\nimport('ok');",
    "javascript",
  );
  assert.deepEqual(
    unsafe.map(({ specifier }) => specifier),
    [null, null, "ok"],
  );
...
test("T217 analyzeGraphFacts reports exact facts for a TS dynamic fixture and matches the scan graph", async () => {
  ...
  assert.deepEqual(facts.bounds, {
    filesInspected: 4,
    fileLimit: GRAPH_FACTS_LIMITS.files,
    filesOmitted: 0,
    edgesInspected: 3,
    edgeLimit: GRAPH_FACTS_LIMITS.edges,
    edgesOmitted: 0,
    capped: false,
  });
  ...
  assert.deepEqual(facts.fanIn, {
    "src/a.ts": 1,
    "src/dyn.ts": 1,
    "src/req.ts": 1,
    "src/index.ts": 0,
  });
  assert.deepEqual(facts.fanOut, {
    "src/a.ts": 0,
    "src/dyn.ts": 0,
    "src/index.ts": 3,
    "src/req.ts": 0,
  ...
  for (const [file, targets] of Object.entries(scanResult.findings.importGraph.graph)) {
    assert.equal(
      facts.fanOut[file],
      targets.length,
      `${file} fan-out must match the preserved graph`,
    );
  }
  assert.equal(
    "graphFacts" in scanResult.findings,
    false,
    "scan() must keep its original findings shape",
  );
...
test("T217 cycles produce Tarjan SCC facts and self-loops are listed", async () => {
  ...
  assert.equal(
    Object.values(facts.edgeKindCounts).reduce((a, b) => a + b, 0),
    3,
  );
});
...
test("T217 the five built-in fixtures keep their preserved import graph and agree with graph facts", async () => {
  ...
  for (const [name, files, ecosystems] of fixtures) {
    ...
    const { scan: scanResult, facts } = await scanFixture(
      `archfacts-parity-${name}`,
      files,
      overview,
    );
    ...
    assert.equal(
      kindTotal,
      scanResult.findings.layers.totalEdges,
      `${name} edge-kind total must match the preserved graph`,
    );
    for (const [file, targets] of Object.entries(graph)) {
      assert.equal(
        facts.fanOut[file],
        targets.length,
        `${name} fan-out must match the preserved graph`,
      );
    }
...
const BANNED_VOICE =
  /\b(?:should|must|poor|good|bad|weak|strong|better|worse|recommended|concern|problem|inconsistent|conflict)\b/i;
...
test("T017 craft: coupling aggregates are exact for a synthetic graph", () => {
  ...
  assert.deepEqual(coupling.fanOut.max, {
    count: 2,
...
test("T017 craft: results are deterministic, deep-frozen, and privacy-safe", () => {
  ...
  assert.throws(() => {
    solid.patternSuffixes.counts.Service = 9;
  }, TypeError);
...
test("T017 catalog: architecture observations carry coupling and design_pattern facts", () => {
  ...
  assert.equal(
    byKey.get("coupling:fan-in-threshold").details.threshold,
    CRAFT_LIMITS.fanInThreshold,
  );
  assert.deepEqual(byKey.get("coupling:fan-in-max").details, {
    count: 4,
    files: ["src/contracts/UserContract.ts"],
    truncated: false,
  });
...
test("T017 catalog: observations avoid banned words and degrade gracefully", () => {
  ...
  assert.ok(
    !observations.some(({ matchedKey }) => matchedKey === "coupling:edge-kinds"),
  ...
  assert.equal(
    ...
    snapshot,
    "scan() findings are never mutated by the derivation",
  );
});
...
function stripCraftProse(markdown) {
  return markdown
    ...
    .replace(
      /^[ \t]*\|([^|]*)\|(.*)\|[ \t]*$/gm,
      (row, firstCell, valueCells) => `|${firstCell}|${maskLine(valueCells)}|`,
    );
}

const CRAFT_VOICE =
  /\b(?:should|must|ought|shall|poor|good|bad|weak|strong|better|worse|recommended|recommendation|ideally|unfortunately|concern|problem|anti-pattern|smell|suboptimal|inadequate|insufficient|contradiction|inconsistent|conflict|lacking)\b/i;
...
test("T005 craft renderer emits an Import Contracts subsection with backticked names", () => {
  ...
  assert.equal(
    CRAFT_VOICE.test(stripCraftProse(craft)),
    false,
    "contract names must be masked by backticks",
  );
});
...
import {
  renderArchitectureCraft,
  renderArchitectureExpanded,
} from "../lib/scan/render/architecture-craft.mjs";
