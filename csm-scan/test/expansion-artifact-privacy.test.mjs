...
test("T206 artifact references enforce lexical, realpath, symlink, and regular-file safety", async (t) => {
  ...
  const resolved = await resolveArtifactReference(root, {
    path: "nested/text.txt",
    sensitivity: "public",
  });
  ...
  for (const path of [
    ...
    await assert.rejects(
      resolveArtifactReference(root, { path, sensitivity: "internal" }),
      ArtifactError,
      path,
    );
  }
  await assert.rejects(
    resolveArtifactReference(root, { path: "linked.txt", sensitivity: "internal" }),
    { code: "SYMLINK" },
  );
  await assert.rejects(
    resolveArtifactReference(root, { path: "nested/escape.txt", sensitivity: "internal" }),
    { code: "SYMLINK" },
  );
  await assert.rejects(
    resolveArtifactReference(root, { path: "nested", sensitivity: "internal" }),
...
test("T206 bounded reads return deterministic T202-compatible typed outcomes and counts", async (t) => {
  ...
  assert.deepEqual(
    result.results.map(({ path, status }) => [path, status]),
    [
...
test("T206 file, byte, record, and structural depth caps never return partial values", async (t) => {
  const root = await fixture(t);
  const fileCap = await readArtifacts(
    root,
    [request("nested/data.json", "json"), request("nested/text.txt")],
    {
      ...LIMITS,
      maxFiles: 1,
    },
  ...
  const recordCap = await readArtifacts(root, [request("nested/records.json", "json")], {
    ...LIMITS,
    maxRecords: 2,
  });
  ...
  const depthCap = await readArtifacts(root, [request("nested/deep.json", "json")], {
    ...LIMITS,
    maxDepth: 2,
  });
...
const CANARIES = Object.freeze({
  ...
  token: "token=ghp_\x61bcdefghijklmnopqrstuvwxyz",
...
test("T206 every privacy canary is rejected from structured and serialized surfaces with safe errors", () => {
  ...
  assert.throws(() => assertPrivacySafe({ subject: "ordinary lowercase words" }), {
...
test("T206 evidence is privacy-validated before persistence and output serialization", () => {
  ...
  const unsafe = evidence({ matchedKey: "token:ghp_\x61bcdefghijklmnopqrstuvwxyz" });
...
test("T206 opaque owner labels and aggregate counts are deterministic and reveal no identities", () => {
  const first = createOpaqueOwnerSummary([
    "alice@example.test",
    "@team-z",
    "alice@example.test",
    "Bob Person",
  ]);
  const second = createOpaqueOwnerSummary([
    "Bob Person",
    "alice@example.test",
    "@team-z",
    "alice@example.test",
  ...
  for (const identity of ["alice@example.test", "@team-z", "Bob Person"])
    ...
    runs: [
      {
        tool: {
          driver: {
            name: "safe-tool",
            rules: [{ id: "RULE-2", helpUri: "https://user:pass@example.test" }],
          },
        },
        results: [
          {
            ...
            locations: [
              {
                physicalLocation: {
                  artifactLocation: { uri: CANARIES.posixPath },
                  region: { snippet: { text: CANARIES.token } },
                },
              },
            ],
            codeFlows: [
              {
                threadFlows: [{ locations: [{ location: { message: { text: CANARIES.email } } }] }],
              },
            ],
          },
        ],
...
test("T206 SBOM projection keeps safe coordinates/licenses/counts and drops nested hazards", () => {
    ...
    components: [
      {
        ...
        externalReferences: [
          { type: "vcs", url: "https://user:pass@example.test/repo?secret=yes" },
        ],
  ...
  assert.deepEqual(projection, {
    ...
    packageCoordinates: ["nested-package@2.0.0", "pkg:npm/%40scope/package@1.2.3"],
  });
  const serialized = JSON.stringify(projection);
  for (const canary of Object.values(CANARIES)) assert.equal(serialized.includes(canary), false);
...
test("T206 Proxy, accessor, depth, and count bombs fail closed without reflecting values", async (t) => {
  ...
  const proxy = new Proxy(
    ...
    {
      ownKeys() {
        throw new Error(CANARIES.secret);
      },
    },
  );
  const accessor = {};
  ...
  for (const bomb of [proxy, accessor]) {
    assert.throws(
      () => assertPrivacySafe(bomb),
      (error) =>
        error instanceof PrivacyError &&
        !error.message.includes(CANARIES.secret) &&
        !error.message.includes(CANARIES.token),
    );
  }
  let deep = "safe";
  ...
  assert.throws(
    () => projectSarif({ version: "2.1.0", runs: Array.from({ length: 4097 }, () => ({})) }),
    PrivacyError,
  );
  assert.throws(
    () => projectSbom({ components: Array.from({ length: 4097 }, () => ({ name: "safe" })) }),
    PrivacyError,
  );
  await assert.rejects(
    readArtifacts(root, [new Proxy(request("nested/text.txt"), {})], LIMITS),
    ArtifactError,
  );
  await assert.rejects(readArtifacts(root, Array(4097).fill(null), LIMITS), {
    code: "BOUND_EXCEEDED",
  });
});
...
test("T206 evidence serialization round-trips realistic lists and still rejects canaries", () => {
  ...
  const flagged = [
    ...records,
    evidence({ locator: "line:99", matchedKey: "access_token:feedfacecafebabe1234" }),
  ];
...
test("T206 bounded reads reopen with O_NOFOLLOW and re-verify inode/realpath containment", async (t) => {
  ...
  await assert.rejects(boundedBytes(join(root, resolved.path), 10_000, resolved), {
    code: "UNREADABLE",
  });
  await assert.rejects(readArtifacts(root, [request("nested/data.json")], LIMITS), {
    code: "SYMLINK",
  });

  const swapped = await resolveArtifactReference(root, {
    path: "nested/text.txt",
    sensitivity: "public",
  });
  ...
  await assert.rejects(boundedBytes(join(root, swapped.path), 10_000, swapped), {
    code: "UNREADABLE",
  });

  const fresh = await resolveArtifactReference(root, {
    path: "nested/text.txt",
    sensitivity: "public",
  });
