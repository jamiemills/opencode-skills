...
import { createAssuranceRenderer, renderAssurance } from "../lib/scan/render/assurance.mjs";
import {
...
test("T216 model: deep-frozen deterministic model with exact summary", () => {
  const records = [
    ...
    },
    {
      ...
      path: "package.json",
      status: "observed",
      details: { package: "lodash", scope: "manifest", version: "4.17.21" },
    },
  ];
...
test("T216 model: matchedKey identities are stable and record ids are prefixed hashes", () => {
  ...
  assert.equal(
    createMatchedKey({
      ...
      category: "standard",
      details: {
        registryId: "std:owasp-cyclonedx:1.7",
        editionKey: "1.7",
        disposition: "metadata_only",
      },
    }),
    "standard:std:owasp-cyclonedx:1.7",
  );
  assert.match(
    recordId({ category: "pin", matchedKey: "pin:requests:2.31.0", path: "requirements.txt" }),
    /^asc-[a-f0-9]{24}$/,
  );
});
...
test("T216 model: invalid categories, statuses, identities, and paths fail with typed errors", () => {
  assert.throws(
    ...
    (e) => e instanceof AssuranceModelError && e.code === "UNKNOWN_CATEGORY",
  );
  ...
  assert.throws(
    () => bad({ details: { format: "package_json", ecosystem: "javascript", extra: 1 } }),
    AssuranceModelError,
  );
  ...
  assert.deepEqual(privacyFiltered.diagnostics, [
    { path: "package.json", status: "unverified", reason: "PRIVACY" },
  ]);
});
...
test("T216 model: privacy violations are dropped and become unverified PRIVACY diagnostics", () => {
  ...
  assert.deepEqual(
    model.manifest.map(({ details }) => details.format),
    ["package_json"],
  );
...
test("T216 standards pack: exact identity joins resolve; unknown identities never resolve", () => {
  assert.equal(
    resolveAssuranceStandard("sbom:CycloneDX:1.7")?.registryId,
    "std:owasp-cyclonedx:1.7",
  );
  ...
  assert.equal(
    resolveAssuranceStandard("sarif:2.1.0")?.registryId,
    "std:oasis-sarif:2.1.0-errata01",
  );
  assert.equal(
    resolveAssuranceStandard("accessibility:WCAG:2.2")?.registryId,
    "std:w3c-wcag:2.2-rec-20241212",
  );
...
test("T216 parsers: classifyAssurancePath recognizes assurance artifacts", () => {
  ...
  assert.deepEqual(classifyAssurancePath("requirements-dev.txt"), {
    kind: "manifest",
    format: "text",
  });
  ...
  assert.deepEqual(classifyAssurancePath(".gitleaks.toml"), {
    kind: "configuration",
    format: "text",
  });
  assert.deepEqual(classifyAssurancePath("osv-scanner.toml"), {
    kind: "configuration",
    format: "text",
  });
  assert.deepEqual(classifyAssurancePath("osv-scanner-results.json"), {
    kind: "tool_result",
    format: "text",
  });
  assert.deepEqual(classifyAssurancePath("accessibility.md"), {
    kind: "accessibility",
    format: "text",
  });
  assert.deepEqual(classifyAssurancePath(".pa11yci.json"), {
    kind: "accessibility",
    format: "text",
  });
  assert.deepEqual(classifyAssurancePath("a.intoto.jsonl"), {
    kind: "attestation",
    format: "text",
  });
  assert.deepEqual(classifyAssurancePath("provenance.intoto.jsonl"), {
    kind: "attestation",
    format: "text",
  });
  assert.deepEqual(classifyAssurancePath("a.sigstore.json"), {
    kind: "attestation",
    format: "text",
  });
...
test("T216 parsers: package.json extracts manifest, license, exact pins, and git sources", () => {
  ...
  assert.deepEqual(
    byCategory("manifest").map((r) => r.details),
    [{ ecosystem: "javascript", format: "package_json" }],
  );
  assert.deepEqual(
    byCategory("license").map((r) => r.details),
    [{ declared: "manifest", identifier: "MIT" }],
  );
  const pins = byCategory("pin").map((r) => r.details);
  assert.ok(
    pins.some((p) => p.package === "lodash" && p.version === "4.17.21" && p.scope === "manifest"),
  );
  assert.equal(
    pins.some((p) => p.package === "express"),
    false,
    "ranges are not pins",
  );
  assert.deepEqual(
    byCategory("source").map((r) => [r.details.kind, r.details.host]),
    [["git", "github.com"]],
  );
});

test("T216 parsers: requirements.txt extracts pins and index sources, skipping ranges", () => {
  const result = textArtifact(
    "requirements.txt",
    [
      "requests==2.31.0",
      "flask>=2.0",
  ...
  assert.deepEqual(
    recordsOf(result)
      .filter((r) => r.category === "pin")
      .map((r) => r.details),
    [{ package: "requests", scope: "requirements", version: "2.31.0" }],
  );
  const sources = recordsOf(result)
...
test("T216 parsers: CycloneDX SBOM 1.7 projects safely and joins its standard", () => {
  ...
  assert.equal(
    JSON.stringify(sboms[0]).includes("alice@example.test"),
    false,
    "T206 projection strips contacts",
  );
  assert.deepEqual(
    recordsOf(result)
      .filter((r) => r.category === "standard")
      .map((r) => r.details.registryId),
    ["std:owasp-cyclonedx:1.7"],
  );
});

test("T216 parsers: SPDX SBOM joins the SPDX standard; unknown spec versions never join", () => {
  const spdx = jsonArtifact("a.spdx.json", {
    spdxVersion: "SPDX-2.3",
    packages: [{ name: "pkg", versionInfo: "1.0.0", licenseConcluded: "MIT" }],
  });
  ...
  const old = jsonArtifact("old.json", {
    bomFormat: "CycloneDX",
    specVersion: "1.6",
    components: [],
  });
  assert.equal(
    recordsOf(old).filter((r) => r.category === "standard").length,
    0,
    "1.6 is not in the registry",
  );
});
...
test("T216 parsers: VEX is projected to metadata only with no vulnerability content", () => {
  ...
  assert.equal(
    /not_affected|affected|justification/i.test(serialized),
    false,
    "no verdict language",
  );
  assert.deepEqual(
...
test("T216 parsers: unknown SBOM/VEX/SARIF schemas are unsupported diagnostics", () => {
  ...
  assert.deepEqual(diagnosticsOf(unknownSbom), [
    { path: "sbom.json", status: "unsupported", reason: "UNKNOWN_SCHEMA" },
  ]);
  const unknownVex = jsonArtifact("openvex.json", {
    "@context": "https://other.dev/ns",
    statements: [],
  });
  assert.deepEqual(diagnosticsOf(unknownVex), [
    { path: "openvex.json", status: "unsupported", reason: "UNKNOWN_SCHEMA" },
  ]);
  const notSarif = jsonArtifact("a.sarif", { version: "2.0", runs: [] });
  assert.equal(
    recordsOf(notSarif).filter((r) => r.category === "sarif").length,
    1,
    "2.0 is still a SARIF document",
  );
...
test("T216 parsers: tool configuration is presence-only with the tool name", () => {
  for (const [path, tool] of [
    ...
    assert.deepEqual(
      recordsOf(result),
      [{ category: "configuration", path, status: "observed", details: { tool } }],
      path,
    );
    assert.deepEqual(diagnosticsOf(result), []);
  }
...
test("T216 parsers: tool results are presence-only and never expose content", () => {
  const result = textArtifact(
    ...
    JSON.stringify({ results: [{ id: "CVE-2024-0001" }] }),
  );
...
test("T216 parsers: accessibility statements declare WCAG and join its standard", () => {
  const result = textArtifact(
    "accessibility.md",
    "This site aims to follow WCAG 2.2 guidelines.\n",
  );
  assert.deepEqual(
    recordsOf(result)
...
test("T216 parsers: attestations are presence-only with declared format and kind", () => {
  assert.deepEqual(
    recordsOf(textArtifact("a.intoto.jsonl", '{"_type":"https://in-toto.io/Statement/v1"}')).map(
      (r) => r.details,
    ),
    [{ format: "in-toto", kind: "statement" }],
  );
  assert.deepEqual(
    recordsOf(textArtifact("build.att", "opaque")).map((r) => r.details),
    [{ format: "in-toto", kind: "link" }],
  );
  assert.deepEqual(
    recordsOf(textArtifact("a.sigstore.json", "opaque")).map((r) => r.details),
...
test("T216 parsers: license files are presence-only and manifest identifiers are bounded", () => {
  assert.deepEqual(
    recordsOf(textArtifact("LICENSE", "opaque text")).map((r) => r.details),
    [{ declared: "file", identifier: null }],
  );
  assert.deepEqual(
    recordsOf(textArtifact("COPYING", "opaque")).map((r) => r.details),
    [{ declared: "file", identifier: null }],
  );
});

test("T216 parsers: unsupported assurance-like content is a typed diagnostic, never a crash", () => {
  const result = extractAssuranceArtifact({
    path: "LICENSES.md",
    text: "x",
    value: null,
    format: "text",
  ...
  assert.deepEqual(diagnosticsOf(result), [
    { path: "LICENSES.md", status: "unsupported", reason: "UNSUPPORTED" },
  ]);
...
test("T216 renderer is INERT: not registered in write or existing-ten renderers", async () => {
  ...
  assert.equal(
    existingTen.includes("render/assurance.mjs"),
    false,
    "existing-ten must not import the assurance renderer",
  );
  const write = await readFile(join(LIB_ROOT, "scan", "write.mjs"), "utf8");
  assert.equal(
    write.includes("render/assurance.mjs"),
    false,
    "write must not import the assurance renderer",
  );
});
...
test("T216 scanner: malformed result artifacts never invalidate manifest evidence", async () => {
  ...
  await withFixture("assurance-malformed", files, async (dir) => {
    ...
    const malformed = artifacts
      .filter((entry) => entry.status === "malformed")
      .map(({ path }) => path);
...
test("T216 scanner: unsupported and unreadable peers are diagnostics without erasing valid evidence", async () => {
  ...
  await withFixture("assurance-atomicity", files, async (dir) => {
    const result = await scanAssurance({ root: dir, files: Object.keys(files) });
    assert.equal(
      result.model.summary.manifests,
      2,
      "package.json and requirements.txt are both manifests",
    );
      ...
      components: [
        {
          name: "safe",
          version: "1.0.0",
          externalReferences: [{ url: "https://user:pass@example.test/repo" }],
        },
      ],
    }),
    "a.sarif": JSON.stringify({
      ...
      runs: [
        {
          ...
          results: [{ message: { text: "Alice Example private file /home/alice/x" } }],
        },
      ],
    }),
  };
...
test("T216 scanner: binary lockfiles are presence-only and never fail decoding", async () => {
  ...
  await withFixture("assurance-binary", files, async (dir) => {
    const result = await scanAssurance({ root: dir, files: Object.keys(files) });
    assert.deepEqual(
      result.model.lock.map(({ details }) => details.format),
      ["bun"],
    );
...
test("T216 scanner: explicit requests drive the scan without discovery", async () => {
  ...
  await withFixture("assurance-requests", files, async (dir) => {
    const result = await scanAssurance({
      root: dir,
...
test("T216 inertness: assurance modules never touch fs, child_process, or execution surfaces", async () => {
  ...
  const providerSource = await readFile(
    join(LIB_ROOT, "scan", "providers", "assurance.mjs"),
    "utf8",
  );
...
test("T216 standards pack: only registered metadata-only registry IDs are referenced", () => {
  ...
  assert.equal(
    serialized.includes("std:owasp-asvs:5.0.0"),
    false,
    "no control-claiming standards are referenced",
  );
