...
test("T201 reviewed test and fixture inputs stay registered and non-skipped", async () => {
  ...
  for (const moduleName of [
    ...
    const module = await import(new URL(moduleName, fixtureUrl));
    assert.ok(
      module.files && Object.keys(module.files).length > 0,
  ...
  assert.deepEqual(
    collected.failures,
    [],
    `reviewed tests failed: ${collected.failures.join(", ")}`,
  );
...
test("T201 supersession records are legacy locks or live recurring replacements", async () => {
  ...
  for (const entry of baseline.entries) {
    assert.ok(
      ["legacy_locked", "superseded"].includes(entry.state),
      `${entry.id} has an invalid state`,
    );
    ...
    if (entry.state === "legacy_locked") {
      ...
      assert.ok(
        lockResults.every(Boolean),
        `${entry.id} legacy locks changed before ${entry.replacementTask}`,
      );
      continue;
    }
    assert.ok(
      lockResults.some((applies) => !applies),
      `${entry.id} cannot be superseded while every legacy lock applies`,
    );
    ...
    const collected = await collectTestNames([join(TEST_ROOT, "..", entry.replacement.testFile)]);
    assert.equal(
      collected.code,
      0,
    ...
    assert.ok(
      collected.names.includes(entry.replacement.testName),
      `${entry.id} replacement test is not registered under its exact name`,
    );
    assert.ok(
      !collected.failures.includes(entry.replacement.testName),
      `${entry.id} replacement test failed to pass`,
    );
    assert.ok(
      !collected.skips.includes(entry.replacement.testName),
