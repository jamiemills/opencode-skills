...
test('T208 registered commands record exact fixed argv arrays and controlled command IDs', async () => {
  ...
  assert.ok(calls.every((call) => call.shell === false), 'shell mode is always disabled');
  assert.deepEqual(calls[0].stdio, ['ignore', 'pipe', 'pipe']);
  assert.equal(calls[0].timeout, COMMAND_LIMITS.rgFilesTimeoutMs);
  assert.deepEqual(calls[0].outputPolicy, { maxBytes: COMMAND_LIMITS.rgFilesMaxBytes, encoding: 'utf8' });
  assert.equal(calls[1].timeout, COMMAND_LIMITS.gitTimeoutMs);
  assert.deepEqual(calls[1].outputPolicy, { maxBytes: COMMAND_LIMITS.gitMaxBytes, encoding: 'utf8' });
});

test('T208 commands run with a reduced deterministic environment', async () => {
  const reduced = buildCommandEnv({ PATH: '/usr/bin:/bin', HOME: '/home/tester', TOKEN: 'secret', FOO: 'bar' });
  ...
  assert.equal(calls[0].env.TOKEN, undefined, 'host tokens must not leak into the child environment');
});
...
test('T208 non-no-match rg failures and runner failures become sanitized typed errors', async () => {
  const { run: failRg } = createRecordingRunner([{ status: 2, stdout: '', stderr: 'error: /private/etc/repo' }]);
  await assert.rejects(
    createCommandBroker({ runner: { run: failRg } }).execute('rg:files', { cwd: '/repo' }),
    (error) => {
      assert.ok(error instanceof CommandError);
      assert.equal(error.code, 'RG_FAILURE');
      assert.ok(!error.message.includes('/private/etc'), 'raw stderr must never leak into error messages');
      return true;
    },
  );
  ...
  await assert.rejects(
    createCommandBroker({ runner: { run: throwRunner } }).execute('git:log-oneline-50', { cwd: '/repo' }),
    (error) => {
      assert.ok(error instanceof CommandError);
      assert.equal(error.code, 'RUN_FAILURE');
      assert.ok(!error.message.includes('configured failure'), 'raw runner messages must never leak');
      return true;
    },
  );
});
...
test('T208 rg:json accepts only bounded literal patterns', async () => {
  ...
  for (const pattern of ['a b', '-flag', 'a;rm -rf /', 'a`b', 'a$b', '', 'a'.repeat(257)]) {
    await assert.rejects(broker.execute('rg:json', { cwd: '/repo', pattern }), (error) => {
      assert.ok(error instanceof CommandError);
      assert.equal(error.code, 'INVALID_PATTERN', `pattern ${JSON.stringify(pattern.slice(0, 8))} must be rejected`);
      return true;
    });
  }
});
...
test('T208 default runner enforces caps and sanitizes OS errors', async () => {
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.ok(!error.message.includes('/definitely-nonexistent'), 'paths must never leak into error messages');
      return true;
    },
  );
});
...
test('T208 migrated callers own no child-process capability outside the broker', () => {
  assert.match(readSource('lib/scan/shared/command.mjs'), /import \{ execFile \} from 'node:child_process';/);
  for (const rel of [
...
test('T208 git scan issues only registered read-only commands through the broker', async () => {
  ...
  try {
    ...
    assert.equal((await scanGit(dir, {}, broker)).findings.remote, 'N/A',
