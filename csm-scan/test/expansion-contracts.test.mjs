...
function applicabilityEvidence(claimId, facts, suffix = "applicability") {
  return evidence(claimId, "applicability", { facts }, suffix, "repository_metadata");
}
...
test("T202R1 exported schema snapshot is exact, complete, and inert", () => {
  ...
  assert.deepEqual(
    DIMENSION_IDS.filter((id) => !PROVIDER_DIMENSION_IDS.includes(id)),
...
test("T202R1 partial, unknown, reordered, mismatched, and duplicate registries fail closed", () => {
  ...
  assert.throws(
    () => validateDimension({ ...definition("DIM-api-v1"), order: 0 }),
  ...
  duplicateClaim[1] = {
    ...duplicateClaim[1],
    expectedClaimIds: duplicateClaim[0].expectedClaimIds,
  };
...
test("T202R1 evidence IDs bind category and canonical structured details", () => {
  ...
  const changed = evidence("CLM-api-search-v1", "search_space", {
    ...SEARCH_OK,
    filesInspected: 4,
  });
  ...
  assert.throws(() => {
    search.details.filesInspected = 0;
  }, TypeError);
});
...
test("T202R1 canonical evidence validation rejects forgery, relabeling, and detail mutation", () => {
  ...
  assert.throws(
    () => validateEvidence({ ...valid, id: `EVD-v1-${"0".repeat(64)}` }),
    /canonical evidence content/,
  );
  assert.throws(
    () => validateEvidence({ ...valid, category: "rpc" }),
    /canonical evidence content/,
  );
  ...
  assert.throws(
    () =>
      validateEvidence({
        ...search,
        details: { ...search.details, recordsInspected: 6 },
      }),
    /canonical evidence content/,
  );
  ...
  assert.throws(
    () => validateClaims([rawClaim(claimId, "observed", [forged])], [forged], claims),
    (error) => error instanceof ClaimContractError && error.code === "INVALID_EVIDENCE",
  );
...
test("T202R1 evidence paths, schemas, search counts, and sensitive fields are bounded", () => {
  ...
  assert.throws(
    () =>
      evidence("CLM-api-bad-count-v1", "search_space", {
        ...SEARCH_OK,
        filesInspected: 101,
        fileLimit: 100,
      }),
  ...
  assert.throws(
    () => evidence("CLM-api-bad-limit-v1", "search_space", { ...SEARCH_OK, fileLimit: 0 }),
    /explicit search bound/,
  );
  assert.throws(
    () =>
      evidence("CLM-api-bad-omitted-v1", "search_space", {
        ...SEARCH_OK,
        complete: false,
        omittedCount: 1,
      }),
  ...
  assert.throws(
    () => validateEvidenceList(Array(EVIDENCE_LIMITS.count + 1).fill(valid)),
    /bounded/,
  );
});
...
test("T202R1 six statuses accept mutually exclusive canonical evidence", () => {
  const ids = [
    ...
    "CLM-api-inferred-v1",
    "CLM-api-absence-v1",
    "CLM-api-unsupported-v1",
    "CLM-api-unverified-v1",
    "CLM-api-na-v1",
  ];
...
test("T202R1 search statuses reject cap, failure, unsupported overlap, omission, and mismatch", () => {
  ...
  for (const cause of [
    ...
    const record = evidence(
      ...
      "search_space",
      details,
      JSON.stringify(cause).replace(/[^a-z]/gi, "") || "cause",
    );
    assert.throws(
      () =>
        validateClaim(
          rawClaim(id, "not_detected", [record], { searchSpace: details }),
          [record],
          dimension,
        ),
  ...
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "unverified", [unsupported], { searchSpace: SEARCH_UNSUPPORTED }),
        [unsupported],
        dimension,
      ),
    /does not match claim status/,
  ...
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "not_detected", [omittedEvidence], { searchSpace: omitted }),
        [omittedEvidence],
        dimension,
      ),
    /does not match claim status/,
  ...
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "not_detected", [matching], { searchSpace: different }),
        [matching],
        dimension,
      ),
    /must be identical/,
...
test("T202R1 applicability evaluates all operators and rejects unrelated or insufficient facts", () => {
  ...
  const dimension = definition("DIM-api-v1", [id]);
  const unrelated = applicabilityEvidence(
    id,
    [{ field: "language", present: true, value: "rust" }],
    "unrelated",
  );
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "not_applicable", [unrelated], {
  ...
  const trueFact = applicabilityEvidence(
    id,
  ...
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "not_applicable", [trueFact], {
  ...
  const insufficient = applicabilityEvidence(
    id,
    [{ field: "repository_kind", present: true, value: "metadata" }],
    "insufficient",
  );
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "not_applicable", [insufficient], {
  ...
  const absent = applicabilityEvidence(
    id,
  ...
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "not_applicable", [absent, trueFact], {
...
test("T202R1 inference requires a derivation ID and direct canonical inputs", () => {
  ...
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "inferred", [direct], {
          inputEvidenceIds: [direct.id],
          derivationId: null,
        }),
        [direct],
        dimension,
  ...
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "inferred", [search], {
          inputEvidenceIds: [search.id],
          derivationId: "DRV-search-only-v1",
        }),
        [search],
        dimension,
      ),
  ...
  assert.throws(
    () =>
      validateClaim(
        rawClaim(id, "observed", [direct], {
          derivationId: "DRV-not-allowed-v1",
        }),
        [direct],
        dimension,
      ),
    /only direct canonical evidence/,
  );
  const applicability = applicabilityEvidence(id, [
    { field: "repository_kind", present: true, value: "metadata" },
  ]);
  assert.throws(
    () => validateClaim(rawClaim(id, "observed", [applicability]), [applicability], dimension),
    /only direct/,
  );
});
...
test("T202R1 aggregate claims enforce registry ownership and one canonical evidence universe", () => {
  ...
  assert.throws(
    () => validateClaims([{ ...valid, dimensionId: "DIM-stack-v1" }], [record], dimensions),
    /not owned/,
  );
  assert.throws(
    () => validateClaims([{ ...valid, dimensionId: "DIM-unknown-v1" }], [record], dimensions),
    /not registered/,
  );
  assert.throws(
    () => validateClaims([{ ...valid, id: "CLM-api-not-expected-v1" }], [record], dimensions),
    /not owned/,
  );
  assert.throws(
    () => validateClaims([valid, valid], [record], dimensions),
...
test("T202R1 coverage requires registry, canonical claims, and registry-owned denominator", () => {
  ...
  assert.throws(
    () => computeCoverage([{ id: observedId, coverageState: "complete" }], records, dimensions),
    /fields do not match/,
  );
...
test("T202R1 providers expose all planned categories and remain strict data-only records", () => {
  ...
  assert.deepEqual(
    provider.dimensions.map(({ dimensionId }) => dimensionId),
  ...
  assert.throws(
    () =>
      validateProvider({
        ...base,
        dimensions: [{ dimensionId: "DIM-api-v1", categories: ["arbitrary"] }],
      }),
    /not allowlisted/,
  );
...
test("T202R2 arrays are canonical dense Array.prototype collections at every contract layer", () => {
  ...
  for (const [name, run] of [
    ...
    [
      "expected claim IDs sparse",
      () =>
        validateDimension({
          ...definition("DIM-api-v1", [id]),
          expectedClaimIds: sparse(),
        }),
    ],
    [
      "rules stripped",
      () =>
        validateDimension({
          ...
          applicability: {
            mode: "all",
            rules: stripped([{ field: "repository_kind", operator: "equals", value: "source" }]),
          },
        }),
    ],
    [
      ...
      () =>
        validateDimension({
          ...
          applicability: {
            mode: "all",
            rules: [{ field: "repository_kind", operator: "in", value: sparse() }],
          },
        }),
    ],
    [
      "claim evidence IDs sparse",
      () =>
        validateClaim(
          {
            ...baseClaim,
...
test("T202R2 collection limits precede descriptor inspection and object descriptor allocation", () => {
  ...
  const tooManyKeys = {};
  for (let index = 0; index <= DATA_LIMITS.maxObjectKeys; index++)
...
test("T202R2 applicability rule and fact values use field-specific types", () => {
  ...
  assert.equal(
    validateDimension({
      ...
      applicability: {
        mode: "all",
        rules: [{ field: "language", operator: "exists", value: true }],
      },
    }).applicability.rules[0].value,
    true,
  );
});
...
test("T202R2 one immutable category map drives dimensions, providers, and claim admissibility", () => {
  ...
  assert.equal(
    validateClaim(
      rawClaim(structureId, "observed", [structure], {
        dimensionId: "DIM-structure-v1",
      }),
      [structure],
      definition("DIM-structure-v1", [structureId]),
    ).status,
    "observed",
  );
  ...
  assert.equal(
    validateClaim(
      rawClaim(gitId, "observed", [git], {
        dimensionId: "DIM-git-v1",
      }),
      [git],
      definition("DIM-git-v1", [gitId]),
    ).status,
    "observed",
  );
});
...
test("T202R1 proxy, depth, node, and reflection bombs fail with typed safe errors", () => {
  ...
  const proxy = new Proxy(
    {},
    {
      getPrototypeOf() {
        traps++;
        throw new Error("secret-proxy-value");
      },
      ownKeys() {
        traps++;
        throw new Error("secret-proxy-value");
  ...
  for (const validate of [
    ...
    const arrayProxy = new Proxy([], {
      get() {
        arrayTraps++;
        throw new Error("secret-array-value");
      },
      getPrototypeOf() {
        arrayTraps++;
        throw new Error("secret-array-value");
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof EvidenceContractError && !error.message.includes("secret-accessor-value"),
  );
});
