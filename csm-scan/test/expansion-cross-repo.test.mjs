...
test('T221 privacy: secret-style workspace references are dropped as unparseable and never emitted', () => {
  const snapshot = synth(
    [{ scanId: 'scan:web', vcs: 'https://github.com/acme/web.git' }],
    [
      { scanId: 'scan:web', kind: 'workspace', value: 'token:ghp_t0kenVaLue123', path: 'package.json', sourceKind: 'manifest' },
      { scanId: 'scan:web', kind: 'workspace', value: 'api_key:AKIAI0S3CR3TKEY', path: 'package.json', sourceKind: 'manifest' },
      { scanId: 'scan:ghost', kind: 'workspace', value: 'token:ghp_unknownOwner', path: 'package.json', sourceKind: 'manifest' },
    ],
  );
  const serialized = JSON.stringify(snapshot);
  for (const forbidden of [
    'ghp_t0kenVaLue123', 'AKIAI0S3CR3TKEY', 'ghp_unknownOwner', 'token:', 'api_key:',
  ]) {
    assert.equal(serialized.includes(forbidden), false, `snapshot leaked ${forbidden}`);
  }
