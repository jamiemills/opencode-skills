...
const EXACT_REFS = [
  ...
  },
  {
    scanId: "scan:worker",
    kind: "vcs",
    value: "https://github.com/acme/web.git",
    path: "Cargo.toml",
    sourceKind: "manifest",
  },
...
test("T221 exact: single-candidate references resolve to canonical sanitized edges", () => {
  ...
  assert.equal(
    byCoordinate["vcs:github.com/acme/web"].sourceRepository,
    "vcs:github.com/acme/worker",
  );
  ...
  assert.equal(
    byCoordinate["event:ingest.completed"].targetRepository,
    "vcs:github.com/acme/worker",
  );
...
test("T221 exact: edge ids are deterministic SHA-256 hashes of canonical content", () => {
  ...
  assert.deepEqual(
    first.edges.edges.map(({ id }) => id),
    second.edges.edges.map(({ id }) => id),
  );
...
test("T221 exact: metrics count cross-repository and self edges separately", () => {
  ...
  assert.equal(
    snapshot.metrics.crossRepositoryEdges + snapshot.metrics.selfEdges,
    snapshot.metrics.edges,
  );
});
...
test("T221 unresolved: zero-candidate and unparseable references remain external records", () => {
    ...
    [
      {
        scanId: "scan:web",
        kind: "vcs",
        value: "https://github.com/acme/unknown.git",
        path: "package.json",
        sourceKind: "manifest",
      },
      {
  ...
  const byReason = Object.fromEntries(
    snapshot.edges.external.map((record) => [record.reason, record]),
  );
...
const AMBIGUOUS_REPOS = [
  ...
  {
    scanId: "scan:site",
    vcs: "https://github.com/acme/site.git",
    componentRoots: ["packages/web"],
  },
...
test("T221 duplicate: same scan id produces unresolved records that are never candidates", () => {
  ...
  assert.ok(
    snapshot.identityTable.unresolved.every((record) => record.reason === "duplicate_scan_id"),
  );
...
test("T221 duplicate: missing scan ids produce unresolved missing_identity records", () => {
  ...
  assert.equal(
    identities.unresolved.filter((record) => record.reason === "missing_identity").length,
    2,
  );
  assert.equal(
    identities.unresolved.filter((record) => record.reason === "duplicate_identity").length,
    2,
  );
});
...
test("T221 duplicate-repo: identical canonical VCS coordinates are duplicates regardless of transport", () => {
  ...
  assert.equal(snapshot.metrics.unresolved, 2);
  assert.ok(
    snapshot.identityTable.unresolved.every((record) => record.reason === "duplicate_identity"),
  );
  assert.deepEqual(
  ...
  assert.equal(
    snapshot.metrics.external,
    1,
    "reference to a duplicate-repo coordinate is external",
  );
  assert.equal(snapshot.metrics.edges, 0);
});
...
test("T221 scoped: fully scoped references disambiguate repositories that share a short name", () => {
  const snapshot = synth(
    ...
    [
      ...
      },
      {
        scanId: "scan:client",
        kind: "vcs",
        value: "git@gitlab.example.com:acme/web.git",
        path: ".",
        sourceKind: "config",
      },
    ],
  ...
  const webEdge = snapshot.edges.edges.find(
    (edge) => edge.coordinate === "vcs:github.com/acme/web",
  );
  const mirrorEdge = snapshot.edges.edges.find(
    (edge) => edge.coordinate === "vcs:gitlab.example.com/acme/web",
  );
...
test("T221 reverse-order: reversed repository and reference order is byte-identical", () => {
  ...
  assert.equal(
    JSON.stringify(forwardTied),
    JSON.stringify(reversedTied),
  ...
  assert.deepEqual(
    externalKinds,
    [...externalKinds].toSorted(),
    "tied external records sort by sourceKind",
  );
  const ambiguousKinds = forwardTied.edges.ambiguous.map((record) => record.sourceKind);
  assert.deepEqual(
    ambiguousKinds,
    [...ambiguousKinds].toSorted(),
    "tied ambiguous records sort by sourceKind",
  );
});
...
test("T221 privacy: credentials, absolute paths, and identities never reach the snapshot", () => {
    ...
    [
      {
        scanId: "scan:web",
        kind: "vcs",
        value: "https://bob:p%40ssw0rd@github.com/acme/worker.git",
        path: "package.json",
        sourceKind: "manifest",
      },
      {
  ...
  assert.equal(
    webRepo.repositoryId,
    "vcs:github.com/acme/web",
    "credentials are stripped from the VCS coordinate",
  );
  ...
  assert.equal(
    vcsExternal.coordinate,
    "vcs:github.com/acme/worker",
    "reference credentials are stripped",
  );
...
test("T221 privacy: secret-style workspace references are dropped as unparseable and never emitted", () => {
    ...
    [
      {
        scanId: "scan:web",
        kind: "workspace",
        value: "token:ghp_\x740kenVaLue123",
        path: "package.json",
        sourceKind: "manifest",
      },
      {
        scanId: "scan:web",
        kind: "workspace",
        value: "api_key:AKIA\x490S3CR3TKEY",
        path: "package.json",
        sourceKind: "manifest",
      },
      {
        scanId: "scan:ghost",
        kind: "workspace",
        value: "token:ghp_\x75nknownOwner",
        path: "package.json",
        sourceKind: "manifest",
      },
    ],
  ...
  assert.ok(
    unparseable.every((record) => record.kind === "workspace" && record.coordinate === null),
    "secret-style workspace references stay unparseable with a null coordinate",
  );
  assert.equal(
    snapshot.edges.external.some((record) => record.reason === "unknown_owner"),
    true,
  );
  assert.equal(snapshot.edges.edges.length, 0);
  assert.equal(snapshot.metrics.external, 3);
...
test("T221 privacy: identity and edge primitives sanitize VCS coordinates", () => {
  ...
  assert.equal(
    vcsCoordinate(normalizeVcsCoordinates("git@github.com:acme/web.git")),
    "vcs:github.com/acme/web",
  );
...
test("T221 caps: candidate lists are capped at the explicit bound with full counts disclosed", () => {
  ...
  for (let index = 0; index < EDGE_LIMITS.candidates + 20; index++) {
    repos.push({
      scanId: `scan:repo-${index}`,
      vcs: `https://github.com/acme/repo-${index}.git`,
      componentRoots: ["packages/web"],
    });
  }
  ...
  assert.equal(
    snapshot.metrics.edges,
    0,
    "candidate-capped ambiguity still never enters graph metrics",
  );
});
...
test("T221 caps: identity synthesis enforces the repository bound", () => {
  ...
  assert.throws(
    () => synthesizeRepositoryIdentities(many),
...
test("T221 errors: unknown reference kinds and malformed references fail closed with typed errors", () => {
  ...
  assert.throws(
    () =>
      resolveReferences({
        ...
        references: [
          { scanId: "scan:web", kind: "fuzzy", value: "x", path: null, sourceKind: "config" },
        ],
      }),
  ...
  assert.throws(
    () =>
      resolveReferences({
        identities: snapshot.identityTable,
        references: [
          { scanId: "scan:web", kind: "vcs", value: "", path: null, sourceKind: "config" },
        ],
      }),
  ...
  assert.throws(
    () => synthesizeCrossRepository(null),
...
test("T221 immutability: snapshot, identities, edges, and metrics are deep-frozen", () => {
  ...
  assert.throws(() => {
    snapshot.metrics.edges = 0;
  }, TypeError);
  assert.throws(() => {
    snapshot.identityTable.repositories[0].repositoryId = "mutated";
  }, TypeError);
});
...
test("T221 inert: no production module outside cross-repo imports the cross-repo modules", async () => {
  ...
  const activatedConsumers = new Set([join(LIB_ROOT, "scan", "pipeline", "run.mjs")]);
  for (const file of files) {
    ...
    for (const crossRepoFile of ["identity.mjs", "edges.mjs", "render.mjs"]) {
      ...
      assert.ok(
        !resolved.includes(target),
        `${file.replace(/\\/g, "/")} imports cross-repo/${crossRepoFile}`,
      );
    }
  }
});
...
test("T221 inert: modules export data and pure factories, never execution surfaces", async () => {
  const identitySource = await readFile(
    join(LIB_ROOT, "scan", "cross-repo", "identity.mjs"),
    "utf8",
  );
  ...
  for (const source of [identitySource, edgesSource]) {
    for (const forbidden of ["execute(", "writeFile", "writeNORMS"]) {
      assert.equal(
        ...
        false,
        `cross-repo module exposes execution surface ${forbidden}`,
      );
    }
  }
