...
import { createDataRenderer, renderData } from "../lib/scan/render/data.mjs";
...
function matchedKeys(result) {
  return result.records
    .map(({ category, signature }) => matchedKeyFor(category, signature))
    .toSorted();
}
  ...
  return {
    records: [
      ...
      },
      {
        ...
        dialect: "sql",
        signature: "orders:id",
        details: { type: "INTEGER", nullable: false },
        path: "migrations/001.sql",
        line: 1,
        status: "observed",
      },
      {
        category: "relation",
        dialect: "sql",
        signature: "orders:users:foreign_key",
        details: { kind: "foreign_key", target: "users" },
        path: "migrations/001.sql",
        line: 3,
        status: "observed",
      },
    ...
    edges: [
      {
        ...
        path: "migrations/001.sql",
        line: 3,
        matchedKey: "relation:orders:users:foreign_key",
      },
    ],
    diagnostics: [],
  };
...
test("T212 model: deep-frozen deterministic model with exact summary and search space", () => {
  ...
  assert.equal(
    ...
    JSON.stringify(buildDataModel({ ...sampleResult(), searchSpace: SEARCH_OK })),
  );
...
test("T212 model: identities are stable prefixed tokens and matchedKeys carry categories", () => {
  ...
  assert.equal(
    encodeMatchedKey("relation:orders:users:foreign_key"),
    "relation:orders:users:foreign_key",
  );
...
test("T212 model: invalid categories, statuses, identities, and paths fail with typed errors", () => {
  ...
  assert.throws(
    () => bad({ category: "table" }),
    (e) => e instanceof DataModelError && e.code === "UNKNOWN_CATEGORY",
  );
  assert.throws(
    () => bad({ status: "observed-evil" }),
    (e) => e instanceof DataModelError && e.code === "INVALID_STATUS",
  );
...
test("T212 model: global caps are disclosed and never drop silently", () => {
  ...
  for (let index = 0; index < DATA_LIMITS.entities + 5; index++) {
    records.push({
      category: "entity",
      dialect: "sql",
      signature: `table_${index}`,
      details: { table: `table_${index}` },
      path: "a/001.sql",
      line: index + 1,
      status: "observed",
    });
...
test("T212 extractor: classifyDataPath and migrationKindOf recognize dialects", () => {
  assert.deepEqual(classifyDataPath("migrations/001.sql"), {
    kind: "sql",
    format: "text",
    ecosystem: null,
  });
  assert.deepEqual(classifyDataPath("schema.prisma"), {
    kind: "prisma",
    format: "text",
    ecosystem: null,
  });
  assert.deepEqual(classifyDataPath("app/models.py"), {
    kind: "python",
    format: "text",
    ecosystem: "python",
  });
  assert.deepEqual(classifyDataPath("models.js"), {
    kind: "javascript",
    format: "text",
    ecosystem: "javascript",
  });
  assert.deepEqual(classifyDataPath("models.ts"), {
    kind: "typescript",
    format: "text",
    ecosystem: "typescript",
  });
  assert.deepEqual(classifyDataPath("src/schema.rs"), {
    kind: "rust",
    format: "text",
    ecosystem: "rust",
  });
  assert.deepEqual(classifyDataPath("README.md"), {
    kind: "other",
    format: "text",
    ecosystem: null,
  });
...
test("T212 extractor: SQL DDL extracts stores, schemas, entities, fields, keys, and FKs", () => {
  ...
  assert.ok(
    keys.includes("key:users:ux_users_email:unique"),
    "unique index kind derives from the UNIQUE keyword, not the name",
  );
...
test("T212 extractor: SQL edge candidates cite explicit FK evidence", () => {
  ...
  assert.deepEqual(result.edges.map(({ from, to, kind }) => `${from}:${to}:${kind}`).toSorted(), [
    "orders:users:foreign_key",
    "users:organizations:foreign_key",
  ]);
...
test("T212 extractor: SQLAlchemy models, fields, keys, FKs, and name-only relationship", () => {
  ...
  assert.ok(keys.includes("relation:orders:users:foreign_key"));
  assert.deepEqual(
    keys.filter((key) => key.startsWith("relation:")),
    ["relation:orders:users:foreign_key"],
  );
  assert.deepEqual(
    result.edges.map(({ from, to }) => `${from}:${to}`),
    ["orders:users"],
  );
  assert.deepEqual(result.diagnostics, [
...
test("T212 extractor: Django models, relation fields, caches, and queues", () => {
  ...
  assert.deepEqual(result.edges.map(({ from, to, kind }) => `${from}:${to}:${kind}`).toSorted(), [
    "Order:Customer:foreign_key",
    "Order:Tag:many_to_many",
  ]);
  assert.deepEqual(result.diagnostics, []);
});
...
test("T212 extractor: real django signals still classify models.Model subclasses and field assignments", () => {
  ...
  assert.ok(
    keys.includes("entity:Draft"),
    "a plain class with models.* assignments plus a django import is a django model",
  );
  assert.ok(keys.includes("field:Draft:text"));

  const inherited = extractSource(
    "audit/models.py",
    [
      "import django",
  ...
  const fieldOnlyNoImport = extractSource(
    "plain/models.py",
    ["class Widget:", "    name = models.CharField(max_length=80)", ""].join("\n"),
    "python",
  );
...
test("T212 extractor: tests and fixtures paths are excluded from data-source extraction", () => {
  ...
  const sqlFixture = extractSource(
    "fixtures/seed.sql",
  ...
  const migrationFixture = extractSource(
    "tests/migrations/0001_init.py",
...
test("T212 extractor: Prisma datasource, models, keys, explicit relation, and name-only list field", () => {
  ...
  assert.deepEqual(result.diagnostics, [
    { path: "prisma/schema.prisma", status: "unverified", reason: "NAME_ONLY", line: 8 },
  ]);
  const model = modelOf(result);
...
test("T212 extractor: Sequelize define, associations, queues, and caches", () => {
  ...
  assert.deepEqual(result.diagnostics, [
    { path: "models.js", status: "unverified", reason: "NAME_ONLY", line: 7 },
  ]);
});
...
test("T212 extractor: Django and Alembic migrations record explicit predecessor aliases", () => {
  const django = extractSource(
    "shop/migrations/0002_orders.py",
    [
      "class Migration(migrations.Migration):",
      "    dependencies = [",
      '        ("shop", "0001_initial"),',
  ...
  const alembic = extractSource(
    "alembic/versions/abc123_create.py",
    ['revision = "abc123"', 'down_revision = "def456"', ""].join("\n"),
    "python",
  );
...
test("T212 extractor: SQL migration files record the migration and extract tables", () => {
  ...
  assert.ok(keys.includes("entity:users"));
  assert.ok(
    result.records.some(({ category, dialect }) => category === "migration" && dialect === "sqlx"),
  );
...
test("T212 extractor: unsupported ecosystems and files produce typed diagnostics", () => {
  ...
  const other = extractDataArtifact({
    path: "docs/guide.md",
    text: "CREATE TABLE x",
    value: null,
    format: "text",
...
test("T212 extractor: credential-bearing SQL is never extracted as a value", () => {
  ...
  assert.deepEqual(
    result.records.map(({ category }) => category),
    ["entity", "field"],
  );
...
test("T212 model: migration order uses explicit predecessor edges only", () => {
  const first = extractSource(
    "shop/migrations/0001_initial.py",
    "class Migration(migrations.Migration):\n    initial = True\n",
    "python",
  );
  const second = extractSource(
    "shop/migrations/0002_orders.py",
...
test("T212 provider: matchedKeys carry admissible evidence references and edge observations", () => {
  ...
  assert.ok(
    observations.some(({ matchedKey }) => matchedKey === "relation:orders:users:foreign_key"),
  );
  assert.ok(
    observations.some(({ matchedKey }) =>
      matchedKey.startsWith("edge:entity@orders:entity@users:foreign_key"),
    ),
  );
...
test("T212 provider: deterministic, immutable, bounded, and empty for empty/foreign input", () => {
  ...
  for (let index = 0; index < 2100; index++) {
    records.push({
      category: "entity",
      dialect: "sql",
      signature: `bulk_${index}`,
      details: { table: `bulk_${index}` },
      path: "big.sql",
      line: index + 1,
      status: "observed",
    });
...
test("T212 renderer: empty model renders a factual no-detected line and disclosed caps", () => {
  ...
  assert.match(
    markdown,
    /No declaration-backed data architecture detected in 2 inspected file\(s\)\./,
  );
  assert.deepEqual(findVoiceHits(markdown), []);
...
test("T212 inertness: data renderer is never registered in the existing-ten map", async () => {
  ...
  assert.equal(
    existingTen.includes("render/data.mjs"),
    false,
    "existing-ten must not import the data renderer",
  );
...
test("T212 scanner: Pydantic self.models and Test* classes yield zero django entities while a real Model subclass is detected", async () => {
  ...
  await withFixture("data-pydantic", files, async (dir) => {
    ...
    assert.deepEqual(
      findings.entities.map(({ signature }) => signature),
      ["Customer"],
    );
...
test("T212 scanner: reader outcomes map to typed diagnostics with line null (crash regression)", async () => {
  ...
  await withFixture("data-outcomes", files, async (dir) => {
    ...
    assert.ok(
      findings.entities.some(({ signature }) => signature === "users"),
      "valid peer records survive",
    );
  });
});
...
test("T212 scanner: data source sampling cap is disclosed and never overclaims completeness", async () => {
  ...
  await withFixture("data-cap560", files, async (dir) => {
    ...
    const serialized = JSON.stringify(findings);
    assert.equal(
      serialized.includes("SKIPPED_CANARY"),
      false,
      "skipped file content must never be invented",
    );
  });
});
