...
test("T207 workflow extracts jobs, containers, commands, environments, and triggers with source locations", async (t) => {
  ...
  const result = await extractDeclarations({
    root,
    requests: requests([".github/workflows/ci.yml"]),
    options: LIMITS,
  });
  assert.deepEqual(kinds(result).toSorted(), [
    "command",
    "command",
    "command",
    "environment",
  ...
  const commands = result.declarations
    .filter(({ kind }) => kind === "command")
    .map(({ label, job }) => [label, job])
    .toSorted();
  ...
  const images = result.declarations
    .filter(({ kind }) => kind === "image")
    .map(({ label }) => label)
    .toSorted();
  assert.deepEqual(images, ["node:20"]);
  const environments = result.declarations
    .filter(({ kind }) => kind === "environment")
    .map(({ label }) => label)
    .toSorted();
  assert.deepEqual(environments, ["CI", "production", "ubuntu-latest"]);
  assert.equal(result.diagnostics.length, 0);
  assert.equal(
    result.declarations.every((entry) => Object.isFrozen(entry)),
    true,
  );
});
...
test("T207 makefile and version files produce targets, commands, and bounded versions", async (t) => {
  ...
  const targets = result.declarations
    .filter(({ kind }) => kind === "target")
    .map(({ label, source }) => [label, source.line]);
  ...
  const versions = result.declarations
    .filter(({ kind }) => kind === "version")
    .map(({ label, value }) => [label, value]);
...
test("T207 anchors, block scalars, malformed JSON, unknown artifacts, and missing files yield diagnostics", async (t) => {
  ...
  assert.ok(
    ["unsupported", "unverified"].includes(byPath[".github/workflows/anchored.yml"].status),
    "anchors must not parse",
  );
  ...
  assert.ok(
    result.declarations.some(({ kind, label }) => kind === "service" && label === "web"),
    "valid peers survive",
  );
...
test("T207 caps bound jobs, commands, and services without partial values", async (t) => {
  ...
  for (let index = 0; index < 70; index++)
    jobs[`job-${index}`] = { "runs-on": "ubuntu-latest", steps: [{ run: "echo ok" }] };
  const root = await fixture(t, {
...
test("T207 secret-bearing command records are omitted with a privacy diagnostic", async (t) => {
  ...
  const result = await extractDeclarations({
    root,
    requests: requests(["Makefile"]),
    options: LIMITS,
  });
  assert.deepEqual(
    result.declarations.map(({ kind }) => kind),
    ["target"],
  );
...
test("T207 Makefile assignments, define bodies, continuations, block-scalar indicators, and FROM flags are handled", async (t) => {
  ...
  assert.deepEqual(
    commands,
    ["mkdir -p dist"],
    "define bodies and continuations must not become commands",
  );
  const images = result.declarations
    .filter(({ kind }) => kind === "image")
    .map(({ label }) => label);
  assert.deepEqual(images, ["node:20"], "FROM flags and stage aliases must be stripped");
  const blockDiagnostic = result.diagnostics.find(
    ({ path }) => path === ".github/workflows/block.yml",
  );
  assert.equal(blockDiagnostic?.status, "unsupported");
  assert.ok(
    ["BLOCK_SCALAR", "PARSE_UNSUPPORTED"].includes(blockDiagnostic?.reason),
    "block scalars must never be parsed as commands",
  );
});
