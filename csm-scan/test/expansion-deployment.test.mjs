...
function assertCited(topology) {
  const declared = new Set(
    [...topology.resources, ...topology.services].map((record) => record.id),
  );
...
test("T213 dockerfile: literal FROM images, stages, and build edges", async (t) => {
  ...
  assert.deepEqual(
    topology.images.map(({ reference, scope }) => [reference, scope]),
    [["node:20", "from"]],
  );
  ...
  assert.deepEqual(
    topology.indicators.map(({ kind }) => kind),
    ["interpolation"],
  );
...
test("T213 dockerfile: undeclared COPY --from becomes a stub, never a resource", async (t) => {
  ...
  assert.deepEqual(
    topology.stubs.map(({ kind, label, source }) => [kind, label, source]),
...
test("T213 compose: services, images, depends_on edges, networks, and volumes", async (t) => {
  ...
  const { topology } = result;
  assert.deepEqual(topology.services.map(({ id }) => id).toSorted(), [
    "service@cache",
    "service@db",
    "service@web",
  ]);
  ...
  assert.deepEqual(
    topology.edges.map(({ from, to, kind }) => `${from}->${to}:${kind}`).toSorted(),
    [
...
test("T213 kubernetes: literal resources, containers, images, and references", async (t) => {
  ...
  assert.deepEqual(
    topology.services.map(({ id, image }) => [id, image]),
    [["container@api:api", "ghcr.io/acme/api:1.0"]],
  );
  assert.deepEqual(
    topology.images.map(({ reference }) => reference),
...
test("T213 kubernetes: block-scalar and anchor documents are diagnostics while valid peers survive", async (t) => {
  ...
  assert.ok(
    diagnostics.some(([doc]) => doc === 1),
    "block-scalar document is recorded",
  );
  assert.ok(
    diagnostics.some(([doc]) => doc === 2),
    "anchor document is recorded",
  );
  assert.ok(
    topology.diagnostics.every(({ status }) => ["malformed", "unsupported"].includes(status)),
...
test("T213 helm: chart metadata is literal and templates are markers without execution", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests([
      "helm/api/Chart.yaml",
      "helm/api/templates/deployment.yaml",
...
test("T213 terraform: dynamic names, counts, and remote functions never fabricate resources", async (t) => {
  ...
  assert.equal(
    topology.resources.some(({ label }) => label.startsWith("${")),
    false,
...
test("T213 cloudformation: literal resources, safe intrinsics, unresolved refs and transforms", async (t) => {
  ...
  assert.ok(
    topology.indicators.some(({ kind }) => kind === "intrinsic"),
    "conditions remain intrinsics",
  );
  assertCited(topology);
});
...
test("T213 cloudformation: JSON template with Fn:: intrinsics", async (t) => {
  ...
  assert.deepEqual(
    topology.edges.map(({ from, to }) => `${from}->${to}`),
    ["topic@Job->bucket@Bucket"],
  );
...
test("T213 serverless: functions, images, events, resources, and variables", async (t) => {
  ...
  assert.deepEqual(
    topology.edges.map(({ from, to }) => `${from}->${to}`),
    ["trigger@WorkerTrigger->queue@WorkerQueue"],
  );
  assert.equal(topology.stubs.length, 0);
  assertCited(topology);
});
...
test("T213 serverless: env/cf variables become unresolved stubs or indicators, never values", async (t) => {
  ...
  assert.ok(
    stubs.some(([source, label]) => source === "resolver" && label === "cf:shared-stack.SecretArn"),
  );
  const kinds = topology.indicators.map(({ kind }) => kind);
  assert.ok(kinds.includes("resolver"), "self references remain indicators");
  const serialized = JSON.stringify(result);
  assert.equal(
    serialized.includes("${env:STAGE}"),
    false,
    "variable expressions are never captured as values",
  );
});
...
test("T213 dynamic constructs produce indicators and stubs, never fabricated resources", async (t) => {
  const root = await fixture(t, {
    ...
    "helm/charts/app/templates/svc.yaml":
      "apiVersion: v1\nkind: Service\nmetadata:\n  name: {{ .Release.Name }}\n",
    "main.tf":
      'resource "aws_instance" "${var.which}" {\n  ami = "ami-1"\n}\nresource "aws_s3_bucket" "fixed" {\n  bucket = var.bucket\n}\nvariable "which" {\n}\n',
  });
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["compose.yaml", "helm/charts/app/templates/svc.yaml", "main.tf"]),
  });
  const { topology } = result;
  ...
  assert.equal(
    topology.resources.some(({ label }) => label.includes("${")),
    false,
  );
  assert.deepEqual(
    topology.resources.map(({ id }) => id).toSorted(),
    ["bucket@fixed", "variable@which"],
...
test("T213 ambiguity: duplicate declarations keep references unresolved", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["app.yaml", "cfg-one.yaml", "cfg-two.yaml"]),
  });
  const { topology } = result;
  assert.deepEqual(topology.edges, [], "ambiguous references never become edges");
  assert.deepEqual(
    topology.stubs.map(({ kind, label }) => [kind, label]),
    [["configmap", "shared"]],
  );
});
...
test("T213 ambiguity: in-artifact edges on duplicate declarations become renderer stubs, never dropped", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["app.yaml", "dup.yaml"]),
  });
  const { topology } = result;
  assert.deepEqual(topology.edges, [], "ambiguous in-artifact edge never becomes an edge");
  assert.deepEqual(
    topology.stubs.map(({ kind, label, source, path }) => [kind, label, source, path]),
    [["configmap", "shared", "env_from", "app.yaml"]],
  );
  const output = renderDeployment("repo", topology);
  assert.ok(
    output.includes("### Unresolved References (1)"),
    "ambiguous edge is retained as a stub in the renderer",
  );
  assert.ok(
    output.includes("| configmap | shared | app.yaml |"),
    "stub row cites kind, reference, and path",
  );
});
...
test("T213 cross-artifact: exact explicit references resolve to edges that cite declarations", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["app.yaml", "cfg.yaml"]),
  });
  const { topology } = result;
  assert.deepEqual(
    topology.edges.map(({ from, to, kind, crossArtifact }) => [from, to, kind, crossArtifact]),
...
test("T213 unresolved references never become edges or resources", async (t) => {
  ...
  assert.deepEqual(
    topology.stubs.map(({ kind, label }) => [kind, label]),
    [["secret", "never-declared"]],
  );
  assert.equal(
    topology.resources.some(({ id }) => id === "secret@never-declared"),
    false,
  );
});
...
test("T213 privacy: sensitive labels reject the artifact without leaking values", async (t) => {
  ...
  assert.deepEqual(
    result.topology.images.map(({ reference }) => reference),
    ["nginx"],
  );
  const serialized = JSON.stringify(result);
...
test("T213 per-artifact atomicity: malformed and unsupported peers never erase valid artifacts", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["bad-compose.yaml", "notes.yaml", "compose.yaml", "missing.txt"]),
  });
  const byPath = Object.fromEntries(result.artifacts.map((entry) => [entry.path, entry]));
  ...
  assert.deepEqual(
    result.topology.services.map(({ id }) => id),
    ["service@web"],
  );
  const statuses = result.topology.diagnostics.map(({ path, status }) => [path, status]);
  assert.ok(
    statuses.some(([path, status]) => path === "bad-compose.yaml" && status === "malformed"),
  );
...
test("T213 repository-level caps truncate deterministically and are disclosed", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: Object.keys(files).slice(0, 40),
    options,
  });
  const { topology } = result;
...
test("T213 unreadable artifacts are reflected in the search space", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["Dockerfile", "gone.yaml"]),
  });
...
test("T213 searchSpace: unsupported candidates fold supported=false and gate complete, peers survive", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["notes.yaml", "Dockerfile"]),
  });
...
test("T213 mixed supported/unsupported manifests keep the deployment claim non-observed (F-020)", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["compose.yaml", "k8s/notes.yaml"]),
  });

  assert.equal(
    result.topology.searchSpace.supported,
    false,
    "NO_EXTRACTOR peer folds supported=false",
  );
...
test("T213 searchSpace: malformed and capped extraction outcomes fold while peers survive", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["bad-compose.yaml", "huge.yaml", "Dockerfile"]),
  });
...
test("T213 provider: DIM-deployment categories via T210 base, immutable and deterministic", async (t) => {
  ...
  const result = await scanDeploymentTopology({
    root,
    requests: requests(["compose.yaml", "Dockerfile", "main.tf"]),
  });
  ...
  const indicator = providerResult.observations.find(
    ({ category }) => category === "template_indicator",
  );
...
test("T213 provider: invalid topology inputs fail with typed errors", () => {
  ...
  assert.throws(
    () => deploymentProviderResults({ topology: [1, 2] }),
...
test("T213 inertness: deployment modules never touch fs, child_process, or execution surfaces", async () => {
  ...
  const providerSource = await readFile(
    join(LIB_ROOT, "scan", "providers", "deployment.mjs"),
    "utf8",
  );
...
test("T213 model constants are exact, frozen, and aligned with the DIM-deployment contract", () => {
  ...
  assert.equal(
    PROVIDER_CATEGORIES["DIM-deployment-v1"].join(","),
