...
test('T222 dimensions: registry validates through the contract and holds every dimension exactly once in canonical order', () => {
  ...
  assert.deepEqual(DIMENSION_REGISTRY.map(({ id }) => id), DIMENSION_IDS);
  assert.deepEqual(
...
test('T222 dimensions: every expected claim ID is stable, pattern-valid, prefixed by its dimension, and globally unique', () => {
  ...
  for (const dimension of DIMENSION_REGISTRY) {
    ...
    for (const claimId of dimension.expectedClaimIds) {
      ...
      assert.ok(claimId.startsWith(`CLM-${short}-`), `${claimId} does not start with CLM-${short}-`);
...
test('T222 dimensions: every DIMENSION_EVIDENCE_CATEGORIES entry has a corresponding claim topic (set inclusion)', () => {
  ...
  for (const topics of Object.values(CATEGORY_TOPIC_COVERAGE)) {
    for (const topic of topics) {
      assert.ok(allTopics.has(topic), `coverage table names topic ${topic}, but no claim registers it`);
    }
  }
  ...
  for (const dimension of DIMENSION_REGISTRY) {
    ...
    const uncovered = DIMENSION_EVIDENCE_CATEGORIES[dimension.id]
      .filter((category) => !CATEGORY_TOPIC_COVERAGE[category].some((topic) => topics.has(topic)));
  ...
  for (const dimension of DIMENSION_REGISTRY) {
    const reachable = new Set(
      DIMENSION_EVIDENCE_CATEGORIES[dimension.id].flatMap((category) => CATEGORY_TOPIC_COVERAGE[category]),
    );
...
test('T222 dimensions: duplicate and unknown failure cases fail with typed contract errors', () => {
  ...
  assert.throws(() => validateDimensions(fifteen),
  ...
  assert.throws(() => validateDimensions(swapped),
  ...
  assert.throws(() => validateDimensions(unknown),
  ...
  assert.throws(() => validateDimensions(duplicatedClaim),
  ...
  assert.throws(() => validateDimensions(badFlag),
  ...
  assert.throws(() => validateDimensions(badRenderer),
  ...
  assert.throws(() => validateDimensions(badRetry),
...
test('T222 dimensions: repeated evaluation is byte-identical and snapshots are deep-frozen', () => {
  ...
  assert.equal(JSON.stringify(validateDimensions(clone(DIMENSION_DEFINITIONS))), JSON.stringify(DIMENSION_REGISTRY));

  assert.throws(() => DIMENSION_REGISTRY.pop(), TypeError);
  assert.throws(() => { DIMENSION_REGISTRY[0].id = 'DIM-mutated-v1'; }, TypeError);
  assert.throws(() => { DIMENSION_REGISTRY[0].retryable = false; }, TypeError);
  assert.throws(() => { DIMENSION_REGISTRY[0].rendererId = 'RND-mutated-v1'; }, TypeError);
  assert.throws(() => { DIMENSION_REGISTRY[0].expectedClaimIds.push('CLM-mutated-v1'); }, TypeError);
  assert.throws(() => { DIMENSION_REGISTRY[0].applicability.rules[0].value = 'mutated'; }, TypeError);
...
test('T222 providers: all 15 provider dimensions are represented, each with one primary builtin provider', () => {
  ...
  for (const dimensionId of PROVIDER_DIMENSION_IDS) {
    ...
    assert.ok(provider.dimensions.some(({ dimensionId: dim }) => dim === dimensionId),
      `${providerId} actually covers ${dimensionId}`);
    assert.notEqual(providerId, GENERIC_BUILTIN_PROVIDER_ID, `primary for ${dimensionId} is not the generic fallback`);
  }
...
test('T222 providers: index mirrors the three catalogs and the generic fallback with globally unique IDs', () => {
  ...
  for (const provider of RUNTIME_CATALOG_PROVIDERS) {
    assert.deepEqual(indexById.get(provider.id), provider, `runtime provider ${provider.id} mirrored`);
  }
  ...
  for (const provider of ASSURANCE_CATALOG_PROVIDERS) {
    assert.deepEqual(indexById.get(provider.id), provider, `assurance provider ${provider.id} mirrored`);
  }
  ...
  assert.deepEqual(
    indexById.get(GENERIC_BUILTIN_PROVIDER_ID).dimensions.map(({ dimensionId, categories }) => ({ dimensionId, categories })),
    GENERIC_BUILTIN_DIMENSIONS,
  );
...
test('T222 providers: duplicate and unknown failure cases fail with typed contract errors', () => {
  ...
  assert.throws(() => validateProviders(duplicated),
  ...
  assert.throws(() => validateProviders(unknown),
  ...
  assert.throws(() => validateProviders(badCategory),
  ...
  empty[0].dimensions = [];
  assert.throws(() => validateProviders(empty),
  ...
  duplicateCategory[0].dimensions[0].categories = duplicateCategory[0].dimensions[0].categories.slice(0, 2);
  duplicateCategory[0].dimensions[0].categories.push(duplicateCategory[0].dimensions[0].categories[0]);
  assert.throws(() => validateProviders(duplicateCategory),
    (error) => error instanceof ProviderContractError && error.code === 'DUPLICATE_ID');
...
test('T222 providers: repeated evaluation is byte-identical and the index is deep-frozen', () => {
  ...
  assert.equal(JSON.stringify(validateProviders(clone(BUILTIN_PROVIDER_INDEX))), JSON.stringify(BUILTIN_PROVIDER_INDEX));
  assert.ok(again.every(({ id }, index) => id === BUILTIN_PROVIDER_INDEX[index].id), 'index stays deterministically sorted');

  assert.throws(() => BUILTIN_PROVIDER_INDEX.pop(), TypeError);
  assert.throws(() => { BUILTIN_PROVIDER_INDEX[0].id = 'PRV-mutated-v1'; }, TypeError);
  assert.throws(() => { BUILTIN_PROVIDER_INDEX[0].dimensions.pop(); }, TypeError);
  assert.throws(() => { BUILTIN_PROVIDER_INDEX[0].dimensions[0].categories.push('mutated'); }, TypeError);
  assert.throws(() => BUILTIN_PROVIDER_IDS.push('PRV-mutated-v1'), TypeError);
  assert.throws(() => { BUILTIN_DIMENSION_TO_PROVIDER['DIM-stack-v1'] = 'PRV-mutated-v1'; }, TypeError);
});
...
test('T222 inert: no production module imports the registry or the builtin index', async () => {
  ...
  assert.deepEqual(consumers, [], 'only the activated pipeline/enrich consumers may use the registration modules');
});
...
test('T222 inert: the builtin index does not import the catalogs, and the production renderer/write path is untouched', async () => {
  const indexSource = await readFile(join(LIB_ROOT, 'scan', 'providers', 'builtin', 'index.mjs'), 'utf8');
  const importSpecifiers = [...indexSource.matchAll(/^\s*import\s+(?:[^'"]*?\s+from\s+)?['"]([^'"]+)['"]/gm)]
    .map((match) => match[1]);
  for (const catalog of ['runtime-catalog', 'analysis-catalog', 'assurance-catalog', 'generic.mjs']) {
    assert.ok(!importSpecifiers.some((specifier) => specifier.includes(catalog)),
  ...
  for (const file of [
    ...
    assert.equal(source.includes('registry/dimensions'), false, `${file} must not reference the dimension registry`);
    assert.equal(source.includes('builtin/index'), false, `${file} must not reference the builtin index`);
    assert.equal(source.includes('CROSS_REPO_GLOBAL_STAGE'), false, `${file} must not reference the global stage`);
    assert.equal(source.includes('DIMENSION_REGISTRY'), false, `${file} must not reference DIMENSION_REGISTRY`);
  }
});
