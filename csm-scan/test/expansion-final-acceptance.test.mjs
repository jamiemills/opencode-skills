...
function assertNeutralText(label, value) {
  const banned =
    /\b(?:should|must|ought|shall|poor|good|bad|weak|strong|better|worse|best|worst|recommended|recommendation|ideally|unfortunately|concern|concerning|problem|anti-pattern|smell|suboptimal|inadequate|insufficient|contradiction|contradictions|inconsistent|inconsistency|conflict|conflicts|lacking)\b/gi;
  const hits = [];
  const prose = value
    .replace(/^(?:```|~~~)[^\n]*\n[\s\S]*?^(?:```|~~~)[ \t]*$/gm, (m) => m.replace(/[^\n]/g, " "))
    .replace(/(`+)[^\n]*?\1/g, (m) => m.replace(/[^\n]/g, " "))
...
function isRegisteredBrokerShape(call) {
  ...
  if (call.executable === "rg") {
    ...
    if (argv[0] === "--json") {
      ...
      return (
        JSON.stringify(prefix) === JSON.stringify(globs) &&
        tail.length === 2 &&
        tail[0] === "--" &&
...
test("T228 AC1 command boundary: sole broker, registered rg/git argv, zero target execution", async (t) => {
  ...
  const gitRepo = makeGitRepo({
    files: {
      "README.md": "AC1 command boundary\n",
  ...
  assert.equal(
    ...
    1,
    "the broker must remain the sole node:child_process owner",
  );
...
test("T228 AC2-AC9/AC15: the python fixture reports declaration-backed facts for all 17 dimensions with statuses and coverage", async (t) => {
  ...
  assert.ok(
    Array.isArray(stack.runtimeDeclarations) && stack.runtimeDeclarations.length >= 2,
    "runtime declarations must carry per-source evidence",
  );
  for (const declaration of stack.runtimeDeclarations) {
    assert.ok(
      typeof declaration.source === "string" && declaration.source.includes("#"),
  ...
  assert.doesNotMatch(
    ...
    /\bactual runtime\b/i,
    "no actual-runtime verdict is emitted",
  );
  ...
  assert.ok(
    data.edges.every((edge) => edge.kind === "foreign_key"),
    "no edge may come from name-only relations",
  );
  assert.ok(
  ...
  assert.ok(
    deployment.counts && deployment.counts.artifacts >= 2,
    "declared artifacts are counted",
  );

  // AC7 — Maintainability: disclosed measurement universe, no scores.
  const maintainability = findingsFor(result, "maintainability");
  assert.ok(
    Number.isSafeInteger(maintainability.summary.filesMeasured) &&
      maintainability.summary.filesMeasured >= 8,
  );
  ...
  assert.ok(
    governance.ownership.assignees.length > 0,
    "CODEOWNERS identities become assignee records",
  );
  for (const assignee of governance.ownership.assignees) {
    assert.match(
      assignee.label,
      /^Owner-\d{3}$/,
      "ownership identities must be report-local opaque tokens",
    );
    assert.ok(assignee.label !== "@platform", "raw owner handles never render");
  }
  assert.doesNotMatch(
    JSON.stringify(governance),
    /@platform|alice\.smith/,
    "no raw identity leaks into governance findings",
  );
  assert.ok(
    governance.searchSpace.complete,
    "governance absence/evidence rests on a complete search",
  );
  ...
  assert.doesNotMatch(
    ...
    /pass|fail|compliant|vulnerab/i,
  ...
  assert.ok(
    result.markdown.includes("## Development Practices"),
  ...
  assert.equal(
    ...
    true,
    "practices presence rests on a complete search",
  );
  assert.equal(
    perDimensionStatus(result, "practices"),
    "observed",
  ...
  assert.equal(
    coverage.excluded,
    2,
    "the non-git fixture excludes the git dimension as not_applicable",
  );
  ...
  assert.deepEqual(
    Object.keys(per),
    ALL_SEVENTEEN,
    "per-dimension coverage follows canonical order",
  );
  ...
  for (const dimension of ALL_SEVENTEEN) {
    assert.ok(
      CLAIM_STATUSES.includes(per[dimension].status),
      `${dimension}: ${per[dimension].status} is a registered status`,
    );
  }
  ...
  assert.match(
    evidence.id,
    /^EVD-v1-[a-f0-9]{64}$/,
    "evidence receives a deterministic canonical identity",
  );
  assert.equal(
    evidence.id,
    createEvidence({
...
test("T228 AC16: the unsupported status is accepted only with a clean unsupported search space", () => {
  ...
  const validated = validateClaim(
    {
      ...
      derivationId: null,
      limitations: ["format is unsupported"],
      searchSpace,
    },
    [evidence],
    api,
  ...
  assert.equal(
    validated.coverageState,
    "unsupported",
    "unsupported is excluded from the complete/eligible coverage set",
  );
});
...
test("T228 AC5: deployment anchors and block scalars are diagnostics, valid peers survive, nothing is fabricated", async (t) => {
  ...
  assert.ok(
    deployment.diagnostics.some(({ status }) => status === "malformed" || status === "unsupported"),
    "anchor/block-scalar documents are recorded as diagnostics",
  );
  assert.ok(
    deployment.diagnostics.every(
      ({ status }) => status === "malformed" || status === "unsupported",
    ),
...
test("T228 AC6: architecture reports dynamic indicators, raw fan-in/fan-out, and Tarjan SCC facts without verdicts", async (t) => {
  ...
  assert.ok(
    Object.keys(facts.fanIn).length > 0 && Object.keys(facts.fanOut).length > 0,
  ...
  assert.ok(
    facts.dynamicIndicators.some(({ kind }) => kind === "dynamic-import"),
  ...
  for (const key of ["bounds", "universe", "edgeKindCounts", "selfLoops"]) {
    assert.doesNotMatch(
      ...
      /\b(?:hub|quality|coupling|critical)\b/i,
      `${key} must stay raw and judgment-free`,
    );
  }

  // The extension renderer turns facts into neutral prose.
  const rendered = createArchitectureExtensionRenderer().render("repository", {
    graphFacts: facts,
  });
...
test("T228 AC10: standards registry is versioned, source-linked, metadata-only, with no copied control text", () => {
  ...
  for (const entry of STANDARDS_REGISTRY) {
    assert.equal(
      entry.disposition,
      "metadata_only",
      `${entry.id}: disposition must be metadata_only`,
    );
    assert.match(
      entry.id,
      /^std:[a-z0-9]+(?:-[a-z0-9]+)*:[a-z0-9]+(?:[._-][a-z0-9]+)*$/,
      `${entry.id}: stable versioned id`,
    );
    ...
    assert.ok(
      !Object.keys(entry).includes("controlText") && !Object.keys(entry).includes("summary"),
  ...
  assert.throws(
    ...
    /metadata_only/,
    "authored_mapping without a proven reuse decision is rejected",
  );
  assert.throws(
    () =>
      validateStandardEntry({
        ...VALID,
        id: "std:test-std:1.0.0-latest",
        editionKey: "1.0.0-latest",
        edition: "1.0.0 latest",
      }),
    /floating marker/,
    "floating edition markers are rejected",
  );
  assert.throws(
    () =>
      validateStandardEntry({ ...VALID, officialUri: "https://user:pass@example.test/standard" }),
    /credentials/,
    "authoritative URIs must not carry credentials",
  );
  assert.throws(
    () => validateStandardEntry({ ...VALID, extra: "prose" }),
    /schema/,
    "unknown fields (including any copied control text) are rejected",
  );
  assert.throws(
    () => validateStandardsRegistry([VALID, { ...VALID, id: "std:test-std:1.0.0" }]),
    /duplicate id/,
    "duplicate standard ids are rejected",
  );
...
test("T228 AC11: ambiguity is retained as records and never becomes an edge", async (t) => {
  ...
  assert.equal(
    full.global.metrics.crossRepositoryEdges,
    0,
...
function acceptancePlugin() {
  const dimensions = PLUGIN_BLUEPRINTS.map(([dimensionId, category]) => ({
    dimensionId,
    categories: [category],
  }));
...
test("T228 AC12: a declarative plugin contributes bounded evidence to all 15 provider dimensions with no evaluation", async (t) => {
  ...
  assert.deepEqual(
    new Set(validated[0].providers[0].dimensions.map((d) => d.dimensionId)),
    new Set(PROVIDER_DIMENSION_IDS),
    "the plugin declares exactly the 15 provider dimensions",
  );
  assert.throws(
    () => validatePlugins([{ ...plugin, hooks: ["node:child_process"] }]),
    (error) => error instanceof PluginSchemaError,
    "executable hooks are rejected by the schema",
  );
  ...
  for (const dimensionId of PROVIDER_DIMENSION_IDS) {
    ...
    assert.ok(
      first.markdown.includes(`RUL-accept-${short}-v1`),
      `${dimensionId}: NORMS.md must contain plugin-labeled evidence`,
    );
  }
  assert.ok(
    first.markdown.includes("PRV-fxlang-v1"),
    "the plugin provider id renders as provenance",
  );
  assert.ok(first.markdown.includes("### Provider Evidence"), "provider evidence sections render");
  assert.ok(
    first.markdown.includes("fixturelang") === false,
    "the plugin token is fxlang, not fixturelang",
  );

  // No plugin code is evaluated: the plugin registry contains only data.
  assert.deepEqual(Object.keys(loaded).toSorted(), [
    "aliases",
    "apiVersion",
    "id",
    "label",
...
test("T228 AC13: the unknown-language fixture receives generic artifact-only evidence with no core source knowledge", async (t) => {
  ...
  assert.ok(
    result.markdown.includes(GENERIC_PROVIDER_ID),
    "the generic artifact fallback renders in NORMS.md",
  );
  assert.ok(
    result.markdown.includes("### Provider Evidence"),
    "generic evidence renders a provider section",
  );
  ...
  assert.ok(
    ...
    "generic file_metric observations are merged into maintainability",
  );
  assert.ok(
    observations.every(({ providerId }) => providerId === GENERIC_PROVIDER_ID),
  ...
  for (const dimension of ["api", "data", "deployment", "maintainability", "governance"]) {
    assert.equal(
      perDimensionStatus(result, dimension),
      "not_detected",
      `${dimension}: the generic fallback never claims first-class semantics`,
    );
  }
  assert.equal(
    perDimensionStatus(result, "assurance"),
    "observed",
    "assurance is observed from artifact presence only",
  );
  assert.equal(
    findingsFor(result, "assurance").manifest.length,
    1,
    "go.mod is inventoried as an artifact",
  );
  assert.equal(
    findingsFor(result, "assurance").lock.length,
    1,
    "go.sum is inventoried as an artifact",
  );
...
test("T228 AC14: the five built-in ecosystems preserve their established facts through the canonical pipeline", async (t) => {
  for (const [name, files, language, ecosystem] of FIVE_FIXTURES) {
    ...
    const overview = result.repos[0].overview;
    assert.ok(
      overview.languages.includes(language),
      `${name}: detected language must include ${language}`,
    );
    assert.equal(
      overview.ecosystems.primary,
      ecosystem,
      `${name}: primary ecosystem must be ${ecosystem}`,
    );
    assert.equal(
      findingsFor(result, "stack").language,
      language,
      `${name}: stack language must be ${language}`,
    );
...
test("T228 AC14/AC20: the 21-case P0 matrix and five fixture pipelines are retained as executable gates", async () => {
  ...
  assert.deepEqual(
    collected.failures,
    [],
    `registered gate tests failed: ${collected.failures.join(", ")}`,
  );
  assert.deepEqual(
    collected.skips,
    [],
    `registered gate tests skipped: ${collected.skips.join(", ")}`,
  );
  const p0TestNames = collected.names.filter((name) => name.startsWith("P0-"));
  assert.deepEqual(
    p0TestNames,
    inventory.p0TestNames,
    "the 21-case P0 regression matrix is retained",
  );
...
test("T228 AC17: structured findings, global snapshot, rendered Markdown, and reporter diagnostics carry zero canaries", async (t) => {
  ...
  const trigger =
    "contact alice.smith@example.test read /etc/privacy/path.conf api_token=privacy-super-secret-token-77";
  assert.throws(
  ...
  assertZeroLeaks(
    "formatError output",
    formatError(new Error(`scan failed at ${CANARIES[1]} for ${CANARIES[2]}`)),
  );
});
...
test("T228 AC18: fixed clock, repeated runs, insertion-order permutations, and repository reversal are byte-identical", async (t) => {
  ...
  for (const order of orders) {
    ...
    markdowns.push(
      (await runExpandedPipeline({ repos: [perm], clock: FIXED_CLOCK, sink: () => "" })).markdown,
    );
  }
  ...
  const forward = await runExpandedPipeline({
    repos: [a, b],
    out: join(outDir, "fwd.md"),
    clock: FIXED_CLOCK,
  });
  const reversed = await runExpandedPipeline({
    repos: [b, a],
    out: join(outDir, "rev.md"),
    clock: FIXED_CLOCK,
  });
  assert.equal(
...
test("T228 AC19: invalid states fail before the sole write with sanitized typed errors", async (t) => {
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.ok(
        ...
        "the pipeline error must not echo the missing repository path",
      );
      return true;
    },
  );
  ...
  await assert.rejects(
    runExpandedPipeline({
      ...
      sink: (findings2, out2, renderer) => {
        pluginSinkCalls++;
        return writeNORMS(findings2, out2, renderer);
      },
    }),
  ...
  assert.ok(
    privacyAt !== -1 && sinkAt !== -1,
    "run.mjs must contain both the privacy gate and the sink call",
  );
...
test("T228 AC20: every named gate executes green — zero failures and zero skips per the node:test machine summary", async () => {
  ...
  assert.notEqual(
    summary.tests,
    null,
    `the named-gate corpus must produce a TAP machine summary\n${tail}`,
  );
  ...
  assert.equal(
    summary.skipped,
    0,
    `named-gate corpus reported ${summary.skipped} skip(s) — runtime t.skip() is banned in named gates\n${tail}`,
  );
...
test("T228 optional real-repo smoke: canonical pipeline to a /tmp output only, never target commands (CSM_REAL_REPO_SMOKE=1)", async (t) => {
  ...
  if (!realRepo || !existsSync(realRepo)) {
    console.warn(
      `[T228] skipping real-repo smoke — set CSM_REAL_REPO_SMOKE=<repo> to enable (${realRepo ?? "unset"})`,
    );
    return;
  }
  ...
  assert.equal(
    ...
    result.markdown,
    "the written Markdown equals the returned Markdown",
  );
  ...
  for (const call of calls) {
    assert.ok(
      ["rg", "git"].includes(call.executable),
      `only rg/git may run; saw ${call.executable}`,
    );
    assert.equal(call.shell, false, "shell mode must always be disabled");
    assert.ok(
      isRegisteredBrokerShape(call),
      `argv ${call.executable} ${call.argv.join(" ")} must be registered`,
    );
  }
  assert.equal(
    calls.filter((call) => TARGET_EXECUTABLES.has(call.executable)).length,
    0,
