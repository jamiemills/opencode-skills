...
import {
  ...
  renderGovernance,
} from '../lib/scan/render/governance.mjs';
...
test('T215 model: category and status snapshots are exact, frozen, and allowlisted', () => {
  assert.deepEqual(GOVERNANCE_CATEGORIES, [
    'contribution', 'decision', 'funding', 'ownership', 'policy', 'reference',
    'release', 'review', 'runbook', 'support',
  ]);
...
test('T215 model: invalid artifacts, statuses, categories, and paths fail with typed errors', () => {
  assert.throws(() => modelOf([{ ...policyArtifact('a.md'), category: 'language' }]),
    (error) => error instanceof GovernanceModelError && error.code === 'UNKNOWN_CATEGORY');
  assert.throws(() => modelOf([{ ...policyArtifact('a.md'), category: 'ownership' }]),
    GovernanceModelError, 'ownership entries must come from parsed CODEOWNERS');
  assert.throws(() => modelOf([{ ...policyArtifact('a.md'), status: 'observed-evil' }]),
    GovernanceModelError);
  assert.throws(() => modelOf([{ ...policyArtifact('a.md'), path: '/etc/passwd' }]),
    GovernanceModelError);
  assert.throws(() => modelOf([{ ...policyArtifact('a.md'), path: '../escape' }]),
    GovernanceModelError);
  assert.throws(() => modelOf([{ ...policyArtifact('a.md'), dialect: 'nonsense' }]),
    GovernanceModelError);
  assert.throws(() => modelOf([policyArtifact('a.md')], [], { isGit: 'yes' }),
    GovernanceModelError);
});
...
test('T215 model: privacy violations downgrade to unverified PRIVACY diagnostics', () => {
  ...
  assert.deepEqual(model.entries.map(({ category }) => category), ['policy']);
...
test('T215 model: a multi-word ADR status downgrades to a diagnostic without crashing peers', () => {
  ...
  const model = modelOf(artifacts);
  assert.ok(model.entries.some(({ path }) => path === 'CODE_OF_CONDUCT.md'),
    'a valid peer artifact survives the downgraded ADR');
  assert.deepEqual(model.diagnostics.map(({ path, status, reason }) => ({ path, status, reason })), [
    { path: 'doc/adr/0001-review.md', status: 'unverified', reason: 'MALFORMED' },
  ]);
  const serialized = JSON.stringify(model);
  assert.equal(serialized.includes('Under Review'), false, 'the multi-word status never reaches the model');
});
...
test('T215 model: encodeMatchedKey percent-encodes every non-token character', () => {
  ...
  assert.equal(encodeMatchedKey('reference:a.md:1:https://example.test/x%20y'), 'reference:a.md:1:https://example.test/x%20y');
...
test('T215 codeowners: comments, blanks, and valid rules parse with last-match precedence', () => {
  ...
  assert.deepEqual(parsed.rules[0], { pattern: '*', anchored: false, owners: ['@global-owner'], line: 3 });
  assert.deepEqual(parsed.rules[1], { pattern: 'docs/', anchored: true, owners: ['@docs-team'], line: 4 });
  ...
  assert.deepEqual(resolveOwners('docs/x/y.md', parsed.rules), ['@docs-team'],
...
test('T215 codeowners: last-match semantics honour file order', () => {
  ...
  assert.deepEqual(resolveOwners('src/app.js', reordered.rules), ['@global'],
...
test('T215 codeowners: pattern matcher supports the documented subset', () => {
  ...
  assert.equal(patternMatches('docs/*', 'a/docs/readme.md'), true);
  assert.equal(patternMatches('docs/*', 'docs/sub/readme.md'), false, 'single-star matches one segment');
...
test('T215 model: raw owners become opaque report-local labels with aggregate counts', () => {
  ...
  const model = modelOf([], [{ path: '.github/CODEOWNERS', rules: parsed.rules, diagnostics: parsed.diagnostics, malformedLines: 0 }]);
  ...
  assert.deepEqual(model.ownership.assignees.map(({ label }) => label), ['Owner-001', 'Owner-002', 'Owner-003']);
  const serialized = JSON.stringify(model);
...
test('T215 model: multiple CODEOWNERS files share one deterministic label universe', () => {
  ...
  const model = modelOf([], [
    { path: '.github/CODEOWNERS', rules: first.rules, diagnostics: first.diagnostics, malformedLines: 0 },
    { path: 'docs/CODEOWNERS', rules: second.rules, diagnostics: second.diagnostics, malformedLines: 0 },
  ]);
  assert.equal(model.ownership.files, 2);
  assert.equal(model.summary.assigneeCount, 2);
  assert.deepEqual(model.entries.filter(({ category }) => category === 'ownership').map(({ path }) => path),
...
test('T215 model: caps truncate deterministically and are disclosed', () => {
  ...
  const parsed = parseCodeowners(
    Array.from({ length: GOVERNANCE_LIMITS.maxRules + 20 }, (_, index) => `p${String(index).padStart(4, '0')} @u`).join('\n'),
    'CODEOWNERS',
  );
  const cappedRules = modelOf([], [{ path: 'CODEOWNERS', rules: parsed.rules, diagnostics: parsed.diagnostics, malformedLines: 0 }]);
...
test('T215 classify: known artifacts, ADR directories, runbooks, and others', () => {
  assert.deepEqual(classifyGovernancePath('.github/CODEOWNERS'), { category: 'ownership', dialect: 'codeowners', parse: 'codeowners' });
  assert.deepEqual(classifyGovernancePath('CODEOWNERS'), { category: 'ownership', dialect: 'codeowners', parse: 'codeowners' });
  assert.deepEqual(classifyGovernancePath('docs/CODEOWNERS'), { category: 'ownership', dialect: 'codeowners', parse: 'codeowners' });
  assert.deepEqual(classifyGovernancePath('.github/FUNDING.yml'), { category: 'funding', dialect: 'funding', parse: null });
  assert.deepEqual(classifyGovernancePath('CODE_OF_CONDUCT.md'), { category: 'policy', dialect: 'code-of-conduct', parse: 'links' });
  assert.deepEqual(classifyGovernancePath('.github/PULL_REQUEST_TEMPLATE.md'), { category: 'review', dialect: 'pr-template', parse: null });
  assert.deepEqual(classifyGovernancePath('doc/adr/0001-x.md'), { category: 'decision', dialect: 'adr', parse: 'adr' });
  assert.deepEqual(classifyGovernancePath('docs/architecture/decisions/0002-y.md'), { category: 'decision', dialect: 'adr', parse: 'adr' });
  assert.deepEqual(classifyGovernancePath('decisions/0003-z.md'), { category: 'decision', dialect: 'adr', parse: 'adr' });
  assert.deepEqual(classifyGovernancePath('runbook.md'), { category: 'runbook', dialect: 'runbook', parse: 'links' });
  assert.deepEqual(classifyGovernancePath('docs/runbooks/ops.md'), { category: 'runbook', dialect: 'runbook', parse: 'links' });
...
test('T215 links: explicit markdown and autolink URLs are extracted and sanitized', () => {
  ...
  assert.equal(JSON.stringify(links).includes('user:secret'), false, 'credentials never survive sanitization');
});
...
test('T215 provider: emits only DIM-governance categories via the provider foundation', () => {
  ...
  const model = modelOf([policyArtifact('CODE_OF_CONDUCT.md', 'code-of-conduct')],
    [{ path: '.github/CODEOWNERS', rules: parsed.rules, diagnostics: parsed.diagnostics, malformedLines: 0 }]);
...
test('T215 provider: observations carry opaque labels and encoded matched keys only', () => {
  ...
  assert.equal(serialized.includes('alice'), false);
  assert.ok(observations.some(({ matchedKey }) => matchedKey.startsWith('rule:.github/CODEOWNERS:%2A.js')),
...
test('T215 provider: over-long links and patterns stay within the foundation key bound', () => {
  ...
  const rules = [{
    pattern: 'b'.repeat(200), anchored: false, owners: ['@alice'], line: 1,
  }];
  const model = modelOf([
    {
      category: 'reference',
  ...
  assert.ok(results[0].observations.some(({ matchedKey }) => /^rule:[a-f0-9]{8}$/.test(matchedKey)),
    'an over-long pattern falls back to a stable short hash');
  assert.ok(results[0].observations.some(({ matchedKey }) => /^reference:[a-f0-9]{8}$/.test(matchedKey)),
    'an over-long link falls back to a stable short hash');
  assert.ok(results[0].observations.some(({ category }) => category === 'policy'),
...
test('T215 model: a >512-char link URL (STRING_LIMIT) downgrades to a diagnostic without crashing peers', () => {
  ...
  assert.equal(JSON.stringify(model).includes(longUrl), false, 'the over-long URL never reaches the model');
  assert.equal(governanceProviderResult(model).length, 1, 'the provider result is never invalidated');
});
...
test('T215 provider: a ~489-char link URL stays within the foundation key bound', () => {
  ...
  assert.match(reference.matchedKey, /^reference:[a-f0-9]{8}$/,
  ...
  assert.equal(JSON.stringify(results).includes(longUrl), false, 'the raw URL is never duplicated into the provider key');
});
...
test('T215 renderer: empty model renders a factual no-detected line and disclosed caps', () => {
  ...
  assert.match(markdown, /No governance or ownership artifacts detected in 3 inspected file\(s\)\./);
  assert.deepEqual(findVoiceHits(markdown), []);

  const cappedModel = modelOf(Array.from({ length: GOVERNANCE_LIMITS.maxEntries + 1 }, (_, index) => policyArtifact(`p${index}.md`)));
...
test('T215 renderer: no-detected line requires a clean search space; otherwise only diagnostics', () => {
  const incomplete = modelOf([], [], {
    searchSpace: {
      supported: true, readable: false, complete: false, capped: false,
      error: true, malformed: false, ambiguous: false,
      filesInspected: 2, fileLimit: 100, bytesInspected: 100, byteLimit: 10_000,
      recordsInspected: 1, recordLimit: 1_000, omittedCount: 0,
    },
    diagnostics: [{ path: 'CODE_OF_CONDUCT.md', line: null, status: 'unverified', reason: 'UNREADABLE' }],
  });
  const markdown = createGovernanceRenderer().render(incomplete);
  assert.equal(markdown.includes('No governance or ownership artifacts detected'), false,
...
test('T215 inertness: governance renderer is never registered in the existing-ten map', async () => {
  ...
  assert.equal(existingTen.includes('render/governance.mjs'), false, 'existing-ten must not import the governance renderer');
  const write = await readFile(join(LIB_ROOT, 'scan', 'write.mjs'), 'utf8');
  assert.equal(write.includes('render/governance.mjs'), false, 'write must not import the governance renderer');
  assert.equal(write.includes('providers/governance.mjs'), false, 'write must not import the governance provider');
});
...
test('T215 scanner: an ADR with a multi-word status never crashes and peers survive', async () => {
  ...
  await withFixture('gov-adr-multiword', files, async (dir) => {
    ...
    assert.ok(findings.entries.some(({ path }) => path === 'CODE_OF_CONDUCT.md'),
...
test('T215 scanner: credential URLs and personal data never reach the model, provider, or markdown', async () => {
  const files = {
    '.github/CODEOWNERS': [
      '* @alice-smith',
      '*.py alice@example.com',
      '',
    ].join('\n'),
    'CODE_OF_CONDUCT.md': 'See [Alice Smith](https://example.test/conduct).\n',
    'CONTRIBUTING.md': 'Please [contribute](https://user:secret@example.test/api).\nContact security@example.com.\n',
  ...
  await withFixture('gov-privacy', files, async (dir) => {
    ...
    for (const canary of ['alice-smith', 'alice@example.com', 'user:secret', 'Alice Smith', 'security@example.com', '@']) {
    ...
    assert.deepEqual(findings.entries.filter(({ category }) => category === 'contribution').map(({ path }) => path),
      ['CONTRIBUTING.md'], 'a contribution guide without links is still inventoried');
  });
});
...
test('T215 scanner: per-file link cap yields a CAP diagnostic and bounded entries', async () => {
  const links = Array.from({ length: GOVERNANCE_LIMITS.maxLinksPerFile + 10 }, (_, index) => (
    `[doc ${index}](https://example.test/${index})`
  ));
...
test('T215 scanner: git usage is broker-only and commit identities never infer ownership', async () => {
  const dir = makeGitRepo({
    commits: [
      ...
      { message: 'fix: second', files: { 'b.txt': 'y' }, user: 'Bob Other', email: 'bob@example.com' },
    ],
  ...
  try {
    ...
    const broker = createCommandBroker({
      runner: {
        async run(executable, argv, options) {
          ...
          return (await import('../lib/scan/shared/command.mjs')).defaultRunner.run(executable, argv, options);
        },
      },
    });
    ...
    assert.equal(calls.some((call) => call.argv.includes('log')), false, 'no commit history may be requested');
    assert.equal(calls.some((call) => call.argv.includes('shortlog')), false, 'no contributor data may be requested');
    const modelJson = JSON.stringify(findings);
...
test('T215 scanner: a 519-char link URL never aborts the scan and peers survive', async () => {
  ...
  await withFixture('gov-longurl-519', files, async (dir) => {
    ...
    assert.equal(findings.entries.some(({ category }) => category === 'reference'), false,
