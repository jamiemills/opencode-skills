...
import {
  DUPLICATE_WINDOW,
  findDuplicateGroups,
} from "../lib/scan/deep/maintainability/duplicates.mjs";
import {
  DEAD_CODE_KINDS,
...
test("T214 tokenizer: constants are exact and frozen", () => {
  ...
  assert.deepEqual(BRANCH_CATEGORIES, [
    "if",
    "else",
    "switch",
    "case",
...
test("T214 tokenizer: hand-calculated branch counts per dialect", () => {
  const cases = [
    ...
    [
      "rust",
      "fn f(x: Option<i32>) -> i32 { match x { Some(v) if v > 0 => v, None => 0, } }",
      { if: 1, match: 1 },
    ],
    [
      "shell",
...
test("T214 tokenizer: shell heredocs, parameter expansions, and $( ) are handled", () => {
  ...
  assert.equal(
    countBranchPoints(sub.tokens, "shell").if,
    1,
    "command substitution content is counted",
  );
});
...
test("T214 tokenizer: js regex literals are skipped so branch keywords inside them do not count", () => {
  ...
  assert.ok(
    division.tokens.filter(({ value }) => value === "/").length >= 2,
...
test("T214 tokenizer: per-function scoping has no brace/indent false positives", () => {
  ...
  assert.deepEqual(
    countFunctionComplexity(jsSource, "javascript", jsTokens).functions,
  ...
  assert.deepEqual(
    countFunctionComplexity(nested, "javascript", nestedTokens).functions,
    [
  ...
  assert.deepEqual(
    countFunctionComplexity(tsx, "typescript", tsxTokens).functions,
    [
  ...
  assert.deepEqual(
    countFunctionComplexity(shellSource, "shell", shellTokens).functions,
    [],
  ...
  assert.deepEqual(
    countFunctionComplexity(asi, "javascript", asiTokens).functions,
    [{ functionName: "anonymous", startLine: 5, endLine: 5, complexity: 1 }],
  ...
  assert.deepEqual(
    countFunctionComplexity(pySource, "python", pyTokens).functions,
...
test("T214 tokenizer: boolean operators count toward per-function complexity", () => {
  const cases = [
    ...
    [
      "typescript",
      "function f(x: boolean, y: boolean): number {\n  if (x && y) { return 1; }\n  if (x || y) { return 2; }\n  return 0;\n}\n",
      4,
    ],
    [
      "rust",
...
test("T214 tokenizer: countFunctionComplexity tokenizes internally and discloses stable names", () => {
  ...
  const direct = countFunctionComplexity(
    source,
    "javascript",
    tokenize(source, "javascript").tokens,
  );
  ...
  assert.equal(
    result.functions[0].functionName,
    "anonymous",
    "non-ASCII names are disclosed as anonymous",
  );
  assert.equal(Object.isFrozen(DIALECT_BOOLEAN_OPERATORS), true);
  assert.deepEqual(
    Object.keys(detectFunctionScopes([], "", "python")),
    [],
    "empty input yields no scopes",
  );
});
...
test("T214 duplicates: exact 50+ token block across two files forms one group", () => {
  ...
  assert.deepEqual(
    groups.groups[0].spans.map(({ path }) => path),
    ["a.js", "b.js"],
  );
  ...
  assert.deepEqual(groups.capped, {
    windows: false,
    groups: false,
    spans: false,
    blocks: false,
...
test("T214 duplicates: within-file duplicates are detected and merged", () => {
  ...
  assert.deepEqual(
    groups.groups[0].spans.map(({ startLine, endLine }) => [startLine, endLine]),
    [
      [1, 60],
      [61, 120],
    ],
...
test("T214 duplicates: invalid inputs fail with typed errors", () => {
  ...
  assert.throws(
    () => findDuplicateGroups([{ path: "a.js", tokens: [{ value: "x" }] }]),
...
test("T214 model: deterministic deep-frozen model with exact summary math", () => {
  ...
  const second = buildMaintainabilityModel(
    modelInput({
      measurement: { ...modelInput().measurement, filesInspected: 9 },
    }),
  );
  assert.notEqual(first, second);
  assert.equal(JSON.stringify(first), JSON.stringify(buildMaintainabilityModel(modelInput())));
  assert.equal(
...
test("T214 model: invalid records fail with typed errors", () => {
  ...
  assert.throws(
    () => buildMaintainabilityModel(badBranch),
  ...
  assert.throws(
    () => buildMaintainabilityModel(modelInput({ searchSpace: null })),
    MaintainabilityModelError,
  );
});
  ...
  const model = buildMaintainabilityModel(
    modelInput({
      measurement: {
        ...
        excludedLanguages: [
          { extension: NO_EXTENSION_LABEL, count: 3 },
    ...
    () =>
      buildMaintainabilityModel(
        modelInput({
          measurement: {
            ...modelInput().measurement,
            excludedLanguages: [{ extension: "", count: 1 }],
          },
        }),
      ),
    MaintainabilityModelError,
    "empty extensions stay invalid outside the scanner",
  );
});
...
test("T214 model: sanitized other sentinel is accepted; raw unsafe extensions stay invalid", () => {
  ...
  assert.deepEqual(model.measurementUniverse.excludedLanguages, [{ extension: "other", count: 3 }]);
  ...
  for (const raw of [".js~", ".日本語", `.${"a".repeat(32)}`]) {
    assert.throws(
      ...
      MaintainabilityModelError,
      `raw extension ${raw} stays invalid outside the scanner`,
    );
  }
});
...
test("T214 model: isValidExcludedExtension admits only the bounded disclosed charset", () => {
  ...
  assert.equal(
    isValidExcludedExtension("." + "a".repeat(31)),
    true,
    "32-char extension fits the bound",
  );
  assert.equal(
    isValidExcludedExtension("." + "a".repeat(32)),
    false,
    "33-char extension exceeds the bound",
  );
  assert.equal(isValidExcludedExtension(".js~"), false, "vim backup suffix is outside the charset");
  assert.equal(
    isValidExcludedExtension(".日本語"),
    false,
    "non-ASCII extension is outside the charset",
  );
  ...
  assert.equal(
    isValidExcludedExtension("no-extension"),
    false,
    "sentinels are not verbatim extensions",
  );
  assert.equal(isValidExcludedExtension(42), false);
});

test("T214 model: duplicate occurrence cap threads into the summary", () => {
  const capped = buildMaintainabilityModel(
    modelInput({
      duplicateCaps: {
        windows: false,
        groups: false,
...
test("T214 model: partial coverage is flagged when caps or omissions occur", () => {
  const partial = buildMaintainabilityModel(
    modelInput({
  ...
  const cappedFiles = buildMaintainabilityModel(
    modelInput({
...
test("T214 model: complexityDistribution computes deterministic nearest-rank percentiles", () => {
  ...
  assert.deepEqual(
    complexityDistribution([-1, 2]),
    { min: 2, median: 2, p95: 2, max: 2 },
...
test("T214 model: complexity records are normalized, frozen, and orphan-checked", () => {
  ...
  const emptyFunctions = buildMaintainabilityModel(
    modelInput({
      complexityRecords: [
        {
          ...record,
  ...
  assert.throws(
    ...
    MaintainabilityModelError,
    "non-positive lines stay invalid",
  );
  assert.throws(
    () =>
      buildMaintainabilityModel(
...
test("T214 model: dead-code entries are normalized, frozen, and kind-allowlisted", () => {
  ...
  assert.throws(
    ...
    MaintainabilityModelError,
    "zero counts stay invalid outside the scanner",
  );
  assert.throws(
    () =>
      buildMaintainabilityModel(
  ...
  const many = Array.from({ length: MAINTAINABILITY_LIMITS.deadCodeEntries + 1 }, (_, index) => ({
    kind: "unused_import",
    path: `f${String(index).padStart(4, "0")}.py`,
    count: 1,
  }));
  assert.throws(
    () => buildMaintainabilityModel(modelInput({ deadCode: many })),
    (e) => e instanceof MaintainabilityModelError && e.code === "BOUND_EXCEEDED",
...
test("T214 generated boundaries: directory markers, filename markers, header comments", () => {
  ...
  const header = detectGeneratedBoundary(
    "gen.go",
  ...
  assert.equal(
    detectGeneratedBoundary("src/handwritten.js", "// hand written\nvar a = 1;\n"),
    null,
  );
...
test("T214 generated boundaries: non-ASCII header text is disclosed as the fixed ASCII marker", () => {
  const boundary = detectGeneratedBoundary(
    "gen.js",
    "// Code generated by ツールX. DO NOT EDIT.\nvar a = 1;\n",
  );
  ...
  assert.equal(
    JSON.stringify(boundary).includes("ツールX"),
    false,
    "raw non-ASCII header text is never disclosed",
  );
  assert.equal(
    detectGeneratedBoundary("src/ツール.min.js", "").source,
    "minified-source",
...
test("T214 tool evidence: config-file presence, manifest sections, and dependencies", () => {
  const config = detectToolEvidence({
    path: ".eslintrc.json",
    format: "text",
    value: "{}",
    text: "{}",
  ...
  assert.ok(
    pkgTools.some(
      (entry) => entry[0] === "eslint" && entry[1] === "manifest" && entry[2] === "eslintConfig",
    ),
  );
  ...
  const cargo = detectToolEvidence({
    path: "Cargo.toml",
    format: "text",
    value: '[lints]\nclippy = { level = "warn" }\n',
    text: '[lints]\nclippy = { level = "warn" }\n',
  ...
  const rustfmt = detectToolEvidence({
    path: "rustfmt.toml",
    format: "text",
    value: "max_width = 100\n",
    text: "max_width = 100\n",
  ...
  assert.deepEqual(
    rustfmt.map(({ tool }) => tool),
    ["rustfmt"],
  );
  assert.deepEqual(
    detectToolEvidence({ path: "README.md", format: "text", value: "docs", text: "docs" }),
    [],
  );
});
...
test("T214 dead-code config signals: declarations are counted once", () => {
  const vulture = detectDeadCodeConfigSignals({
    path: "pyproject.toml",
    format: "text",
    text: "[tool.vulture]\nmin-confidence = 90\n",
  });
  ...
  assert.deepEqual(
    detectDeadCodeConfigSignals({ path: "pyproject.toml", format: "text", text: "[tool.ruff]\n" }),
    [],
  );
  const toml = detectDeadCodeConfigSignals({
    path: "vulture.toml",
    format: "text",
    text: "min-confidence = 90\n",
  });
  assert.deepEqual(toml, [{ kind: "vulture_config", path: "vulture.toml", count: 1 }]);
  const whitelist = detectDeadCodeConfigSignals({
    path: ".vulture_whitelist.py",
    format: "text",
    text: "# whitelist\nfoo\n",
  });
  assert.deepEqual(whitelist, [
    { kind: "vulture_whitelist", path: ".vulture_whitelist.py", count: 1 },
  ]);
  const ts = detectDeadCodeConfigSignals({
    path: "tsconfig.json",
    format: "json",
  ...
  assert.deepEqual(
    detectDeadCodeConfigSignals({
      path: "jsconfig.json",
      format: "json",
      value: { compilerOptions: { noUnusedLocals: false } },
      text: "",
    }),
    [],
    "a false flag is not a declaration",
  );
  const eslint = detectDeadCodeConfigSignals({
    path: ".eslintrc.json",
    format: "text",
    value: "{}",
    text: '{"rules": {"no-unused-vars": "error"}}',
  });
  assert.deepEqual(eslint, [{ kind: "no_unused_vars", path: ".eslintrc.json", count: 1 }]);
  const eslintTs = detectDeadCodeConfigSignals({
    path: "eslint.config.js",
    format: "text",
    value: "{}",
    text: 'export default [{ rules: { "@typescript-eslint/no-unused-vars": "warn" } }];',
  });
...
test("T214 provider: emits only DIM-maintainability categories via the foundation", async () => {
  ...
  await withFixture("maint-prov", files, async (dir) => {
    ...
    for (const observation of result.observations) {
      assert.ok(
        allowed.includes(observation.category),
        `category ${observation.category} is allowlisted`,
      );
      assert.ok(EVIDENCE_SOURCE_KINDS.includes(observation.sourceKind), observation.sourceKind);
      seen.add(observation.category);
    }
    assert.deepEqual([...seen].toSorted(), ["branch_point", "file_metric", "measurement_universe"]);
    const universe = result.observations.find(
      ({ category }) => category === "measurement_universe",
    );
    ...
    assert.deepEqual(maintainabilityObservations({}), [
      { dimensionId: "DIM-maintainability-v1", observations: [] },
    ]);
  });
});

test("T214 provider: deterministic, immutable, and empty for foreign input", () => {
  const observations = maintainabilityObservations({
    files: [],
    branchPoints: [],
    duplicateGroups: [],
  });
  assert.deepEqual(
    observations[0].observations.filter(({ category }) => category !== "measurement_universe"),
    [],
  );
});
...
test("T214 renderer: complexity distribution and unused-code markers render neutrally", async () => {
  ...
  await withFixture("maint-render-craft", files, async (dir) => {
    ...
    assert.equal(
      markdown.includes("dead code"),
      false,
      'the phrase "dead code" must never appear in prose',
    );
    assert.equal(
      markdown.includes("dead_code"),
      false,
      "the attribute token is disclosed via its label only",
    );
    assert.deepEqual(findVoiceHits(markdown), []);
    assert.equal(markdown.includes("\r"), false);
  });
...
test("T214 renderer: repo-wide aggregate complexity line uses nearest-rank stats across files", () => {
  ...
  const model = buildMaintainabilityModel(
    modelInput({
      ...
      complexityRecords: [fileFor("src/a.js", 0, 11), fileFor("src/b.js", 11, 10)],
    }),
  );
  const markdown = renderMaintainability("repo", model);
  assert.match(
    markdown,
    /Aggregate per-function complexity across the measured files: 21 function\(s\), median 11, p95 20, max 21\./,
  );
...
test("T214 scanner: five-ecosystem fixture with hand-calculated metrics", async () => {
  ...
  await withFixture("maint-eco", files, async (dir) => {
    ...
    assert.deepEqual(
      findings.duplicateGroups[0].spans.map(({ path }) => path),
      ["src/b.js", "src/c.js"],
    );
...
test("T214 scanner: generated boundaries, declared tools, and measurement universe", async () => {
  ...
  await withFixture("maint-bound", files, async (dir) => {
    const { findings } = await scan(dir, {});
    const boundaries = findings.generatedBoundaries.map(({ path, reason, marker }) => [
      path,
      reason,
      marker,
    ]);
    ...
    assert.ok(
      tools.some(
        (entry) => entry[0] === "eslint" && entry[1] === "config" && entry[2] === ".eslintrc.json",
      ),
    );
    assert.ok(
      tools.some(
        (entry) => entry[0] === "eslint" && entry[1] === "manifest" && entry[2] === "package.json",
      ),
    );
...
test("T214 scanner: malformed UTF-8 eligible file is disclosed without a false file-cap note", async () => {
  ...
  await withFixture("maint-badutf", files, async (dir) => {
    ...
    assert.ok(
      findings.diagnostics.some(
        ({ reason, status }) => reason === "MALFORMED" && status === "unverified",
      ),
    );
    const markdown = renderMaintainability("repo", findings);
...
test("T214 scanner: rust and shell dialects contribute measurements without extrapolation", async () => {
  ...
  await withFixture("maint-rs", files, async (dir) => {
    ...
    assert.deepEqual(
      findings.measurementUniverse.excludedLanguages,
      [{ extension: ".go", count: 1 }],
      "unsupported language is excluded with disclosure",
    );
  });
});
...
test("T214 scanner: complexity distributions and unused-code markers end to end", async () => {
  ...
  await withFixture("maint-craft", files, async (dir) => {
    ...
    const byPath = Object.fromEntries(
      findings.complexityRecords.map((record) => [record.path, record]),
    );
    assert.deepEqual(
      byPath["src/a.js"].functions.map(({ functionName, complexity }) => [
        functionName,
        complexity,
      ]),
      [
    ...
    assert.deepEqual(
      byPath["app.py"].functions.map(({ functionName, complexity }) => [functionName, complexity]),
      [["run", 1]],
    );
    assert.deepEqual(
      byPath["src/lib.rs"].functions.map(({ functionName, complexity }) => [
        functionName,
        complexity,
      ]),
      [["value", 0]],
...
test("T214 scanner: a token=... line in a dead-code artifact is never retained", async () => {
  ...
  await withFixture("maint-craft-privacy", files, async (dir) => {
    ...
    assert.equal(
      ...
      false,
      "the token= value must never be retained",
    );
    assert.equal(
      serialized.includes("ghp_\x73uperSecretValue"),
      false,
      "the secret literal must never be retained",
    );
    assert.equal(serialized.includes(dir), false);
    assert.ok(
      findings.deadCode.some(({ kind }) => kind === "allow_dead_code"),
      "allow attributes are still counted",
    );
    assert.ok(
      findings.deadCode.some(({ kind }) => kind === "unused_import"),
      "noqa markers are still counted",
    );
    assert.ok(
      findings.complexityRecords.length >= 1,
      "complexity records survive privacy filtering",
    );
    assert.equal(findings.summary.partialCoverage, false);
  });
});
...
test("T214 scanner: per-file function caps are disclosed as COMPLEXITY_CAP", async () => {
  ...
  await withFixture("maint-fncap", files, async (dir) => {
    ...
    assert.ok(
      findings.diagnostics.some(
        ({ path, reason }) => path === "src/many.js" && reason === "COMPLEXITY_CAP",
      ),
    );
    const markdown = renderMaintainability("repo", findings);
...
test("T214 inertness: maintainability modules never touch execution surfaces", async () => {
  ...
  const providerSource = await readFile(
    join(LIB_ROOT, "scan", "providers", "maintainability.mjs"),
    "utf8",
  );
  const rendererSource = await readFile(
    join(LIB_ROOT, "scan", "render", "maintainability.mjs"),
    "utf8",
  );
