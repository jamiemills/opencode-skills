...
test("T227 negative: malformed plugin JSON aborts at the loader with a sanitized error and no output", async () => {
  ...
  try {
    ...
    await assert.rejects(loadPlugins({ skillRoot }), (error) =>
      sanitizedErrorMatch(error, PluginLoaderError, "MALFORMED_JSON", [
        skillRoot,
        "not-valid-json",
      ]),
    );
    assert.deepEqual(
      ...
      [],
      "a failed plugin load must not produce any output file",
    );
  } finally {
...
test("T227 negative: plugin schema violations are rejected typed before any evaluation", () => {
  ...
  const invalidCases = [
    ...
    [
      "rule with a path token",
      { ...base, rules: [{ ...base.rules[0], artifactTokens: ["../../etc/passwd"] }] },
    ],
  ...
  try {
    ...
    await assert.rejects(
      runExpandedPipeline({
        ...
        sink: (findings, out, renderer) => {
          sinkCalls++;
          return writeNORMS(findings, out, renderer);
        },
      }),
    ...
    assert.deepEqual(
      await readdir(outDir),
      [],
      "a malformed plugin rule must not create any output file",
    );
      ...
      providers: [
        {
          id: "PRV-fixturelang-v1",
          apiVersion: 1,
          dimensions: [{ dimensionId: "DIM-does-not-exist-v1", categories: ["route"] }],
        },
      ],
      rules: [
        {
    ...
    assert.equal(
      sinkCalls,
      1,
      "the run with an unregistered rule dimension must complete and write exactly once",
    );
    const markdown = await readFile(join(outDir, "NORMS.md"), "utf8");
    assert.equal(
      markdown.includes("DIM-does-not-exist-v1"),
      false,
      "an unregistered dimension must never be fabricated",
    );
    assert.equal(
      markdown.includes("fixturelang"),
      false,
      "a rule with an unregistered dimension must produce no evidence",
    );
...
test("T227 negative: invalid evidence records are rejected by the canonical contract", () => {
  ...
  assert.match(
    created.id,
    /^EVD-v1-[a-f0-9]{64}$/,
    "a canonical evidence record receives a deterministic identity",
  );
      ...
      () =>
        createRenderRegistry({
          entries: [
            ...
            {
              dimension: "foreign-canary",
              rendererId: "RND-foreign-canary-v1",
              label: "x",
              prose: ["x"],
              render: () => "",
            },
...
test("T227 negative: standards-policy violations are rejected with sanitized typed errors", () => {
  ...
  assert.throws(() => reuseDisposition({ bogus: 1 }), /unknown field/);
...
test("T227 negative: unsupported, ambiguous, and unresolved constructs never fabricate facts", async () => {
  ...
  try {
    const result = await runExpandedPipeline({
      repos: [unsupported],
      out: join(outDir, "u.md"),
      clock: FIXED_CLOCK,
    });
    const api = result.repos[0].deep.find(({ dimension }) => dimension === "api");
    assert.deepEqual(
      api.findings.operations,
      [],
      "an unsupported template must never invent operations",
    );
    assert.ok(
      api.findings.diagnostics.some(
    ...
    assert.equal(
      api.findings.searchSpace.complete,
      true,
    ...
    try {
      const cross = await runExpandedPipeline({
        repos: [a, b],
        out: join(outDir, "c.md"),
        clock: FIXED_CLOCK,
      });
      assert.equal(
        cross.global.metrics.crossRepositoryEdges,
        0,
        "an ambiguous exact reference never becomes an edge",
      );
      assert.equal(
        cross.global.metrics.ambiguous,
        2,
        "ambiguous references are retained as disclosed records",
      );
      assert.deepEqual(
        cross.global.edges.edges,
        [],
    ...
    try {
      const data = (
        await runExpandedPipeline({
          repos: [pythonRepo],
          out: join(outDir, "d.md"),
          clock: FIXED_CLOCK,
        })
      ).repos[0].deep.find(({ dimension }) => dimension === "data");
      assert.ok(
        data.findings.diagnostics.some(
...
test("T227 constraint: the pipeline issues only registered broker command IDs and zero target commands", async () => {
  ...
  try {
    ...
    assert.equal(
      targetCalls.length,
      0,
      `no target command may execute (saw ${targetCalls.length})`,
    );
  } finally {
    cleanupFixture(repo);
...
test("T227 constraint: exactly one production write per run", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      await readdir(outDir),
      ["NORMS.md"],
      "a run must produce exactly one output file",
    );
  } finally {
    cleanupFixture(repo);
    await rm(outDir, { recursive: true, force: true });
...
test("T227 constraint: recorded git and rg argv forms are strictly read-only", () => {
    ...
    for (const token of form) {
      ...
      if (token === "config") {
        assert.equal(
          form.join(" "),
          "config --get remote.origin.url",
          "the only registered config form is config --get",
        );
      }
    }
  }
  ...
  assert.ok(
    !rgFiles.some((token) => token.startsWith("--replace") || token.startsWith("--write")),
