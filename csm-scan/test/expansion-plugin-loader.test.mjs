...
test('T203 API and valid deterministic registry snapshot are exact and immutable', async (t) => {
  ...
  const zeta = plugin('zeta', {
    ...
    rules: [rule('RUL-zeta-runtime-v1', 'DIM-stack-v1', 'runtime', {
      extensions: ['.zeta'],
      basenames: ['Zetafile'],
  ...
  const alpha = plugin('alpha', {
    aliases: ['a-lang'],
    rules: [rule('RUL-alpha-route-v1', 'DIM-api-v1', 'route', {
      extensions: ['.alpha'], literal: 'route:',
    })],
  });
  ...
  assert.deepEqual(loaded, [
    {
      id: 'alpha', apiVersion: 1, label: 'Alpha language', aliases: ['a-lang'],
      providers: [{
        id: 'PRV-alpha-v1', apiVersion: 1,
        dimensions: [{ dimensionId: 'DIM-api-v1', categories: ['route'] }],
      }],
      rules: [{
        id: 'RUL-alpha-route-v1', label: 'Fixture artifact', extensions: ['.alpha'],
        basenames: [], manifestNames: [], artifactTokens: [], dimensionId: 'DIM-api-v1',
        category: 'route', literal: 'route:', regexSource: null,
      }],
    },
    {
      id: 'zeta', apiVersion: 1, label: 'Zeta language', aliases: ['z-lang', 'zeta-lang'],
      providers: [{
        id: 'PRV-zeta-v1', apiVersion: 1,
        dimensions: [{ dimensionId: 'DIM-stack-v1', categories: ['language', 'runtime'] }],
      }],
      rules: [{
        id: 'RUL-zeta-runtime-v1', label: 'Fixture artifact', extensions: ['.zeta'],
        basenames: ['Zetafile'], manifestNames: ['zeta.json'], artifactTokens: ['config/zeta'],
        dimensionId: 'DIM-stack-v1', category: 'runtime', literal: null,
        regexSource: '^runtime:[ ]+[A-Za-z0-9._-]+$',
      }],
    },
  ]);
...
test('T203 rejects malformed and oversized JSON without reflecting content', async (t) => {
  ...
  await assert.rejects(loadPlugins({ skillRoot: oversizedRoot }), (error) => error.code === 'FILE_TOO_LARGE');
});
...
test('T203 rejects symlink skill roots, plugin roots, plugin directories, and files', async (t) => {
  ...
  await assert.rejects(loadPlugins({ skillRoot: linkedSkill }), (error) => error.code === 'SYMLINK');
  ...
  await assert.rejects(loadPlugins({ skillRoot: pluginRootSkill }), (error) => error.code === 'SYMLINK');
  ...
  await assert.rejects(loadPlugins({ skillRoot: directorySkill }), (error) => error.code === 'SYMLINK');
...
test('T203 rejects relative, non-normalized, symlink-contained, and non-direct layouts', async (t) => {
  await assert.rejects(loadPlugins({ skillRoot: 'relative/root' }), (error) => error.code === 'INVALID_ROOT');
  const root = await temporaryRoot(t);
  await assert.rejects(
  ...
  await writeFile(join(nested, 'plugins', 'group', 'nested', 'plugin.json'), JSON.stringify(plugin('nested')));
  await assert.rejects(loadPlugins({ skillRoot: nested }), (error) => error.code === 'INVALID_LAYOUT');
  ...
  await assert.rejects(loadPlugins({ skillRoot: extra }), (error) => error.code === 'INVALID_LAYOUT');
});
...
test('T203 enforces directory identity and safe relative artifact selectors', async (t) => {
  ...
  await assert.rejects(loadPlugins({ skillRoot: mismatch }), (error) => error.code === 'DIRECTORY_ID_MISMATCH');
  ...
  for (const [field, value] of [
    ...
    const valueUnderTest = field === 'id'
      ? plugin(value)
      : field === 'aliases'
        ? plugin('safe', { aliases: value })
        : plugin('safe', { rules: [rule('RUL-safe-route-v1', 'DIM-api-v1', 'route', { [field]: value })] });
...
test('T203 rejects versions, unknown fields, executable fields, invalid types, and bounds', () => {
  ...
  for (const value of [
    ...
    { ...base, aliases: Array.from({ length: PLUGIN_LIMITS.aliases + 1 }, (_, index) => `alias-${index}`) },
    { ...base, providers: [] },
  ...
  assert.throws(() => validatePlugin({ ...base, run() {} }), (error) => error.code === 'DATA_ONLY');
});
...
test('T203 bounds strings and rejects regex syntax and complexity hazards under fixed flags', () => {
  ...
  const withRegex = (regexSource) => ({
    ...base,
    rules: [rule('RUL-regex-route-v1', 'DIM-api-v1', 'route', { extensions: ['.rx'], regexSource })],
  });
  assert.equal(validatePlugin(withRegex('^route\\s+[A-Za-z0-9._-]+$')).rules[0].regexSource, '^route\\s+[A-Za-z0-9._-]+$');
  for (const source of [
  ...
  assert.throws(() => validatePlugin({
    ...base,
    rules: [rule('RUL-regex-route-v1', 'DIM-api-v1', 'route', {
      extensions: ['.rx'], literal: 'route', regexSource: 'route',
    })],
  }), (error) => error.code === 'INVALID_MATCH');
  assert.throws(() => validatePlugin({
    ...base,
    rules: [rule('RUL-regex-route-v1', 'DIM-api-v1', 'route', {
      extensions: ['.rx'], literal: 'x'.repeat(PLUGIN_LIMITS.literal + 1),
    })],
  }), PluginSchemaError);
});
...
test('T203R1 rejects overlapping quantified atoms while keeping safe partition patterns', () => {
  ...
  const withRegex = (regexSource) => ({
    ...base,
    rules: [rule('RUL-partition-route-v1', 'DIM-api-v1', 'route', { extensions: ['.px'], regexSource })],
  });
  for (const source of [
    'a*a*',
    '[a-z]*[a-z]*',
    ...
    assert.throws(() => validatePlugin(withRegex(source)), (error) => {
...
test('T203 rejects duplicate plugin identities, aliases, rules, providers, and capabilities', () => {
  ...
  assert.throws(() => validatePlugins([first, first]), (error) => error.code === 'DUPLICATE_ALIAS');
  assert.throws(() => validatePlugins([
    first,
    plugin('second', { aliases: ['first-alias'] }),
  ]), (error) => error.code === 'DUPLICATE_ALIAS');
  assert.throws(() => validatePlugin({
    ...first, rules: [first.rules[0], first.rules[0]],
  }), (error) => error.code === 'DUPLICATE_RULE');
  assert.throws(() => validatePlugins([
    first,
    plugin('second', {
      providers: [provider('PRV-second-v1', [capability('DIM-stack-v1', ['runtime'])])],
      rules: [{ ...rule(first.rules[0].id, 'DIM-stack-v1', 'runtime'), extensions: ['.second'] }],
    }),
  ]), (error) => error.code === 'DUPLICATE_RULE');
  assert.throws(() => validatePlugin({
    ...first, providers: [first.providers[0], first.providers[0]],
  }), (error) => error.code === 'DUPLICATE_PROVIDER');
  assert.throws(() => validatePlugins([
    first,
    plugin('second', { providers: [{ ...first.providers[0] }] }),
  ]), (error) => error.code === 'DUPLICATE_PROVIDER');
  assert.throws(() => validatePlugin({
    ...first,
    providers: [provider('PRV-first-v1', [
      capability('DIM-api-v1', ['route']), capability('DIM-api-v1', ['rpc']),
    ])],
  }), (error) => error.code === 'INVALID_PROVIDER');
  const sharing = validatePlugins([
    first,
    plugin('second', { providers: [provider('PRV-second-v1', [capability('DIM-api-v1', ['route'])])] }),
  ]);
  assert.equal(sharing.length, 2, 'distinct plugins may both contribute the same dimension/category');
});
...
test('T203 errors are typed and sanitized across filesystem, JSON, schema, and regex failures', async (t) => {
  ...
  await writePlugin(root, plugin('unsafe', {
    rules: [rule('RUL-unsafe-route-v1', 'DIM-api-v1', 'route', {
      extensions: ['.unsafe'], regexSource: `(?=${secret})`,
    })],
  }));
