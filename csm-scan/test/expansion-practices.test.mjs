...
test("T005 model: invalid entries, statuses, categories, and paths throw typed errors", () => {
  assert.throws(
    () => modelOf([{ ...entry("a.md"), category: "language" }]),
    (error) => error instanceof PracticesModelError && error.code === "UNKNOWN_CATEGORY",
  );
  assert.throws(
    () => modelOf([{ ...entry("a.md"), status: "observed-evil" }]),
    PracticesModelError,
  );
  assert.throws(() => modelOf([{ ...entry("/etc/passwd") }]), PracticesModelError);
  assert.throws(() => modelOf([{ ...entry("../escape") }]), PracticesModelError);
  assert.throws(() => modelOf([{ ...entry("a.md"), matchedKey: "bad key!" }]), PracticesModelError);
  assert.throws(() => modelOf([{ ...entry("a.md"), count: -1 }]), PracticesModelError);
  assert.throws(() => modelOf([{ ...entry("a.md"), kinds: ["ok", 3] }]), PracticesModelError);
  assert.throws(() => modelOf([entry("a.md")], { isGit: "yes" }), PracticesModelError);
});
...
test("T005 model: deterministic deep-frozen output with exact summary", () => {
  ...
  assert.deepEqual(first.entries.find((item) => item.path === "a.md").kinds, [
    "behave",
    "hypothesis",
  ]);
});
...
test("T005 model: privacy violations downgrade to unverified PRIVACY diagnostics", () => {
  ...
  assert.deepEqual(
    model.entries.map((item) => item.path),
    ["a.md"],
  );
...
test("T002 foundations: maxKinds accepts up to 256 deduplicated kind tokens", () => {
  ...
  const manyKinds = Array.from(
    { length: 256 },
    (_, index) => `kind-${String(index).padStart(3, "0")}`,
  );
  const model = modelOf([entry("a.md", { kinds: manyKinds })]);
  assert.equal(model.entries[0].kinds.length, 256);
  assert.throws(
    () => modelOf([entry("a.md", { kinds: [...manyKinds, "overflow"] })]),
...
test("T005 model: encodeMatchedKey percent-encodes every non-token character", () => {
  ...
  assert.equal(
    encodeMatchedKey("quality_gate:gate:quality/gates.conf"),
    "quality_gate:gate:quality/gates.conf",
  );
...
test("T005 classify: static practice paths across all seven categories", () => {
  assert.deepEqual(classifyPracticePath("features/login.feature"), {
    category: "methodology",
    kind: "bdd-feature",
  });
  assert.deepEqual(classifyPracticePath("strategies.py"), {
    category: "methodology",
    kind: "hypothesis-strategies",
  });
  assert.deepEqual(classifyPracticePath("fuzz_corpus/seed.txt"), {
    category: "methodology",
    kind: "fuzz-corpus",
  });
  assert.deepEqual(classifyPracticePath("commitlint.config.js"), {
    category: "enforcement",
    kind: "commitlint",
  });
  assert.deepEqual(classifyPracticePath(".gitlint"), { category: "enforcement", kind: "gitlint" });
  assert.deepEqual(classifyPracticePath(".github/release-drafter.yml"), {
    category: "automation",
    kind: "release-drafter",
  });
  assert.deepEqual(classifyPracticePath("dependabot.yml"), {
    category: "automation",
    kind: "dependabot",
  });
  assert.deepEqual(classifyPracticePath("renovate.json"), {
    category: "automation",
    kind: "renovate",
  });
  assert.deepEqual(classifyPracticePath("mkdocs.yml"), { category: "automation", kind: "mkdocs" });
  assert.deepEqual(classifyPracticePath(".github/pull_request_template.md"), {
    category: "ritual",
    kind: "pr-template",
  });
  assert.deepEqual(classifyPracticePath("quality/gates.conf"), {
    category: "quality_gate",
    kind: "gates-conf",
  });
  assert.deepEqual(classifyPracticePath("test/baselines/coverage.json"), {
    category: "quality_gate",
    kind: "baseline",
  });
  assert.deepEqual(classifyPracticePath("AGENTS.md"), {
    category: "agent_workflow",
    kind: "agents",
  });
  assert.deepEqual(classifyPracticePath(".agents/plans/feature-csm.md"), {
    category: "agent_workflow",
    kind: "csm-plan",
  });
  assert.deepEqual(classifyPracticePath("ruff.toml"), { category: "style_guide", kind: "ruff" });
  assert.deepEqual(classifyPracticePath(".prettierrc"), {
...
test("T002 foundations: static makefile and contributing paths classify with distinct kinds", () => {
  ...
  assert.deepEqual(classifyPracticePath("makefile"), { category: "automation", kind: "makefile" });
  assert.deepEqual(classifyPracticePath("GNUmakefile"), {
    category: "automation",
    kind: "makefile",
  });
  assert.deepEqual(classifyPracticePath("contributing.md"), {
    category: "style_guide",
    kind: "contributing",
  });
  assert.deepEqual(classifyPracticePath("CONTRIBUTING.md"), {
  ...
  assert.deepEqual(classifyPracticePath(".github/contributing.md"), {
    category: "style_guide",
    kind: "contributing",
  });
...
test("T002 foundations: static and content kinds coexist on the same path without dedup loss", () => {
  ...
  assert.equal(
    ...
    2,
    "static and content kinds must both survive first-wins dedup",
  );
  assert.deepEqual(model.entries.map((item) => item.matchedKey).toSorted(), [
    "automation:make-targets:Makefile",
    "automation:makefile:Makefile",
  ]);
  const content = model.entries.find(
    (item) => item.matchedKey === "automation:make-targets:Makefile",
  );
...
test("T003 policy: gate values render as bounded per-key counts and slugs, token keys never survive", () => {
  ...
  assert.deepEqual(records.map((record) => record.kind).toSorted(), [
    "gate-value:maxcomplexity",
    "gate-value:maxlines",
    "gate-value:mincoverage",
  ]);
  ...
  assert.equal(
    JSON.stringify(records).includes("sk-supersecret123"),
    false,
    "non-allowlisted token key still never survives",
  );
  assert.equal(
    JSON.stringify(records).includes("token="),
    false,
    "raw KEY=value strings never survive",
  );
});
...
test("T003 policy: extended gate vocabulary renders count 85, slug B, and key-presence", () => {
  ...
  assert.deepEqual(
    byKind.get("gate-value:distancethreshold").kinds,
    ["0.3"],
    "floats render as slug kinds",
  );
  assert.deepEqual(
    byKind.get("gate-value:radonccgrade").kinds,
    ["B"],
  ...
  assert.deepEqual(
    byKind.get("gate-value:semgrepseverity").kinds,
    ["--severity%20ERROR"],
    "SEMGREP_SEVERITY carries its real value through the value channel",
  );
  assert.equal(
    byKind.get("gate-value:semgrepseverity").count,
    undefined,
  ...
  assert.equal(
    JSON.stringify(records).includes("KEY="),
    false,
    "raw KEY=value strings never survive",
  );
});
...
test("T005 scanner: positive fixture spans all seven categories", async () => {
  await withFixture("prac-positive", POSITIVE_FILES, async (dir) => {
    ...
    assert.ok(
      paths.includes(".agents/plans/feature-csm.md"),
      "hidden .agents/plans artifact probed",
    );
    assert.ok(
      paths.includes(".devcontainer/devcontainer.json"),
      "hidden .devcontainer artifact probed",
    );
    ...
    const minCoverage = findings.entries.find(
      (item) => item.matchedKey === "quality_gate:gate-value:mincoverage:quality/gates.conf",
    );
    assert.ok(minCoverage, "quality gates.conf per-key entry present");
    assert.equal(minCoverage.count, 85, "MIN_COVERAGE=85 renders as a bounded count");
    assert.equal(
      findings.entries.some(
        (item) => item.matchedKey === "quality_gate:gate-thresholds:quality/gates.conf",
      ),
      false,
      "the aggregated gate-thresholds entry is replaced by per-key entries",
    );

    const mutmutConfig = findings.entries.find(
      (item) => item.matchedKey === "methodology:mutation-config:pyproject.toml",
    );
    assert.ok(mutmutConfig, "[tool.mutmut] section detected");

    const deps = findings.entries.find(
      (item) => item.matchedKey === "methodology:test-deps:pyproject.toml",
    );
    assert.ok(deps && deps.kinds.includes("hypothesis"), "hypothesis dependency detected");

    const styles = findings.entries.find(
      (item) => item.matchedKey === "style_guide:style-values:pyproject.toml",
    );
    assert.ok(styles && styles.kinds.includes("line-length"), "ruff line-length value detected");

    const prettierValues = findings.entries.find(
      (item) => item.matchedKey === "style_guide:style-values:.prettierrc",
    );
    assert.ok(prettierValues, "prettier style values present");
    assert.ok(
      prettierValues.kinds.includes("print-width") && prettierValues.kinds.includes("single-quote"),
    );

    assert.ok(
      findings.entries.some((item) => item.status === "inferred"),
      "regex-derived signals are inferred",
    );
    assert.ok(
      findings.entries.some((item) => item.matchedKey === "ritual:release-notes:CHANGELOG.md"),
...
test("T005 scanner: empty repository yields not_detected after a complete search", async () => {
  ...
  await withFixture("prac-empty", files, async (dir) => {
    ...
    assert.deepEqual(
      findings.summary.byCategory,
      Object.fromEntries(PRACTICES_CATEGORIES.map((name) => [name, 0])),
    );
  });
});
...
test("T005 scanner: token values never reach the model, provider, or outputs", async () => {
  ...
  await withFixture("prac-privacy", files, async (dir) => {
    ...
    assert.equal(
      JSON.stringify(practicesProviderResult(findings)).includes(canary),
      false,
      "provider leaked the token",
    );
  });
});
...
test("T005 scanner: malformed artifacts degrade to unverified, never crash", async () => {
  ...
  await withFixture("prac-unreadable", files, async (dir) => {
    ...
    assert.ok(
      findings.entries.some(({ path }) => path === "features/login.feature"),
  ...
  await withFixtureIsolated(
    ...
    async (dir) => {
      ...
      assert.ok(
        ...
        "node_modules content is never collected",
      );
      assert.ok(
        !paths.some((path) => path.includes("/coverage/")),
        "coverage artifacts are never collected",
      );
      assert.ok(
        paths.includes(".opencode/eslint.config.mjs"),
        "real agent configs are still collected",
      );
      assert.ok(
        paths.some((path) => path === "quality/gates.conf"),
        "quality gate file still collected",
      );
    },
  );
});
...
test("T003 style: ruff select and ignore codes emit as separate facts", () => {
  ...
  assert.ok(
    select.kinds.includes("E") && select.kinds.includes("S") && select.kinds.includes("UP"),
    "rule families are slug kinds",
  );
  assert.ok(
    select.kinds.includes("S101") && select.kinds.includes("UP035"),
  ...
  assert.ok(
    ignored.kinds.includes("E501") && ignored.kinds.includes("D105") && ignored.kinds.includes("D"),
    "ignored codes render as slug kinds",
  );
  assert.equal(
    records.some((record) => record.kind === "ruff-rules"),
    false,
...
test("T003 style: ruff extractor handles dedicated ruff.toml root tables", () => {
  ...
  assert.equal(lineLength.count, 100);
  assert.deepEqual(
    extractRuffRules({ path: "pyproject.toml", text: "[tool.black]\nline-length = 100\n" }),
    [],
...
test("T003 style: Makefile targets extract with count and slug kinds", () => {
  ...
  assert.equal(
    extractMakeTargets({ path: "src/Makefile", text: "x:\n" }).length,
    1,
...
test("T003 style: lefthook stages and jobs extract as separate records with pipeline semantics", () => {
  ...
  assert.equal(
    stages.count,
    2,
    "stage count is the number of declared hooks, never summed with jobs",
  );
  ...
  assert.ok(
    preCommit.kinds.includes("piped") && preCommit.kinds.includes("abort-on-failure"),
  ...
  assert.deepEqual(
    extractLefthookStages({ path: ".github/workflows/ci.yml", text: "jobs:\n  lint: {}\n" }),
    [],
  );
});
...
test("T003 style: opencode deny rules and plugin inventory extract as kinds", () => {
  ...
  assert.deepEqual(
    extractOpencodeWorkflow({ path: "src/opencode.jsonc", text: OPENCODE_JSONC }),
    [],
  );
});
...
test("T003 style: exceptions hub reports role count and exit-code taxonomy pairs", () => {
  ...
  assert.deepEqual(
    constants.kinds.toSorted(),
    ["general-failure-1", "success-0"],
  ...
  assert.equal(
    records.some((record) => record.kind === "exit-code-exception"),
    false,
...
test("T003 style: multi-word names become bounded slugs, never space tokens", () => {
  assert.equal(
    slugToken("adapter must not import non-allowed layers"),
...
test("T003 style: style extractors feed the scanner without space-token throws", async () => {
  ...
  await withFixture("t003-style-signals", files, async (dir) => {
    ...
    const conventions = findings.entries.find(
      (item) => item.matchedKey === "style_guide:declared-conventions:CONTRIBUTING.md",
    );
    assert.ok(conventions, "declared-conventions entry present");
    assert.deepEqual(conventions.kinds, ["code-quality", "contributing"]);
    const targets = findings.entries.find(
      (item) => item.matchedKey === "automation:make-targets:Makefile",
    );
    assert.ok(targets, "make-targets entry present alongside the static makefile kind");
    assert.equal(targets.count, 2);
    const hub = findings.entries.find(
      (item) => item.matchedKey === "style_guide:exceptions-hub:src/exit_codes.py",
    );
...
test("T005 policy: CHECK_* toggles emit as quality_gate:check-toggle facts", () => {
  ...
  assert.equal(
    records.some((record) => record.kind === "check-toggle:check-lint"),
    true,
  );
  assert.equal(
    records.some((record) => record.kind === "check-toggle:min-coverage"),
    false,
...
test("T005 policy: gates.conf header semantics become tokenized policy kinds", () => {
  ...
  assert.equal(
    JSON.stringify(header).includes("coding agents"),
    false,
...
test("T005 style: lefthook piped-stage facts report 5 pre-commit / 4 pre-push stages with stdin ownership", () => {
  ...
  assert.ok(
    prePush.kinds.includes("stdin-owner:gitleaks-detect"),
...
test("T005 style: make check toggles and ci-quality membership emit as automation facts", () => {
  ...
  assert.deepEqual(membership.kinds.toSorted(), [
    "deptry",
    "format-check",
    "lint",
    "ratchets",
...
test("T005 style: exit-code taxonomy emits constants, exception table and HTTP special-casing", () => {
  const records = extractExceptionsHub({
    path: "src/perplexity_cli/exit_codes.py",
    text: EXIT_CODES_TAXONOMY,
  });
  const constants = records.find((record) => record.kind === "exit-code-constant");
  assert.equal(constants.count, 5, "ALL_CAPS = <int> assignments only");
  assert.ok(
    constants.kinds.includes("auth-required-4") && constants.kinds.includes("interrupted-130"),
  );
  assert.ok(
    !constants.kinds.some((kind) => kind.includes("http")),
...
test("T005 render: style-guide block renders value fidelity, toggles, stages and exit-code pairs", () => {
  const entries = [
    {
      category: "quality_gate",
      matchedKey: "quality_gate:gate-value:semgrepseverity:quality/gates.conf",
      path: "quality/gates.conf",
      status: "observed",
      kinds: ["--severity%20ERROR%20--severity%20WARNING"],
    },
    {
      category: "quality_gate",
      matchedKey: "quality_gate:gate-value:mincoverage:quality/gates.conf",
      path: "quality/gates.conf",
      status: "observed",
      count: 85,
    },
    {
      category: "quality_gate",
      matchedKey: "quality_gate:check-toggle:check-format:quality/gates.conf",
      path: "quality/gates.conf",
      status: "observed",
      count: 1,
    },
    {
      category: "quality_gate",
      matchedKey: "quality_gate:check-toggle:check-semgrep:quality/gates.conf",
      path: "quality/gates.conf",
      status: "observed",
      count: 0,
    },
    {
      category: "quality_gate",
      matchedKey: "quality_gate:gates-header:quality/gates.conf",
      path: "quality/gates.conf",
      status: "observed",
      kinds: ["denied-to-agents", "human-change"],
    },
    {
      category: "enforcement",
      matchedKey: "enforcement:hook-stages:lefthook.yml",
      path: "lefthook.yml",
      status: "observed",
      count: 2,
    ...
    {
      category: "enforcement",
      matchedKey: "enforcement:hook-jobs:lefthook.yml",
      path: "lefthook.yml",
      status: "observed",
      count: 53,
    },
    {
      category: "enforcement",
      matchedKey: "enforcement:hook-stage:pre-commit:lefthook.yml",
      path: "lefthook.yml",
      status: "observed",
      count: 5,
      kinds: ["piped", "abort-on-failure"],
    },
    {
      category: "enforcement",
      matchedKey: "enforcement:hook-stage:pre-push:lefthook.yml",
      path: "lefthook.yml",
      status: "observed",
      count: 4,
      kinds: ["piped", "abort-on-failure", "stdin-owner:gitleaks-detect"],
    },
    {
      category: "style_guide",
      matchedKey: "style_guide:exit-code-constant:src/exit_codes.py",
      path: "src/exit_codes.py",
      status: "observed",
      count: 9,
      kinds: ["success-0", "interrupted-130"],
    },
  ];
  const model = modelOf(entries);
  const markdown = renderPractices("repository", model);
  assert.ok(
    markdown.includes("| semgrepseverity | `--severity ERROR --severity WARNING` |"),
    "semgrepseverity renders its real value verbatim in the gate table",
  );
  assert.ok(
    markdown.includes("| check-format | on |") && markdown.includes("| check-semgrep | off |"),
    "check toggles render as on/off states",
  );
  assert.ok(
    markdown.includes("pre-commit (5 stages: `abort-on-failure`, `piped`)"),
    "per-hook stage counts render with pipeline semantics",
  );
  assert.ok(
    markdown.includes(
      "pre-push (4 stages: `abort-on-failure`, `piped`, `stdin-owner:gitleaks-detect`)",
    ),
...
const CONTENT_FIXTURE_FILES = {
  ...
  ".opencode/package.json": JSON.stringify({
    scripts: {
      check:
        "npm run lint && npm run test && npm run typecheck && npm run check:config && npm run test:coverage",
...
test("T006 content: suppression-policy kinds emit from both gate scripts", () => {
  ...
  assert.ok(
    suppPolicy.kinds.includes("file-line-type-detail"),
    "file:line:type[:detail] identity tokenized",
  );
  assert.ok(
    suppPolicy.kinds.includes("block-new-moved-broadened"),
    "block new/moved/broadened tokenized",
  );
...
test("T006 content: suppression-baseline reports the grandfathered identity count", () => {
  ...
  assert.deepEqual(
    extractSuppressionBaseline({ path: "quality/baselines/other.json", text: "{}" }),
    [],
  );
});
...
test("T006 content: mutation exit codes, actionable categories, scope and schedule", () => {
  ...
  assert.deepEqual(
    exit.kinds,
    ["clean-0", "findings-1", "tool-error-2"],
    "0/1/2 exit taxonomy tokenized",
  );
  ...
  assert.deepEqual(
    waivers.kinds,
    ["unsupported"],
    "waivers-unsupported inferred when no waiver mechanism",
  );
  ...
  assert.deepEqual(
    schedule.find((record) => record.kind === "mutation-schedule").kinds,
    ["full-policy", "weekly-sunday-0200-utc"],
    "cron 0 2 * * 0 maps to weekly Sunday 02:00 UTC",
  );
  ...
  assert.deepEqual(scope.find((record) => record.kind === "mutation-scope").kinds, [
    "diff",
    "full",
  ]);
});
...
test("T006 content: fuzz replay contract, decomposition, platform gate and blocking CI", () => {
  ...
  assert.deepEqual(replay.find((record) => record.kind === "fuzz-replay-contract").kinds, [
    ...
    "json-state-file",
    "lexicographic-seed-order",
  ]);
  assert.equal(
    replay.find((record) => record.kind === "fuzz-seeds").count,
    2,
    "seed rows counted from the inventory table",
  );
  ...
  assert.equal(
    decomposition.find((record) => record.kind === "fuzz-iterations").count,
    5000,
  ...
  assert.deepEqual(
    platform.find((record) => record.kind === "fuzz-platform-gate").kinds,
    ["atheris", "linux-x86-64"],
    "atheris linux/x86_64 gating detected",
  );

  const make = extractFuzzReplay({ path: "Makefile", text: CONTENT_FIXTURE_FILES.Makefile });
  assert.deepEqual(
    make.find((record) => record.kind === "fuzz-ci-blocking").kinds,
    ["blocking", "ci-fuzz-status"],
    "blocking fuzz CI detected from the ci-fuzz-status target",
  );
});
...
test("T006 content: plugin behaviours and conventions block are tokenized", () => {
  ...
  assert.equal(
    JSON.stringify(block).includes("cyclomatic complexity <= 5"),
    false,
...
test("T006 content: methodology aggregator emits csm-planning, no-bdd and plan-gate-removed", () => {
  const records = aggregateMethodology([
    {
      category: "agent_workflow",
      matchedKey: "agent_workflow:csm-plan:.agents/plans/feature-csm.md",
      path: ".agents/plans/feature-csm.md",
    },
    {
      category: "methodology",
      matchedKey: "methodology:plan-gate-meta-test:tests/test_removed_plan_gate.py",
  ...
  assert.ok(
    records.some((record) => record.kind === "no-bdd"),
    "zero .feature files in a CSM-planned repo yields no-bdd",
  );
  assert.ok(
    records.some((record) => record.kind === "plan-gate-removed"),
    "meta-test presence certifies plan-gate removal",
  );

  const noPlans = aggregateMethodology([
    {
      category: "agent_workflow",
      matchedKey: "agent_workflow:agents:AGENTS.md",
      path: "AGENTS.md",
    },
  ]);
...
test("T006 scanner: content fixture emits suppression, ratchet, mutation, methodology and plugin facts", async () => {
  await withFixture("t006-content", CONTENT_FIXTURE_FILES, async (dir) => {
    ...
    const reasonsPolicy = contentEntry(
      findings,
      "quality_gate:suppression-policy:scripts/check_suppression_reasons.py",
    );
    assert.ok(reasonsPolicy, "suppression-policy entry for the reasons gate");
    assert.ok(
      reasonsPolicy.kinds.includes("owner-required") &&
        reasonsPolicy.kinds.includes("reason-required"),
    );

    const suppressionsPolicy = contentEntry(
      findings,
      "quality_gate:suppression-policy:scripts/check_suppressions.py",
    );
    assert.ok(suppressionsPolicy.kinds.includes("file-line-type-detail"));

    const baseline = contentEntry(
      findings,
      "quality_gate:suppression-baseline:quality/baselines/suppressions.json",
    );
    assert.equal(baseline.count, 2);

    const ratchet = contentEntry(findings, "quality_gate:ratchet-engine:scripts/_ratchet.py");
    assert.ok(
      ratchet.kinds.includes("fingerprint-diff") && ratchet.kinds.includes("update-baseline"),
    );

    const mutationExit = contentEntry(
      findings,
      "quality_gate:mutation-exit-code:scripts/mutation_policy.py",
    );
    ...
    const planGate = contentEntry(
      findings,
      "methodology:plan-gate-removed:tests/test_removed_plan_gate.py",
    );
    assert.ok(planGate, "plan-gate-removed aggregated");

    const conventionsBlock = contentEntry(
      findings,
      "agent_workflow:conventions-block:.opencode/plugins/pxcli-quality.ts",
    );
    assert.equal(conventionsBlock.count, 20);
    assert.ok(
      conventionsBlock.kinds.includes("british-english") &&
        conventionsBlock.kinds.includes("version-floors"),
    );

    const override = contentEntry(
      findings,
      "agent_workflow:quality-gate-override:.opencode/plugins/quality-gate.ts",
    );
    assert.deepEqual(override.kinds, ["override-env:opencode-disable-quality-gate"]);

    const prePush = contentEntry(
      findings,
      "agent_workflow:pre-push-docs-block:.opencode/plugins/pre-push-docs-check.ts",
    );
    assert.ok(prePush.kinds.includes("first-push-block"));

    const schedule = contentEntry(
      findings,
      "quality_gate:mutation-schedule:.github/workflows/mutation-scheduled.yml",
    );
...
test("T006 render: behaviour facts render inside their category groups", async () => {
  await withFixture("t006-render", CONTENT_FIXTURE_FILES, async (dir) => {
    ...
    assert.ok(
      markdown.includes("- **CSM planning**: `.agents/plans`: 1 plan"),
      "methodology behaviour line",
    );
    assert.ok(markdown.includes("- **BDD**: `.agents/plans`: `no-bdd`"), "no-bdd behaviour line");
    assert.ok(
      markdown.includes(
        "- **Suppression baseline**: `quality/baselines/suppressions.json`: 2 identities",
      ),
      "suppression baseline renders with correct plural",
    );
    assert.ok(
      markdown.includes(
        "- **Mutation exit codes**: `scripts/mutation_policy.py`: `clean-0`, `findings-1`, `tool-error-2`",
      ),
      "mutation exit codes render",
    );
    assert.ok(
      markdown.includes(
        "- **Enforced conventions block**: `.opencode/plugins/pxcli-quality.ts`: 20 rules:",
      ),
      "enforced conventions block renders",
    );
    assert.ok(
      markdown.includes("`british-english`") && markdown.includes("`version-floors`"),
      "tokenized rule facts render",
    );
    assert.equal(
      markdown,
      renderPractices("repository", findings),
      "behaviour rendering stays deterministic",
    );
  });
});
