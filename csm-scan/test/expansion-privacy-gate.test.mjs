...
const CANARIES = Object.freeze([
  ...
  'ghp_privacy_fixture_token_88', // GitHub PAT shape
...
test('T227 privacy: the T206/T224 primitives reject and redact every canary class', () => {
  ...
  for (const trigger of PRIMITIVE_TRIGGERS) {
    ...
    assert.ok(
      !CANARIES.some((canary) => canary.length > 2 && trigger.includes(canary) && sanitized.includes(canary)),
      `sanitizeText must remove the canary embedded in ${trigger}`,
    );
    const redacted = redactText(trigger);
    assert.ok(!CANARIES.some((canary) => canary.length > 2 && trigger.includes(canary) && redacted.includes(canary)),
...
test('T227 privacy: zero leaks across every sink in one combined run', async () => {
  ...
  try {
    ...
    const reporter = createReporter({
      out: { write: (chunk) => { captured.push(String(chunk)); return true; } },
      err: { write: (chunk) => { captured.push(String(chunk)); return true; } },
    });
    const result = await runExpandedPipeline({
      repos: [gitRepo],
      out: join(outDir, 'NORMS.md'),
