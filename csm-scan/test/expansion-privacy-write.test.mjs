...
test("T005 write path renders relative Path, basename gitRoot, redacted description, and no script bodies", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      false,
      "NORMS.md must not contain the fixture root absolute path",
    );
    ...
    assert.equal(
      markdown.includes(DESCRIPTION_TOKEN),
      false,
      "the description token must be redacted",
    );
  ...
  try {
    ...
    assert.equal(
      markdown.includes(gitRepo),
      false,
      "NORMS.md must not contain the git fixture root absolute path",
    );
    assert.equal(
      markdown.includes(SCRIPT_BODY),
      false,
      "script bodies must not render in a git repository either",
    );
  } finally {
  ...
  try {
    ...
    for (const token of [
      ...
      assert.equal(
        ...
        false,
        `structured-channel token must be redacted: ${token}`,
      );
    }
    ...
    assert.match(
      markdown,
      /- `src\/leak\.js`: `import crypto from 'node:crypto'; \/\/ \[redacted\]`/,
    );
  } finally {
    cleanupFixture(repo);
  }
