...
const REGISTRY_SHORTS = DIMENSION_REGISTRY.map(({ id }) =>
  ...
  test(`T204 ${c.name} fixture: expanded pipeline projects the ten legacy dimensions onto the established baseline`, async () => {
    await withFixture(`t204-${c.name}`, c.files, async (repoPath) => {
      ...
      assert.equal(
        ...
        `${c.name}: reused legacy mirror no longer matches the baseline`,
      );
      assert.equal(
        old.markdownSha256,
      ...
      try {
        ...
        const legacyDeep = result.repos[0].deep.filter(({ dimension }) =>
          EXISTING_TEN_DIMENSIONS.includes(dimension),
        );
        const projected = await DEFAULT_SINK(
          {
            generated: "2026-01-01",
            repos: [{ overview: result.repos[0].overview, deep: legacyDeep }],
          },
        ...
        assert.equal(
          projected,
          old.markdown,
          `${c.name}: expanded legacy projection must equal the legacy mirror`,
        );
        assert.equal(
        ...
        assert.deepEqual(
          crossObs,
          result.semantic[0].validated.contradictions,
          `${c.name}: cross-observations wiring mismatch`,
        );
      } finally {
        await rm(root, { recursive: true, force: true });
      }
...
test("T204 pipeline dispatches each registered dimension exactly once in canonical order without retry", async () => {
  await withFixture("t204-once", pythonFiles, async (repoPath) => {
    ...
    assert.deepEqual(
      initial.map((entry) => entry.dimension),
      REGISTRY_SHORTS,
    );
    const counts = new Map();
    for (const entry of initial)
      counts.set(entry.dimension, (counts.get(entry.dimension) || 0) + 1);
...
test("T204 retry loop caps at two iterations when coverage cannot recover", async () => {
  ...
  assert.ok(
    validated.needsRetry.includes("config"),
    "config must remain below the retry threshold",
  );
});
  ...
  await withFixture("t204-runner", pythonFiles, async (repoPath) => {
    ...
    for (const call of calls) {
      assert.ok(
        ...
        `unexpected executable ${call.executable}`,
      );
...
test("T204 pipeline preserves single and multi-repo renderer semantics", async () => {
  ...
  await withFixture("t204-multi-a", pythonFiles, async (a) => {
    await withFixture("t204-multi-b", rustFiles, async (b) => {
      ...
      assert.equal(
        result.trace.filter((entry) => entry.repoIndex === 0 && entry.phase === "initial").length,
        REGISTRY_SHORTS.length,
      );
      assert.equal(
        result.trace.filter((entry) => entry.repoIndex === 1 && entry.phase === "initial").length,
        REGISTRY_SHORTS.length,
      );
      ...
      try {
        ...
        for (const repo of legacyRepos) {
          assert.ok(
            markdown.includes(repo.overview.name),
            `${repo.overview.name} must be rendered`,
          );
        }
      } finally {
        await rm(root, { recursive: true, force: true });
