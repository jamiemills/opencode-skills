...
function assertArchitectureParity(architecture, findings, facts) {
  ...
  assert.deepEqual(
    ...
    findings.modules.slice().toSorted(),
    "module paths",
  );
  for (const entry of modules) {
    ...
    assert.equal(
      entry.details.selfLoop,
      facts.selfLoops.includes(entry.path),
      `selfLoop ${entry.path}`,
    );
    assert.equal(entry.sourceKind, "source");
  }

  const entryPoints = architecture.observations.filter(
    ({ category }) => category === "entry_point",
  );
  assert.deepEqual(
    entryPoints.map(({ path }) => path),
    (findings.layers.entryPoints || []).slice().toSorted(),
    "entry-point paths",
  );
  ...
  assert.ok(
    edges.every(({ sourceKind }) => sourceKind === "source"),
    "import-edge source kind",
  );

  const indicators = architecture.observations.filter(
    ({ category }) => category === "dynamic_indicator",
  );
  assert.deepEqual(
    indicators.map(({ path, details }) => ({
      file: path,
      kind: details.kind,
  ...
  assert.equal(
    fan.details.filesWithOutboundEdges,
    fanOutValues.filter((value) => value > 0).length,
  );
...
function assertDocumentationParity(documentation, findings, repoPath) {
  ...
  assert.deepEqual(license.details, {
    present: findings.license.present,
    name: findings.license.name,
  });
  ...
  assert.deepEqual(changelog.details, {
    present: findings.changelog.present,
    format: findings.changelog.format,
  });
  ...
  assert.deepEqual(
    adrs.details.paths,
    findings.adrs.map(({ path }) => path),
  );
  assert.equal(
    adrs.details.count,
    findings.adrs.reduce((sum, { count }) => sum + count, 0),
  );
...
test("T219 parity: five-ecosystem snapshot mirrors built-in architecture findings", async () => {
  for (const [name, files] of FIVE_ECOSYSTEM_FIXTURES) {
    ...
    assertArchitectureParity(
      providerFor(results, "DIM-architecture-v1"),
...
test("T219 parity: five-ecosystem snapshot mirrors built-in documentation findings", async () => {
  for (const [name, files] of FIVE_ECOSYSTEM_FIXTURES) {
    ...
    assertDocumentationParity(
      providerFor(results, "DIM-documentation-v1"),
...
test("T219 parity: practices adapter maps synthetic model entries to category-safe observations", () => {
  ...
  assert.deepEqual(observations.map(({ category }) => category).toSorted(), [
    "enforcement",
    "methodology",
    "quality_gate",
  ]);
...
test("T219 practices: bounded matched keys stay within the foundation bound", () => {
  ...
  assert.match(
    observations[0].matchedKey,
    /^[A-Za-z0-9][A-Za-z0-9._:/#@+%()[\],-]*$/,
    "matched key token pattern",
  );
});
...
test("T219 deterministic: insertion-order changes never alter observations or results", () => {
  ...
  const left = architectureObservations({
    findings: { modules, layers: { entryPoints: ["m.js"] }, importGraph: { graph } },
    facts,
  });
...
test("T219 unique IDs: repeated catalog results carry unique, stable provider/dimension pairs", async () => {
  ...
  assert.deepEqual(
    results.map(({ providerId, dimensionId }) => providerId + dimensionId),
...
test("T219 plugin: matches convert to bounded artifact observations on the analysis dimensions only", () => {
  ...
  assert.deepEqual(
    ...
    ["DIM-architecture-v1", "DIM-documentation-v1"],
  );
...
test("T219 plugin: observations are appended after built-in and never replace or rewrite findings", () => {
  const builtin = builtinResult(ANALYSIS_PROVIDER_IDS.architecture, "DIM-architecture-v1", [
    {
      category: "module",
      path: "src/a.js",
      matchedKey: "module:src/a.js",
      details: { fanIn: 1, fanOut: 0, selfLoop: false },
      sourceKind: "source",
    },
    {
      category: "import_edge",
      path: "src/a.js",
      matchedKey: "import-edge:src/a.js:src/b.js",
      details: { target: "src/b.js" },
      sourceKind: "source",
    },
  ]);
  const plugin = analysisPluginProviderResults({
    matches: [pluginMatch({ category: "dynamic_indicator", path: "src/a.js" })],
  })[0];
  ...
  assert.equal(
    result.providerId,
    ANALYSIS_PROVIDER_IDS.architecture,
    "provider identity is the built-in",
  );
  assert.equal(
    result.observations.length,
    3,
    "built-in module + built-in edge + plugin dynamic indicator",
  );
  assert.deepEqual(
    result.observations.slice(0, 2).map(({ category }) => category),
    ["import_edge", "module"],
  );
  assert.equal(result.observations[2].category, "dynamic_indicator");
  assert.deepEqual(
    result.observations[0].details,
    { target: "src/b.js" },
    "built-in edge is untouched",
  );
  ...
  const plugin = builtinResult(ANALYSIS_PLUGIN_PROVIDER_ID, "DIM-documentation-v1", [
    {
      ...
      path: null,
      matchedKey: "readme",
      details: { present: true, plugin: true },
      sourceKind: "artifact_metadata",
    },
  ]);
...
test("T219 plugin: dimension mismatch and duplicate/unknown categories fail with typed errors", () => {
  ...
  assert.deepEqual(
    analysisPluginProviderResults({
      matches: [pluginMatch({ dimensionId: "DIM-api-v1", category: "route" })],
    }),
    [],
  );
  assert.deepEqual(
...
test("T219 generic: unknown-language repositories receive documentation artifacts only", () => {
  ...
  for (const forbidden of [
    ...
    assert.equal(
      serialized.includes(`"category":"${forbidden}"`),
      false,
      `generic claimed ${forbidden}`,
    );
  }
...
test("T219 cap: observation lists are bounded and truncation is disclosed", () => {
  ...
  const { results, capped } = analysisProviderResults({
    architecture: {
      findings: { modules, layers: { entryPoints: [] }, importGraph: { graph } },
      facts,
    },
  });
  ...
  for (const matchedKey of [
    ...
    assert.ok(
      results[0].observations.some(({ matchedKey: key }) => key === matchedKey),
      `${matchedKey} survives`,
    );
  }
...
test("T219 matchedKey bound: long-path import edges and dynamic indicators assemble without aborting", () => {
  ...
  assert.equal(
    edge.matchedKey,
    edgeRaw.slice(0, 128),
    "import-edge matchedKey is final-truncated at the 128 bound",
  );
  assert.ok(edge.matchedKey.length <= 128, "import-edge matchedKey is bounded");
  assert.match(
    edge.matchedKey,
    /^[A-Za-z0-9][A-Za-z0-9._:/#@+%()[\],-]*$/,
    "import-edge matchedKey stays within the foundation pattern",
  );
  assert.equal(edge.path, longA, "import-edge path keeps the full source");
  assert.equal(
    edge.details.target,
    longB,
    "import-edge details keep the full target (truncation disclosure)",
  );

  const indicator = architecture.observations.find(
    ({ category }) => category === "dynamic_indicator",
  );
  ...
  assert.equal(
    indicator.matchedKey,
    indicatorRaw.slice(0, 128),
    "dynamic-indicator matchedKey is final-truncated at the 128 bound",
  );
  assert.ok(indicator.matchedKey.length <= 128, "dynamic-indicator matchedKey is bounded");
  assert.match(
    indicator.matchedKey,
    /^[A-Za-z0-9][A-Za-z0-9._:/#@+%()[\],-]*$/,
    "dynamic-indicator matchedKey stays within the foundation pattern",
  );
  assert.equal(indicator.path, longA, "dynamic-indicator path keeps the full file");
  assert.equal(
    indicator.details.kind,
    kind,
    "dynamic-indicator details keep the full kind (truncation disclosure)",
  );
  assert.equal(
    indicator.details.line,
    12345,
    "dynamic-indicator details keep the line (truncation disclosure)",
  );
  ...
  assert.equal(
    ...
    "long-path assembly is deterministic",
  );
...
test("T219 immutability: catalog results, observations, and details are deep-frozen", async () => {
  ...
  for (const result of results) {
    ...
    assert.throws(() => {
      result.observations[0].matchedKey = "x";
    }, TypeError);
    assert.throws(() => {
      result.observations[0].details.fanIn = 99;
    }, TypeError);
  }
  ...
  const merged = mergeAnalysisResults({
    builtin: [builtinResult(ANALYSIS_PROVIDER_IDS.architecture, "DIM-architecture-v1", [])],
    plugin,
  });
...
test("T219 empty input: foreign and null models produce empty results without throwing", () => {
  ...
  assert.deepEqual(architectureProviderResults({ findings: null, facts: null }), {
    results: [],
    capped: false,
  });
...
test("T219 inert: the catalog module is a pure data adapter with no execution surface", async () => {
  const source = await readFile(
    join(LIB_ROOT, "lib", "scan", "providers", "analysis-catalog.mjs"),
    "utf8",
  );
  for (const forbidden of [
    "node:fs",
  ...
  assert.equal(
    (source.match(/export\s+async\s+function/g) || []).length,
    0,
    "catalog exports no async functions",
  );
});
