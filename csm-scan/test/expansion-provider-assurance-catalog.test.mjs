...
function assertCanonicalOrder(observations) {
  assert.deepEqual(
    ...
    observations
      ...
      .map(({ category, path, matchedKey }) => ({ category, path, matchedKey })),
...
function assertValidObservation(observation, dimensionId) {
  ...
  assert.ok(
    observation.matchedKey.length <= 128,
    `${dimensionId} matchedKey exceeds the 128 bound`,
  );
  assert.match(
    observation.matchedKey,
    FOUNDATION_KEY_PATTERN,
    `${dimensionId} matchedKey is not bounded stable ASCII`,
  );
}
...
async function scanAll(repoPath, name) {
  const overview = {
    files: Object.keys(FIVE_ECOSYSTEM_FIXTURES.find(([fixtureName]) => fixtureName === name)[1]),
    languages: [name],
  };
...
test("T220 registry: versioned, sorted, deep-frozen, and duplicate-free", () => {
  ...
  assert.ok(
    ASSURANCE_CATALOG_PROVIDERS.every(
  ...
  assert.equal(
    JSON.stringify(ASSURANCE_CATALOG_PROVIDERS),
    JSON.stringify(ASSURANCE_CATALOG_PROVIDERS),
  );
});
...
test("T220 registry: every dimension/category is allowlisted and unique", () => {
  ...
  for (const provider of ASSURANCE_CATALOG_PROVIDERS) {
    ...
    for (const { dimensionId, categories } of provider.dimensions) {
      assert.ok(
        ASSURANCE_DIMENSION_IDS.includes(dimensionId) ||
          ["DIM-maintainability-v1", "DIM-assurance-v1", "DIM-documentation-v1"].includes(
            dimensionId,
          ),
...
test("T220 catalog set: all 15 provider dimensions are represented exactly once", () => {
  const runtimeBuiltIn = new Set(
    RUNTIME_CATALOG_PROVIDERS.filter(({ id }) => id !== GENERIC_PROVIDER_ID).flatMap(
      ({ dimensions }) => dimensions.map(({ dimensionId }) => dimensionId),
    ),
  );
  const analysisBuiltIn = new Set([
    "DIM-architecture-v1",
    "DIM-conventions-v1",
    "DIM-documentation-v1",
    "DIM-practices-v1",
  ]);
  const assuranceBuiltIn = new Set(
    ASSURANCE_CATALOG_PROVIDERS.filter(({ id }) => id !== GENERIC_PROVIDER_ID).flatMap(
      ({ dimensions }) => dimensions.map(({ dimensionId }) => dimensionId),
    ),
  );

  assert.deepEqual([...runtimeBuiltIn].toSorted(), [
    "DIM-config-v1",
    "DIM-stack-v1",
    "DIM-testing-v1",
  ]);
  ...
  assert.equal(
    new Set(all).size,
    all.length,
    "a provider dimension is covered by more than one catalog",
  );
  assert.deepEqual(
    [...new Set(all)].toSorted(),
    [...PROVIDER_DIMENSION_IDS].toSorted(),
    "the three catalogs must cover all 15 provider dimensions exactly once",
  );
});

const genericCoverage = (registry) =>
  registry
...
test("T220 catalog set: provider ids are globally unique with a single shared generic fallback", () => {
  ...
  const analysisIds = new Set([
    ...Object.values(ANALYSIS_PROVIDER_IDS),
    ANALYSIS_PLUGIN_PROVIDER_ID,
  ]);

  const nonGeneric = [...runtimeIds, ...analysisIds, ...assuranceIds].filter(
    (id) => id !== GENERIC_PROVIDER_ID,
  );
  assert.equal(
    new Set(nonGeneric).size,
    nonGeneric.length,
  ...
  assert.deepEqual(genericCoverage(RUNTIME_CATALOG_PROVIDERS), [
    "DIM-assurance-v1",
    "DIM-documentation-v1",
    "DIM-maintainability-v1",
  ]);
  assert.deepEqual(
    genericCoverage(ASSURANCE_CATALOG_PROVIDERS),
    genericCoverage(RUNTIME_CATALOG_PROVIDERS),
...
const EXPECTED_SECURITY = [
  {
    ...
    details: { package: "passport", label: "Passport", type: null },
    matchedKey: "auth:passport",
    path: null,
    sourceKind: "manifest",
  },
  {
  ...
  },
  {
    ...
    details: { pattern: "AWS Access Key", totalFiles: 1 },
    matchedKey: keyFor("secret", "AWS Access Key"),
    path: "src/creds.js",
    sourceKind: "search_result",
  },
  {
    category: "security_tool",
    details: { tool: "snyk", source: "dependency", location: "manifest" },
    matchedKey: keyFor("audit-tool", "snyk:dependency:manifest"),
    path: null,
  ...
  {
    category: "security_tool",
    details: { name: ".gitleaks.toml" },
    matchedKey: "security-tool:.gitleaks.toml",
    path: null,
    sourceKind: "config",
  },
  {
    category: "security_tool",
    details: { name: "bandit" },
    matchedKey: "security-tool:bandit",
    path: null,
    sourceKind: "manifest",
  },
  {
    category: "validation",
    details: { codeReferences: 2 },
  ...
  {
    category: "validation",
    details: { package: "express-rate-limit", label: "Express Rate Limit", control: "rate_limit" },
    matchedKey: "rate-limit:express-rate-limit",
    path: null,
    sourceKind: "manifest",
  },
  {
    category: "validation",
    details: { package: "joi", label: "Joi" },
    matchedKey: "validation:joi",
    path: null,
    sourceKind: "manifest",
  },
];
...
const EXPECTED_OPERATIONS = [
  ...
  },
  {
    ...
    matchedKey: "dockerfile:Dockerfile",
    path: "Dockerfile",
    sourceKind: "container",
  },
  {
    category: "deployment_declaration",
    details: { networks: ["front"], volumes: ["data"] },
  ...
  {
    category: "deployment_declaration",
    details: { file: "compose.yaml", serviceCount: 1, services: ["web"] },
    matchedKey: "compose:compose.yaml",
    path: "compose.yaml",
    sourceKind: "config",
  },
  {
    category: "health_check",
    details: { detected: true, references: ["src/server.js"] },
  ...
  {
    category: "health_check",
    details: { pattern: "SIGTERM handler", fileCount: 1 },
    matchedKey: keyFor("shutdown", "SIGTERM handler"),
    path: null,
    sourceKind: "search_result",
  },
  {
    category: "monitoring",
    details: { package: "@sentry/node", label: "Sentry" },
    matchedKey: "monitoring:@sentry/node",
    path: null,
    sourceKind: "manifest",
  },
  {
    category: "workflow",
    details: { platform: "GitHub Actions", jobs: ["build"], triggers: ["push"], workflowCount: 1 },
    matchedKey: keyFor("workflow", "GitHub Actions"),
    path: null,
    sourceKind: "workflow",
  },
];
  ...
  operations: [
    {
      ...
      matchedKey: "route:GET:/api/users",
      id: "op-fixed",
    },
  ],
  diagnostics: [],
  ...
  entries: [
    {
      category: "policy",
      dialect: "governance",
      matchedKey: "policy:GOVERNANCE.md",
      path: "GOVERNANCE.md",
      status: "observed",
      details: { kind: "governance" },
  ...
  manifest: [
    {
      ...
      matchedKey: "manifest:package_json",
      id: "asc-fixed",
    },
  ],
  lock: [],
...
test("T220 parity: security adapter projects scanner findings exactly", () => {
  ...
  assert.deepEqual(
    raw.slice().toSorted(canonicalOrder),
    EXPECTED_SECURITY,
    "raw adapter observation set matches",
  );
  ...
  assert.deepEqual(
    result.observations,
    EXPECTED_SECURITY,
    "result observations are canonically ordered",
  );
  assert.equal(
    JSON.stringify(result.observations),
    JSON.stringify(EXPECTED_SECURITY),
  ...
  const cases = [
    {
      ...
      expected: [
        {
          category: "route",
          details: { signature: "GET:/api/users", dialect: "express", status: "observed" },
          matchedKey: "route:GET:/api/users",
          path: "src/routes.js",
          sourceKind: "source",
        },
    ...
    },
    {
      ...
      expected: [
        {
          category: "entity",
          details: { signature: "User", dialect: "prisma", status: "observed" },
          matchedKey: "entity:User",
          path: "schema.prisma",
          sourceKind: "schema",
        },
    ...
    },
    {
      ...
      expected: [
        {
          category: "image",
          details: { reference: "nginx:1.25", scope: "container", line: 1 },
          matchedKey: "image:nginx:1.25",
          path: "Dockerfile",
          sourceKind: "container",
        },
        {
          category: "service",
          details: { id: "service@web", kind: "service", label: "web", image: "nginx:1.25" },
          matchedKey: "service:service@web",
          path: "compose.yaml",
          sourceKind: "config",
        },
        {
          category: "template_indicator",
          details: { kind: "dynamic", count: 1 },
          matchedKey: "indicator:dynamic",
          path: null,
        ...
        },
        {
          ...
          matchedKey: "edge:service@web:service@db:depends_on",
          path: "compose.yaml",
          sourceKind: "config",
        },
      ],
      ...
      expected: [
        {
          ...
          matchedKey: "branch-point:src/a.js",
          path: "src/a.js",
          sourceKind: "source",
        },
        {
          ...
          matchedKey: "file-metric:src/a.js",
          path: "src/a.js",
          sourceKind: "source",
        },
        {
      ...
      expected: [
        {
          category: "ownership",
          details: { count: 1 },
          matchedKey: "assignee:Owner-1",
          path: null,
          sourceKind: "policy",
        },
        {
          category: "ownership",
          details: { pattern: "src/", anchored: false, labels: ["Owner-1"], line: 1 },
          matchedKey: "rule:CODEOWNERS:src/",
          path: "CODEOWNERS",
          sourceKind: "policy",
        },
        {
          category: "policy",
          details: { dialect: "governance", status: "observed" },
          matchedKey: "policy:GOVERNANCE.md",
          path: "GOVERNANCE.md",
          sourceKind: "policy",
        },
    ...
    },
    {
      ...
      expected: [
        {
          category: "manifest",
          details: { status: "observed", ecosystem: "javascript", format: "package_json" },
          matchedKey: "manifest:package_json",
          path: "package.json",
          sourceKind: "manifest",
        },
...
test("T220 generic: known ecosystems keep built-in mode", () => {
  assert.equal(
    assuranceCatalogResults({ languages: ["Python"], ecosystems: ["python"], files: [] }).mode,
    "builtin",
  );
  assert.equal(
    assuranceCatalogResults({ manifestEcosystems: ["rust"], files: [] }).mode,
    "builtin",
  );
});
...
test("T220 generic: more than 2048 distinct extensions thread the capped flag through the envelope", () => {
  ...
  assert.equal(
    ...
    "file_metric observations are truncated after the universe record",
  );
...
test("T220 plugin: declarative matches convert to bounded artifact observations on the catalog dimensions only", () => {
  const matches = [
    ...
    {
      ruleId: "RUL-api-v1",
      label: "Zeta api marker",
      dimensionId: "DIM-api-v1",
      category: "route",
      path: "src/zeta.js",
    },
    {
      ruleId: "RUL-ignored-v1",
      label: "Ignored",
  ...
  assert.deepEqual(
    groups.map(({ dimensionId }) => dimensionId),
    ["DIM-api-v1", "DIM-security-v1"],
  );
  ...
  assert.deepEqual(
    results.map(({ dimensionId }) => dimensionId),
    ["DIM-api-v1", "DIM-security-v1"],
  );
...
test("T220 plugin: observations are appended after built-in and never replace or rewrite findings", () => {
  ...
  const plugin = assurancePluginResult("DIM-security-v1", [
    {
      category: "security_tool",
      path: null,
      matchedKey: "plugin-rule:RUL-zeta-v1",
      details: { ruleId: "RUL-zeta-v1", label: "Zeta tool" },
      sourceKind: "artifact_metadata",
    },
  ]);
  ...
  const builtinLock = builtin.observations.find(
    ({ matchedKey }) => matchedKey === "dependency-lock",
  );
...
test("T220 plugin: exact duplicate plugin observations are dropped and never rewrite built-ins", () => {
  ...
  const plugin = assurancePluginResult("DIM-operations-v1", [
    duplicate,
    {
      category: "monitoring",
      path: null,
      matchedKey: "plugin-rule:RUL-mon-v1",
      details: { ruleId: "RUL-mon-v1", label: "Zeta monitoring" },
      sourceKind: "artifact_metadata",
    },
  ]);
  const merged = mergeAssurancePlugin({ builtin, plugin });
  assert.equal(
    merged.observations.filter(({ matchedKey }) => matchedKey === "workflow:ci").length,
    1,
...
test("T220 plugin: assuranceCatalogResults merges per-dimension plugin observations", () => {
      ...
      "DIM-security-v1": [
        {
          category: "authentication",
          path: null,
          matchedKey: "plugin-auth:zeta",
          details: { package: "zeta-auth", label: "Zeta" },
          sourceKind: "artifact_metadata",
        },
      ],
    },
  ...
  const authIdx = securityResult.observations.findIndex(
    ({ matchedKey }) => matchedKey === "plugin-auth:zeta",
  );
  assert.ok(authIdx >= 0, "plugin observation contributed");
  assert.ok(
    securityResult.observations
      .slice(0, authIdx)
      .some(({ matchedKey }) => matchedKey === "dependency-lock"),
...
test("T220 plugin: empty built-in findings still emit a plugin-only provider result", () => {
      ...
      "DIM-api-v1": [
        {
          ...
          details: { ruleId: "RUL-zeta-v1", label: "Zeta route" },
          sourceKind: "artifact_metadata",
        },
      ],
    },
  ...
  assert.equal(
    apiResult.providerId,
    ASSURANCE_PLUGIN_PROVIDER_ID,
    "plugin-only result uses the plugin carrier id, never the built-in id",
  );
  assert.deepEqual(
    apiResult.observations.map(({ matchedKey }) => matchedKey),
    ["plugin-route:zeta"],
  );
  assert.ok(
    !providerFor(envelope.results, "DIM-security-v1"),
    "empty built-in never fabricates a security result",
  );
  assert.ok(
    !providerFor(envelope.results, "DIM-assurance-v1"),
    "dimensions with no built-in and no plugin are absent",
  );
});
...
test("T220 plugin: unknown and duplicate categories and dimension mismatch fail with typed errors", () => {
  ...
  const security = assurancePluginProviderResults({
    matches: [
      {
        ruleId: "RUL-sec-v1",
        label: "L",
...
test("T220 cap: observation lists are bounded and truncation is threaded through the catalog", () => {
  ...
  for (let index = 0; index < count; index++) {
    entities.push({
      ...
      matchedKey: `entity:Entity${index}`,
      id: `rec-${index}`,
    });
  }
...
test("T220 matchedKey bound: long scanner values assemble without aborting and stay within 128 chars", () => {
  ...
  const longFindings = {
    ...
    secrets: {
      count: 1,
      findings: [{ pattern: longPattern, files: ["src/creds.js"], totalFiles: 1 }],
    },
  };
  ...
  assert.equal(
    secret.matchedKey,
    keyFor("secret", longPattern),
    "unsafe long value is deterministically hashed",
  );
  ...
  assert.equal(
    JSON.stringify(again.results),
    JSON.stringify(second.results),
    "long-value assembly is deterministic",
  );
  for (const result of again.results) {
    for (const observation of result.observations) {
      assert.ok(
        observation.matchedKey.length <= 128,
        `${result.dimensionId} matchedKey is bounded`,
      );
      assert.match(
        observation.matchedKey,
        FOUNDATION_KEY_PATTERN,
        `${result.dimensionId} matchedKey pattern`,
      );
    }
  }
});
...
test("T220 immutability: catalog and every produced result are deep-frozen", () => {
  ...
  assert.throws(() => {
    ASSURANCE_CATALOG_PROVIDERS[0].dimensions.pop();
  }, TypeError);
  assert.throws(() => {
    ASSURANCE_CATALOG_PROVIDERS[0].id = "PRV-mutated-v1";
  }, TypeError);
  ...
  assert.throws(() => {
    security.observations[0].details.pattern = "mutated";
  }, TypeError);
  assert.throws(() => {
    security.dimensionId = "DIM-api-v1";
  }, TypeError);
  ...
  assert.throws(() => {
    api[0].observations.pop();
  }, TypeError);

  const plugin = assurancePluginProviderResults({
    matches: [
      {
        ruleId: "RUL-x-v1",
        label: "L",
...
test("T220 inert: catalog avoids T201 forbidden capabilities on import", async () => {
  const source = await readFile(
    join(LIB_ROOT, "scan", "providers", "assurance-catalog.mjs"),
    "utf8",
  );
