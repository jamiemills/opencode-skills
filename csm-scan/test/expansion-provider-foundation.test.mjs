...
import { manifestObservations, manifestProviderResult } from "../lib/scan/shared/manifest.mjs";
import {
  DATABASE_INDICATORS,
  matchDep,
...
test("T210 base: provider results are deep-frozen and immutable", () => {
  ...
  assert.throws(() => {
    result.observations[0].details.id = "mutated";
  }, TypeError);
  assert.throws(() => {
...
test("T210 base: identical inputs are byte-identical regardless of insertion order", () => {
  ...
  assert.deepEqual(
    first.observations.map(({ matchedKey }) => matchedKey),
...
test("T210 base: explicit per-dimension category constraints reject unknown and cross-dimension categories", () => {
  for (const dimensionId of PROVIDER_DIMENSION_IDS) {
    ...
    for (const category of allowed) {
      assert.doesNotThrow(
        () => resultFor(dimensionId, [observation({ category })]),
        `${dimensionId}:${category}`,
      );
    }
  }
  assert.throws(
    () => resultFor("DIM-stack-v1", [observation({ category: "route" })]),
    (error) => error instanceof ProviderResultError && error.code === "UNKNOWN_CATEGORY",
  );
  assert.throws(
    () => resultFor("DIM-api-v1", [observation({ category: "language" })]),
    (error) => error instanceof ProviderResultError && error.code === "UNKNOWN_CATEGORY",
  );
  assert.throws(
    () => resultFor("DIM-stack-v1", [observation({ category: "arbitrary" })]),
    ProviderResultError,
  );
});
...
test("T210 base: duplicate observations and unknown/unsafe identities are rejected", () => {
  assert.throws(
    () => resultFor("DIM-stack-v1", [observation(), observation()]),
    (error) => error instanceof ProviderResultError && error.code === "DUPLICATE_OBSERVATION",
  );
  assert.throws(
    () => resultFor("DIM-stack-v1", [observation({ matchedKey: "../escape" })]),
    ProviderResultError,
  );
  assert.throws(
    () => resultFor("DIM-stack-v1", [observation({ matchedKey: "a b" })]),
    ProviderResultError,
  );
  assert.throws(
    () =>
...
test("T210 base: paths are normalized repository-relative and source kinds are allowlisted", () => {
  for (const bad of [
    ...
    assert.throws(
      () => resultFor("DIM-stack-v1", [observation({ path: bad })]),
      ProviderResultError,
      bad,
    );
  }
  ...
  assert.throws(
    () => resultFor("DIM-stack-v1", [observation({ sourceKind: "not-a-kind" })]),
    ProviderResultError,
  );
...
test("T210 base: non-data details and unbounded inputs fail with typed safe errors", () => {
  for (const details of [{ run() {} }, /regex/u, new Map(), Promise.resolve()]) {
    assert.throws(
      () => resultFor("DIM-stack-v1", [observation({ details })]),
  ...
  assert.throws(
    () => resultFor("DIM-stack-v1", observations),
...
test("T210 base: merge places built-in first and appends plugin observations only", () => {
  const builtin = resultFor("DIM-stack-v1", [
    observation(),
    observation({
      matchedKey: "runtime:Node.js",
      category: "runtime",
      details: { name: "Node.js" },
    }),
  ]);
  ...
  assert.deepEqual(
    merged.observations.map(({ matchedKey }) => matchedKey),
...
test("T210 base: merge rejects dimension mismatch and revalidates categories", () => {
  ...
  assert.throws(
    () => mergeProviderResults({ builtin, plugin: other }),
...
test("T210 rules: artifact metadata is normalized and bounded, with no path escape", () => {
  ...
  for (const bad of ["/abs", "../x", "a/../../b", "C:/x", "src\\x", "a//b"]) {
    assert.throws(
      () => validateArtifactMetadata({ path: bad, size: 0, content: "" }),
      RuleEvaluationError,
      bad,
    );
  }
  assert.throws(
    () => validateArtifactMetadata({ path: "x", size: -1, content: "" }),
    RuleEvaluationError,
  );
  assert.throws(
    () => validateArtifactMetadata({ path: "x", size: 1.5, content: "" }),
    RuleEvaluationError,
  );
  const capped = validateArtifactMetadata({
    path: "x.txt",
    size: 1,
    content: "z".repeat(RULE_EVALUATION_LIMITS.contentBytes + 10),
  });
...
test("T210 rules: every selector matches its artifact positively", () => {
  ...
  assert.equal(
    evaluateRule(rule({ extensions: [".zeta"] }), artifact),
    true,
  ...
  assert.equal(
    evaluateRule(rule({ artifactTokens: ["config/zeta"] }), named),
    true,
    "artifactTokens",
  );
  assert.equal(
    evaluateRule(rule({ artifactTokens: ["config"] }), named),
    true,
    "artifactTokens dir prefix",
  );
  assert.equal(evaluateRule(rule({ literal: "runtime: 3.1" }), named), true, "literal");
  assert.equal(
    evaluateRule(rule({ regexSource: "^runtime:[ ]+[0-9.]+" }), named),
    true,
    "regexSource",
  );
});
...
test("T210 rules: negative cases do not match", () => {
  ...
  assert.equal(
    evaluateRule(rule({ artifactTokens: ["srcother"] }), artifact),
    false,
    "no loose prefix escape",
  );
...
test("T210 rules: selectors are OR alternatives within one rule", () => {
  ...
  assert.equal(
    evaluateRule(rule({ extensions: [".zeta"], basenames: ["Other"], literal: "x" }), artifact),
    false,
  );
});
...
test("T210 rules: malformed rules are rejected with typed errors and the fixed regex policy applies", () => {
  assert.throws(
    () => validateRuleForEvaluation(rule({ literal: "a", regexSource: "a" })),
  ...
  assert.throws(
    () => validateRuleForEvaluation(rule({ regexSource: "(a+)+b" })),
...
test("T210 rules: unvalidated catastrophic regex sources fail at evaluation before compilation", () => {
  ...
  for (const source of ["(a+)+b", "(a*)*x", "(a|aa)+b", "a*a*b"]) {
    assert.throws(
      () => evaluateRule(rule({ regexSource: source }), artifact),
      (error) => error instanceof RuleEvaluationError && error.code === "REGEX_COMPLEXITY",
      source,
    );
    assert.throws(
      () => evaluateRules({ rules: [rule({ regexSource: source })], artifacts: [artifact] }),
      (error) => error instanceof RuleEvaluationError && error.code === "REGEX_COMPLEXITY",
      source,
    );
  }
...
test("T210 rules: evaluation is bounded and deterministic with typed caps", () => {
  ...
  const floodArtifacts = Array.from({ length: 300 }, (_, index) => ({
    path: `f/${index}.x`,
    size: 1,
    content: "",
  }));
  ...
  assert.throws(
    () => evaluateRules({ rules: tooManyRules, artifacts: floodArtifacts }),
    RuleEvaluationError,
  );
});
...
test("T210 rules: regexes compile only from fixed-u policy-validated sources at evaluate time", () => {
  ...
  assert.equal(
    evaluateRule(rule({ regexSource: "^[A-Za-z][0-9]$" }), { ...artifact, content: "A1" }),
    true,
  );
  assert.equal(
    evaluateRule(rule({ regexSource: "^[A-Za-z0-9._-]+$" }), { ...artifact, content: "A1" }),
    true,
  );
  assert.throws(
    () => evaluateRule(rule({ regexSource: "\\p{L}+\\d" }), artifact),
    (error) => error instanceof RuleEvaluationError && error.code === "REGEX_COMPLEXITY",
  );
});
...
test("T210 generic: activation requires an unknown language and no known ecosystem", () => {
  ...
  assert.equal(
    isUnknownLanguageEcosystem({ languages: ["Zeta"], manifestEcosystems: ["python"] }),
    false,
  );
  ...
  for (const result of results) {
    for (const { category, details } of result.observations) {
      ...
      assert.ok(
        ...
        `unexpected generic detail field ${JSON.stringify(details)}`,
      );
    }
  }
});
...
test("T210 generic: no import, runtime, source-syntax, or first-class-depth claims", () => {
  ...
  const categories = results.flatMap(({ observations }) =>
...
test("T210 generic: metadata observations reflect path, extension, size, and directory", () => {
  ...
  const maintainability = results.find(
  ...
  assert.deepEqual(metrics, [
    {
      ...
      path: null,
      matchedKey: "file-metric:.md",
      details: { extension: ".md", count: 1, bytes: 50 },
      sourceKind: "file_metadata",
    },
    {
      category: "file_metric",
      path: null,
      matchedKey: "file-metric:.zzz",
      details: { extension: ".zzz", count: 2, bytes: 400 },
      sourceKind: "file_metadata",
    },
  ]);
  const universe = maintainability.observations.find(
    ({ matchedKey }) => matchedKey === "measurement-universe",
  );
...
test("T210 generic: manifest, lockfile, and known doc artifact observations carry paths", () => {
  ...
  assert.deepEqual(
    manifests.map(({ path, details }) => [path, details.name]),
    [["pyproject.toml", "pyproject.toml"]],
  );
  assert.deepEqual(
    locks.map(({ path, details }) => [path, details.name]),
...
test("T210 generic: does not fire for the five built-in ecosystems and empty input", () => {
  ...
  assert.deepEqual(genericProviderResults({ languages: ["Zeta"], files: [] }), {
    results: [],
    capped: false,
  });
...
test("T210 generic: deterministic and immutable envelope results; typed input errors", () => {
  ...
  for (const bad of [["/etc/passwd"], ["../x"], ["a/../../b"], ["C:/x"]]) {
    assert.throws(
      () => genericProviderResults({ languages: ["Zeta"], files: bad }),
...
test("T210 generic: long-extension file_metric matchedKeys stay bounded with the full extension disclosed", () => {
  ...
  assert.ok(
    metric.matchedKey.length <= GENERIC_LIMITS.matchedKey,
    "file-metric matchedKey is bounded",
  );
  assert.ok(
    metric.matchedKey.startsWith("file-metric:"),
    "file-metric matchedKey keeps its prefix",
  );
  assert.equal(
    metric.matchedKey,
  ...
  assert.equal(
    metric.details.extension,
    extension,
    "full extension stays in details (truncation disclosure)",
  );
  ...
  assert.equal(
    JSON.stringify({ results, capped }),
    JSON.stringify(second),
    "long-extension assembly is deterministic",
  );
});
...
test("T210 generic: caps file_metric observations at the declared bound with a disclosed flag", () => {
  ...
  assert.ok(
    maintainability.observations.every(
      ({ category }) => category === "file_metric" || category === "measurement_universe",
    ),
  );
  assert.deepEqual(maintainability.observations.map(({ matchedKey }) => matchedKey).slice(0, 2), [
    "file-metric:.ext000000",
    "file-metric:.ext000001",
  ]);
  assert.equal(
...
test("T210 parity: ecosystem adapter matches descriptor data for all five ecosystems", () => {
  for (const id of ECOSYSTEM_IDS) {
    ...
    assert.ok(
      results.every(({ providerId }) => providerId === `PRV-${id}-builtin-v1`),
      id,
    );
    const byDimension = new Map(
      results.map(({ dimensionId, observations }) => [dimensionId, observations]),
    );
    ...
    assert.deepEqual(
      stack
        .filter(({ category }) => category === "package_manager")
        .map(({ details }) => details.name)
        .toSorted(),
    ...
    assert.deepEqual(
      assurance
        .filter(({ category }) => category === "manifest")
        .map(({ details }) => details.name)
        .toSorted(),
...
test("T210 parity: manifest adapter matches normalized manifest fields", () => {
      ...
      observations: [
        {
          ...
          details: { name: "python", label: "Python" },
          sourceKind: "manifest",
        },
        {
          category: "runtime",
          path: null,
          matchedKey: "runtime:Python",
          details: { name: "Python", declared: ">=3.12" },
          sourceKind: "manifest",
        },
      ],
    },
  ...
  assert.deepEqual(jsRuntimes, [
    {
      ...
      path: null,
      matchedKey: "runtime:Node.js",
      details: { name: "Node.js", declared: ">=18.0.0" },
      sourceKind: "manifest",
    },
  ]);
  ...
  assert.deepEqual(
    rust[0].observations.filter(({ category }) => category === "runtime")[0].details,
    {
      name: "Rust",
      declared: "1.75",
    },
...
test("T210 parity: manifest adapter results are immutable and deterministic", () => {
  ...
  assert.ok(first.every(({ observations }) => Object.isFrozen(observations)));
  assert.deepEqual(
    first.map(({ providerId }) => providerId),
    ["PRV-manifest-v1"],
  );
});
...
test("T210 parity: detection adapter matches matchDep results for each supported category", () => {
  ...
  assert.deepEqual(monitoring.details, {
    name: "tracing",
    label: "tracing",
    type: "Logging/Tracing",
  });
  ...
  assert.deepEqual(securityTool.details, {
    name: "cargo-audit",
    label: "cargo-audit",
...
test("T210 parity: detection adapter emits only allowlisted provider categories", () => {
  const groups = detectionObservations({
    ecosystem: "javascript",
    deps: ["express-rate-limit", "zod", "winston", "axios", "prisma"],
  });
  const categories = groups.flatMap(({ observations }) =>
    observations.map(({ category }) => category),
  );
...
test("T210 parity: comment adapter is deterministic, immutable, and path-aware", () => {
  ...
  const second = commentProviderResult({
    ecosystem: "javascript",
    text: "// line\n/* block */\nx = 1",
    path: "src/index.js",
  });
  ...
  assert.throws(() => {
    first[0].observations[0].details.commentLines = 99;
  }, TypeError);
...
test("T210 inert: only the four shared contribution points import provider modules in production", async () => {
  ...
  for (const file of await libScanFiles()) {
    ...
    if (
      resolved.some((path) => path === providersRoot || path.startsWith(`${providersRoot}${sep}`))
    ) {
  ...
  for (const consumer of consumers) {
    ...
    assert.ok(
      inProviders || allowed.includes(consumer),
      `unexpected provider consumer outside the adapters: ${consumer}`,
    );
  }
  for (const adapter of allowed) {
    assert.ok(
      consumers.includes(adapter),
      `expected ${adapter} to expose a provider contribution point`,
    );
  }
});
...
test("T210 inert: provider modules export only pure factories, never scan/run/execute entry points", async () => {
  for (const [path, names] of [
    ...
    [
      "providers/rules.mjs",
      ["validateArtifactMetadata", "validateRuleForEvaluation", "evaluateRule", "evaluateRules"],
    ],
