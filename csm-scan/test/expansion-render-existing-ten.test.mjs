...
test('T205 unchanged write facade matches the pre-existing deterministic bytes with one write result', async () => {
  ...
  try {
    const content = await writeNORMS({ generated: '2026-01-15', repos: [{ overview, deep }] }, outPath);
  ...
  assert.equal(source.split("import { writeFile } from 'node:fs/promises';").length - 1, 1);
  assert.equal(source.split("await writeFile(outPath, content, 'utf-8');").length - 1, 1);
});

test('T205 base centralizes safe scalars, Markdown escaping, privacy, and LF finalization', () => {
  const seen = [];
  const context = createRenderContext({ privacyHook(value) { seen.push(value); return value === 'private' ? 'filtered' : value; } });
...
test('T205 existing-ten definitions are complete while partial findings preserve caller order', () => {
  ...
  const sections = renderer.render([
    ...
    { dimension: 'stack', findings: { runtime: 'Node', language: 'JS', framework: 'none', packageManager: 'npm' } },
  ]);
...
test('T205 injectable renderer definitions fail typed and sanitized', () => {
  ...
  assertRendererError(
    () => createExistingTenRenderer({ order: [...EXISTING_TEN_RENDERER_ORDER.slice(0, -1), 'structure'] }),
    'DUPLICATE_RENDERER',
  );
  assertRendererError(
    () => createExistingTenRenderer({ rendererMap: Object.entries(EXISTING_TEN_RENDERER_MAP).slice(0, -1) }),
    'MISSING_RENDERER',
  );
  assertRendererError(
    () => createExistingTenRenderer({ rendererMap: [...Object.entries(EXISTING_TEN_RENDERER_MAP), ['stack', () => '']] }),
    'DUPLICATE_RENDERER',
  );
  const renderer = createExistingTenRenderer();
  assertRendererError(
