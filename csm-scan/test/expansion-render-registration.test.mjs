...
function sixteenDeep() {
  return [
    { dimension: 'structure', signal: 'high', findings: { tree: '.', fileCounts: {}, totalFiles: 0 } },
    { dimension: 'stack', signal: 'high', findings: { runtime: 'Node.js (declared)', language: 'JavaScript', framework: '(none)', packageManager: 'npm' } },
    { dimension: 'config', signal: 'high', findings: {} },
    { dimension: 'testing', signal: 'high', findings: { framework: [], fileCount: 0, naming: [], sampleFiles: [], testDirs: [] } },
    ...
    {
      dimension: 'api', signal: 'high',
      findings: {
        summary: { operations: 0, routes: 0, contracts: 0, rpcs: 0, events: 0, cliCommands: 0, publicExports: 0, capped: {} },
        operations: [], searchSpace: { filesInspected: 1 }, diagnostics: [],
      },
    },
    {
      dimension: 'data', signal: 'high',
      findings: {
        summary: { entities: 0, fields: 0, keys: 0, relations: 0, migrations: 0, stores: 0, caches: 0, queues: 0, edges: 0, capped: {} },
        searchSpace: { filesInspected: 1 },
        stores: [], schemas: [], entities: [], fields: [], keys: [], relations: [],
        migrations: [], caches: [], queues: [], edges: [], diagnostics: [],
      },
    },
    { dimension: 'deployment', signal: 'high', findings: { counts: { artifacts: 0 } } },
    {
      dimension: 'maintainability', signal: 'high',
      findings: {
        summary: { filesMeasured: 0, tokens: 0, dialects: [], branchPoints: 0, duplicateGroups: 0, generatedFiles: 0, toolEvidence: 0, eligibleFiles: 0, partialCoverage: false, capped: {} },
        measurementUniverse: { filesInspected: 0, bytesInspected: 0, recordsInspected: 0, measuredFiles: 0, eligibleFiles: 0, omittedCount: 0, excludedFiles: 0, supportedDialects: [], excludedLanguages: [] },
        searchSpace: { capped: false },
        sizeDistribution: [], branchPoints: [], duplicateGroups: [],
        generatedBoundaries: [], toolEvidence: [], files: [], diagnostics: [],
      },
    },
    {
      dimension: 'governance', signal: 'high',
      findings: {
        summary: { filesInspected: 0, isGit: false, defaultBranch: null, entries: 0, capped: {} },
        ownership: { files: 0, patterns: 0, assigneeCount: 0, assignmentCount: 0, rules: [], assignees: [] },
        searchSpace: { complete: true, supported: true, readable: true, capped: false, error: false, malformed: false, filesInspected: 0 },
        entries: [], diagnostics: [],
      },
    },
    {
      dimension: 'assurance', signal: 'high',
      findings: {
        summary: { manifests: 0, locks: 0, pins: 0, sources: 0, licenses: 0, sboms: 0, vexes: 0, sarifs: 0, configurations: 0, toolResults: 0, accessibility: 0, attestations: 0, standards: 0, records: 0, capped: {} },
...
test('T223 registry: snapshot registers all 16 dimensions in canonical order plus the global renderer', () => {
  ...
  assert.equal(new Set(RENDERER_SNAPSHOT.map(({ rendererId }) => rendererId)).size, RENDERER_SNAPSHOT.length);
  ...
  assert.throws(() => { RENDERER_SNAPSHOT[0].label = 'mutated'; }, TypeError);
...
test('T223 registry: unknown, missing, and duplicate renderers fail typed and sanitized', () => {
  ...
  assertRegistryError(
    () => createRenderRegistry({ entries: [...DIMENSION_RENDERER_ENTRIES, DIMENSION_RENDERER_ENTRIES[0]] }),
    'DUPLICATE_RENDERER',
  );
  // Unknown dimension entry (16 entries, one is not a registered dimension).
  assertRegistryError(
    () => createRenderRegistry({ entries: [...DIMENSION_RENDERER_ENTRIES.slice(1), entryFor('private-canary')] }),
    'UNKNOWN_RENDERER',
    'private-canary',
  );
  // Non-function render.
  assertRegistryError(
    () => createRenderRegistry({ entries: [...DIMENSION_RENDERER_ENTRIES.slice(0, 15), entryFor('assurance', { render: null })] }),
    'INVALID_RENDERER',
  );
  // Invalid renderer ID.
  assertRegistryError(
    () => createRenderRegistry({ entries: [...DIMENSION_RENDERER_ENTRIES.slice(0, 15), entryFor('assurance', { rendererId: 'not-a-renderer' })] }),
    'INVALID_RENDERER',
  );
  // Empty label.
  assertRegistryError(
    () => createRenderRegistry({ entries: [...DIMENSION_RENDERER_ENTRIES.slice(0, 15), entryFor('assurance', { label: '' })] }),
    'INVALID_LABEL',
  );
  // Empty prose.
  assertRegistryError(
    () => createRenderRegistry({ entries: [...DIMENSION_RENDERER_ENTRIES.slice(0, 15), entryFor('assurance', { prose: [] })] }),
    'INVALID_PROSE',
  );
  // Judgmental prose.
  assertRegistryError(
    () => createRenderRegistry({ entries: [...DIMENSION_RENDERER_ENTRIES.slice(0, 15), entryFor('assurance', { prose: ['This should be improved.'] })] }),
    'VOICE_HIT',
  );
  // Privacy hazard in prose.
  assertRegistryError(
    () => createRenderRegistry({ entries: [...DIMENSION_RENDERER_ENTRIES.slice(0, 15), entryFor('assurance', { prose: ['Contact admin@example.test for details.'] })] }),
    'PRIVACY_HAZARD',
  );
...
test('T223 registry: injected findings with an unknown dimension fail typed and sanitized before rendering', () => {
  ...
  assert.throws(() => registry.render(null), (error) => {
    assert.ok(error instanceof RenderRegistryError);
  ...
  try {
    ...
    for (const dimension of DIMENSION_RENDERER_ORDER) {
      const heading = dimension === 'structure' ? '## Repository Structure'
        ...
        : dimension === 'config' ? '## Configuration'
        : dimension === 'testing' ? '## Testing'
        : dimension === 'conventions' ? '## Code Conventions'
        : dimension === 'git' ? '## Git Practices'
        ...
        : dimension === 'api' ? '## API Surface'
        : dimension === 'data' ? '## Data Architecture'
...
test('T223 injected: injected missing or unknown renderer fails before any write', async () => {
  ...
  try {
    ...
    assert.throws(() => createRenderRegistry({ entries: DIMENSION_RENDERER_ENTRIES.slice(0, 15) }),
    ...
    assert.deepEqual(await readdir(missing), [], 'no file may be written when injected rendering fails');
  } finally {
...
test('T223 default: the six new renderers stay inert in the default write path', async () => {
  ...
  try {
    const out = join(root, 'NORMS.md');
    const content = await writeNORMS({ generated: '2026-01-15', repos: [{ overview: fixedOverview(), deep: tenDeep() }] }, out);
...
test('T223 inert: no production module imports the renderer registry and the write seam stays minimal', async () => {
  ...
  const activatedConsumers = new Set([
    join(ROOT, 'lib', 'scan', 'pipeline', 'run.mjs'),
  ]);
  const consumers = [];
...
test('T223 voice: every registered renderer label and static prose is neutral', () => {
  for (const entry of RENDERER_SNAPSHOT) {
    ...
    assert.deepEqual(hits, [], `${entry.dimension} static prose has judgmental voice:\n${JSON.stringify(hits, null, 2)}`);
  }
});
...
test('T223 voice: registered renderer prose stays clean under the established neutral prose masking', () => {
  ...
  for (const entry of RENDERER_SNAPSHOT.slice(0, 16)) {
    ...
    assert.deepEqual(findVoiceHits(markdown), [], `${entry.dimension} prose fails the voice gate in context`);
  }
});
