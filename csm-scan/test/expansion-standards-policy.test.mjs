...
const SNAPSHOT = [
  [
    "std:aicpa-soc2-tsc:2017-rpof-2022",
    "2017-rpof-2022",
  ...
  [
    ...
    "2.2-rec-20241212",
    "2.2, W3C Recommendation 12 December 2024 (REC-WCAG22-20241212)",
  ],
];
...
test("registry access is immutable and returns validated identities", () => {
  ...
  assert.throws(() => {
    STANDARDS_REGISTRY[0].edition = "changed";
  }, TypeError);
});
...
test("registry validation sorts deterministically without mutating input", () => {
  ...
  assert.deepEqual(
    result.map(({ id }) => id),
    ["std:another-format:2", "std:example-format:1.0"],
  );
...
test("entry validator rejects malformed records and metadata-boundary bypasses", () => {
  ...
  assert.throws(
    () => validateStandardEntry({ ...VALID_ENTRY, officialUri: "http://example.test" }),
    TypeError,
  );
  assert.throws(
    () => validateStandardEntry({ ...VALID_ENTRY, publicationDate: "2026-13" }),
    TypeError,
  );
  assert.throws(
    () => validateStandardEntry({ ...VALID_ENTRY, publicationDate: "2026-02-30" }),
    TypeError,
  );
  assert.throws(
    () => validateStandardEntry({ ...VALID_ENTRY, disposition: "undecided" }),
    TypeError,
  );
  assert.throws(
    () => validateStandardEntry({ ...VALID_ENTRY, disposition: "authored_mapping" }),
    /metadata_only/,
  );
  assert.throws(
    () => validateStandardEntry({ ...VALID_ENTRY, summary: "Free-form prose." }),
    TypeError,
  );
  assert.throws(() => validateStandardEntry({ ...VALID_ENTRY, copiedText: false }), TypeError);
  assert.throws(
    () => validateStandardEntry({ ...VALID_ENTRY, controlText: "source prose" }),
    TypeError,
  );
  assert.throws(() => {
...
test("registry validator rejects duplicate IDs and official URIs", () => {
  ...
  assert.throws(
    () => validateStandardsRegistry([VALID_ENTRY, { ...VALID_ENTRY, id: "std:other-format:1.0" }]),
