...
test('git fixture with remediation branches reports the remediation depth structure without identities', async () => {
  ...
  try {
    ...
    const result = await scan(dir, {});
    assert.ok(result.findings.branchPattern.includes('remediation/<date>/<id>/attempt-N'),
      `branch fact must report the remediation structure: ${result.findings.branchPattern}`);
    assert.ok(result.findings.branchPattern.includes('feature/*'),
    ...
    assert.ok(!result.findings.branchPattern.includes('attempt-1'), 'must not leak an attempt number');
  } finally {
    cleanupGitRepo(dir);
  }
});
