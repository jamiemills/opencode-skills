...
test("manifest repair: Cargo classes, ranges, negation, globstar, question, and exclusions resolve safely", async () => {
  ...
  await withFixture(
    ...
    async (dir) => {
      const manifest = readManifest(dir);
      assert.deepEqual(manifest.workspace.members, [
        "crates/[a-c]",
        "plugins/**",
        "tools/?ne",
        "ignored/[!x]",
...
test("manifest repair: Cargo globstar stops safely at the directory budget", async () => {
  await withFixture(
    ...
    async (dir) => {
      assert.doesNotThrow(() =>
        expandRepositoryDirectoryPatterns(dir, "**", {
          marker: "Cargo.toml",
          maxDirectories: 2,
        }),
      );
...
test("manifest repair: repository directory glob rejects absolute and parent traversal patterns", async () => {
  await withFixture(
    ...
    async (dir) => {
      ...
      assert.deepEqual(
        expandRepositoryDirectoryPatterns(dir, "../*", { marker: "Cargo.toml" }),
        [],
      );
      assert.deepEqual(
        expandRepositoryDirectoryPatterns(dir, "/tmp/*", { marker: "Cargo.toml" }),
        [],
      );
    },
  );
