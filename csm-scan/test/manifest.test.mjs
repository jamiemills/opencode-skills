...
test('readManifest: pyproject fixture (python ecosystem, hatchling, src-layout)', () => {
  ...
  try {
    ...
    for (const expected of ['click', 'rich', 'httpx', 'mcp']) {
      assert.ok(expected in m.dependencies, `dependencies missing ${expected}: ${JSON.stringify(m.dependencies)}`);
    }
...
test('readManifest: buildBackend explicit wins / normalizes dotted form', () => {
  const dir = makeTempRepo();
  try {
    writeFileSync(join(dir, 'pyproject.toml'), `# perplexity-cli-style: setuptools.build_meta
[build-system]
...
test('readManifest: package.json fixture resolves to typescript ecosystem', () => {
  ...
  try {
    ...
    assert.ok(m.entrypoints.some((e) => e.startsWith('demo-pkg=')), 'bin captured as entrypoint');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('readManifest: Cargo.toml fixture -> rust ecosystem', () => {
  ...
  try {
    ...
    assert.ok(m.entrypoints.some((e) => e.startsWith('mycli=')), 'cargo [[bin]] captured');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('readManifest: package.json exports/engines/workspaces/peerDependencies surfaced', () => {
  ...
  try {
    ...
    assert.ok(m.entrypoints.some((e) => e === 'main=dist/index.cjs'), `main entrypoint: ${JSON.stringify(m.entrypoints)}`);
    assert.ok(m.entrypoints.some((e) => e === 'module=dist/index.mjs'), 'module entrypoint');
...
anyhow = "1.0"
`);
    writeFileSync(join(dir, 'crate-a', 'Cargo.toml'), `
[package]
name = "crate-a"
version = "0.1.0"
edition = "2021"
...
serde = "1.0"
`);
    writeFileSync(join(dir, 'crate-b', 'Cargo.toml'), `
[package]
name = "crate-b"
version = "0.2.0"
...
`);
    ...
    // Unioned member deps.
    assert.ok('serde' in m.dependencies, `crate-a serde unioned: ${JSON.stringify(m.dependencies)}`);
...
`);
    ...
    assert.ok('default' in m.features && 'std' in m.features && 'async' in m.features, 'features captured');
...
`);
    ...
    assert.ok('atheris' in m.devDependencies, 'atheris (marker stripped) from dependency-groups.dev');
    assert.equal(m.devDependencies.pytest, '', 'bare name -> empty spec');
    assert.equal(m.devDependencies.atheris, '>=3.1.0', 'environment marker stripped, version spec kept');
...
test('readManifest: requirements.txt + requirements-dev.txt parsed into deps/devDeps', () => {
  ...
  try {
    ...
    writeFileSync(join(dir, 'requirements-dev.txt'), [
      'pytest>=7.0',
      'ruff  # linter',
      '',
    ].join('\n'));
    ...
    assert.ok(Object.keys(m.dependencies).length > 0, 'dependencies non-empty from requirements.txt');
    ...
    assert.equal(m.devDependencies.pytest, '>=7.0', 'requirements-dev.txt routed to devDependencies');
...
`);
    const m = readManifest(dir);
    assert.ok(m.entrypoints.some((e) => e === 'gui-app=ep_test.gui:main'), `gui-scripts captured: ${JSON.stringify(m.entrypoints)}`);
    assert.ok(
