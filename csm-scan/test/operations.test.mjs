...
test("operations: contract findings keys preserved", async () => {
  await withFixture("ops-contract", { "README.md": "x" }, async (dir) => {
    const r = await runScanner(OPS, dir);
    assert.deepEqual(
      Object.keys(r.findings).toSorted(),
      [
        "dockerCompose",
        "dockerfiles",
        "envConfig",
...
test("operations: step scan detects practice tools across names, run and uses", async () => {
  await withFixture(
    "ops-tools",
    { ".github/workflows/quality.yml": TOOL_RICH_WORKFLOW },
    async (dir) => {
      const r = await runScanner(OPS, dir);
      const gh = r.findings.ci.find((c) => c.platform === "GitHub Actions");
      assert.equal(gh.stepToolScan, "verified");
...
test("operations: real perplexity-cli", async (t) => {
  ...
  assert.ok(
    gh.triggers.includes("workflow_dispatch"),
    "workflow_dispatch captured from quoted on:",
  );
  ...
  assert.ok(
    gh.workflowCount >= expectedWorkflows,
    `multiple workflows present (expected >= ${expectedWorkflows}, got ${gh.workflowCount})`,
  );
...
test("operations: workflow anatomy — SHA pins, escalated permissions and per-job semantics", async () => {
  await withFixture(
    "ops-pinned",
    { ".github/workflows/release.yml": PINNED_RELEASE_WORKFLOW },
    async (dir) => {
      const r = await runScanner(OPS, dir);
      const gh = r.findings.ci.find((c) => c.platform === "GitHub Actions");
      assert.ok(gh, "GitHub Actions entry present");
      ...
      assert.ok(
        !rawJson.includes("3d3c42e5aac5ba805825da76410c181273ba90b1"),
        "full declared SHA must not reach the model",
      );
      ...
      assert.deepEqual(workflow.releasePipeline, {
        declared: true,
        oidc: true,
        skipExisting: true,
        tripleMatch: true,
...
test("operations: renderer emits pins, permissions and release-pipeline facts", async () => {
  await withFixture(
    ...
    async (dir) => {
      ...
      assert.ok(
        markdown.includes("`actions/checkout` @ `3d3c42e5` (# v7.0.1)"),
        "pin token rendered",
      );
      assert.ok(
        !markdown.includes("3d3c42e5aac5ba805825da76410c181273ba90b1"),
        "full SHA not rendered",
      );
      assert.ok(
        markdown.includes("Escalated permissions: `contents: write`, `id-token: write`"),
        "escalation rendered",
      );
      assert.ok(
        markdown.includes("Release pipeline: oidc: true; skip-existing: true; triple-match: true"),
        "release pipeline rendered",
      );
      assert.ok(markdown.includes("if: `github.event_name == 'push'`"), "job if rendered");
      assert.ok(
        markdown.includes("matrix: python-version: `3.12`, `3.13`"),
        "matrix rows rendered",
      );
