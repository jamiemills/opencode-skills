...
test('parseToml: synthetic pyproject produces expected structure', () => {
  ...
  assert.deepEqual(p.tool.setuptools.packages.find.where, ['src'], 'inline table inside dotted path');
});
...
test('parseToml: synthetic Cargo.toml [package] + [dependencies] + [[bin]]', () => {
  ...
  assert.deepEqual(p.dependencies.serde, { version: '1.0', features: ['derive'] },
...
test('parseYamlShallow: throws on anchor', () => {
  assert.throws(
    () => parseYamlShallow('foo: &anchor value'),
