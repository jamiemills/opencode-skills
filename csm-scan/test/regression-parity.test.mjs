...
import {
  files as javascriptFiles,
  parityFiles as javascriptParityFiles,
} from "./fixtures/javascript.mjs";
...
export const P0_CASES = [
  {
    ...
    run: async () => {
      ...
      assert.ok(
        edges.includes("src/util.ts"),
        `runtime alias edge missing: ${JSON.stringify(edges)}`,
      );
      assert.ok(
        !edges.includes("src/types.ts"),
        `type-only edge emitted: ${JSON.stringify(edges)}`,
      );
      assert.ok(
        !result.findings.modules.includes("src/public.d.ts"),
        ".d.ts entered the runtime module set",
      );
    },
  },
  {
  ...
  },
  {
    ...
    run: async () => {
      ...
      assert.ok(
        edges.includes("src/demo/parts/alpha.py"),
        `alpha edge missing: ${JSON.stringify(edges)}`,
      );
      assert.ok(
        edges.includes("src/demo/parts/beta.py"),
        `beta edge missing: ${JSON.stringify(edges)}`,
      );
      assert.ok(
        edges.includes("src/acme/plugins/loader.py"),
        `PEP 420 edge missing: ${JSON.stringify(edges)}`,
      );
    },
  },
  {
    ...
    run: async () => {
      ...
      assert.ok(
        parseInt(coverage, 10) > 0,
        `expected positive Python docstring coverage, got ${coverage}`,
      );
    },
  },
  {
    ...
    run: async () => {
      ...
      assert.ok(
        style.relativeImports > 0,
        `relative imports not counted: ${JSON.stringify(style)}`,
      );
    },
  },
  {
  ...
  },
  {
    ...
    run: async () => {
      ...
      assert.ok(
        edges.includes("src/a/b/foo.rs"),
        `nested module edge missing: ${JSON.stringify(edges)}`,
      );
      assert.ok(
        !edges.includes("src/a/foo.rs"),
        `false sibling edge emitted: ${JSON.stringify(edges)}`,
      );
    },
  },
  {
    ...
    run: async () => {
      ...
      assert.ok(
        !edges.some((edge) => /serde/i.test(edge)),
        `external serde edge emitted: ${JSON.stringify(edges)}`,
      );
    },
  },
  {
  ...
  },
  {
    ...
    run: async () => {
      ...
      assert.ok(
        result.findings.linters.some(
          (tool) => tool.name === "eslint" && tool.config === "eslint.config.ts",
        ),
      );
