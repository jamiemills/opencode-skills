...
test('F-024: markdownSafe contexts escape the HTML-significant token set', () => {
  ...
  assert.equal(safe.escapeField('<img src=x onerror=alert(1)>'), '\\<img src=x onerror=alert(1)\\>');
...
test('F-024: writeNORMS renders a hostile repo-controlled description on one neutral line', async () => {
  ...
  assert.ok(!content.split('\n').some((line) => line.startsWith('# fake heading')), 'an injected heading must not render as a heading');
  ...
  assert.ok(descriptionLine && descriptionLine.includes('top line'), 'description must stay on one line');
