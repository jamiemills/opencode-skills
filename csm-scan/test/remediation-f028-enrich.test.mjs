...
test('F-028: enrichValidateRetry returns the post-retry enriched, not the stale first one', async () => {
  await withFixture('f028-retry', pythonFiles, async (repoPath) => {
    ...
    assert.ok(trace.some((entry) => entry.phase === 'retry'), 'retry must have fired');
    ...
    assert.equal(config.signal, 'high', 'config signal must be the post-retry high, not the stale low');
  });
});
