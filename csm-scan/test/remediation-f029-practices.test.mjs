...
function modelWithKinds(kinds) {
  return {
    ...
    diagnostics: [],
    searchSpace: { complete: true, supported: true, readable: true, capped: false, error: false, malformed: false, filesInspected: 1 },
  };
}

test('F-029: kind tokens containing backticks and pipes are escaped, not interpolated raw', () => {
  const hostile = 'D212`|evil';
  const markdown = renderPractices('repo', modelWithKinds([hostile, 'E501']), DEFAULT_RENDER_CONTEXT);
  ...
  assert.ok(markdown.includes('D212\\`\\|evil'), 'backtick and pipe inside a kind token must be escaped');
});
