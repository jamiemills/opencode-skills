...
test("F-031: a chatty rule that hits its per-rule cap does not suppress later rules", () => {
  ...
  assert.equal(
    perRule.get("RUL-test-v1"),
    RULE_EVALUATION_LIMITS.maxMatchesPerRule,
    "chatty rule capped at its per-rule limit",
  );
  assert.equal(
    perRule.get("RUL-later-v1"),
    RULE_EVALUATION_LIMITS.maxMatchesPerRule,
    "later rule must still be evaluated",
  );
