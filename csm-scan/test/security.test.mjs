...
test("security: Rust fixture with argon2 + validator detects auth + inputValidation", async () => {
  await withFixture("rust-sec", rustFixtureWithSecurity(), async (dir) => {
    ...
    assert.ok(f.auth.detected, `auth should be detected via argon2: ${JSON.stringify(f.auth)}`);
    assert.ok(
...
test("security: JS fixture still detects its JS auth libs (no regression)", async () => {
  await withFixture("js-auth", jsFixtureWithAuth(), async (dir) => {
    ...
    assert.ok(
      pkgs.includes("jsonwebtoken"),
      `expected jsonwebtoken: ${JSON.stringify(f.auth.frameworks)}`,
    );
  });
});
...
test("security: dependency-only Python fixture reports declared dependency evidence without a package script", async () => {
  ...
  await withFixture(
    "py-audit-dep",
    { ...pythonFiles, "pyproject.toml": pyproject },
    async (dir) => {
...
test("security: findings preserve the write.mjs contract keys and add securityTools", async () => {
  await withFixture("contract", pythonFixtureWithSecurity(), async (dir) => {
    ...
    for (const k of expected) {
      assert.ok(
        k in res.findings,
        `missing finding key '${k}' in: ${JSON.stringify(Object.keys(res.findings))}`,
      );
    }
...
test("security: dependabot branch evidence — recording broker with dependabot/* branches reports inferred", async () => {
  ...
  await withFixture(
    ...
    async (dir) => {
      ...
      assert.ok(
        calls.some((call) => call.executable === "git" && call.argv.join(" ") === "branch -a"),
...
test("security: AWS key regex matches UPPER and lower env-var forms, never prose (F-003)", async () => {
  ...
  await withFixture("sec-aws-case", files, async (dir) => {
    ...
    assert.ok(
      access,
      `uppercase/lowercase access-key forms must both be detected: ${JSON.stringify(f.secrets)}`,
    );
    ...
    assert.ok(
      secret,
      `lowercase aws_secret_access_key must be detected: ${JSON.stringify(f.secrets)}`,
    );
    ...
    for (const finding of f.secrets.findings) {
      assert.ok(
        !finding.files.includes("docs.md"),
        `prose must never match: ${JSON.stringify(finding)}`,
      );
    }
  });
});
...
test("security: >400-file window prioritizes config files and discloses the truncation (F-002)", async () => {
  const files = {};
  for (let i = 0; i < 410; i++)
    files[`filler-${String(i).padStart(3, "0")}.txt`] = "placeholder content\n";
  ...
  await withFixture(
    ...
    async (dir) => {
      const res = await scan(
        ...
        createCommandBroker({ runner: { run: okRun } }),
      );
...
test("security: gitignored .env canary detected via the hidden/gitignored pass (F-018)", async () => {
  ...
  try {
    ...
    const overview = await surveyOverview(dir);
    assert.ok(
      !overview.files.includes(".env"),
      `.env must be invisible to the survey enumeration: ${JSON.stringify(overview.files)}`,
    );
    ...
    assert.ok(
      secret,
      `gitignored .env canary must be detected by the hidden pass: ${JSON.stringify(res.findings.secrets)}`,
    );
...
test("security: real perplexity-cli -> uv.lock + gitleaks + SECURITY.md, pydantic validation + bandit/pip-audit", async (t) => {
  ...
  assert.equal(
    f.hasLockfile,
    true,
    `perplexity-cli must have uv.lock, got hasLockfile=${f.hasLockfile}`,
  );
  assert.ok(
