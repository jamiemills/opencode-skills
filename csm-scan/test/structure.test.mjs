...
test("structure renderer reports dual-scope totals with a scope caveat", () => {
  ...
  assert.match(
    out,
    /\| \.toml \| 0 \| 1 \|/,
    "extensions only in the git scope must render with a zero rg count",
  );
...
test("structure scan omits gitTracked keys when git scope is unavailable (coverage stays 100%)", async () => {
  ...
  await withFixture("structnogit", files, async (dir) => {
    ...
    assert.ok(
      !("gitTrackedTotalFiles" in f),
      "gitTracked keys must be omitted when git scope is absent",
    );
    assert.ok(
      !("gitTrackedFileCounts" in f),
      "gitTracked keys must be omitted when git scope is absent",
    );
