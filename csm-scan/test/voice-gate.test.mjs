...
test("T116 voice matcher catches prescriptive and evaluative prose", () => {
  assert.deepEqual(findVoiceHits("This repo should use tests\nConfiguration is inadequate"), [
...
function assertNeutralNORMS(name, markdown) {
  ...
  assert.deepEqual(
    hits,
    [],
    `${name}: judgmental renderer prose:\n${JSON.stringify(hits, null, 2)}`,
  );
  assert.match(
    markdown,
  ...
  const relatedHeadings =
    markdown.match(/^#{1,6}\s+.*(?:cross-observ|contradiction|conflict).*$/gim) || [];
  for (const heading of relatedHeadings) {
    assert.equal(
      heading,
      "## Cross-observations",
      `${name}: non-neutral cross-observation heading`,
    );
  }
}
...
test("T116 perplexity-cli: rendered NORMS uses neutral factual voice", async () => {
  if (RESOLVED_REAL_REPO.repo === null) {
    console.warn(
      `[T116] CSM_SCAN_REAL_REPO is set but does not exist (${RESOLVED_REAL_REPO.missing}); running against the pxcli-mini fallback fixture`,
    );
  }
