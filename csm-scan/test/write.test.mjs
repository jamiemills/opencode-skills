    ...
    repos: [
      {
        crossObservations: [
          {
            description:
              'conventions import style is ESM (import/export); package.json type is "commonjs"',
        ...
        deep: [
          ...
          },
          {
            ...
            findings: {
              ...
              unsafeCount: {
                count: 0,
                kinds: { block: 0, fn: 0, impl: 0, trait: 0, extern: 0, other: 0 },
              },
              shellHygiene: {
                totalShellFiles: 5,
                filesWithPipefail: 4,
...
test("renders the G3 richness findings (stack/config/conventions/operations)", async () => {
  ...
  // config richness
  assert.ok(
    content.includes("**Build tools**: vite (`vite.config.ts`)"),
    "buildTools not rendered",
  );
  assert.ok(
    content.includes("**Alternative runtimes/manifests**: deno (`deno.json`)"),
    "runtimes not rendered",
  );
  assert.ok(
    content.includes("**Markers present**: MANIFEST.in, py.typed, .python-version"),
    "markers not rendered",
  );
  assert.ok(content.includes("module: NodeNext"), "expanded typescript module not rendered");
  assert.ok(
    content.includes("moduleResolution: NodeNext"),
    "expanded typescript moduleResolution not rendered",
  );
  ...
  assert.ok(
    content.includes("**Symbol naming**: snake_case dominant (866 symbols)"),
    "symbolNaming not rendered",
  );
  assert.ok(
    content.includes("**Async/await usage**: 12 async declaration(s), 45 await reference(s)"),
    "asyncUsage not rendered",
  );
  assert.ok(content.includes("**Unsafe blocks**: 0"), "unsafeCount not rendered");
  assert.ok(
    content.includes("**Shell hygiene**: pipefail adopted in 80.0% (4/5 shell files)"),
    "shellHygiene not rendered",
  );
  assert.ok(content.includes("Shebangs present in 5 file(s)"), "shellHygiene shebang not rendered");
  assert.ok(
    content.includes("**Type hints**: 99.9% of defs annotated"),
    "pythonTypeHints not rendered",
  );
  assert.ok(
    content.includes("`from __future__ import annotations` present"),
    "futureAnnotations not rendered",
  );
  assert.ok(
    content.includes(
      "**TS annotations**: 45 interface(s), 20 type alias(es); 62.5% annotation density",
    ),
    "tsAnnotations not rendered",
  );
...
test("renders cross-observations neutrally (no severity badges, neutral heading)", async () => {
  ...
  assert.ok(
    content.includes("## Cross-observations"),
    "neutral Cross-observations heading missing",
  );
  assert.ok(
    content.includes(
      '- conventions import style is ESM (import/export); package.json type is "commonjs"',
  ...
  assert.ok(
    !/\b(high|medium|low|critical)\s+severity\b/i.test(content),
    "severity badges must not appear",
  );
  assert.ok(
    !content.includes("## Contradictions"),
    'must not use judgmental "Contradictions" heading',
  );
...
test("supports the single-repo top-level cross-observation compatibility fallback", async () => {
  ...
  findings.contradictions = [
    { description: "top-level compatibility fact", dimensions: ["stack"] },
  ];
...
test("renders authoritative cross-observations within each repository", async () => {
  ...
  const findings = {
    ...
    repos: [
      {
        ...first,
        crossObservations: [{ description: "first repository fact", dimensions: ["stack"] }],
      },
      {
        ...first,
  ...
  assert.match(
    content,
    /first repository fact[\s\S]*Repository Overview[\s\S]*Name\*\*: second-repo[\s\S]*second repository fact/,
  );
...
test("rendered output contains no banned judgmental voice terms", async () => {
  ...
  const banned =
    /\b(should|must|ought|poor|poorly|good|bad|weak|strong|recommended|anti-pattern|smell|unfortunately|ideally|suboptimal)\b/i;
...
test(
  "perplexity-cli pipeline renders the new richness findings and the neutral coverage line",
  { timeout: 120000 },
  (t) => {
    if (PERPLEXITY === null) {
