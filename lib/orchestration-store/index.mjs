...
import { validateProgressDocument as validateCanonicalProgressDocument } from "../../csm-orchestrate/lib/progress.mjs";
...
const PROGRESS_ID = /^progress-[a-z0-9][a-z0-9-]{1,127}$/;
const PHASE_ID = /^phase-[a-z0-9][a-z0-9-]{1,127}$/;
const CHILD_RUN_ID = /^run-[a-z0-9][a-z0-9-]{1,127}$/;
const WORKER_ID = /^worker-[a-z0-9][a-z0-9-]{1,127}$/;
...
export class WorkerLeaseError extends OrchestrationStoreError {
  constructor(info) {
    super(
      info.reason === "expired"
        ? `worker lease for ${info.workerId} has expired`
        : `worker lease for ${info.workerId} is held until ${info.heldUntil}`,
      info,
    );
...
CREATE TABLE IF NOT EXISTS child_attempts (
  ...
  receipt_schema_digest TEXT NOT NULL,
  evidence_schema_digest TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_child_attempts_key ON child_attempts(logical_key);
CREATE INDEX IF NOT EXISTS idx_child_attempts_child ON child_attempts(child_run_id);
`;
...
CREATE INDEX IF NOT EXISTS idx_worker_leases_run ON worker_leases(run_id);
CREATE INDEX IF NOT EXISTS idx_worker_leases_state ON worker_leases(state);
`;
    ...
    migrate() {
      ...
      engine.transaction(() => {
        ...
        for (const migration of MIGRATIONS) {
          ...
          // Idempotent under concurrent first-open: a racing opener that already
          // recorded this version must not fail the whole migration.
...
function childAttemptFromRow(row) {
  if (!row) return null;
  return {
    schema: "csm-orchestrate-child-attempt/1",
    ...
    handlerDigest: row.handler_digest,
    receiptSchemaDigest: row.receipt_schema_digest,
    evidenceSchemaDigest: row.evidence_schema_digest,
    configDigest: row.config_digest,
    sideEffectClass: row.side_effect_class,
    dispatchIntentId: row.dispatch_intent_id,
    state: row.state,
    response: row.response === null ? null : parseJson(row.response, "child attempt response"),
...
export function createSqliteStore(options = {}) {
  ...
  if (!driver.available) {
    if (driverPreference === "memory-js" || mode === "memory") {
      engineBackend = createMemoryJsEngine();
    } else {
      throw new OrchestrationStoreError(
        "node:sqlite unavailable; durable WAL mode requires Node >= 22.13. Use mode:'memory' or driver:'memory-js' for non-durable testing.",
      );
    }
  } else if (driverPreference === "memory-js") {
    engineBackend = createMemoryJsEngine();
  } else if (mode === "memory") {
    engineBackend = createSqliteEngine({ ...options, databasePath: ":memory:" });
  } else {
    databasePath =
  ...
  const store = {
    ...
    async saveProgress(progress, saveOptions = {}) {
      guard();
      if (!progress || progress.schema !== "csm-progress/1") fail("progress document is required");
      validateCanonicalProgressDocument(progress);
      assertCanonical(progress.runId, RUN_ID, "runId");
      assertCanonical(progress.progressId, PROGRESS_ID, "progressId");
      if (!Number.isInteger(progress.revision) || progress.revision < 0)
        fail("progress revision must be a non-negative integer");
      const cursorId = `progress:${progress.progressId}`;
      const expectedRevision = saveOptions.expectedRevision ?? progress.revision - 1;
      const providedToken = saveOptions.fencingToken ?? null;
      return engineBackend.transaction(() => {
        const rows = engineBackend
          .selectWhere("events", { cursor_id: cursorId })
          .filter((row) => row.event_type === "progress-snapshot")
          .toSorted((a, b) => Number(b.sequence) - Number(a.sequence));
        const current = rows[0] ? parseJson(rows[0].payload, "progress snapshot") : null;
        const currentRevision = Number(current?.revision ?? 0);
        if (expectedRevision !== currentRevision)
          throw new CasMismatchError({ cursorId, expectedRevision, currentRevision });
        const fence = fencingRow(cursorId);
        if (fence && providedToken === null)
          throw new StaleFenceError({
            cursorId,
            providedToken: null,
            currentToken: Number(fence.token),
          });
        if (providedToken !== null && (!Number.isInteger(providedToken) || providedToken < 1))
          throw new StaleFenceError({
            cursorId,
            providedToken,
            currentToken: Number(fence?.token ?? 1),
          });
        if (providedToken !== null && fence && providedToken < Number(fence.token))
          throw new StaleFenceError({ cursorId, providedToken, currentToken: Number(fence.token) });
        if (providedToken !== null && !fence)
        ...
        else if (providedToken !== null && providedToken > Number(fence.token))
          engineBackend.updateWhere(
            "fencing_tokens",
            { cursor_id: cursorId },
            { token: providedToken, issued_at: now() },
          );
        if (progress.revision !== currentRevision + 1)
          fail("progress revision must advance exactly once");
        appendEventTx(cursorId, "progress-snapshot", progress);
        return { ...progress };
      });
    },

    peekFencingToken(cursorId) {
      guard();
      const fence = fencingRow(cursorId);
      return fence ? Number(fence.token) : null;
    },
    async loadProgress(progressId, loadOptions = {}) {
      guard();
      assertCanonical(progressId, PROGRESS_ID, "progressId");
      const rows = engineBackend
        .selectWhere("events", { cursor_id: `progress:${progressId}` })
        .filter((row) => row.event_type === "progress-snapshot")
        .toSorted((a, b) => Number(b.sequence) - Number(a.sequence));
      if (!rows[0]) return null;
      const progress = parseJson(rows[0].payload, "progress snapshot");
      validateCanonicalProgressDocument(progress);
      assertCanonical(progress.progressId, PROGRESS_ID, "progressId");
      if (progress.progressId !== progressId) fail("progressId does not match lookup key");
      const fence = fencingRow(`progress:${progressId}`);
      const providedToken = loadOptions.fencingToken ?? null;
      if (fence && providedToken === null)
        throw new StaleFenceError({
          cursorId: `progress:${progressId}`,
          providedToken: null,
          currentToken: Number(fence.token),
        });
      if (
        providedToken !== null &&
        (!Number.isInteger(providedToken) || providedToken < Number(fence?.token ?? 1))
      )
        throw new StaleFenceError({
          cursorId: `progress:${progressId}`,
          providedToken,
    ...
    async beginChildAttempt(record) {
      ...
      if (!record || typeof record !== "object" || Array.isArray(record))
        fail("child attempt is required");
      for (const field of [
        "attemptId",
        "logicalKey",
        "requestDigest",
        ...
        assertNonEmptyString(record[field], `child attempt ${field}`);
      assertCanonical(record.parentRunId, RUN_ID, "parentRunId");
      assertCanonical(record.childRunId, CHILD_RUN_ID, "childRunId");
      assertCanonical(record.phaseId, PHASE_ID, "phaseId");
      if (!Number.isInteger(record.attempt) || record.attempt < 0)
        fail("child attempt number must be a non-negative integer");
      const createdAt = now();
      return engineBackend.transaction(() => {
        const existing = engineBackend
          .selectWhere("child_attempts", { logical_key: record.logicalKey })
          .find((item) => Number(item.attempt) === record.attempt);
        if (existing) {
          const comparable = {
            requestDigest: existing.request_digest,
            capabilityDigest: existing.capability_digest,
            contractDigest: existing.contract_digest,
            handlerDigest: existing.handler_digest,
            receiptSchemaDigest: existing.receipt_schema_digest,
            evidenceSchemaDigest: existing.evidence_schema_digest,
            configDigest: existing.config_digest,
          };
          const changed = Object.entries(comparable).some(([key, value]) => value !== record[key]);
          if (changed)
            throw new DuplicateIdempotencyError({
          ...
          return childAttemptFromRow(existing);
        }
        engineBackend.insert("child_attempts", {
          ...
          handler_digest: record.handlerDigest,
          receipt_schema_digest: record.receiptSchemaDigest,
          evidence_schema_digest: record.evidenceSchemaDigest,
          config_digest: record.configDigest,
          side_effect_class: record.sideEffectClass,
          dispatch_intent_id: record.dispatchIntentId ?? null,
          state: record.state ?? "dispatched",
          response: record.response === undefined ? null : JSON.stringify(record.response),
          created_at: createdAt,
          completed_at: record.completedAt ?? null,
        });
        appendEventTx(`child:${record.childRunId}`, "child-attempt-started", {
          attemptId: record.attemptId,
          logicalKey: record.logicalKey,
          attempt: record.attempt,
        });
        return { ...record, state: record.state ?? "dispatched", createdAt };
      });
    },
    ...
    async saveChildAttemptResult(attemptId, response, state = "terminal") {
      ...
      if (!response || typeof response !== "object" || Array.isArray(response))
        fail("child response is required");
      if (!new Set(["terminal", "UNKNOWN"]).has(state)) fail("invalid child attempt state");
      const completedAt = now();
      return engineBackend.transaction(() => {
        const existing = engineBackend.get("child_attempts", attemptId);
        if (!existing) fail(`unknown child attempt ${attemptId}`);
        if (existing.response !== null) return childAttemptFromRow(existing);
        engineBackend.updateWhere(
          "child_attempts",
          { attempt_id: attemptId },
          { state, response: JSON.stringify(response), completed_at: completedAt },
        );
        appendEventTx(`child:${existing.child_run_id}`, "child-attempt-completed", {
          attemptId,
          state,
        });
        return childAttemptFromRow(engineBackend.get("child_attempts", attemptId));
      });
    },

    async loadChildAttempts(childRunId) {
      guard();
      assertCanonical(childRunId, CHILD_RUN_ID, "childRunId");
      return engineBackend
        .selectWhere("child_attempts", { child_run_id: childRunId })
        .map(childAttemptFromRow);
    },

    async loadChildAttemptByKey(logicalKey) {
      guard();
      assertNonEmptyString(logicalKey, "logical key");
      return (
        engineBackend
          .selectWhere("child_attempts", { logical_key: logicalKey })
          .toSorted((a, b) => Number(a.attempt) - Number(b.attempt))
          .map(childAttemptFromRow)
          .at(-1) ?? null
      );
    },
    ...
    // Terminal receipts are keyed by the run that produced them: the
    // run-level receipt's `runId`. A child run id is never a `terminal_receipts`
    // key, so this reader is run-scoped only. Child-run terminal state is
    // authoritative in `child_attempts.child_run_id` and is read via
    // loadChildAttempts/loadChildAttemptByKey; callers must not expect a
    // child-keyed result here (and this must not fabricate a childRunId from
    // run_id). The true-resume already-persisted check calls this with the
    // run-level `durable.runId`, which is the correct key.
    async loadTerminalRecords(runId) {
      guard();
      assertCanonical(runId, RUN_ID, "runId");
      return engineBackend.selectWhere("terminal_receipts", { run_id: runId }).map((row) => {
    ...
    async claimWorker({ workerId, runId, taskId = null, attempt = 0, leaseMs = 30_000 } = {}) {
      guard();
      assertCanonical(workerId, WORKER_ID, "workerId");
      assertCanonical(runId, RUN_ID, "runId");
      if (!Number.isInteger(leaseMs) || leaseMs < 1) fail("leaseMs must be a positive integer");
      const claimedAt = now();
      return engineBackend.transaction(() => {
        const existing = engineBackend.get("worker_leases", workerId);
        if (
          existing &&
          existing.released_at === null &&
          Date.parse(existing.expires_at) > Date.parse(claimedAt)
        )
          throw new WorkerLeaseError({ workerId, heldUntil: existing.expires_at });
        const leaseToken = randomUUID();
        const fencingToken = existing ? Number(existing.fencing_token) + 1 : 1;
        const expiresAt = new Date(Date.parse(claimedAt) + leaseMs).toISOString();
        const row = {
          worker_id: workerId,
          run_id: runId,
          task_id: taskId,
        ...
        if (existing) engineBackend.updateWhere("worker_leases", { worker_id: workerId }, row);
        else engineBackend.insert("worker_leases", row);
        return {
          workerId,
          runId,
          taskId,
    ...
    async heartbeatWorker({ workerId, leaseToken, leaseMs = 30_000 } = {}) {
      guard();
      assertCanonical(workerId, WORKER_ID, "workerId");
      if (typeof leaseToken !== "string" || !leaseToken.length) fail("leaseToken is required");
      if (!Number.isInteger(leaseMs) || leaseMs < 1) fail("leaseMs must be a positive integer");
      const beatAt = now();
      return engineBackend.transaction(() => {
        const existing = engineBackend.get("worker_leases", workerId);
        if (!existing || existing.lease_token !== leaseToken || existing.released_at !== null)
          throw new WorkerLeaseError({
            workerId,
            reason: "expired",
            heldUntil: existing?.expires_at ?? null,
          });
        if (Date.parse(existing.expires_at) <= Date.parse(beatAt))
          throw new WorkerLeaseError({
            workerId,
            reason: "expired",
            heldUntil: existing.expires_at,
          });
        const expiresAt = new Date(Date.parse(beatAt) + leaseMs).toISOString();
        engineBackend.updateWhere(
          "worker_leases",
          { worker_id: workerId },
          { heartbeat_at: beatAt, expires_at: expiresAt },
        );
        return {
          workerId,
          heartbeatAt: beatAt,
    ...
    async releaseWorker({ workerId, leaseToken, state = "completed" } = {}) {
      guard();
      assertCanonical(workerId, WORKER_ID, "workerId");
      if (typeof leaseToken !== "string" || !leaseToken.length) fail("leaseToken is required");
      ...
      return engineBackend.transaction(() => {
        ...
        engineBackend.updateWhere(
          "worker_leases",
          { worker_id: workerId },
          { state: String(state), released_at: releasedAt, expires_at: releasedAt },
        );
        return { workerId, state: String(state), releasedAt };
      });
    },
    ...
    async loadWorker(workerId) {
      guard();
      assertCanonical(workerId, WORKER_ID, "workerId");
      const row = engineBackend.get("worker_leases", workerId);
      if (!row) return null;
      return {
    ...
    async reconcileStaleWorkers({ at = now() } = {}) {
      guard();
      const resolvedAt = at;
      return engineBackend.transaction(() => {
        const stale = [];
        for (const row of engineBackend.selectWhere("worker_leases", { state: "active" })) {
          if (row.released_at !== null) continue;
          if (Date.parse(row.expires_at) > Date.parse(resolvedAt)) continue;
          engineBackend.updateWhere(
            "worker_leases",
            { worker_id: row.worker_id },
            { state: "stale", released_at: resolvedAt },
          );
          stale.push({
            workerId: row.worker_id,
            runId: row.run_id,
