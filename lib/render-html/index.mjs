...
function renderItem(item, section, sectionIndex, itemIndex, urlPolicy, idRegistry) {
  const label = element(
    "dt",
    { id: idRegistry.generate("csm-label", section.id, sectionIndex, item.path, itemIndex) },
    [text(item.label)],
  );
...
export function renderHtml({
        ...
        [
          ...
          element(
            ...
            section.items.flatMap((item, itemIndex) =>
              renderItem(item, section, sectionIndex, itemIndex, profile.urlPolicy, idRegistry),
            ),
          ),
        ],
      );
  ...
  const descriptor = {
    ...
    profileDigest,
    generatedAt: projection.generatedAt ?? generatedAt,
    outputDigest,
    status: "untrusted-presentation",
    approval: {
      status: "pending",
  ...
  return {
    ...
    metadata: { ...metadata, generatedAt, outputDigest },
  };
}
