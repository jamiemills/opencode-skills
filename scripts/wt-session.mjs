...
export function createWorktree(root, slug, base) {
  ...
  if (gitOk(root, ['show-ref', '--verify', '--quiet', `refs/heads/${branch}`])) {
    throw new Error(`branch ${branch} already exists — pick another goal slug or merge/nuke it first`);
  }
  const dir = path.join(base, slug);
  if (gitOk(root, ['worktree', 'list', '--porcelain']) && git(root, ['worktree', 'list', '--porcelain']).includes(`worktree ${dir}`)) {
...
export function mergeWorktree(root, slug, { push = false } = {}) {
  ...
  const wt = git(root, ['worktree', 'list', '--porcelain'])
    ...
    .find((d) => gitOk(root, ['-C', d, 'rev-parse', '--abbrev-ref', 'HEAD']) && git(root, ['-C', d, 'rev-parse', '--abbrev-ref', 'HEAD']) === branch);
  const cwd = wt || root;
  ...
  if (cur !== 'main') throw new Error(`main checkout is on "${cur}" — switch it to main before merging`);
...
export function removeWorktree(root, slug, { force = false } = {}) {
  ...
  const entry = entries.find((e) =>
    e.branch === `refs/heads/${branch}` || e.branch === `refs/heads/${slug}` || path.basename(e.dir) === slug);
  if (!entry) throw new Error(`no worktree for ${slug} found`);
  if (path.resolve(entry.dir) === path.resolve(root)) throw new Error('refusing to remove the main checkout');
  const status = git(root, ['-C', entry.dir, 'status', '--porcelain']);
  if (status !== '' && !force) throw new Error(`worktree ${entry.dir} has uncommitted changes — commit/stash or pass --force`);
  ...
  if (!merged && !force) throw new Error(`branch ${branch} is not merged into main — merge it first or pass --force`);
...
function main() {
  ...
  try {
    if (args.action === 'create') {
      if (!args.slug || !SLUG_RE.test(args.slug)) throw new Error('usage: wt-session create <goal-slug> (lowercase, hyphens)');
