...
import { canonicalJson, digest, FIXED_PACKAGE_POLICY, policyToSign, SHELL_DENYLIST, validateEnvelope } from './protocol/trust-policy.mjs';
...
async function makeTlsFixture(dir) {
  await execFileAsync('openssl', ['req', '-x509', '-newkey', 'rsa:2048', '-nodes', '-keyout', join(dir, 'key.pem'), '-out', join(dir, 'cert.pem'), '-days', '1', '-subj', '/CN=localhost', '-addext', 'subjectAltName=DNS:localhost,IP:127.0.0.1'], { stdio: 'ignore' });
...
test('accepts the committed valid local HTTPS envelope and binds canonical steps', async () => {
  ...
  try {
    ...
    assert.deepEqual(validateEnvelope(fetched.body, keyring, { now, origin: `https://localhost:${port}` }), { trusted: true, steps: 'guidance-only', key: 'fixture-2026' });
    await assert.rejects(fetchEnvelope(`https://localhost:${port}/redirect`, tls.cert), error => error.code === 'REDIRECT');
    await assert.rejects(fetchEnvelope(`https://localhost:${port}/oversized`, tls.cert), error => error.code === 'CONTENT_TOO_LARGE');
    await assert.rejects(fetchEnvelope(`https://localhost:${port}/malformed`, tls.cert), error => error.code === 'MALFORMED');
    await assert.rejects(fetchEnvelope(`https://localhost:${port}/notfound`, tls.cert), error => error.code === 'HTTP_STATUS');
...
test('rejects trust and guidance boundary violations', async () => {
  ...
  for (const [name, changes, expected] of cases) assert.throws(() => validateEnvelope({ ...envelope, ...changes }, keyring, { now, bytes: name === 'oversized bytes' ? 65537 : 1 }), error => error.code === expected, name);
  const missing = { ...envelope }; delete missing.audience;
  assert.throws(() => validateEnvelope(missing, keyring, { now }), error => error.code === 'SCHEMA');
  assert.throws(() => validateEnvelope(envelope, expiredKeyring, { now }), error => error.code === 'KEY_EXPIRED');
  assert.throws(() => validateEnvelope(envelope, keyring, { now, origin: 'http://localhost' }), error => error.code === 'ORIGIN');
  assert.throws(() => validateEnvelope(envelope, keyring, { now, origin: 'https://evil.example' }), error => error.code === 'ORIGIN');
  assert.throws(() => validateEnvelope(envelope, keyring, { now, origin: 'not-a-url' }), error => error.code === 'ORIGIN');
  assert.throws(() => validateEnvelope({ ...envelope, signature: { ...envelope.signature, value: Buffer.from('bad').toString('base64') } }, keyring, { now }), error => error.code === 'BAD_SIGNATURE');
});
...
test('F-046: a freshly signed envelope binds payload_index_sha256 into the signed policy and refuses a mismatch', () => {
  ...
  const keyring = {
    keys: [{ id, algorithm: 'Ed25519', public_key_der_base64: spki.toString('base64'), fingerprint, not_before: '2026-01-01T00:00:00.000Z', not_after: '2027-01-01T00:00:00.000Z', revoked: false }]
  };
  ...
  const base = {
    ...
    policy: {
      package: { name: '@jamiemills/csm-skills-bootstrap', version: '0.1.0', bin: 'csm-skills-bootstrap', registry: 'https://registry.npmjs.org' },
      payload_release: 'ephemeral-2026-08',
      limits: { max_bytes: 65536, max_redirects: 0, allowed_origin: 'https://localhost' }
    },
    steps_markdown: '# Guidance\n\nThis is signed guidance. It adds no commands, paths, or permissions.',
    steps_sha256: null,
    payload_index_sha256: indexSha256
  };
  base.steps_sha256 = digest(base.steps_markdown);
  const signature = { algorithm: 'Ed25519', value: sign(null, Buffer.from(canonicalJson(policyToSign(base))), privateKey).toString('base64') };
  const envelope = { ...base, signature };
  assert.deepEqual(validateEnvelope(envelope, keyring, { now, indexSha256 }), { trusted: true, steps: 'guidance-only', key: id });
  assert.throws(() => validateEnvelope({ ...envelope, payload_index_sha256: '0'.repeat(64) }, keyring, { now, indexSha256 }), error => error.code === 'PAYLOAD_INDEX_MISMATCH');
  assert.throws(() => validateEnvelope({ ...envelope, signature: { ...signature, value: Buffer.from('bad').toString('base64') } }, keyring, { now, indexSha256 }), error => error.code === 'BAD_SIGNATURE');
  assert.throws(() => validateEnvelope({ ...envelope, steps_markdown: 'altered' }, keyring, { now, indexSha256 }), error => error.code === 'STEPS_DIGEST');
});
...
test('F-045: the shipped bin embeds the keyring canonically and its verify subcommand enforces the signed boundary', async () => {
  ...
  assert.equal(canonicalJson(embedded), canonicalJson(fileKeyring), 'embedded keyring must equal bootstrap/keyring.json canonically');
  ...
  try {
    ...
    const envelope = JSON.parse(await readFile(fixturePath, 'utf8'));
    const accepted = await execFileAsync(process.execPath, [shippedBin, 'verify', fixturePath], { encoding: 'utf8' });
    assert.equal(JSON.parse(accepted.stdout).verification.ok, true);

    await writeFile(join(dir, 'tampered.json'), JSON.stringify({ ...envelope, signature: { ...envelope.signature, value: Buffer.from('bad').toString('base64') } }));
    const rejected = await execFileAsync(process.execPath, [shippedBin, 'verify', join(dir, 'tampered.json')], { encoding: 'utf8' }).then(out => ({ stdout: out.stdout, code: 0 })).catch(error => ({ stdout: error.stdout || '', code: error.code }));
...
test('R2: the envelope schema matches the when-present runtime and validates the fixture and a no-signature envelope', async () => {
  ...
  assert.deepEqual([...schema.required].toSorted(), ['audience', 'expires_at', 'key', 'policy', 'schema', 'steps_markdown', 'steps_sha256']);
  assert.ok(schema.properties.payload_index_sha256, 'payload_index_sha256 must keep a property definition');
...
test('R4: an envelope with no signature field is the documented local flow and passes the validator and the shipped bin', async () => {
  ...
  assert.deepEqual(validateEnvelope(envelope, keyring, { now }), { trusted: true, steps: 'guidance-only', key: 'fixture-2026' });
  const dir = await mkdtemp('/tmp/csm-bootstrap-'); await chmod(dir, 0o700);
  try {
    await execFileAsync('tar', ['-xf', packHolder.value.tarball, '-C', dir]);
    const shippedBin = join(dir, 'package', 'bin', 'csm-skills-bootstrap.js');
    ...
    const accepted = await execFileAsync(process.execPath, [shippedBin, 'verify', noSigPath], { encoding: 'utf8' });
...
test('R5: the shipped bin embeds the shared shell denylist and fixed package policy without drift', async () => {
  ...
  for (const [, field, value] of policyMatch[1].matchAll(/([a-z]+): '([^']*)'/g)) embeddedPolicy[field] = value;
