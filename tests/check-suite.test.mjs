...
test("corpus family (plan): a marker-less plan exits 1 with the format-marker message", () => {
  ...
  try {
    ...
    assertGateFails(
      dir,
      /plan corpus \.agents\/plans\/zz-plant-csm\.md missing\/unknown format marker/,
      "corpus-plan",
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("corpus family (review): a marker-less review exits 1 with the format-marker message", () => {
  ...
  try {
    ...
    assertGateFails(
      dir,
      /review corpus \.agents\/reviews\/zz-plant-review\.md missing\/unknown format marker/,
      "corpus-review",
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("README family (F-052): a fenced-only skill reference does not satisfy the prose path scan", () => {
  ...
  try {
    ...
    const withoutRefs = content
      .split("\n")
      .filter((l) => !l.includes("csm-upload/SKILL.md"))
      .join("\n");
    write(
      dir,
      "README.md",
...
test("contracts family (F-052): a fenced-only needle does not satisfy the CONTRACTS declaration", () => {
  ...
  try {
    ...
    assertGateFails(
      dir,
      /contract plan-save-path: consumer csm-build\/SKILL\.md lacks "\.agents\/plans\/"/,
      "contracts",
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("DEFERRED-citation family: a non-COMPLETE plan with an uncited DEFERRED task exits 1", () => {
  ...
  try {
    write(
      dir,
      ".agents/plans/zz-deferred-csm.md",
      [
        "format: csm-plan/1",
        "# Plant Plan",
        "",
    ...
    assertGateFails(
      dir,
      /plan corpus \.agents\/plans\/zz-deferred-csm\.md: DEFERRED task 1 has no ledger citation \[DEF:<slug>\]/,
      "deferred-citation",
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("plan-signal family: a <...> placeholder in an acceptance signal exits 1", () => {
  ...
  try {
    ...
    assertGateFails(
      dir,
      /plan corpus \.agents\/plans\/zz-signal-csm\.md line \d+: placeholder token <bar>/,
      "plan-signal",
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("F-050 hard-enforcement: the template-format-marker check fails on a bad marker post-prune", () => {
  ...
  try {
    ...
    assertGateFails(
      dir,
      /csm-plan\/SKILL\.md template format marker: first line inside the "## Required Plan Document" template fence is "not-a-marker" — expected "format: csm-plan\/<n>"/,
...
test("F-053 (D15): with a .git the untracked plant is ignored; without .git it is seen", () => {
  ...
  try {
    ...
    assert.equal(
      withGitRun.status,
      0,
      `untracked plant must be ignored when a .git exists\n${combined(withGitRun)}`,
    );
    assert.doesNotMatch(
      combined(withGitRun),
      /zz-untracked-review/,
      "untracked review not validated",
    );
    ...
    assert.notEqual(
      noGitRun.status,
      0,
      "without .git the planted review must be visible and fail the gate",
    );
    assert.match(
      combined(noGitRun),
...
test("F-069-1: sync --write fails loudly when a synced section is missing", () => {
  ...
  try {
    ...
    assert.match(
      combined(r),
      /sync-skill-boilerplate: csm-plan\/SKILL\.md is missing the synced section "## Tmux Session Bootstrap"/,
      "F-069-1 loud failure",
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
...
test("F-069-3 regression: the offline grammar carries a real --no- negation, not a bare --no", () => {
  const grammar = JSON.parse(
    fs.readFileSync(path.join(REPO, "bootstrap", "runtime-commands.json"), "utf8"),
  );
  assert.ok(
    !grammar.offlineFlags.includes("--no"),
    'bare "--no" must not be recorded as a load-bearing flag',
  );
  ...
  assert.ok(
    grammar.argvTemplates.offline.includes(negation[0]),
    "argvTemplates.offline must match offlineFlags",
  );
});
...
test("F-054: canonical splitLines/fenceMap edge cases (top-level fences required for plants)", () => {
  ...
  assert.deepEqual(
    fenceMask("    ```\na\n    ```"),
    [false, false, false],
    "4-space-indented fence does not open (J5 R3: use top-level fences in plants)",
  );
  assert.deepEqual(
    fenceMask("   ```\na\n   ```"),
    [true, true, true],
    "up-to-3-space indent opens",
  );
  assert.deepEqual(fenceMask("~~~\na\n~~~"), [true, true, true], "tilde fences open/close");
  assert.deepEqual(
    fenceMask("```js `code`\na"),
    [false, false],
    "inline-backtick opener does not open a fence",
  );
  ...
  assert.deepEqual(
    fenceMask("```\r\na\r\n```"),
    [true, true, true],
    "CRLF content splits and fences identically",
  );
});
...
test("F-052: containsOutsideFences / README path-class boundary / anchor generation", () => {
  ...
  assert.equal(
    ...
    false,
    "fenced needle is not a declaration",
  );
  assert.equal(containsOutsideFences(null, "x"), false);
  assert.equal(containsOutsideFences("no needle", "x"), false);

  assert.deepEqual(
    pathMatches("see csm-browse/SKILL.md."),
    ["csm-browse/SKILL.md"],
    "sentence-ending period is not captured",
  );
  assert.deepEqual(pathMatches("csm-upload/SKILL.md and csm-scan/scripts/scan.mjs"), [
    "csm-upload/SKILL.md",
    "csm-scan/scripts/scan.mjs",
  ]);
  ...
  assert.equal(
    githubAnchor("Development & testing"),
    "development--testing",
