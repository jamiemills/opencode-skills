...
import {
  ...
  SKILL_NAME,
  resolveSkillConfig,
} from "../../csm-grill/lib/config.mjs";
import { AUTHORITY_FIELDS, envelope, fixture, layerEnv, writeJson } from "./helpers.mjs";

test("csm-grill: no config resolves to native defaults (differential)", async () => {
  ...
  try {
    ...
    assert.equal(viaResolver.schema, "csm-grill-config/1");
...
test("csm-grill: valid namespace applies settings over defaults", async () => {
  const ctx = await fixture();
  try {
    await writeJson(`${ctx.project}/.csm-skills.json`, {
      schema: "csm-skills-config/1",
      skills: { [SKILL_NAME]: { verbosity: "verbose" } },
    });
    const resolved = await resolveConfig({ projectRoot: ctx.project, env: layerEnv(ctx) });
    const { config, schema, source } = resolveSkillConfig(resolved.effective);
    assert.deepEqual(config, { verbosity: "verbose" });
...
test("csm-grill: unknown namespace key is rejected", async () => {
  const ctx = await fixture();
  try {
    await writeJson(`${ctx.project}/.csm-skills.json`, {
      schema: "csm-skills-config/1",
...
test("csm-grill: invalid types and enum values are rejected", () => {
  for (const namespace of [{ verbosity: 3 }, { verbosity: "loud" }, { verbosity: null }]) {
    assert.throws(
...
test("csm-grill: malformed effective config fails closed", () => {
  assert.throws(
  ...
  assert.throws(
    () => resolveSkillConfig(envelope({ [SKILL_NAME]: ["verbose"] })),
    (error) => error.code === "config-schema",
  );
});

test("csm-grill: unrelated namespaces are ignored", () => {
  const effective = envelope({
    "csm-plan": { batchSize: 999, verbosity: "nope" },
    "csm-scan": { outputFormat: "markdown", maxRetries: 99 },
...
test("csm-grill: authority fields are not accepted", () => {
