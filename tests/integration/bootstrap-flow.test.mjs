...
import { checkArgv, checkSpec, grammar, makeSandbox, npmVersion, verifyCacheManifest } from '../offline/commands.mjs';
...
test('pack, payload audit, capable install, offline boundary, malicious refusal, and managed upgrade compose end to end', async () => {
  ...
  try {
    ...
    assert.deepEqual(installed.report.hashVerification, { algorithm: 'sha256', verified: placedEntries.length, total: placedEntries.length });
    for (const entry of placedEntries) {
      ...
      assert.equal((await lstat(join(destination, rel))).mode & 0o777, parseInt(entry.mode, 8), rel);
    }
    ...
    assert.equal(checkArgv(online.map(part => (part === `--package=${grammar.package.spec}` ? `--package=${floating}` : part))).ok, false);

    const toolchainSandbox = await makeSandbox();
    dirs.push(toolchainSandbox.dir);
    const toolchain = { node: process.versions.node, npm: await npmVersion(toolchainSandbox), platform: `${process.platform}-${process.arch}` };
    ...
    assert.deepEqual(verifyCacheManifest(manifest, tarball, { expectedToolchain: toolchain }), { ok: true, errors: [] });
    assert.equal(verifyCacheManifest({ ...manifest, npm: '99.0.0' }, tarball, { expectedToolchain: toolchain }).ok, false);
    ...
    assert.deepEqual(upgraded.report.hashVerification, { algorithm: 'sha256', verified: placedEntries.length, total: placedEntries.length });
    ...
    const planIndex = JSON.parse(await readFile(join(root, 'bootstrap/payload-index.json'), 'utf8'));
    const planEntry = planIndex.classes.skills.find(entry => entry.path === 'payload/skills/csm-plan/SKILL.md');
    assert.equal(sha256(await readFile(join(upgraded.report.backupPath, 'csm-plan', 'SKILL.md'))), planEntry.sha256);
