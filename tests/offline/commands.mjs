...
const grammar = JSON.parse(
  await readFile(join(root, "bootstrap", "runtime-commands.json"), "utf8"),
);
...
if (!fixedPolicyMatches())
  throw new Error("offline grammar package policy drifted from the shared trust module");
...
function classifySpec(spec) {
  ...
  if (/^git[+:]/.test(spec) || spec.startsWith("github:") || spec.includes("#semver:"))
    return "git";
...
function checkArgv(argv, { offline = false } = {}) {
  if (!Array.isArray(argv) || argv.length === 0 || argv.some((part) => typeof part !== "string"))
    return { ok: false, reason: "argv-type" };
  for (const part of argv)
    if (shellMetacharRe.test(part)) return { ok: false, reason: "shell", detail: part };
  ...
  for (const part of argv) {
    ...
    if (!offline && grammar.offlineFlags.includes(part))
      return { ok: false, reason: "extra-flags", detail: part };
  }
  for (const flag of grammar.requiredFlags)
...
function verifyCacheManifest(manifest, tarballData, { expectedToolchain } = {}) {
  if (!Buffer.isBuffer(tarballData) && !(tarballData instanceof Uint8Array))
    return { ok: false, errors: ["tarballData: expected bytes"] };
  const errors = [];
  const push = (message) => errors.push(message);
  const manifestKeys = keysOf(manifest);
  if (manifestKeys === null) return { ok: false, errors: ["manifest: expected an object"] };
  for (const key of manifestKeys)
    if (!manifestFields.includes(key)) push(`manifest: unexpected property "${key}"`);
  for (const key of manifestFields)
    if (!Object.hasOwn(manifest, key)) push(`manifest: missing required property "${key}"`);
  if (manifest.schema !== "csm-cache-manifest/1")
    push('schema: expected const "csm-cache-manifest/1"');
  for (const key of ["node", "npm"]) {
    if (typeof manifest[key] !== "string" || !versionRe.test(manifest[key]))
      push(`${key}: expected an x.y.z version string`);
  }
  if (typeof manifest.platform !== "string" || !platformRe.test(manifest.platform))
    push("platform: expected a platform-arch string");
  if (expectedToolchain !== undefined && expectedToolchain !== null) {
    for (const key of ["node", "npm", "platform"]) {
      if (Object.hasOwn(expectedToolchain, key) && manifest[key] !== expectedToolchain[key])
        push(`${key}: does not match the recorded toolchain (${expectedToolchain[key]})`);
    }
  }
  ...
  if (pkgKeys === null) {
    ...
    for (const key of pkgKeys)
      if (!packageFields.includes(key)) push(`package: unexpected property "${key}"`);
    for (const key of packageFields)
      if (!Object.hasOwn(pkg, key)) push(`package: missing required property "${key}"`);
    if (pkg.name !== grammar.package.name) push(`package.name: expected "${grammar.package.name}"`);
    if (pkg.version !== grammar.package.version)
      push(`package.version: expected "${grammar.package.version}"`);
    if (typeof pkg.integrity !== "string" || !integrityRe.test(pkg.integrity))
      push("package.integrity: expected a sha256 hex digest");
    if (!Number.isInteger(pkg.bytes) || pkg.bytes < 1)
      push("package.bytes: expected a positive integer");
    if (
      typeof pkg.integrity === "string" &&
      integrityRe.test(pkg.integrity) &&
      pkg.integrity !== sha256(tarballData)
    )
      push("package.integrity: does not match the tarball sha256");
    if (Number.isInteger(pkg.bytes) && pkg.bytes >= 1 && pkg.bytes !== tarballData.length)
      push("package.bytes: does not match the tarball size");
  }
  if (!Array.isArray(manifest.dependencyClosure)) push("dependencyClosure: expected an array");
  else if (manifest.dependencyClosure.length !== 0)
    push("dependencyClosure: must be empty for this package");
  ...
  if (verificationKeys === null) {
    ...
    for (const key of verificationKeys)
      if (!verificationFields.includes(key)) push(`verification: unexpected property "${key}"`);
    for (const key of verificationFields)
      if (!Object.hasOwn(verification, key))
        push(`verification: missing required property "${key}"`);
    if (typeof verification.checkedAt !== "string" || !checkedAtRe.test(verification.checkedAt))
      push("verification.checkedAt: expected an ISO-8601 UTC timestamp");
