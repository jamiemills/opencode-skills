...
before(async () => {
  ...
  try {
    toolchain = { node: process.versions.node, npm: await npmVersion(sandbox), platform: `${process.platform}-${process.arch}` };
  } finally {
...
test('warm cache then offline replay against a dead registry both print the fixed version from identical cache bytes', async () => {
  const sandbox = await makeSandbox();
  try {
    const warmArgs = [...grammar.requiredFlags, `--package=file:${pack.tarball}`, grammar.package.bin, '--version'];
    const warm = await runNpx(sandbox, warmArgs);
    assert.equal(warm.stdout.trim(), '0.1.0');
    const contentTree = () => hashTree(sandbox.cache).then(entries => entries.filter(([rel]) => rel.startsWith('_npx/')));
    ...
    const verify = await promisify(execFile)('npm', ['cache', 'verify'], { cwd: sandbox.cwd, encoding: 'utf8', env: sandbox.env, timeout: 120000 });
...
test('cold-cache offline run resolves the file spec from the local tarball path and fails closed once the artifact is absent', async () => {
  ...
  const offlineArgs = () => [...grammar.offlineFlags, ...grammar.requiredFlags, `--package=file:${pack.tarball}`, grammar.package.bin, '--version'];
  try {
...
test('missing tarball makes the file spec invocation exit nonzero', async () => {
  ...
  try {
    await rename(pack.tarball, hidden);
    await assert.rejects(runNpx(sandbox, [...grammar.requiredFlags, `--package=file:${pack.tarball}`, grammar.package.bin, '--version']), nonzeroExit);
  } finally {
    try {
      await rename(hidden, pack.tarball);
    } catch {}
...
test('checkSpec accepts only the exact recorded literal and rejects every other grammar', () => {
  ...
  for (const [spec, reason] of rejections) {
    ...
    assert.ok(Object.hasOwn(grammar.rejections.rules, reason), `grammar has no recorded rule for ${reason}`);
  }
});

test('checkArgv enforces the recorded invocation rules for online and offline forms', () => {
  const online = ['npx', ...grammar.requiredFlags, `--package=${grammar.package.spec}`, grammar.package.bin, '--version'];
  const offline = ['npx', ...grammar.offlineFlags, ...grammar.requiredFlags, `--package=${grammar.package.spec}`, grammar.package.bin, '--version'];
  ...
  const cases = [
    ...
    ['shell', ['npx', ...grammar.requiredFlags, `--package=${grammar.package.spec}`, '; rm -rf /', grammar.package.bin]],
    ['shell', ['npx', ...grammar.requiredFlags, `--package=${grammar.package.spec}`, grammar.package.bin, '$(id)']],
    ['extra-flags', ['npx', ...grammar.offlineFlags, ...grammar.requiredFlags, `--package=${grammar.package.spec}`, grammar.package.bin]],
    ['extra-flags', ['npx', ...grammar.requiredFlags, `--package=${grammar.package.spec}`, grammar.package.bin, '--frozen']],
    ['dist-tag', ['npx', ...grammar.requiredFlags, `--package=@jamiemills/csm-skills-bootstrap@latest`, grammar.package.bin]],
    ...
    ['extra-flags', ['node', ...grammar.requiredFlags, `--package=${grammar.package.spec}`, grammar.package.bin]],
    ['extra-flags', ['npx', ...grammar.requiredFlags, `--package=${grammar.package.spec}`, `--package=${grammar.package.spec}`, grammar.package.bin]],
    ['extra-flags', ['npx', ...grammar.requiredFlags, `--package=${grammar.package.spec}`, grammar.package.bin, grammar.package.bin]]
  ];
  for (const [reason, argv] of cases) {
    ...
    assert.ok(Object.hasOwn(grammar.rejections.invocationRules, reason) || Object.hasOwn(grammar.rejections.rules, reason), `grammar has no recorded rule for ${reason}`);
  }
});
...
test('cache-manifest verifier accepts the real pack manifest and rejects every alteration', async () => {
  ...
  const base = () => ({
    ...
    package: { name: grammar.package.name, version: grammar.package.version, integrity: pack.sha256, bytes: pack.bytes },
    dependencyClosure: [],
    verification: { checkedAt: fixedClock, ok: true }
  });
  assert.deepEqual(verifyCacheManifest(base(), tarball), { ok: true, errors: [] });
  assert.deepEqual(verifyCacheManifest(base(), tarball, { expectedToolchain: toolchain }), { ok: true, errors: [] });
  for (const key of ['node', 'npm', 'platform']) {
    ...
    assert.ok(mismatched.errors.some(error => error.startsWith(`${key}: does not match the recorded toolchain`)), key);
  }
