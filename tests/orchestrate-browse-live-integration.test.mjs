...
test("live typed browse dispatch provisions, translates, validates, redacts, and cleans only its session", async () => {
  ...
  try {
    ...
    const result = await orchestrate({
      ...
      approvals: async ({ phase, node, childRunId }) => ({
        ...
        binding: { parentRunId, childRunId, phaseId: phase.phaseId, edgeId: `edge-${node.nodeId}` },
...
test("live browse adapter fails closed for unsafe sensitive input, binary omission, and resolver failure", async () => {
  const fixture = await startFixture();
  const request = {
...
test("live browse cancellation after dispatch cleans the real session", async () => {
  const fixture = await startFixture();
  const context = {
...
test("live native console evidence redacts a controlled session token", async () => {
  const fixture = await startFixture();
  const context = { runId: `run-live-browse-log-${process.pid}`, owner: "csm-browse", attempt: 1 };
  const sid = sessionIdFor(context);
  let nativeLog;
  const adapter = createCsmBrowseAdapter({
    ensureSession: async ({ sid: requestedSid }) => ensureSession(requestedSid),
