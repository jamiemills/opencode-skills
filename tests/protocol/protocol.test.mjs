...
test('capable agent materializes verified payload copies and emits a schema-valid report', async () => {
  ...
  try {
    ...
    const result = await runProtocol(capableInput({ destination, sandbox, reloadAction: 'restart the agent host' }));
    ...
    assert.deepEqual(result.report.hashVerification, { algorithm: 'sha256', verified: expected.length, total: expected.length });
    ...
    assert.deepEqual(result.report.reloadAction, { status: 'declared', action: 'restart the agent host' });
...
test('ambiguous destination asks the user and refuses without confirmation, proceeds with it', async () => {
  ...
  try {
    ...
    assert.equal(proceeded.report.states.find(state => state.state === 'CONFIRM_IF_NEEDED').action, 'confirmed');
