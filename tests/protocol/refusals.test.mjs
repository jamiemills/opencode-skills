...
test("missing npx or file-write capability refuses before any mutation", async () => {
  ...
  try {
    ...
    const noNpx = await runProtocol(
      capableInput({ destination, sandbox, capabilities: { ...capable, hasNpx: false } }),
    );
    assert.equal(noNpx.exitCode, EXIT_CODES.E_NO_NPX);
    assert.deepEqual(noNpx.report.refusal, { code: "E_NO_NPX", state: "DISCOVER" });
    const noWrite = await runProtocol(
      capableInput({ destination, sandbox, capabilities: { ...capable, hasFileWrite: false } }),
    );
...
test("payload index schema mismatch refuses as unsupported format", async () => {
  ...
  try {
    ...
    const result = await runProtocol(
      capableInput({ destination: join(sandbox, "skills"), sandbox, index }),
    );
    ...
    const shapeResult = await runProtocol(
      capableInput({ destination: join(sandbox, "skills"), sandbox, index: malformed }),
    );
...
test("shell-bearing steps refuse as malicious with zero mutation", async () => {
  ...
  try {
    ...
    for (const steps of cases) {
      ...
      const result = await runProtocol(
        capableInput({
          destination: join(sandbox, `skills-${run}`),
          sandbox,
          envelope: { ...envelope, steps_markdown: steps },
        }),
      );
...
test("symlinked destination path component refuses with zero mutation", async () => {
  ...
  try {
    ...
    const result = await runProtocol(
      capableInput({ destination: join(sandbox, "link", "skills"), sandbox }),
    );
...
test("traversal or duplicate payload index entries refuse before any copy", async () => {
  ...
  try {
    ...
    traversal.classes.skills.push({
      path: "payload/skills/../../../etc/evil",
      sha256: "0".repeat(64),
      bytes: 1,
      mode: "0644",
    });
    const relative = await runProtocol(
      capableInput({ destination: join(sandbox, "skills"), sandbox, index: traversal }),
    );
    ...
    absolute.classes.supportingFiles.push({
      path: "/etc/passwd",
      sha256: "0".repeat(64),
      bytes: 1,
      mode: "0644",
    });
    const absoluteResult = await runProtocol(
      capableInput({ destination: join(sandbox, "skills"), sandbox, index: absolute }),
    );
    ...
    const duplicateResult = await runProtocol(
      capableInput({ destination: join(sandbox, "skills"), sandbox, index: duplicate }),
    );
    ...
    for (const refused of [relative, absoluteResult, duplicateResult])
      assert.deepEqual(validateSchema(refused.report, schema), []);
    assert.deepEqual((await readdir(sandbox)).toSorted(), []);
  } finally {
    await rm(sandbox, { recursive: true, force: true });
...
test("untrusted envelopes refuse at trust before any mutation", async () => {
  ...
  try {
    ...
    for (const [name, forged] of cases) {
      const result = await runProtocol(
        capableInput({ destination: join(sandbox, "skills"), sandbox, envelope: forged }),
      );
...
test("missing sandbox or out-of-scope placed entry refuses at materialize", async () => {
  ...
  try {
    ...
    const index = await loadIndex();
    index.classes.skills.push({
      path: "payload/other/csm-evil/SKILL.md",
      sha256: "0".repeat(64),
      bytes: 1,
      mode: "0644",
    });
    const misplaced = await runProtocol(
      capableInput({ destination: join(sandbox, "skills"), sandbox, index }),
    );
    assert.equal(misplaced.exitCode, EXIT_CODES.E_UNSUPPORTED_FORMAT);
    assert.deepEqual(misplaced.report.refusal, {
      code: "E_UNSUPPORTED_FORMAT",
      state: "MATERIALIZE",
    });
    for (const refused of [noSandbox, misplaced])
      assert.deepEqual(validateSchema(refused.report, schema), []);
    assert.deepEqual((await readdir(sandbox)).toSorted(), []);
  } finally {
    await rm(sandbox, { recursive: true, force: true });
...
test("modified existing destination refuses unmanaged and replaces managed with rollback reported", async () => {
  ...
  try {
    ...
    const placed = await runProtocol(
      capableInput({ destination: join(sandbox, "fresh"), sandbox }),
    );
    ...
    assert.equal(
      await readFile(join(sandbox, "backup", "csm-plan", "SKILL.md"), "utf8"),
      "drifted after first install\n",
    );
  } finally {
    await rm(sandbox, { recursive: true, force: true });
  }
