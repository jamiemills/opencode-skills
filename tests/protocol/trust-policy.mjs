...
export const SHELL_DENYLIST = /\b(npx|npm|node|nodejs|bash|sh|python|python3|pip|pip3|git|curl|wget|sudo|rm|powershell|eval|exec|chmod|chown|docker|uvx|bunx|deno)\b/i;
...
export const ALLOWED_ENVELOPE_KEYS = ['audience', 'expires_at', 'key', 'payload_index_sha256', 'policy', 'schema', 'signature', 'steps_markdown', 'steps_sha256'];
export const REQUIRED_ENVELOPE_KEYS = ['audience', 'expires_at', 'key', 'policy', 'schema', 'steps_markdown', 'steps_sha256'];
...
export const checkStepsShellPolicy = steps => steps.includes('`') || steps.includes('~~~') || SHELL_DENYLIST.test(steps);
...
export function validateEnvelopeShape(envelope) {
  ...
  for (const required of REQUIRED_ENVELOPE_KEYS) if (!Object.prototype.hasOwnProperty.call(envelope, required)) reject('SCHEMA', `missing field ${required}`);
  for (const present of Object.keys(envelope)) if (!ALLOWED_ENVELOPE_KEYS.includes(present)) reject('UNEXPECTED_FIELD', `unsigned field ${present} is not allowed`);
  if (Object.prototype.hasOwnProperty.call(envelope, 'payload_index_sha256') && (typeof envelope.payload_index_sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(envelope.payload_index_sha256))) {
  ...
  if (!Number.isInteger(limits.max_bytes) || limits.max_bytes < 1 || limits.max_bytes > HARDWIRE_CAP) reject('SCHEMA', 'max_bytes out of bounds');
...
export function validateEnvelope(envelope, keyring, { origin, bytes = Buffer.byteLength(JSON.stringify(envelope)), now = new Date(), indexSha256 } = {}) {
