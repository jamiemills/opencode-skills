...
function extractRequiredPlanDocument(skillContent) {
  ...
  const headingIdx = lines.findIndex((l, i) => !inFence[i] && l.trim() === '## Required Plan Document');
  ...
  for (let i = headingIdx + 1; i < lines.length; i += 1) {
    ...
    if (openChar !== null && m && m[1][0] === openChar && m[1].length >= openLen && m[2].trim() === '') break;
...
test('PAUSED -> RECOVER negatives: bad transition and bad journal Next state both fail', async () => {
  const badTransition = await readFile(join(root, 'tests', 'fixtures', 'resume', 'bad-transition-csm.md'), 'utf8');
  ...
  const badJournal = await readFile(join(root, 'tests', 'fixtures', 'resume', 'bad-journal-csm.md'), 'utf8');
  assert.deepEqual(validatePlanControl(badJournal), []);
  const journalFailures = validatePlanJournal(badJournal);
  assert.ok(journalFailures.length >= 1, 'FROBULATE journal Next state must fail journal validation');
...
test('journal and control consistency over the plan corpus', async () => {
  ...
  for (const f of files) {
    ...
    assert.ok(control.status !== null && control.currentState !== null && control.nextTransition !== null, `${label}: Control fields must be present`);
  }
});
...
test('csm-build "## Pause On Quota" documents the full quota-signal set and the resume marker', async () => {
  ...
  assert.ok(enumLine !== undefined, 'Pause On Quota must have a "Quota signal set:" enumeration line');
  for (const signal of ['HTTP 429', 'rate-limit', 'quota-exceeded', 'out-of-credits', 'billing', 'context-length-exceeded']) {
    assert.ok(enumLine.includes(signal), `Quota signal set must enumerate "${signal}"`);
  }
  assert.ok(section.includes('PAUSED -> RECOVER'), 'Pause On Quota must contain the PAUSED -> RECOVER marker');
});
