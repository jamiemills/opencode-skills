...
import { createWorktree, listWorktrees, mergeWorktree, removeWorktree } from '../scripts/wt-session.mjs';
...
test('removeWorktree refuses dirty and committed-unmerged, then removes cleanly after merge; an empty branch nukes safely', () => {
  ...
  try {
    ...
    assert.equal(emptyRemoved, empty.dir, 'an unadvanced branch has nothing to lose and nukes safely');
  } finally {
    fs.rmSync(base, { recursive: true, force: true });
    fs.rmSync(root, { recursive: true, force: true });
  }
